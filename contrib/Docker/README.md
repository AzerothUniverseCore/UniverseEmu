...
The Circle CI Linux pch job uses the Dockerfile contained in the same folder as this README to create an image with the binaries built for Linux, and stores that in the job artifacts. For the 3.3.5 and master branches, it also pushes the images to https://hub.docker.com/r/syphrenacore/syphrenacore .

The instructions below expect a basic knowledge of how to configure SyphrenaCore and how to use Docker.
...
1. Copy the .conf files from the SyphrenaCore GitHub repository to a local folder which will be passed on as a mapped volume to Docker.
