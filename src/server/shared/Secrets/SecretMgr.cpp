...
static Optional<BigNumber> GetHexFromConfig(char const* configKey, int bits)
{
    ...
    {
        SC_LOG_FATAL("server.loading", "Invalid value for '{}' - specify a hexadecimal integer of up to {} bits with no prefix.", configKey, bits);
        ABORT();
    }
    ...
    {
        SC_LOG_ERROR("server.loading", "Value for '{}' is out of bounds (should be an integer of up to {} bits with no prefix). Truncated to {} bits.", configKey, bits, bits);
        secret %= threshold;
    }
...
void SecretMgr::AttemptLoad(Secrets i, LogLevel errorLevel, std::unique_lock<std::mutex> const&)
{
    ...
    if (
        ...
        (oldDigest && !Syphrena::Crypto::Argon2::Verify(currentValue->AsHexStr(), *oldDigest)) // there is an old digest, and the current secret does not match it
        )
    {
        if (info.owner != THIS_SERVER_PROCESS)
        {
            if (currentValue)
                SC_LOG_MESSAGE_BODY("server.loading", errorLevel, "Invalid value for '{}' specified - this is not actually the secret being used in your auth DB.", info.configKey);
            else
                SC_LOG_MESSAGE_BODY("server.loading", errorLevel, "No value for '{}' specified - please specify the secret currently being used in your auth DB.", info.configKey);
        ...
        if (oldDigest && info.oldKey) // there is an old digest, so there might be an old secret (if possible)
        {
            ...
            if (oldSecret && !Syphrena::Crypto::Argon2::Verify(oldSecret->AsHexStr(), *oldDigest))
            {
                SC_LOG_MESSAGE_BODY("server.loading", errorLevel, "Invalid value for '{}' specified - this is not actually the secret previously used in your auth DB.", info.oldKey);
                _secrets[i].state = Secret::LOAD_FAILED;
                return;
            }
        }
        ...
        if (error)
        {
            SC_LOG_MESSAGE_BODY("server.loading", errorLevel, "Your value of '{}' changed, but we cannot transition your database to the new value:\n{}", info.configKey, error->c_str());
            _secrets[i].state = Secret::LOAD_FAILED;
            return;
        }

        SC_LOG_INFO("server.loading", "Successfully transitioned database to new '{}' value.", info.configKey);
    }
...
Optional<std::string> SecretMgr::AttemptTransition(Secrets i, Optional<BigNumber> const& newSecret, Optional<BigNumber> const& oldSecret, bool hadOldSecret) const
{
    ...
    switch (i)
    {
        case SECRET_TOTP_MASTER_KEY:
        {
            ...
            if (result) do
            {
                ...
                {
                    if (!oldSecret)
                        return Syphrena::StringFormat("Cannot decrypt old TOTP tokens - add config key '{}' to authserver.conf!", secret_info[i].oldKey);

                    bool success = Syphrena::Crypto::AEDecrypt<Syphrena::Crypto::AES>(totpSecret, oldSecret->ToByteArray<Syphrena::Crypto::AES::KEY_SIZE_BYTES>());
                    if (!success)
                        return Syphrena::StringFormat("Cannot decrypt old TOTP tokens - value of '{}' is incorrect for some users!", secret_info[i].oldKey);
                }

                if (newSecret)
                    Syphrena::Crypto::AEEncryptWithRandomIV<Syphrena::Crypto::AES>(totpSecret, newSecret->ToByteArray<Syphrena::Crypto::AES::KEY_SIZE_BYTES>());
    ...
    if (newSecret)
    {
        BigNumber salt;
        salt.SetRand(128);
        Optional<std::string> hash = Syphrena::Crypto::Argon2::Hash(newSecret->AsHexStr(), salt);
        if (!hash)
