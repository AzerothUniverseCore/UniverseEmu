...
public:
    ...
    virtual bool StartNetwork(Syphrena::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port, int threadCount)
    {
        ...
        {
            SC_LOG_ERROR("network", "Exception caught in SocketMgr.StartNetwork ({}:{}): {}", bindIp, port, err.what());
            return false;
        }

        if (!acceptor->Bind())
        {
            SC_LOG_ERROR("network", "StartNetwork failed to bind socket acceptor");
            delete acceptor;
            return false;
        }
    ...
    virtual void OnSocketOpen(tcp::socket&& sock, uint32 threadIndex)
    {
        ...
        {
            SC_LOG_WARN("network", "Failed to retrieve client's remote address {}", err.what());
        }
    }
