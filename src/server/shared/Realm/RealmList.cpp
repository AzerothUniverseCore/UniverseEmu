...
void RealmList::Initialize(Syphrena::Asio::IoContext& ioContext, uint32 updateInterval)
{
    _updateInterval = updateInterval;
    _updateTimer = std::make_unique<Syphrena::Asio::DeadlineTimer>(ioContext);
    _resolver = std::make_unique<Syphrena::Asio::Resolver>(ioContext);
...
void RealmList::UpdateRealms(boost::system::error_code const& error)
{
    ...
    SC_LOG_DEBUG("server.authserver", "Updating Realm List...");
    ...
    if (result)
    {
        do
        {
            try
            {
                ...
                if (!externalAddress)
                {
                    SC_LOG_ERROR("server.authserver", "Could not resolve address {} for realm \"{}\" id {}", externalAddressString, name, realmId);
                    continue;
                }
                ...
                if (!localAddress)
                {
                    SC_LOG_ERROR("server.authserver", "Could not resolve localAddress {} for realm \"{}\" id {}", localAddressString, name, realmId);
                    continue;
                }
                ...
                if (!localSubmask)
                {
                    SC_LOG_ERROR("server.authserver", "Could not resolve localSubnetMask {} for realm \"{}\" id {}", localSubmaskString, name, realmId);
                    continue;
                }
                ...
                if (!existingRealms.count(id))
                    SC_LOG_INFO("server.authserver", "Added realm \"{}\" at {}:{}.", name, externalAddressString, port);
                else
                    SC_LOG_DEBUG("server.authserver", "Updating realm \"{}\" at {}:{}.", name, externalAddressString, port);

                existingRealms.erase(id);
            }
            catch (std::exception& ex)
            {
                SC_LOG_ERROR("server.authserver", "Realmlist::UpdateRealms has thrown an exception: {}", ex.what());
                ABORT();
            }
        }
        while (result->NextRow());
    ...
    for (auto itr = existingRealms.begin(); itr != existingRealms.end(); ++itr)
        SC_LOG_INFO("server.authserver", "Removed realm \"{}\".", itr->second);
