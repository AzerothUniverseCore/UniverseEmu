...
{
    uint8   cmd;
    Syphrena::Crypto::SRP6::EphemeralKey A;
    Syphrena::Crypto::SHA1::Digest clientM;
    Syphrena::Crypto::SHA1::Digest crc_hash;
...
typedef struct AUTH_LOGON_PROOF_S
{
    uint8   cmd;
    uint8   error;
    Syphrena::Crypto::SHA1::Digest M2;
    uint32  AccountFlags;
    uint32  SurveyId;
    uint16  LoginFlags;
} sAuthLogonProof_S;
...
typedef struct AUTH_RECONNECT_PROOF_C
{
    uint8   cmd;
    uint8   R1[16];
    Syphrena::Crypto::SHA1::Digest R2, R3;
    uint8   number_of_keys;
...
void AuthSession::Start()
{
    ...
    SC_LOG_TRACE("session", "Accepted connection from {}", ip_address);
...
void AuthSession::CheckIpCallback(PreparedQueryResult result)
{
    if (result)
    {
        ...
        if (banned)
        {
            ...
            SendPacket(pkt);
            SC_LOG_DEBUG("session", "[AuthSession::CheckIpCallback] Banned ip '{}:{}' tries to login!", GetRemoteIpAddress().to_string(), GetRemotePort());
            return;
        }
    }
...
bool AuthSession::HandleLogonChallenge()
{
    ...
    SC_LOG_DEBUG("server.authserver", "[AuthChallenge] '{}'", login);
...
void AuthSession::LogonChallengeCallback(PreparedQueryResult result)
{
    ...
    {
        SC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is locked to IP - '{}' is logging in from '{}'", _accountInfo.Login, _accountInfo.LastIP, ipAddress);
    ...
    else
    {
        ...
        SC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is not locked to ip", _accountInfo.Login);
        if (_accountInfo.LockCountry.empty() || _accountInfo.LockCountry == "00")
            SC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is not locked to country", _accountInfo.Login);
        else if (!_ipCountry.empty())
        {
            SC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is locked to country: '{}' Player country is '{}'", _accountInfo.Login, _accountInfo.LockCountry, _ipCountry);
    ...
    if (_accountInfo.IsBanned)
    {
        if (_accountInfo.IsPermanenetlyBanned)
        {
            pkt << uint8(WOW_FAIL_BANNED);
            SendPacket(pkt);
            SC_LOG_INFO("server.authserver.banned", "'{}:{}' [AuthChallenge] Banned account {} tried to login!", ipAddress, port, _accountInfo.Login);
            return;
        }
        else
        {
            ...
            SendPacket(pkt);
            SC_LOG_INFO("server.authserver.banned", "'{}:{}' [AuthChallenge] Temporarily banned account {} tried to login!", ipAddress, port, _accountInfo.Login);
            return;
        }
    }
    ...
    if (!fields[9].IsNull())
    {
        ...
        {
            bool success = Syphrena::Crypto::AEDecrypt<Syphrena::Crypto::AES>(*_totpSecret, *secret);
            if (!success)
            {
                pkt << uint8(WOW_FAIL_DB_BUSY);
                SC_LOG_ERROR("server.authserver", "[AuthChallenge] Account '{}' has invalid ciphertext for TOTP token key stored", _accountInfo.Login);
                SendPacket(pkt);
                return;
            }
        }
    ...
    _srp6.emplace(
        _accountInfo.Login,
        fields[10].GetBinary<Syphrena::Crypto::SRP6::SALT_LENGTH>(),
        fields[11].GetBinary<Syphrena::Crypto::SRP6::VERIFIER_LENGTH>()
    );
    ...
    if (AuthHelper::IsAcceptedClientBuild(_build))
    {
        ...
        SC_LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] account {} is using '{}' locale ({})",
...
{
    SC_LOG_DEBUG("server.authserver", "Entering _HandleLogonProof");
    _status = STATUS_CLOSED;
    ...
    if (_expversion == NO_VALID_EXP_FLAG)
    {
        ...
        SC_LOG_DEBUG("network", "Client with invalid version, patching is not implemented");
        return false;
    }
    ...
    if (std::optional<SessionKey> K = _srp6->VerifyChallengeResponse(logonProof->A, logonProof->clientM))
    {
        ...
        if (sentToken && _totpSecret)
        {
            ...
            tokenSuccess = Syphrena::Crypto::TOTP::ValidateToken(*_totpSecret, incomingToken);
        ...
        SC_LOG_DEBUG("server.authserver", "'{}:{}' User '{}' successfully authenticated", GetRemoteIpAddress().to_string(), GetRemotePort(), _accountInfo.Login);
        ...
        Syphrena::Crypto::SHA1::Digest M2 = Syphrena::Crypto::SRP6::GetSessionVerifier(logonProof->A, logonProof->clientM, _sessionKey);
    ...
    else
    {
        ...
        SC_LOG_INFO("server.authserver.hack", "'{}:{}' [AuthChallenge] account {} tried to login with invalid password!",
        ...
        if (MaxWrongPassCount > 0)
        {
            ...
            if (++_accountInfo.FailedLogins >= MaxWrongPassCount)
            {
                ...
                if (WrongPassBanType)
                {
                    ...
                    SC_LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] account {} got banned for '{}' seconds because it failed to authenticate '{}' times",
                ...
                else
                {
                    ...
                    SC_LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] IP got banned for '{}' seconds because account {} failed to authenticate '{}' times",
...
bool AuthSession::HandleReconnectChallenge()
{
    ...
    std::string login((char const*)challenge->I, challenge->I_len);
    SC_LOG_DEBUG("server.authserver", "[ReconnectChallenge] '{}'", login);
...
void AuthSession::ReconnectChallengeCallback(PreparedQueryResult result)
{
    ...
    Syphrena::Crypto::GetRandomBytes(_reconnectProof);
...
{
    SC_LOG_DEBUG("server.authserver", "Entering _HandleReconnectProof");
    _status = STATUS_CLOSED;
    ...
    {
        SC_LOG_ERROR("server.authserver.hack", "'{}:{}' [ERROR] user {} tried to login, but session is invalid.", GetRemoteIpAddress().to_string(),
...
{
    SC_LOG_DEBUG("server.authserver", "Entering _HandleRealmList");
...
bool AuthSession::VerifyVersion(uint8 const* a, int32 aLength, Syphrena::Crypto::SHA1::Digest const& versionProof, bool isReconnect)
{
    if (!sConfigMgr->GetBoolDefault("StrictVersionCheck", false))
        return true;

    Syphrena::Crypto::SHA1::Digest zeros = { };
    Syphrena::Crypto::SHA1::Digest const* versionHash = nullptr;
    if (!isReconnect)
    {
