...
void SCSoapThread(const std::string& host, uint16 port)
{
    struct soap soap;
    soap_init(&soap);
    ...
    {
        SC_LOG_ERROR("network.soap", "Couldn't bind to {}:{}", host, port);
        exit(-1);
    }

    SC_LOG_INFO("network.soap", "Bound to http://{}:{}", host, port);
    ...
    while (!World::IsStopped())
    {
        ...
        SC_LOG_DEBUG("network.soap", "Accepted connection from IP={}.{}.{}.{}", (int)(soap.ip>>24)&0xFF, (int)(soap.ip>>16)&0xFF, (int)(soap.ip>>8)&0xFF, (int)soap.ip&0xFF);
...
{
    SC_LOG_TRACE("network.soap", "SOAPWorkingThread::process_message");
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    {
        SC_LOG_INFO("network.soap", "Client didn't provide login information");
        return 401;
    }
    ...
    if (!accountId)
    {
        SC_LOG_INFO("network.soap", "Client used invalid username '{}'", soap->userid);
        return 401;
    }

    if (!AccountMgr::CheckPassword(accountId, soap->passwd))
    {
        SC_LOG_INFO("network.soap", "Invalid password for account '{}'", soap->userid);
        return 401;
    }

    if (AccountMgr::GetSecurity(accountId, realm.Id.Realm) < SEC_ADMINISTRATOR)
    {
        SC_LOG_INFO("network.soap", "{}'s gmlevel is too low", soap->userid);
        return 403;
    }
    ...
    SC_LOG_INFO("network.soap", "Received command '{}'", command);
...
{   { "SOAP-ENV", "http://schemas.xmlsoap.org/soap/envelope/", NULL, NULL }, // must be first
    ...
    { "ns1", "urn:SC", NULL, NULL },     // "ns1" namespace prefix
    { NULL, NULL, NULL, NULL }
};
