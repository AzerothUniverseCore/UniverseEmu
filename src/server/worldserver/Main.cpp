...
/// \addtogroup Syphrenad Syphrena Daemon
/// @{
/// \file
...
#ifndef _SYPHRENA_CORE_CONFIG
    #define _SYPHRENA_CORE_CONFIG  "worldserver.conf"
#endif

#ifndef _SYPHRENA_CORE_CONFIG_DIR
    #define _SYPHRENA_CORE_CONFIG_DIR "worldserver.conf.d"
#endif

#ifdef _WIN32
#include "ServiceWin32.h"
char serviceName[] = "worldserver";
char serviceLongName[] = "SyphrenaCore world service";
char serviceDescription[] = "SyphrenaCore World of Warcraft emulator world service";
/*
...
{
public:
    FreezeDetector(Syphrena::Asio::IoContext& ioContext, uint32 maxCoreStuckTime)
...
private:
    Syphrena::Asio::DeadlineTimer _timer;
...
AsyncAcceptor* StartRaSocketAcceptor(Syphrena::Asio::IoContext& ioContext);
bool StartDB();
void StopDB();
void WorldUpdateLoop();
void ClearOnlineAccounts();
void ShutdownCLIThread(std::thread* cliThread);
bool LoadRealmInfo(Syphrena::Asio::IoContext& ioContext);
variables_map GetConsoleArguments(int argc, char** argv, fs::path& configFile, fs::path& configDir, std::string& winServiceAction);

/// Launch the Syphrena server
extern int main(int argc, char** argv)
{
    Syphrena::Impl::CurrentServerProcessHolder::_type = SERVER_PROCESS_WORLDSERVER;
    signal(SIGABRT, &Syphrena::AbortHandler);

    auto configFile = fs::absolute(_SYPHRENA_CORE_CONFIG);
    auto configDir  = fs::absolute(_SYPHRENA_CORE_CONFIG_DIR);
    ...
    std::shared_ptr<Syphrena::Asio::IoContext> ioContext = std::make_shared<Syphrena::Asio::IoContext>();

    sLog->RegisterAppender<AppenderDB>();
    // If logs are supposed to be handled async then we need to pass the IoContext into the Log singleton
    sLog->Initialize(sConfigMgr->GetBoolDefault("Log.Async.Enable", false) ? ioContext.get() : nullptr);

    Syphrena::Banner::Show("worldserver-daemon",
        [](char const* text)
        {
            SC_LOG_INFO("server.worldserver", "{}", text);
        },
        []()
        {
            SC_LOG_INFO("server.worldserver", "Using configuration file {}.", sConfigMgr->GetFilename());
            SC_LOG_INFO("server.worldserver", "Using SSL version: {} (library: {})", OPENSSL_VERSION_TEXT, OpenSSL_version(OPENSSL_VERSION));
            SC_LOG_INFO("server.worldserver", "Using Boost version: {}.{}.{}", BOOST_VERSION / 100000, BOOST_VERSION / 100 % 1000, BOOST_VERSION % 100);
        }
    );

    for (std::string const& key : overriddenKeys)
        SC_LOG_INFO("server.worldserver", "Configuration field '{}' was overridden with environment variable.", key);
    ...
    if (!pidFile.empty())
    {
        if (uint32 pid = CreatePIDFile(pidFile))
            SC_LOG_INFO("server.worldserver", "Daemon PID: {}\n", pid);
        else
        {
            SC_LOG_ERROR("server.worldserver", "Cannot create PID file {}.\n", pidFile);
            return 1;
        }
    }
    ...
    std::shared_ptr<Syphrena::ThreadPool> threadPool = std::make_shared<Syphrena::ThreadPool>(numThreads);
    ...
    {
        SC_METRIC_VALUE("online_players", sWorld->GetPlayerCount());
        SC_METRIC_VALUE("db_queue_login", uint64(LoginDatabase.QueueSize()));
        SC_METRIC_VALUE("db_queue_character", uint64(CharacterDatabase.QueueSize()));
        SC_METRIC_VALUE("db_queue_world", uint64(WorldDatabase.QueueSize()));
    });

    SC_METRIC_EVENT("events", "Worldserver started", "");

    std::shared_ptr<void> sMetricHandle(nullptr, [](void*)
    {
        SC_METRIC_EVENT("events", "Worldserver shutdown", "");
        sMetric->Unload();
    });
    ...
    {
        soapThread.reset(new std::thread(SCSoapThread, sConfigMgr->GetStringDefault("SOAP.IP", "127.0.0.1"), uint16(sConfigMgr->GetIntDefault("SOAP.Port", 7878))),
            [](std::thread* thr)
        {
            thr->join();
            delete thr;
    ...
    if (networkThreads <= 0)
    {
        SC_LOG_ERROR("server.worldserver", "Network.Threads must be greater than 0");
    ...
    {
        SC_LOG_ERROR("server.worldserver", "Failed to initialize network");
        World::StopNow(ERROR_EXIT_CODE);
        return 1;
    }
    ...
    if (int coreStuckTime = sConfigMgr->GetIntDefault("MaxCoreStuckTime", 60))
    {
        ...
        SC_LOG_INFO("server.worldserver", "Starting up anti-freeze thread (%u seconds max stuck time)...", coreStuckTime);
    }

    SC_LOG_INFO("server.worldserver", "{} (syphrenabotserver-daemon) amorcer...", GitRevision::GetFullVersion());
    ...
    SC_LOG_INFO("server.worldserver", "Halting process...");

    // 0 - normal shutdown
    // 1 - shutdown at error
    // 2 - restart command used, this code can be used by restarter for restart Syphrenad
...
void ShutdownCLIThread(std::thread* cliThread)
{
    if (cliThread != nullptr)
    {
        ...
        if (!CancelSynchronousIo(cliThread->native_handle()))
        {
            ...
            SC_LOG_DEBUG("server.worldserver", "Error cancelling I/O of CliThread, error code {}, detail: {}", uint32(errorCode), errorBuffer);
    ...
    while (!World::IsStopped())
    {
        ...
        if (diff < minUpdateDiff)
        {
            ...
            if (sleepTime >= halfMaxCoreStuckTime)
                SC_LOG_ERROR("server.worldserver", "WorldUpdateLoop() waiting for {} ms with MaxCoreStuckTime set to {} ms", sleepTime, maxCoreStuckTime);
    ...
    if (!error)
    {
        if (std::shared_ptr<FreezeDetector> freezeDetector = freezeDetectorRef.lock())
        {
            ...
            else
            {
                ...
                {
                    SC_LOG_ERROR("server.worldserver", "World Thread hangs for %u ms, forcing a crash!", msTimeDiff);
...
AsyncAcceptor* StartRaSocketAcceptor(Syphrena::Asio::IoContext& ioContext)
{
    ...
    if (!acceptor->Bind())
    {
        SC_LOG_ERROR("server.worldserver", "Failed to bind RA socket acceptor");
        delete acceptor;
        return nullptr;
    }
...
bool LoadRealmInfo(Syphrena::Asio::IoContext& ioContext)
{
    ...
    Syphrena::Asio::Resolver resolver(ioContext);
    ...
    if (!externalAddress)
    {
        SC_LOG_ERROR("server.worldserver", "Could not resolve address {}", fields[2].GetString());
        return false;
    }
    ...
    if (!localAddress)
    {
        SC_LOG_ERROR("server.worldserver", "Could not resolve address {}", fields[3].GetString());
        return false;
    }
    ...
    if (!localSubmask)
    {
        SC_LOG_ERROR("server.worldserver", "Could not resolve address {}", fields[4].GetString());
        return false;
    }
...
bool StartDB()
{
    ...
    if (!realm.Id.Realm)
    {
        SC_LOG_ERROR("server.worldserver", "Realm ID not defined in configuration file");
        return false;
    }

    SC_LOG_INFO("server.worldserver", "Realm running as realm ID {}", realm.Id.Realm);
    ...
    SC_LOG_INFO("server.worldserver", "Using World DB: {}", sWorld->GetDBVersion());
    return true;
}
...
variables_map GetConsoleArguments(int argc, char** argv, fs::path& configFile, fs::path& configDir, [[maybe_unused]] std::string& winServiceAction)
{
    ...
    all.add_options()
        ...
        ("config,c", value<fs::path>(&configFile)->default_value(fs::absolute(_SYPHRENA_CORE_CONFIG)),
                     "use <arg> as configuration file")
					 ("config-dir,cd", value<fs::path>(&configDir)->default_value(fs::absolute(_SYPHRENA_CORE_CONFIG_DIR)),
