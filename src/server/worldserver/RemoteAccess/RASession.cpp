...
void RASession::Start()
{
    ...
    SC_LOG_INFO("commands.ra", "Accepting RA connection from user {} (IP: {})", username, GetRemoteIpAddress());
    ...
    SC_LOG_INFO("commands.ra", "User {} (IP: {}) authenticated correctly to RA", username, GetRemoteIpAddress());
...
bool RASession::CheckAccessLevel(const std::string& user)
{
    ...
    if (!result)
    {
        SC_LOG_INFO("commands.ra", "User {} does not exist in database", user);
        return false;
    }
    ...
    {
        SC_LOG_INFO("commands.ra", "User {} has no privilege to login", user);
        return false;
    }
    else if (fields[2].GetInt32() != -1)
    {
        SC_LOG_INFO("commands.ra", "User {} has to be assigned on all realms (with RealmID = '-1')", user);
        return false;
    }
...
bool RASession::CheckPassword(const std::string& user, const std::string& pass)
{
    ...
    if (PreparedQueryResult result = LoginDatabase.Query(stmt))
    {
        ...
        if (Syphrena::Crypto::SRP6::CheckLogin(safe_user, safe_pass, salt, verifier))
            return true;
    }

    SC_LOG_INFO("commands.ra", "Wrong password for user: {}", user);
    return false;
}
...
bool RASession::ProcessCommand(std::string& command)
{
    ...
    SC_LOG_INFO("commands.ra", "Received command: {}", command);
