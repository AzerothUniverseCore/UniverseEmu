...
bool DBUpdaterUtil::CheckExecutable()
{
    ...
    {
        exe = Syphrena::SearchExecutableInPath("mysql");
        ...
        SC_LOG_FATAL("sql.updates", "Didn't find any executable MySQL binary at \'{}\' or in path, correct the path in the *.conf (\"MySQLExecutable\").",
...
{
    SC_LOG_INFO("sql.updates", "Database \"{}\" does not exist, do you want to create it? [yes (default) / no]: ",
    ...
    SC_LOG_INFO("sql.updates", "Creating database \"{}\"...", pool.GetConnectionInfo()->database);
    ...
    if (!file.is_open())
    {
        SC_LOG_FATAL("sql.updates", "Failed to create temporary query file \"{}\"!", temp.generic_string());
        return false;
    }
    ...
    {
        SC_LOG_FATAL("sql.updates", "Failed to create database {}! Does the user (named in *.conf) have `CREATE`, `ALTER`, `DROP`, `INSERT` and `DELETE` privileges on the MySQL server?", pool.GetConnectionInfo()->database);
        boost::filesystem::remove(temp);
        return false;
    }

    SC_LOG_INFO("sql.updates", "Done.");
    boost::filesystem::remove(temp);
    return true;
}
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool)
{
    ...
    SC_LOG_INFO("sql.updates", "Updating {} database...", DBUpdater<T>::GetTableName());

    Path const sourceDirectory(BuiltInConfig::GetSourceDirectory());

    if (!is_directory(sourceDirectory))
    {
        SC_LOG_ERROR("sql.updates", "DBUpdater: The given source directory {} does not exist, change the path to the directory where your sql directory exists (for example c:\\source\\syphrenacore). Shutting down.", sourceDirectory.generic_string());
        return false;
    }
    ...
    std::string const info = Syphrena::StringFormat("Containing {} new and {} archived updates.",
        result.recent, result.archived);

    if (!result.updated)
        SC_LOG_INFO("sql.updates", ">> {} database is up-to-date! {}", DBUpdater<T>::GetTableName(), info);
    else
        SC_LOG_INFO("sql.updates", ">> Applied {} {}. {}", result.updated, result.updated == 1 ? "query" : "queries", info);
...
bool DBUpdater<T>::Populate(DatabaseWorkerPool<T>& pool)
{
    ...
    SC_LOG_INFO("sql.updates", "Database {} is empty, auto populating it...", DBUpdater<T>::GetTableName());

    std::string const p = DBUpdater<T>::GetBaseFile();
    if (p.empty())
    {
        SC_LOG_INFO("sql.updates", ">> No base file provided, skipped!");
        return true;
    }
    ...
    if (!exists(base))
    {
        switch (DBUpdater<T>::GetBaseLocationType())
        {
            ...
            {
                SC_LOG_ERROR("sql.updates", ">> Base file \"{}\" is missing. Try fixing it by cloning the source again.",
                    base.generic_string());
            ...
            case LOCATION_DOWNLOAD:
            {
                ...
                SC_LOG_ERROR("sql.updates", ">> File \"{}\" is missing, download it from \"https://github.com/DreamCoreRev/SyphrenaCoreLegacy\"" \
    ...
    // Update database
    SC_LOG_INFO("sql.updates", ">> Applying \'{}\'...", base.generic_string());
    try
    {
        ApplyFile(pool, base);
    }
    ...
    Path const& path)
{
    ...
    // Execute sql file
    args.emplace_back("-e");
    args.emplace_back(Syphrena::StringFormat("BEGIN; SOURCE {}; COMMIT;", path.generic_string()));
    ...
    int const ret = Syphrena::StartProcess(DBUpdaterUtil::GetCorrectedMySQLExecutable(), args,
                                 "sql.updates", "", true);

    if (ret != EXIT_SUCCESS)
    {
        SC_LOG_FATAL("sql.updates", "Applying of file \'{}\' to database \'{}\' failed!" \
            " If you are a user, please pull the latest revision from the repository. "
            "Also make sure you have not applied any of the databases with your sql client. "
            "You cannot use auto-update system and import sql files from SyphrenaCore repository with your sql client. "
...
template class SC_DATABASE_API DBUpdater<LoginDatabaseConnection>;
template class SC_DATABASE_API DBUpdater<WorldDatabaseConnection>;
template class SC_DATABASE_API DBUpdater<CharacterDatabaseConnection>;
