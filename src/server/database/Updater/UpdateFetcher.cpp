...
void UpdateFetcher::FillFileListRecursively(Path const& path, LocaleFileStorage& storage, State const state, uint32 const depth) const
{
    ...
    for (directory_iterator itr(path); itr != end; ++itr)
    {
        ...
        {
            SC_LOG_TRACE("sql.updates", "Added locale file \"{}\".", itr->path().filename().generic_string());
            ...
            {
                SC_LOG_FATAL("sql.updates", "Duplicate filename \"{}\" occurred. Because updates are ordered " \
...
UpdateFetcher::DirectoryStorage UpdateFetcher::ReceiveIncludedDirectories() const
{
    ...
    do
    {
        ...
        if (!is_directory(p))
        {
            SC_LOG_WARN("sql.updates", "DBUpdater: Given update include directory \"{}\" does not exist, skipped!", p.generic_string());
            continue;
        }
        ...
        SC_LOG_TRACE("sql.updates", "Added applied file \"{}\" from remote.", p.filename().generic_string());
...
std::string UpdateFetcher::ReadSQLUpdate(boost::filesystem::path const& file) const
{
    ...
    if (!in.is_open())
    {
        SC_LOG_FATAL("sql.updates", "Failed to open the sql update \"{}\" for reading! "
                                   ...
                                   int32 const cleanDeadReferencesMaxCount) const
{
    ...
    {
        SC_LOG_DEBUG("sql.updates", "Checking update \"{}\"...", availableQuery.first.filename().generic_string());
        ...
        if (iter != applied.end())
        {
            ...
            if (!redundancyChecks)
            {
                SC_LOG_DEBUG("sql.updates", ">> Update is already applied, skipping redundancy checks.");
                applied.erase(iter);
                continue;
            }
            ...
            {
                SC_LOG_DEBUG("sql.updates", ">> Update is archived and marked as archived in database, skipping redundancy checks.");
                applied.erase(iter);
                continue;
            }
        }
        ...
        std::string const hash = ByteArrayToHexStr(Syphrena::Crypto::SHA1::GetDigestOf(ReadSQLUpdate(availableQuery.first)));
        ...
        if (iter == applied.end())
        {
            ...
            if (hashIter != hashToName.end())
            {
                ...
                {
                    SC_LOG_WARN("sql.updates", ">> It seems like the update \"{}\" \'{}\' was renamed, but the old file is still there! " \
                ...
                {
                    SC_LOG_INFO("sql.updates", ">> Renaming update \"{}\" to \"{}\" \'{}\'.",
            ...
            {
                SC_LOG_INFO("sql.updates", ">> Applying update \"{}\" \'{}\'...",
                    availableQuery.first.filename().string(), hash.substr(0, 7));
            }
        }
        ...
        else if (allowRehash && iter->second.hash.empty())
        {
            ...
            SC_LOG_INFO("sql.updates", ">> Re-hashing update \"{}\" \'{}\'...", availableQuery.first.filename().string(),
                hash.substr(0, 7));
        }
        else
        {
            ...
            {
                SC_LOG_INFO("sql.updates", ">> Reapplying update \"{}\" \'{}\' -> \'{}\' (it changed)...", availableQuery.first.filename().string(),
            ...
            else
            {
                ...
                {
                    SC_LOG_DEBUG("sql.updates", ">> Updating the state of \"{}\" to \'{}\'...",
                ...
                SC_LOG_DEBUG("sql.updates", ">> Update is already applied and matches the hash \'%s\'.", hash.substr(0, 7).c_str());
    ...
    if (!applied.empty())
    {
        ...
        for (auto const& entry : applied)
        {
            SC_LOG_WARN("sql.updates", ">> The file \'{}\' was applied to the database, but is missing in" \
                " your update directory now!", entry.first);

            if (doCleanup)
                SC_LOG_INFO("sql.updates", "Deleting orphaned entry \'{}\'...", entry.first);
        }
        ...
        {
            SC_LOG_ERROR("sql.updates", "Cleanup is disabled! There were  {} dirty files applied to your database, " \
