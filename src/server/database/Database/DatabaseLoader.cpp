...
DatabaseLoader& DatabaseLoader::AddDatabase(DatabaseWorkerPool<T>& pool, std::string const& name)
{
    ...
    _open.push([this, name, updatesEnabledForThis, &pool]() -> bool
    {
        ...
        if (dbString.empty())
        {
            SC_LOG_ERROR(_logger, "Database {} not specified in configuration file!", name);
            return false;
        }
        ...
        {
            SC_LOG_ERROR(_logger, "{} database: invalid number of worker threads specified. "
        ...
        if (uint32 error = pool.Open())
        {
            ...
            if (error)
            {
                SC_LOG_ERROR("sql.driver", "\nDatabasePool {} NOT opened. There were errors opening the MySQL connections. Check your SQLDriverLogFile "
                    "for specific errors. Read wiki at http://www.github.com/DreamCoreRev/SyphrenaCoreLegacy", name);
    ...
    if (updatesEnabledForThis)
    {
        _populate.push([this, name, &pool]() -> bool
        {
            ...
            {
                SC_LOG_ERROR(_logger, "Could not populate the {} database, see log for details.", name);
                return false;
            }
            return true;
        });
        ...
        _update.push([this, name, &pool]() -> bool
        {
            ...
            {
                SC_LOG_ERROR(_logger, "Could not update the {} database, see log for details.", name);
                return false;
            }
            return true;
        });
    ...
    _prepare.push([this, name, &pool]() -> bool
    {
        ...
        {
            SC_LOG_ERROR(_logger, "Could not prepare statements of the {} database, see log for details.", name);
            return false;
        }
        return true;
    });
...
{
    if (!_updateFlags)
        SC_LOG_INFO("sql.updates", "Automatic database updates are disabled for all databases!");
