...
static uint32 SizeForType(MYSQL_FIELD* field)
{
    switch (field->type)
    {
        ...
        case MYSQL_TYPE_GEOMETRY:
            ...
            */
        default:
            SC_LOG_WARN("sql.sql", "SQL::SizeForType(): invalid field type {}", uint32(field->type));
            return 0;
    }
}
...
DatabaseFieldTypes MysqlTypeToFieldType(enum_field_types type, uint32 flags)
{
    switch (type)
    {
        ...
        default:
            SC_LOG_WARN("sql.sql", "MysqlTypeToFieldType(): invalid field type {}", uint32(type));
            break;
    }
...
m_metadataResult(result)
{
    ...
    {
        SC_LOG_WARN("sql.sql", "{}:mysql_stmt_store_result, cannot bind result from MySQL server. Error: {}", __FUNCTION__, mysql_stmt_error(m_stmt));
        delete[] m_rBind;
        delete[] m_isNull;
        delete[] m_length;
        return;
    ...
    {
        SC_LOG_WARN("sql.sql", "{}:mysql_stmt_bind_result, cannot bind result from MySQL server. Error: {}", __FUNCTION__, mysql_stmt_error(m_stmt));
...
bool ResultSet::NextRow()
{
    ...
    if (!lengths)
    {
        SC_LOG_WARN("sql.sql", "{}:mysql_fetch_lengths, cannot retrieve value lengths. Error {}.", __FUNCTION__, mysql_error(_result->handle));
        CleanUp();
        return false;
    }
