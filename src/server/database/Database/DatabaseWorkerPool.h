...
class DatabaseWorkerPool
{
    ...
    public:
        ...
        */
        ...
        void PExecute(Syphrena::FormatString<Args...> sql, Args&&... args)
        {
            if (Syphrena::IsFormatEmptyOrNull(sql))
                return;

            this->Execute(Syphrena::StringFormat(sql, std::forward<Args>(args)...).c_str());
        }
        ...
        */
        ...
        //! This method should only be used for queries that are only executed once, e.g during startup.
        template<typename... Args>
        void DirectPExecute(Syphrena::FormatString<Args...> sql, Args&&... args)
        {
            if (Syphrena::IsFormatEmptyOrNull(sql))
                return;

            this->DirectExecute(Syphrena::StringFormat(sql, std::forward<Args>(args)...).c_str());
        }
        ...
        */
        ...
        //! Returns reference counted auto pointer, no need for manual memory management in upper level code.
        template<typename... Args>
        QueryResult PQuery(Syphrena::FormatString<Args...> sql, T* conn, Args&&... args)
        {
            if (Syphrena::IsFormatEmptyOrNull(sql))
                return QueryResult(nullptr);

            return this->Query(Syphrena::StringFormat(sql, std::forward<Args>(args)...).c_str(), conn);
        }

        //! Directly executes an SQL query in string format -with variable args- that will block the calling thread until finished.
        //! Returns reference counted auto pointer, no need for manual memory management in upper level code.
        template<typename... Args>
        QueryResult PQuery(Syphrena::FormatString<Args...> sql, Args&&... args)
        {
            if (Syphrena::IsFormatEmptyOrNull(sql))
                return QueryResult(nullptr);

            return this->Query(Syphrena::StringFormat(sql, std::forward<Args>(args)...).c_str());
        }
