      ...
      _async_threads(0), _synch_threads(0)
{
    ...
    WPFatal(mysql_get_client_version() >= MIN_MARIADB_CLIENT_VERSION, "SyphrenaCore does not support MariaDB versions below " MIN_MARIADB_CLIENT_VERSION_STRING " (found %s id %lu, need id >= %u), please update your MariaDB client library", mysql_get_client_info(), mysql_get_client_version(), MIN_MARIADB_CLIENT_VERSION);
    WPFatal(mysql_get_client_version() == MARIADB_PACKAGE_VERSION_ID, "Used MariaDB library version (%s id %lu) does not match the version id used to compile SyphrenaCore (id %u). Search on forum for TCE00011.", mysql_get_client_info(), mysql_get_client_version(), MARIADB_PACKAGE_VERSION_ID);
#else
    WPFatal(mysql_get_client_version() >= MIN_MYSQL_CLIENT_VERSION, "SyphrenaCore does not support MySQL versions below " MIN_MYSQL_CLIENT_VERSION_STRING " (found %s id %lu, need id >= %u), please update your MySQL client library", mysql_get_client_info(), mysql_get_client_version(), MIN_MYSQL_CLIENT_VERSION);
    WPFatal(mysql_get_client_version() == MYSQL_VERSION_ID, "Used MySQL library version (%s id %lu) does not match the version id used to compile SyphrenaCore (id %u). Search on forum for TCE00011.", mysql_get_client_info(), mysql_get_client_version(), MYSQL_VERSION_ID);
#endif
}
...
uint32 DatabaseWorkerPool<T>::Open()
{
    ...
    SC_LOG_INFO("sql.driver", "Opening DatabasePool '{}'. "
    ...
    if (!error)
    {
        SC_LOG_INFO("sql.driver", "DatabasePool '{}' opened successfully. "
...
{
    SC_LOG_INFO("sql.driver", "Closing down DatabasePool '{}'.", GetDatabaseName());

    //! Closes the actualy MySQL connection.
    _connections[IDX_ASYNC].clear();

    SC_LOG_INFO("sql.driver", "Asynchronous connections on DatabasePool '{}' terminated. "
    ...
    SC_LOG_INFO("sql.driver", "All connections on DatabasePool '{}' closed.", GetDatabaseName());
}
        ...
        for (auto& connection : connections)
        {
            ...
            for (size_t i = 0; i < preparedSize; ++i)
            {
                ...
                if (MySQLPreparedStatement* stmt = connection->m_stmts[i].get())
                {
                    ...
                    // SC only supports uint8 indices.
...
void DatabaseWorkerPool<T>::CommitTransaction(SQLTransaction<T> transaction)
{
    ...
    {
    case 0:
        SC_LOG_DEBUG("sql.driver", "Transaction contains 0 queries. Not executing.");
        return;
    case 1:
        SC_LOG_DEBUG("sql.driver", "Warning: Transaction only holds 1 query, consider removing Transaction context in code.");
        break;
    default:
        break;
    }
...
uint32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, uint8 numConnections)
{
    for (uint8 i = 0; i < numConnections; ++i)
    {
        ...
        {
#ifndef LIBMARIADB
            SC_LOG_ERROR("sql.driver", "SyphrenaCore does not support MySQL versions below " MIN_MYSQL_SERVER_VERSION_STRING " (found id {}, need id >= {}), please update your MySQL server", connection->GetServerVersion(), MIN_MYSQL_SERVER_VERSION);
#else
            SC_LOG_ERROR("sql.driver", "SyphrenaCore does not support MariaDB versions below " MIN_MARIADB_SERVER_VERSION_STRING " (found id {}, need id >= {}), please update your MySQL server", connection->GetServerVersion(), MIN_MARIADB_SERVER_VERSION);
#endif
...
T* DatabaseWorkerPool<T>::GetFreeConnection()
{
    ...
    if (_warnSyncQueries)
    {
        ...
        SC_LOG_WARN("sql.performances", "Sync query at:\n{}", ss.str());
    }
#endif
...
{
    if (Syphrena::IsFormatEmptyOrNull(sql))
        return;
...
template class SC_DATABASE_API DatabaseWorkerPool<LoginDatabaseConnection>;
template class SC_DATABASE_API DatabaseWorkerPool<WorldDatabaseConnection>;
template class SC_DATABASE_API DatabaseWorkerPool<CharacterDatabaseConnection>;
