...
{
    std::vector<std::string_view> tokens = Syphrena::Tokenize(infoString, ';', true);
...
uint32 MySQLConnection::Open()
{
    ...
    if (!mysqlInit)
    {
        SC_LOG_ERROR("sql.sql", "Could not initialize Mysql connection to database `{}`", m_connectionInfo.database);
    ...
    if (m_Mysql)
    {
        if (!m_reconnecting)
        {
            SC_LOG_INFO("sql.sql", "MySQL client library: {}", mysql_get_client_info());
            SC_LOG_INFO("sql.sql", "MySQL server ver: {} ", mysql_get_server_info(m_Mysql));
            // MySQL version above 5.1 IS required in both client and server and there is no known issue with different versions above 5.1
            // if (mysql_get_server_version(m_Mysql) != mysql_get_client_version())
            //     SC_LOG_INFO("sql.sql", "[WARNING] MySQL client/server version mismatch; may conflict with behaviour of prepared statements.");
        }

        SC_LOG_INFO("sql.sql", "Connected to MySQL database at {}", m_connectionInfo.host);
    ...
    {
        SC_LOG_ERROR("sql.sql", "Could not connect to MySQL database at {}: {}", m_connectionInfo.host, mysql_error(mysqlInit));
...
bool MySQLConnection::Execute(char const* sql)
{
...

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            ...
            SC_LOG_INFO("sql.sql", "SQL: {}", sql);
            SC_LOG_ERROR("sql.sql", "[{}] {}", lErrno, mysql_error(m_Mysql));
            ...
            SC_LOG_DEBUG("sql.sql", "[{} ms] SQL: {}", getMSTimeDiff(_s, getMSTime()), sql);
    }
...
bool MySQLConnection::Execute(PreparedStatementBase* stmt)
{
    ...
    SC_LOG_DEBUG("sql.sql", "[{} ms] SQL(p): {}", getMSTimeDiff(_s, getMSTime()), m_mStmt->getQueryString());
    ...
    for (auto itr = queries.begin(); itr != queries.end(); ++itr)
    {
        ...
        switch (itr->type)
        {
            case SQL_ELEMENT_PREPARED:
            {
                ...
                if (!Execute(stmt))
                {
                    SC_LOG_WARN("sql.sql", "Transaction aborted. {} queries not executed.", (uint32)queries.size());
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ...
    if (!ret)
        SC_LOG_ERROR("sql.sql", "Could not fetch prepared statement {} on database `{}`, connection type: {}.",
...
void MySQLConnection::PrepareStatement(uint32 index, std::string const& sql, ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        SC_LOG_ERROR("sql.sql", "In mysql_stmt_init() id: {}, sql: \"{}\"", index, sql);
        SC_LOG_ERROR("sql.sql", "{}", mysql_error(m_Mysql));
        m_prepareError = true;
    }
    else
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "In mysql_stmt_prepare() id: {}, sql: \"{}\"", index, sql);
            SC_LOG_ERROR("sql.sql", "{}", mysql_stmt_error(stmt));
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo, uint8 attempts /*= 5*/)
{
    switch (errNo)
    {
        ...
        case CR_SERVER_LOST_EXTENDED:
        {
            if (m_Mysql)
            {
                SC_LOG_ERROR("sql.sql", "Lost the connection to the MySQL server!");
        ...
        {
            SC_LOG_INFO("sql.sql", "Attempting to reconnect to the MySQL server...");
            ...
            if (!lErrno)
            {
                ...
                {
                    SC_LOG_FATAL("sql.sql", "Could not re-prepare statements!");
                    std::this_thread::sleep_for(std::chrono::seconds(10));
                    ABORT();
                }

                SC_LOG_INFO("sql.sql", "Successfully reconnected to {} @{}:{} ({}).",
            ...
            if ((--attempts) == 0)
            {
                ...
                SC_LOG_FATAL("sql.sql", "Failed to reconnect to the MySQL server, "
        ...
        case ER_NO_SUCH_TABLE:
            SC_LOG_ERROR("sql.sql", "Your database structure is not up to date. Please make sure you've executed all queries in the sql/updates folders.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            ABORT();
            return false;
        case ER_PARSE_ERROR:
            SC_LOG_ERROR("sql.sql", "Error while parsing SQL. Core fix required.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            ABORT();
            return false;
        default:
            SC_LOG_ERROR("sql.sql", "Unhandled MySQL errno {}. Unexpected behaviour possible.", errNo);
            return false;
    }
}
