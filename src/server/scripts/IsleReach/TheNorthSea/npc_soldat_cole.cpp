...
namespace
{
    uint32 const NPC_SOLDAT_COLE = 160664;
    uint32 const QUEST_DISCIPLE_CHALLENGE_COLE = 58209;
...
class npc_soldat_cole : public CreatureScript
{
public:
    npc_soldat_cole() : CreatureScript("npc_soldat_cole") { }

    struct npc_soldat_coleAI : public ScriptedAI
    {
        explicit npc_soldat_coleAI(Creature* creature) : ScriptedAI(creature) { }
        ...
        bool OnGossipHello(Player* player) override
        {
            ...
            if (!player->IsActiveQuest(QUEST_DISCIPLE_CHALLENGE_COLE))
            {
        ...
        void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo*/) override
        {
            ...
            {
                ChatHandler(player->GetSession()).SendSysMessage("Victory! Private Cole surrenders.");

                if (player->IsActiveQuest(QUEST_DISCIPLE_CHALLENGE_COLE))
                    player->CompleteQuest(QUEST_DISCIPLE_CHALLENGE_COLE);
            }
    ...
    private:
        void StartChallenge(Player* player)
        {
            ...
            npc_soldat_coleAI* cloneAI = dynamic_cast<npc_soldat_coleAI*>(clone->AI());
            if (!cloneAI)
            {
                player->SetPhaseMask(PHASEMASK_NORMAL, true);
                DisciplePhaseManager::Instance().Release(phaseMask);
    ...
    {
        return new npc_soldat_coleAI(creature);
    }
};
