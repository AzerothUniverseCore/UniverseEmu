    ...
    bool OnGossipHello(Player* player) override
    {
        ...
        for (uint8 i = 0; i < graveyard.size(); i++)
            if (graveyard[i]->GetControlTeamId() == player->GetTeamId())
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, player->GetSession()->GetSyphrenaString(((BfGraveyardWG*)graveyard[i])->GetTextId()), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + i);
...
struct npc_wg_queue : public ScriptedAI
{
    ...
    bool OnGossipHello(Player* player) override
    {
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, player->GetSession()->GetSyphrenaString(WG_NPCQUEUE_TEXTOPTION_JOIN), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
