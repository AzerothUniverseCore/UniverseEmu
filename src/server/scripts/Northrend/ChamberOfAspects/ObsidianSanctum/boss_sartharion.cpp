...
struct boss_sartharion : public BossAI
{
    ...
    void Initialize()
    {
        ...
        ResetPlayerHitByLavaStrike();

        playerHit = false;

        if (GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
            achievement = sAchievementStore.LookupEntry(VOLCANO_BLOWS_ACHIEVEMENT_10_MAN);
        else
            achievement = sAchievementStore.LookupEntry(VOLCANO_BLOWS_ACHIEVEMENT_25_MAN);
    }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        // Condition used to check for the achievement "Gonna Go When the Volcano Blows" for each player if they have not been hit by the "Lava Strike" spell
        Map::PlayerList const& players = instance->instance->GetPlayers();

        if (!GetPlayerHitByLavaStrike().empty())
        {
            playerHit = true;

            std::set<Player*> playerHitByLavaStrikeList = GetPlayerHitByLavaStrike();
            ...
            for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
            {
                ...
                auto it = std::find(playerHitByLavaStrikeList.begin(), playerHitByLavaStrikeList.end(), player);

                if (it == playerHitByLavaStrikeList.end())
    ...
    void CastLavaStrikeOnTarget(Unit* target)
    {
        ...
        Syphrena::AllCreaturesOfEntryInRange checker(me, NPC_FIRE_CYCLONE, 200.0f);
        Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher(me, fireCyclonesList, checker);
        Cell::VisitAllObjects(me, searcher, 200.0f);

        if (fireCyclonesList.empty())
            return;

        Syphrena::Containers::SelectRandomContainerElement(fireCyclonesList)->CastSpell(target, SPELL_LAVA_STRIKE, true);
    }
