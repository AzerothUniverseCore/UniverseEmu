...
// Necessary for achievement "Volcano Blows" - used by Sartharion for get list of player hit by "Lava Strike"
std::set<Player*> playerHitByLavaStrike;

std::set<Player*> GetPlayerHitByLavaStrike()
{
    return playerHitByLavaStrike;
}

void ResetPlayerHitByLavaStrike()
{
    playerHitByLavaStrike.clear();
}

struct npc_fire_cyclone : public ScriptedAI
{
    npc_fire_cyclone(Creature* creature) : ScriptedAI(creature)
    {
    }

    void SpellHitTarget(WorldObject* target, SpellInfo const* spell) override
    {
        if (spell->Id == SPELL_LAVA_STRIKE && target->IsPlayer())
        {
            playerHitByLavaStrike.insert(target->ToPlayer());
        }
    }
};
...
void AddSC_obsidian_sanctum()
{
    ...
    RegisterObsidianSanctumCreatureAI(npc_fire_cyclone);
