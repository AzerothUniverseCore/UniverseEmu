...
class spell_fjord_rivenwood_captives_not_on_quest : public SpellScript
{
    ...
    {
        GetHitUnit()->CastSpell(GetCaster(), Syphrena::Containers::SelectRandomContainerElement(CocoonSummonSpells), true);
    }
...
class spell_fjord_rivenwood_captives_on_quest : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (roll_chance_i(80))
            target->CastSpell(caster, Syphrena::Containers::SelectRandomContainerElement(CocoonSummonSpells), true);
        else
