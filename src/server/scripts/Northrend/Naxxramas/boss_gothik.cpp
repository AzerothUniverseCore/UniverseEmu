        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_SUMMON:
                {
                    ...
                    {
                        SC_LOG_INFO("scripts", "GothikAI: Wave count {} is out of range for difficulty {}.", _waveCount, static_cast<uint32>(GetDifficulty()));
                        break;
                    }
...
class spell_gothik_shadow_bolt_volley : public SpellScript
{
    ...
    {
        targets.remove_if(Syphrena::UnitAuraCheck(false, SPELL_SHADOW_MARK));
    }
