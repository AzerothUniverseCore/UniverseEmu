...
enum EventPlanarAnomaly
{
    EVENT_PLANAR_BLAST = 1
};
...
struct npc_planar_anomaly : public ScriptedAI
{
    npc_planar_anomaly(Creature* creature) : ScriptedAI(creature)
    {
        Initialized();
    }
    ...
    {
        DoZoneInCombat();
        events.ScheduleEvent(EVENT_PLANAR_BLAST, 15s);

        DoCastSelf(SPELL_PLANAR_DISTORTION);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (events.ExecuteEvent() == EVENT_PLANAR_BLAST)
        {
            me->CastSpell(me, SPELL_PLANAR_BLAST, true);
            me->GetMotionMaster()->Clear();
        }
    }
...
class achievement_gen_eregos_void : public AchievementCriteriaScript
{
 ...
 void AddSC_boss_eregos()
 {
    ...
    RegisterOculusCreatureAI(npc_planar_anomaly);
