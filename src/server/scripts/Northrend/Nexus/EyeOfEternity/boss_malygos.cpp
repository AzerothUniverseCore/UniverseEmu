...
class spell_malygos_arcane_storm : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        if (GetSpellInfo()->Id == SPELL_ARCANE_STORM_P_III)
        {
            ...
            Syphrena::Containers::RandomResize(targets, check, (malygos->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL ? 4 : 10));
        }
        else
            Syphrena::Containers::RandomResize(targets, (malygos->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL ? 4 : 10));
    }
    ...
    void FilterMeleeHoverDiskPassangers(std::list<WorldObject*>& targets)
    {
        ...
        if (targets.size() > 1)
            if (malygos && malygos->AI()->GetGUID(DATA_LAST_TARGET_BARRAGE_GUID))
                targets.remove_if(Syphrena::ObjectGUIDCheck(malygos->AI()->GetGUID(DATA_LAST_TARGET_BARRAGE_GUID)));
        ...
        if (!targets.empty())
            targets.remove_if(Syphrena::UnitAuraCheck(true, SPELL_ARCANE_BARRAGE_DAMAGE));
...
class spell_malygos_surge_of_power_warning_selector_25 : public SpellScript
{
    ...
    void SendThreeTargets(std::list<WorldObject*>& targets)
    {
        ...
        Syphrena::Containers::RandomResize(selectedTargets, 3);
