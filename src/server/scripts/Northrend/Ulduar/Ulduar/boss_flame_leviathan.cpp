...
class spell_pursue : public SpellScriptLoader
{
    public:
        ...
        private:
            ...
            {
                Syphrena::Containers::RandomResize(targets, FlameLeviathanPursuedTargetSelector(), 1);
                if (targets.empty())
                {
                ...
                if (targets.HasTraj())
                    if (Vehicle* vehicle = GetCaster()->GetVehicleKit())
                        if (Unit* passenger = vehicle->GetPassenger(damage - 1))
                        {
                            ...
                            Syphrena::WorldObjectSpellAreaTargetCheck check(99, GetExplTargetDest(), GetCaster(), GetCaster(), GetSpellInfo(), TARGET_CHECK_DEFAULT, nullptr);
                            Syphrena::WorldObjectListSearcher<Syphrena::WorldObjectSpellAreaTargetCheck> searcher(GetCaster(), targetList, check);
