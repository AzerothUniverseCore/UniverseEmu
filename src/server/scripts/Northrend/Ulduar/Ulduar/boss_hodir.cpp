    ...
    public:
        ...
        struct boss_hodirAI : public BossAI
        {
            ...
            void FlashFreeze()
            {
                ...
                Syphrena::AnyUnfriendlyUnitInObjectRangeCheck checker(me, me, 100.0f);
                Syphrena::UnitListSearcher<Syphrena::AnyUnfriendlyUnitInObjectRangeCheck> searcher(me, TargetList, checker);
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            std::list<Unit*> TargetList;
                            Syphrena::AnyFriendlyUnitInObjectRangeCheck checker(me, me, 30.0f);
                            Syphrena::UnitListSearcher<Syphrena::AnyFriendlyUnitInObjectRangeCheck> searcher(me, TargetList, checker);
