        ...
        switch (action)
        {
            ...
            case ACTION_ROTFACE_COMBAT:
                ...
                if (Creature* rotface = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_ROTFACE)))
                {
                    ...
                    if (list.size() > 4)
                    {
                        list.sort(Syphrena::ObjectDistanceOrderPred(rotface));
                        do
                        {
                            list.pop_back();
                        } while (list.size() > 4);
...
class spell_putricide_unbound_plague : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if(Syphrena::UnitAuraCheck(true, sSpellMgr->GetSpellIdForDifficulty(SPELL_UNBOUND_PLAGUE, GetCaster())));
