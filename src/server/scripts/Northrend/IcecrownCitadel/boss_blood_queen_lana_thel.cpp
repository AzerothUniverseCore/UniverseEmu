        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_PACT_OF_THE_DARKFALLEN:
                {
                    std::list<Player*> targets;
                    SelectRandomTarget(false, &targets);
                    Syphrena::Containers::RandomResize(targets, Is25ManRaid() ? 3 : 2);
                    if (targets.size() > 1)
                    {
                ...
                case EVENT_TWILIGHT_BLOODBOLT:
                {
                    std::list<Player*> targets;
                    SelectRandomTarget(false, &targets);
                    Syphrena::Containers::RandomResize(targets, uint32(Is25ManRaid() ? 4 : 2));
                    for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
...
private:
    ...
    Player* SelectRandomTarget(bool includeOfftank, std::list<Player*>* targetList = nullptr)
    {
        ...
        if (includeOfftank)
        {
            tempTargets.sort(Syphrena::ObjectDistanceOrderPred(me->GetVictim()));
            return tempTargets.front();
        }

        return Syphrena::Containers::SelectRandomContainerElement(tempTargets);
    }
...
class spell_blood_queen_bloodbolt : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        Syphrena::Containers::RandomResize(targets, targetCount);
...
class spell_blood_queen_pact_of_the_darkfallen : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if(Syphrena::UnitAuraCheck(false, SPELL_PACT_OF_THE_DARKFALLEN));
...
class spell_blood_queen_pact_of_the_darkfallen_dmg_target : public SpellScript
{
    ...
    {
        unitList.remove_if(Syphrena::UnitAuraCheck(true, SPELL_PACT_OF_THE_DARKFALLEN));
