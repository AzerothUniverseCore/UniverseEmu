...
class spell_sindragosa_unchained_magic : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        for (WorldObject* target : targets)
        {
            ...
            if (Syphrena::Helpers::Entity::IsPlayerHealer(player))
            {
            ...
            switch (player->GetClass())
            {
                ...
                case CLASS_WARLOCK:
                    ...
                    break;
                case CLASS_SHAMAN:
                    if (Syphrena::Helpers::Entity::GetPlayerSpecialization(player) != SPEC_SHAMAN_ENHANCEMENT)
                        casters.push_back(target);
                    break;
                case CLASS_DRUID:
                    if (Syphrena::Helpers::Entity::GetPlayerSpecialization(player) != SPEC_DRUID_FERAL)
                        casters.push_back(target);
                    break;
                default:
                    break;
        ...
        if (!healers.empty())
        {
            Syphrena::Containers::RandomResize(healers, size_t(is25ManRaid ? 3 : 1));
            while (!healers.empty())
            {
        ...
        if (!casters.empty())
        {
            Syphrena::Containers::RandomShuffle(casters);
...
class spell_frostwarden_handler_order_whelp : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if(Syphrena::ObjectTypeIdCheck(TYPEID_PLAYER, false));
        if (targets.empty())
            return;
    ...
    void HandleForcedCast(SpellEffIndex effIndex)
    {
        ...
        Syphrena::Containers::SelectRandomContainerElement(unitList)->CastSpell(GetHitUnit(), uint32(GetEffectValue()), true);
    }
