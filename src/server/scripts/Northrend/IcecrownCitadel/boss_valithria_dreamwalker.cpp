...
class ValithriaDespawner : public BasicEvent
{
    public:
        ...
        {
            Syphrena::CreatureWorker<ValithriaDespawner> worker(_creature, *this);
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_CHECK_PLAYER:
                {
                    ...
                    Syphrena::AnyPlayerInObjectRangeCheck check(me, 5.0f);
...
class spell_dreamwalker_summoner : public SpellScript
{
    ...
    {
        targets.remove_if(Syphrena::UnitAuraCheck(true, SPELL_RECENTLY_SPAWNED));
        if (targets.empty())
            return;
...
class spell_dreamwalker_summon_suppresser : public AuraScript
{
    ...
    void HandleSummon(Unit* caster)
    {
        ...
        for (uint8 i = 0; i < suppresserNumber; ++i)
            caster->CastSpell(Syphrena::Containers::SelectRandomContainerElement(summoners), SPELL_SUMMON_SUPPRESSER, true);
    }
