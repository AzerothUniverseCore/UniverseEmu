...
struct boss_the_lich_king : public BossAI
{
    ...
    void Reset() override
    {
        ...
        Syphrena::GameObjectWorker<FrozenThroneResetWorker> worker(me, reset);
            ...
            switch (eventId)
            {
                ...
                case EVENT_FROSTMOURNE_HEROIC:
                    if (TempSummon* terenas = me->GetMap()->SummonCreature(NPC_TERENAS_MENETHIL_FROSTMOURNE_H, TerenasSpawnHeroic, nullptr, 50000))
                    {
                        ...
                        if (!triggers.empty())
                        {
                            triggers.sort(Syphrena::ObjectDistanceOrderPred(terenas, true));
            ...
            case POINT_CHARGE:
                if (Player* target = ObjectAccessor::GetPlayer(*me, _grabbedPlayer))
                {
                    ...
                    if (GameObject* platform = ObjectAccessor::GetGameObject(*me, _instance->GetGuidData(DATA_ARTHAS_PLATFORM)))
                    {
                        ...
                        triggers.sort(Syphrena::ObjectDistanceOrderPred(me));
...
private:
    ...
    {
        targets.sort(Syphrena::ObjectDistanceOrderPred(GetCaster()));
        if (targets.size() < 2)
            return;
...
class spell_the_lich_king_defile : public SpellScript
{
    ...
    void CorrectRange(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if(Syphrena::UnitAuraCheck(true, SPELL_HARVEST_SOUL_VALKYR));
    }
...
class spell_the_lich_king_valkyr_target_search : public SpellScript
{
    ...
    void SelectTarget(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if(Syphrena::UnitAuraCheck(true, GetSpellInfo()->Id));
        if (targets.empty())
            return;

        _target = Syphrena::Containers::SelectRandomContainerElement(targets);
        targets.clear();
