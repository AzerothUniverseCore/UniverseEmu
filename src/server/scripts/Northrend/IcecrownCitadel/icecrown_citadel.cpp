...
struct npc_icc_orb_controller : public ScriptedAI
{
    ...
    void Reset() override
    {
        _scheduler.Schedule(1s, [this](TaskContext /*initialize*/)
        {
            ...
            Syphrena::CreatureListSearcher<ICCOrbControllerMinionSearch> searcher(me, creatures, check);
    ...
    void ScheduleVisualChannel(bool evading)
    {
        ...
        {
            ObjectGuid guid = Syphrena::Containers::SelectRandomContainerElement(_minionGuids);
    ...
    void SetGUID(ObjectGuid const& guid, int32 id) override
    {
        ...
        if (Unit* minion = ObjectAccessor::GetUnit(*me, Syphrena::Containers::SelectRandomContainerElement(_minionGuids)))
    ...
    void Reset() override
    {
        ...
        .Schedule(1s, 10s, [this](TaskContext emote)
        {
            ...
            if (roll_chance_i(20))
            {
                ...
                if (!creatures.empty())
                {
                    Creature* friendly = Syphrena::Containers::SelectRandomContainerElement(creatures);
                    DoCast(friendly, SPELL_POLYMORPH_ALLY);
                }
            }
            Scheduler.Schedule(1s, [this](TaskContext /*emote*/)
            {
                me->HandleEmoteCommand(Syphrena::Containers::SelectRandomContainerElement(DarkFallensEmotes));
            });
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_DARK_MENDING:
                {
                    ...
                    Unit* target = nullptr;
                    Syphrena::MostHPPercentMissingInRange u_check(me, 40.0f, 1, 75);
                    Syphrena::UnitLastSearcher<Syphrena::MostHPPercentMissingInRange> searcher(me, target, u_check);
...
class spell_darkfallen_blood_mirror : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        _targets = targets;
        Syphrena::Containers::RandomResize(_targets, 2);
    }
...
class spell_icc_sprit_alarm : public SpellScript
{
    ...
    void HandleEvent(SpellEffIndex effIndex)
    {
        ...
        wards.sort(Syphrena::ObjectDistanceOrderPred(GetGObjCaster()));
    ...
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/) override
        {
            ...
            if (instance->GetData(DATA_COLDFLAME_JETS) == NOT_STARTED)
            {
                ...
                traps.sort(Syphrena::ObjectDistanceOrderPred(player));
                bool instant = false;
