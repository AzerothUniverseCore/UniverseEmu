                ...
                default: // Show items you can use
                    {
                        ...
                        if (const ItemTemplate* itemTemplate = sObjectMgr->GetItemTemplate(selection.item))
                        {
                            ...
                            SC_LOG_DEBUG("network", "WORLD: Sent SMSG_LIST_INVENTORY");

                            Creature* vendor = player->GetNPCIfCanInteractWith(creature->GetGUID(), UNIT_NPC_FLAG_VENDOR);
                            if (!vendor)
                            {
                                SC_LOG_DEBUG("network", "WORLD: SendListInventory - Unit (GUID: {}) not found or you can not interact with him.", creature->GetGUID().GetCounter());
        ...
        if (result)
        {
            do
            {
                ...
                else
                {
                    ...
                    // SC_LOG_ERROR("custom.transmog", "Item entry (Entry: {}, itemGUID: {}, playerGUID: {}) does not exist, ignoring.", fakeEntry, GUID_LOPART(itemGUID), player->GetGUID().GetCounter());
...
public:
    ...
    void OnStartup() override
    {
        ...
        for (auto const& v : sObjectMgr->GetItemTemplateStore())
        {
            ...
            {
                SC_LOG_INFO("server.loading", "Too many items for transmogrification: Class: {} SubClass: {} InventoryType: {} Quality: {}", temp.Class, temp.SubClass, TransmogDisplayVendorMgr::getCorrectInvType(temp.InventoryType), temp.Quality);
            }
        }
