...
local function TID(name, reqguid, onetime)
    syphrenaID[name] = {TIDs, reqguid, onetime}
    TIDs = TIDs+1
end
...
function GOMove:Move(ID, input)
    ...
    if(not syphrenaID[ID] or not tonumber(syphrenaID[ID][1])) then
        return
    end
    if(not syphrenaID[ID][2]) then
        SendChatMessage(".gomove "..syphrenaID[ID][1].." "..(0).." "..ARG)
    elseif(syphrenaID[ID][3] and tonumber(ARG) and tonumber(ARG) > 0) then
        SendChatMessage(".gomove "..syphrenaID[ID][1].." "..ARG.." "..(0))
    else
        local did = false
        for GUID, NAME in pairs(GOMove.Selected) do
            if(tonumber(GUID)) then
                SendChatMessage(".gomove "..syphrenaID[ID][1].." "..GUID.." "..ARG)
                if(ID == "GOTO") then
                    return
                end
                did = true
