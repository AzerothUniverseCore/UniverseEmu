...
public:
    ...
    struct npc_1v1arenaAI : public ScriptedAI
    {
        ...
        bool JoinQueueArena(Player* player, Creature* me, bool isRated)
        {
            ...
            if (!bg)
            {
                SC_LOG_ERROR("Arena", "Battleground: template bg (all arenas) not found");
                return false;
            }
        ...
        bool CreateArenateam(Player* player, Creature* me)
        {
            ...
            ChatHandler(player->GetSession()).SendSysMessage("|cffFF4500[Arène 1v1]:|r L'équipe d'arène 1v1 a été créée avec succès !");
        ...
        bool OnGossipHello(Player* player)
        {
            ...
            {
                ChatHandler(player->GetSession()).SendSysMessage("|cffFF4500[Arène 1v1]:|r L'arène 1v1 est désactivée !");
                return true;
            }

            if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Quitter la file", GOSSIP_SENDER_MAIN, 3, "Êtes-vous sûr ?", 0, false);

            if (player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5)) == 0)
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Créer une nouvelle équipe", GOSSIP_SENDER_MAIN, 1, "Êtes-vous sûr ?", config_Costs, false);
            else
            {
                if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5) == false)
                {
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "S'inscrire en arène 1v1", GOSSIP_SENDER_MAIN, 2);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Quitter l'équipe", GOSSIP_SENDER_MAIN, 5, "Êtes-vous sûr ?", 0, false);
                }

                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Voir les statistiques", GOSSIP_SENDER_MAIN, 4);
            }
        ...
        bool OnGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId)
        {
            ...
            case 1: // Create new Arenateam
            {
                ...
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("|cffFF4500[Arène 1v1]:|r Vous devez avoir le niveau {} pour créer une arène 1v1.", config_minLevel);
            ...
            case 2: // Join Queue Arena (rated)
            {
                if (Arena1v1CheckTalents(player) && JoinQueueArena(player, me, true) == false)
                    ChatHandler(player->GetSession()).SendSysMessage("|cffFF4500[Arène 1v1]:|r Quelque chose s'est mal passé en rejoignant la file d'attente.");
            ...
            case 4: // get statistics
            {
                ...
                {
                    std::stringstream s;
                    s << "Cote équipe: " << at->GetStats().Rating;
                    s << "\nGrade: " << at->GetStats().Rank;
                    s << "\nSaison Matchs: " << at->GetStats().SeasonGames;
                    s << "\nSaison Victoire: " << at->GetStats().SeasonWins;
                    s << "\nMatchs de la Semaine: " << at->GetStats().WeekGames;
                    s << "\nVictoire de la Semaine: " << at->GetStats().WeekWins;
            ...
            case 5: // Disband arenateam
            {
                ...
                ChatHandler(player->GetSession()).SendSysMessage("|cffFF4500[Arène 1v1]:|r L'équipe d'arène a été supprimée !");
                CloseGossipMenuFor(player);
                return true;
            }
            break;
