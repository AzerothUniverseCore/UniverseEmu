...
public:
    ...
    struct boss_vizier_jinbakAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            if (Creature* trigger = GetPuddleCreature())
        ...
        void JustEnteredCombat(Unit* /*who*/) override
        {
            ...
            Creature* sapPuddle = GetPuddleCreature();
            ...
            {
                float angle = sapPuddle->GetRelativeAngle(itr); // Need to verify
            ...
            Talk(SAY_AGGRO);
            events.ScheduleEvent(EVENT_SUMMON_GLOBULES, 10s);
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            {
                if (Creature* puddle = GetPuddleCreature())
                {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (visualTimer)
            {
                if (visualTimer <= diff)
                {
                    if (Creature* trigger = me->FindNearestCreature(NPC_SAP_PUDDLE, 20.0f))
                    {
                        DoCast(trigger, SPELL_DETONATE_VISUAL, false);
                        visualTimer = 0;
                    }
                }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_DETONATE:
                    ...
                    events.ScheduleEvent(EVENT_DETONATE, 45500ms);
                    break;
                case EVENT_SUMMON_GLOBULES:
                {
                    ...
                    float minSectorAngle = 100.0f * 3.1415927f / 180.0f;
                    uint8 count = 3;
                    ...
                    for (int i = 0; i < 13; ++i)
                    {
                        ...
                        {
                            // passed the 2xPI
                            lastAngle -= 6.2831853f;
                        }

                        if (fabs(itr->first - lastAngle) > minSectorAngle)
                        {
                            lastAngle = itr->first;
                            if (Creature* trigger = me->FindNearestCreature(NPC_GLOBULE_SUMMON_DEST, itr->second))
                    ...
                    Talk(EMOTE_GLOBULE);
                    events.ScheduleEvent(EVENT_SUMMON_GLOBULES, 45500ms);
        ...
        Creature* GetPuddleCreature()
        {
            return me->FindNearestCreature(NPC_SAP_PUDDLE, 500.0f);
        }
...
public:
    ...
    struct npc_sap_globuleAI : public ScriptedAI
    {
        ...
        {
            if (Creature* jinbak = me->FindNearestCreature(NPC_VIZIER_JINBAK, 500.0f))
        ...
        void DoAction(int32 actionId) override
        {
            if (actionId == 1)
            {
                ...
                if (!growAura)
                {
                    ...
                    // Create an object AuraCreateInfo
                    AuraCreateInfo createInfo(spell, MAX_EFFECT_MASK, me);
                    createInfo.SetCaster(me);

                    int32 tempSolution = 1;
                    createInfo.SetBaseAmount(&tempSolution);

                    growAura = Aura::TryCreate(createInfo);
                }
    ...
    class spell_jinbak_detonate_AuraScript : public AuraScript
    {
        ...
        void OnTick(AuraEffect const* /*aurEff*/)
        {
            if (Unit* target = GetTarget())
            {
                ...
                target->CastSpell(target, SPELL_DETONATE_DAMAGE, &damage); // Need to verify
