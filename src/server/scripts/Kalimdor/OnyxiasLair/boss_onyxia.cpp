            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_BELLOWING_ROAR: // Phase PHASE_END
                    {
                        ...
                        Syphrena::GameObjectInRangeCheck check(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 15);
                        Syphrena::GameObjectLastSearcher<Syphrena::GameObjectInRangeCheck> searcher(me, Floor, check);
