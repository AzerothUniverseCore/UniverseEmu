...
public:
    ...
    struct npc_arthas_stratholmeAI : public ScriptedAI
    {
        ...
        {
            SC_LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::AdvanceToState: advancing to 0x{:X}", newState);
            if (!_progressRP)
            {
                SC_LOG_WARN("scripts.cos", "npc_arthas_stratholmeAI::AdvanceToState: advancing to instance state 0x{:X}, but RP is paused. Overriding!", newState);
                _progressRP = true;
            }
            ...
            if (itr != ArthasSnapbackPositions.end())
            {
                ...
                SC_LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::AdvanceToState: has snapback for this state, distance = {}", target.SnapbackPosition->GetExactDist(me));
                ...
                switch (event)
                {
                    ...
                    case RP2_EVENT_REACT5:
                    {
                        ...
                        if (!nearbyVictims.empty())
                        {
                            ...
                            Creature* victim = Syphrena::Containers::SelectRandomContainerElement(nearbyVictims);
                            if (victim->IsAlive())
                                victim->HandleEmoteCommand(Syphrena::Containers::SelectRandomContainerElement(emotes));
                        }
                        break;
                    }
        ...
        {
            SC_LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustEngagedWith: RP in progress? '{}'", _progressRP ? "YES" : "NO");
            if (_progressRP)
            {
                _progressRP = false;
                ...
                if (!_resumeMovement.Empty())
                    SC_LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustEngagedWith: spline chain motion paused");
                else
                    SC_LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustEngagedWith: entered combat without pathing, pausing RP regardless");
            }
        ...
        {
            SC_LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::EnterEvadeMode: why = {} ", why);
        ...
        void JustReachedHome() override
        {
            ...
            {
                SC_LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustReachedHome: Resuming motion");
                me->GetMotionMaster()->ResumeSplineChain(_resumeMovement);
                _resumeMovement.Clear();
            }
            else
                SC_LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustReachedHome: Back at leash pos, resuming RP");
        ...
        bool OnGossipSelect(Player* player, uint32 /*sender*/, uint32 listId) override
        {
            ...
            SC_LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::GossipSelect: '{}' selects action '{}' on '{}'", player->GetGUID().ToString(), action, me->GetGUID().ToString());
