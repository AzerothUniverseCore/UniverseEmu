...
void hyjalAI::SummonNextWave(const Wave wave[18], uint32 Count, float Base[4][3])
{
    ...
    if (!wave[Count].IsBoss)
    {
        ...
        {
            NextWaveTimer = 15000;
            SC_LOG_DEBUG("scripts", "HyjalAI: debug mode is enabled. Next Wave in 15 seconds");
        }
    }
    else
    {
...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    Syphrena::AllFriendlyCreaturesInGrid creature_check(me);
    Syphrena::CreatureListSearcher<Syphrena::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    Syphrena::WorldObjectWorker<Syphrena::RespawnDo> worker(me, u_do);
