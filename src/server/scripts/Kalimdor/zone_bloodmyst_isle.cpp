        ...
        void WaypointReached(uint32 waypointId, uint32 /*pathId*/) override
        {
            ...
            switch (waypointId)
            {
                ...
                case WP_SIRONAS_HILL:
                {
                    ...
                    Syphrena::AllCreaturesOfEntryInRange check(me, NPC_SIRONAS, SIZE_OF_GRIDS);
                    Syphrena::CreatureSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher(me, sironas, check);
...
class spell_free_webbed : public SpellScript
{
    ...
    {
        GetCaster()->CastSpell(GetCaster(), Syphrena::Containers::SelectRandomContainerElement(CocoonSummonSpells), true);
    }
...
class spell_free_webbed_on_quest : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (roll_chance_i(66))
            caster->CastSpell(caster, Syphrena::Containers::SelectRandomContainerElement(CocoonSummonSpells), true);
        else
