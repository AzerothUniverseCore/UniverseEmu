        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_START:
                {
                    ...
                    {
                        Syphrena::AllCreaturesOfEntryInRange pred(me, entry);
                        Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher(me, dummies, pred);
...
struct npc_infernal_rain_hellfire : public ScriptedAI
{
    ...
    void RebuildTargetList()
    {
        ...
        Syphrena::AllCreaturesOfEntryInRange pred(me, NPC_INFERNAL_RAIN);
        Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher(me, others, pred);
        Cell::VisitAllObjects(me, searcher, 500.0f);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_CAST:
                {
                    if (Creature* target = ObjectAccessor::GetCreature(*me, Syphrena::Containers::SelectRandomContainerElement(_targets)))
                    {
