...
static uint32 GetRandomBossExcept(uint32 exception)
{
    ...
    return Syphrena::Containers::SelectRandomContainerElement(bossData);
}
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            case EVENT_BLESS:
            {
                ...
                Syphrena::AnyFriendlyUnitInObjectRangeCheck checker(me, me, 100.0f);
                ...
                if (!TargetList.empty())
                {
                    Unit* target = Syphrena::Containers::SelectRandomContainerElement(TargetList);
