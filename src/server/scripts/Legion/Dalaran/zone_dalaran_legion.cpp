...
struct npc_mageguard_dalaran_legion : public ScriptedAI
{
    npc_mageguard_dalaran_legion(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_minigob_manabonk_legion : public ScriptedAI
{
    npc_minigob_manabonk_legion(Creature* creature) : ScriptedAI(creature)
    {
        me->setActive(true);
    }
    ...
    void GetPlayersInDalaranLegion(std::vector<Player*>& playerList) const
    {
        Map::PlayerList const& players = me->GetMap()->GetPlayers();
        for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
            if (Player* player = itr->GetSource()->ToPlayer())
                if (player->GetZoneId() == ZONE_DALARAN_LEGION && !player->IsFlying() && !player->IsMounted() && !player->IsGameMaster())
                    playerList.push_back(player);
    }

    static Player* SelectTargetInDalaranLegion(std::vector<Player*>& PlayerInDalaranLegionList)
    {
        if (PlayerInDalaranLegionList.empty())
            return nullptr;

        return Syphrena::Containers::SelectRandomContainerElement(PlayerInDalaranLegionList);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                {
                    std::vector<Player*> PlayerInDalaranLegionList;
                    GetPlayersInDalaranLegion(PlayerInDalaranLegionList);

                    // Increases chance of event based on player count in Dalaran Legion (100 players or more = 100% else player count%)
                    if (PlayerInDalaranLegionList.empty() || urand(1, 100) > PlayerInDalaranLegionList.size())
                        me->AddObjectToRemoveList();

                    me->SetVisible(true);
                    DoCastSelf(SPELL_TELEPORT_VISUAL);
                    if (Player* player = SelectTargetInDalaranLegion(PlayerInDalaranLegionList))
                    {
...
{
    RegisterCreatureAI(npc_mageguard_dalaran_legion);
    RegisterCreatureAI(npc_minigob_manabonk_legion);
}
