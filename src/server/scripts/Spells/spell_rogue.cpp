        ...
        if (Unit* target = GetHitUnit())
        {
            ...
            for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot)
            {
                ...
                for (uint8 s = 0; s < 3; ++s)
                {
                    ...
                    if (!spellInfo)
                    {
                        SC_LOG_ERROR("spells", "Player::CastItemCombatSpell Enchant {}, player (Name: {}, {}) cast unknown spell {}", enchant->ID, player->GetName(), player->GetGUID().ToString(), enchant->EffectArg[s]);
                        continue;
                    }
    ...
    public:
        ...
        class spell_rog_killing_spree_AuraScript : public AuraScript
        {
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
            {
                ...
                {
                    ObjectGuid guid = Syphrena::Containers::SelectRandomContainerElement(_targets);
