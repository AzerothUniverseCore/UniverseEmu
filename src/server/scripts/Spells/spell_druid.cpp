...
class spell_dru_glyph_of_innervate : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        int32 amount = CalculatePct(static_cast<int32>(caster->GetMaxPower(POWER_MANA)), aurEff->GetAmount());
...
class spell_dru_t10_restoration_4p_bonus : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        if (Creature* bot = GetCaster()->ToCreature())
        {
            ...
            Unit* target = Syphrena::Containers::SelectRandomContainerElement(tempTargets);
            targets.clear();
