...
class spell_gen_arena_drink : public AuraScript
{
    ...
    bool Validate(SpellInfo const* spellInfo) override
    {
        ...
        {
            SC_LOG_ERROR("spells", "Aura {} structure has been changed - first aura is no longer SPELL_AURA_MOD_POWER_REGEN", GetId());
            return false;
        }
...
class spell_gen_cannibalize : public SpellScript
{
    ...
    SpellCastResult CheckIfCorpseNear()
    {
        ...
        Syphrena::AnyDeadUnitSpellTargetInRangeCheck check(caster, max_range, GetSpellInfo(), TARGET_CHECK_ENEMY);
        Syphrena::WorldObjectSearcher<Syphrena::AnyDeadUnitSpellTargetInRangeCheck> searcher(caster, result, check);
...
class spell_gen_gift_of_naaru : public AuraScript
{
    ...
    void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
    {
        ...
        case SPELLFAMILY_KNIGHT:
        case SPELLFAMILY_DEMONHUNTER:
        case SPELLFAMILY_MONK:
        case SPELLFAMILY_HERO:
...
enum Mounts
{
    ...
    SPELL_LIGHTFORGED_60 = 190021,
    SPELL_LIGHTFORGED_100 = 190022,
    SPELL_LIGHTFORGED_150 = 190023,
    SPELL_LIGHTFORGED_310 = 190025,
...
class spell_gen_replenishment : public SpellScript
{
    ...
    void RemoveInvalidTargets(std::list<WorldObject*>& targets)
    {
        ...
        {
            targets.sort(Syphrena::PowerPctOrderPred(POWER_MANA));
...
class spell_gen_spectator_cheer_trigger : public SpellScript
{
    ...
    {
        if (roll_chance_i(40))
            GetCaster()->HandleEmoteCommand(Syphrena::Containers::SelectRandomContainerElement(EmoteArray));
    }
    ...
    void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
    {
        if (GetCaster()->HasAura(SPELL_MIXOLOGY) && GetCaster()->HasSpell(GetEffectInfo(EFFECT_0).TriggerSpell))
        {
            ...
            case SPELL_ELIXIR_OF_GIANTH_GROWTH:
                ...
                break;
            default:
                SC_LOG_ERROR("spells", "SpellId {} couldn't be processed in spell_gen_mixology_bonus", GetId());
                break;
            }
            amount += bonus;
        }
...
class spell_corrupting_plague_aura : public AuraScript
{
    ...
    void OnPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        Syphrena::CreatureListSearcher<CorruptingPlagueSearcher> creature_searcher(owner, targets, creature_check);
...
class spell_stasis_field_aura : public AuraScript
{
    ...
    void OnPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        Syphrena::CreatureListSearcher<StasisFieldSearcher> creature_searcher(owner, targets, creature_check);
        Cell::VisitGridObjects(owner, creature_searcher, 15.0f);
...
void AddSC_generic_spell_scripts()
{
    ...
    RegisterSpellScriptWithArgs(spell_gen_mount, "spell_lightforged", 0, SPELL_LIGHTFORGED_60, SPELL_LIGHTFORGED_100, SPELL_LIGHTFORGED_150, SPELL_LIGHTFORGED_310);
