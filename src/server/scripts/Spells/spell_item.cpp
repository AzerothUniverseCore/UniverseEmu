...
class spell_item_aura_of_madness : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        PreventDefaultAction();
        Unit* caster = eventInfo.GetActor();
        uint32 spellId = Syphrena::Containers::SelectRandomContainerElement(triggeredSpells[caster->GetClass()]);
        ...
        class spell_item_deathbringers_will_AuraScript : public AuraScript
        {
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                static std::vector<uint32> const triggeredSpells[MAX_CLASSES] =
                {
                    ...
                    { Strength, Critical, Haste },
                    //CLASS_HERO
                    { Strength, Critical, Haste, AttackPower },
                    //CLASS_EVOKER
                    { },
                    //CLASS_CHRONOMANCER
                    { Agility, Haste, AttackPower }
                ...
                uint32 spellId = Syphrena::Containers::SelectRandomContainerElement(randomSpells);
                caster->CastSpell(caster, spellId, aurEff);
            }
...
enum DireBrew
{
    ...
    MODEL_CLASS_RUBIS_MALE         = 25237,
    MODEL_CLASS_RUBIS_FEMALE       = 25238
};
...
class spell_item_echoes_of_light : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.sort(Syphrena::HealthPctOrderPred());
...
class spell_item_mingos_fortune_generator : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        GetCaster()->CastSpell(GetCaster(), Syphrena::Containers::SelectRandomContainerElement(CreateFortuneSpells), true);
    }
...
enum Shadowmournerework
{
    SPELL_SHADOWMOURNEREWORK_CHAOS_BANE_DAMAGE = 2071904,
    SPELL_SHADOWMOURNEREWORK_SOUL_FRAGMENT = 2071905,
    SPELL_SHADOWMOURNEREWORK_VISUAL_LOW = 72521,
    SPELL_SHADOWMOURNEREWORK_VISUAL_HIGH = 72523,
    SPELL_SHADOWMOURNEREWORK_CHAOS_BANE_BUFF = 2073422,
};

// 71903 - Item - Shadowmourne Legendary
class spell_item_shadowmournerework : public AuraScript
{
    PrepareAuraScript(spell_item_shadowmournerework);

    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
            {
                SPELL_SHADOWMOURNEREWORK_CHAOS_BANE_DAMAGE,
                SPELL_SHADOWMOURNEREWORK_SOUL_FRAGMENT,
                SPELL_SHADOWMOURNEREWORK_CHAOS_BANE_BUFF
            });
    }

    bool CheckProc(ProcEventInfo& eventInfo)
    {
        if (GetTarget()->HasAura(SPELL_SHADOWMOURNEREWORK_CHAOS_BANE_BUFF)) // cant collect shards while under effect of Chaos Bane buff
            return false;
        return eventInfo.GetProcTarget() && eventInfo.GetProcTarget()->IsAlive();
    }
    ...
    {
        PreventDefaultAction();
        GetTarget()->CastSpell(GetTarget(), SPELL_SHADOWMOURNEREWORK_SOUL_FRAGMENT, aurEff);

        // this can't be handled in AuraScript of SoulFragments because we need to know victim
        if (Aura* soulFragments = GetTarget()->GetAura(SPELL_SHADOWMOURNEREWORK_SOUL_FRAGMENT))
        {
            if (soulFragments->GetStackAmount() >= 10)
            {
                GetTarget()->CastSpell(eventInfo.GetProcTarget(), SPELL_SHADOWMOURNEREWORK_CHAOS_BANE_DAMAGE, aurEff);
                soulFragments->Remove();
            }
        }
    }
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        GetTarget()->RemoveAurasDueToSpell(SPELL_SHADOWMOURNEREWORK_SOUL_FRAGMENT);
    }

    void Register() override
    {
        DoCheckProc += AuraCheckProcFn(spell_item_shadowmournerework::CheckProc);
        OnEffectProc += AuraEffectProcFn(spell_item_shadowmournerework::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
        AfterEffectRemove += AuraEffectRemoveFn(spell_item_shadowmournerework::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};

// 71905 - Soul Fragment
class spell_item_shadowmournerework_soul_fragment : public AuraScript
{
    PrepareAuraScript(spell_item_shadowmournerework_soul_fragment);

    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
            {
                SPELL_SHADOWMOURNEREWORK_VISUAL_LOW,
                SPELL_SHADOWMOURNEREWORK_VISUAL_HIGH,
                SPELL_SHADOWMOURNEREWORK_CHAOS_BANE_BUFF
            });
    }
    ...
    void OnStackChange(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        {
        case 1:
            target->CastSpell(target, SPELL_SHADOWMOURNEREWORK_VISUAL_LOW, true);
            break;
        case 6:
            target->RemoveAurasDueToSpell(SPELL_SHADOWMOURNEREWORK_VISUAL_LOW);
            target->CastSpell(target, SPELL_SHADOWMOURNEREWORK_VISUAL_HIGH, true);
            break;
        case 10:
            target->RemoveAurasDueToSpell(SPELL_SHADOWMOURNEREWORK_VISUAL_HIGH);
            target->CastSpell(target, SPELL_SHADOWMOURNEREWORK_CHAOS_BANE_BUFF, true);
            break;
        default:
            break;
        }
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_item_shadowmournerework_soul_fragment::OnStackChange, EFFECT_0, SPELL_AURA_MOD_STAT, AuraEffectHandleModes(AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY));
        AfterEffectRemove += AuraEffectRemoveFn(spell_item_shadowmournerework_soul_fragment::OnRemove, EFFECT_0, SPELL_AURA_MOD_STAT, AURA_EFFECT_HANDLE_REAL);
    }
};
...
class spell_item_taunt_flag_targeting : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        Syphrena::Containers::RandomResize(targets, 1);
    }
...
class spell_item_mad_alchemists_potion : public SpellScript
{
    ...
    void SecondaryEffect()
    {
        ...
        uint32 chosenElixir = Syphrena::Containers::SelectRandomContainerElement(availableElixirs);
...
void AddSC_item_spell_scripts()
{
    ...
    RegisterSpellScript(spell_item_shadowmournerework);
    RegisterSpellScript(spell_item_shadowmournerework_soul_fragment);
