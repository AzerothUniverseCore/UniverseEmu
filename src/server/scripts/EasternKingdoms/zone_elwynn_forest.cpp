    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            ...
            case EVENT_GIVE_RIGHT_CLICK_INSTRUCTIONS:
            {
                ...
                Syphrena::AnyPlayerInObjectRangeCheck checker(me, 50.0f);
                Syphrena::PlayerListSearcher<Syphrena::AnyPlayerInObjectRangeCheck> searcher(me, players, checker);
...
struct npc_cameron : public ScriptedAI
{
    ...
    void MoveTheChildren()
    {
        ...
        Syphrena::Containers::RandomShuffle(MovePosPositions);
