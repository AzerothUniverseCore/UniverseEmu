        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_SUMMON_SOLAR_POWER:
                {
                    ...
                    if (Player* player = GetPlayer())
                    {
                        WorldLocation targetLoc(player->GetMapId(), x, y, pos.GetPositionZ() + 50.0f);
                        CastSpellTargetArg targets(targetLoc);
                        CastSpellExtraArgs args;
                        args.TriggerFlags = TRIGGERED_FULL_MASK;

                        player->CastSpell(targets, SPELL_CREATE_RANDOM_SUN_POWER, args);
                    }
                ...
                case EVENT_CREATE_GHOUL:
                {
                    ...
                    if (Player* player = GetPlayer())
                    {
                        WorldLocation targetLoc(player->GetMapId(), pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), angle);
                        CastSpellTargetArg targets(targetLoc);
                        CastSpellExtraArgs args;
                        args.TriggerFlags = TRIGGERED_FULL_MASK;

                        player->CastSpell(targets, spellId, args);
                    }
...
private:
    ...
    void EnterNextStage()
    {
        ...
        if (Player* player = GetPlayer())
            player->ModifyPower(POWER_ALL, 1);
    }
...
struct npc_brazie_fertilitize_o_tron_2000 : public ScriptedAI
{
    ...
    void OnIsSummonedBy(Unit* summoner)
    {
        me->SetDisplayId(1);
...
class spell_brazie_summon_plant : public SpellScript
{
    ...
    {
        (&spell_brazie_summon_plant::CheckLocation);
        (&spell_brazie_summon_plant::HandleDummy, EFFECT_1, SPELL_EFFECT_DUMMY);
        (&spell_brazie_summon_plant::PreventEffect, EFFECT_0, SPELL_EFFECT_TRANS_DOOR);
    }
};
...
class spell_brazie_highlight : public SpellScript
{
    ...
    void Register() override
    {
        (&spell_brazie_highlight::SetDest, EFFECT_1, TARGET_DEST_NEARBY_ENTRY);
    }
};
...
class spell_brazie_create_random_seed_sack : public SpellScript
{
    ...
    void Register() override
    {
        (&spell_brazie_create_random_seed_sack::SetDest, EFFECT_0, TARGET_DEST_NEARBY_ENTRY);
    }
};
...
class spell_brazie_spit : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        {
            targets.sort(Syphrena::ObjectDistanceOrderPred(GetCaster(), true));
            targets.resize(1);
        }
        else
        {
            std::list<WorldObject*> targetsBackup = targets;
            targetsBackup.remove_if(Syphrena::UnitAuraCheck(true, SPELL_FREEZYA_BLAST));
    ...
    void Register() override
    {
        (&spell_brazie_spit::FilterTargets, EFFECT_ALL, TARGET_UNIT_CONE_ENEMY_24);
    }
};
}
