...
public:
    ...
    static bool HandleDebugSendOpcodeCommand(ChatHandler* handler)
    {
        ...
        while (!parsedStream.eof() && !parsedStream.fail())
        {
            ...
            {
                SC_LOG_ERROR("misc", "Sending opcode that has unknown type '{}'", type);
                break;
            }
        }
        SC_LOG_DEBUG("network", "Sending opcode {}", data.GetOpcode());
        data.hexlike();
    ...
    static bool HandleDebugEnterVehicleCommand(ChatHandler* handler, uint32 entry, Optional<int8> seatId)
    {
        ...
        else
        {
            ...
            Syphrena::AllCreaturesOfEntryInRange check(handler->GetPlayer(), entry, 20.0f);
            Syphrena::CreatureSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher(handler->GetPlayer(), passenger, check);
    ...
    static bool HandleWPGPSCommand(ChatHandler* handler)
    {
        ...
        SC_LOG_INFO("sql.dev", "(@PATH, XX, {:.3f}, {:.3f}, {:.5f}, {:.5f}, 0, 0, 0, 100, 0),", player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), player->GetOrientation());
