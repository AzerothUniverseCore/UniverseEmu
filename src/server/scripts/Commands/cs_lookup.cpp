    ...
    static bool HandleLookupEventCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < events.size(); ++id)
        {
            ...
            if (Utf8FitTo(descr, wNamePart))
            {
                ...
                char const* active = activeEvents.find(id) != activeEvents.end() ? handler->GetSyphrenaString(LANG_ACTIVE) : "";
            ...
            if (factionEntry)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (factionState) // and then target != NULL also
                    {
                        ...
                        std::string rankName = handler->GetSyphrenaString(index);

                        ss << ' ' << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ')';

                        if (factionState->Flags & FACTION_FLAG_VISIBLE)
                            ss << handler->GetSyphrenaString(LANG_FACTION_VISIBLE);
                        if (factionState->Flags & FACTION_FLAG_AT_WAR)
                            ss << handler->GetSyphrenaString(LANG_FACTION_ATWAR);
                        if (factionState->Flags & FACTION_FLAG_PEACE_FORCED)
                            ss << handler->GetSyphrenaString(LANG_FACTION_PEACE_FORCED);
                        if (factionState->Flags & FACTION_FLAG_HIDDEN)
                            ss << handler->GetSyphrenaString(LANG_FACTION_HIDDEN);
                        if (factionState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                            ss << handler->GetSyphrenaString(LANG_FACTION_INVISIBLE_FORCED);
                        if (factionState->Flags & FACTION_FLAG_INACTIVE)
                            ss << handler->GetSyphrenaString(LANG_FACTION_INACTIVE);
                    }
                    else
                        ss << handler->GetSyphrenaString(LANG_FACTION_NOREPUTATION);
                        ...
                        if (target)
                        {
                            switch (target->GetQuestStatus(questTemplatePair.first))
                            {
                                case QUEST_STATUS_COMPLETE:
                                    statusStr = handler->GetSyphrenaString(LANG_COMMAND_QUEST_COMPLETE);
                                    break;
                                case QUEST_STATUS_INCOMPLETE:
                                    statusStr = handler->GetSyphrenaString(LANG_COMMAND_QUEST_ACTIVE);
                                    break;
                                case QUEST_STATUS_REWARDED:
                                    statusStr = handler->GetSyphrenaString(LANG_COMMAND_QUEST_REWARDED);
                                    break;
                                default:
                                    break;
                            }
        ...
        for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
        {
            ...
            if (skillInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    {
                        knownStr = handler->GetSyphrenaString(LANG_KNOWN);
                        uint32 curValue = target->GetPureSkillValue(id);
                        uint32 maxValue  = target->GetPureMaxSkillValue(id);
                        uint32 permValue = target->GetSkillPermBonusValue(id);
                        uint32 tempValue = target->GetSkillTempBonusValue(id);

                        char const* valFormat = handler->GetSyphrenaString(LANG_SKILL_VALUES);
        ...
        for (uint32 id = 0; id < sSpellMgr->GetSpellInfoStoreSize(); ++id)
        {
            ...
            if (spellInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (rank)
                        ss << handler->GetSyphrenaString(LANG_SPELL_RANK) << rank;
                    ...
                    if (talent)
                        ss << handler->GetSyphrenaString(LANG_TALENT);
                    if (passive)
                        ss << handler->GetSyphrenaString(LANG_PASSIVE);
                    if (learn)
                        ss << handler->GetSyphrenaString(LANG_LEARN);
                    if (known)
                        ss << handler->GetSyphrenaString(LANG_KNOWN);
                    if (active)
                        ss << handler->GetSyphrenaString(LANG_ACTIVE);
        ...
        for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
        {
            ...
            if (titleInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    char const* knownStr = target && target->HasTitle(titleInfo) ? handler->GetSyphrenaString(LANG_KNOWN) : "";

                    char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->MaskID
                        ? handler->GetSyphrenaString(LANG_ACTIVE)
                        : "";
            ...
            if (MapEntry const* mapInfo = sMapStore.LookupEntry(id))
            {
                ...
                if (Utf8FitTo(name, wNamePart) && locale < TOTAL_LOCALES)
                {
                    ...
                    if (mapInfo->IsContinent())
                        ss << handler->GetSyphrenaString(LANG_CONTINENT);

                    switch (mapInfo->InstanceType)
                    {
                        case MAP_INSTANCE:
                            ss << handler->GetSyphrenaString(LANG_INSTANCE);
                            break;
                        case MAP_RAID:
                            ss << handler->GetSyphrenaString(LANG_RAID);
                            break;
                        case MAP_BATTLEGROUND:
                            ss << handler->GetSyphrenaString(LANG_BATTLEGROUND);
                            break;
                        case MAP_ARENA:
                            ss << handler->GetSyphrenaString(LANG_ARENA);
                            break;
                    }
        ...
        do
        {
            ...
            if (result2)
            {
                ...
                do
                {
                    ...
                    handler->PSendSysMessage(LANG_LOOKUP_PLAYER_CHARACTER, name.c_str(), guid, online ? handler->GetSyphrenaString(LANG_ONLINE) : "");
                    ++counter;
                }
