...
public:
    ...
    static void NotifyModification(ChatHandler* handler, Unit* target, SyphrenaStrings resourceMessage, SyphrenaStrings resourceReportMessage, Args&&... args)
    {
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (strchr(args, 'g') || strchr(args, 's') || strchr(args, 'c'))
            ...
            moneyToAddO = Syphrena::StringTo<int64>(args);
        ...
        if (moneyToAdd < 0)
        {
            ...
            SC_LOG_DEBUG("misc", "{}", handler->PGetParseString(LANG_CURRENT_MONEY, targetMoney, moneyToAdd, newmoney));
            if (newmoney <= 0)
            {
    ...
    static bool HandleModifyRepCommand(ChatHandler* handler, char const* args)
    {
        ...
        if ((amount == 0) && (rankTxt[0] != '-') && !isdigit((unsigned char)rankTxt[0]))
        {
            ...
            {
                std::string rank = handler->GetSyphrenaString(ReputationRankStrIndex[r]);
                if (rank.empty())
                    continue;
