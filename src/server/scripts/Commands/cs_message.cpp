...
public:
    ...
    static bool HandleNotifyCommand(ChatHandler* handler, Tail message)
    {
        ...
        std::string str = handler->GetSyphrenaString(LANG_GLOBAL_NOTIFY);
        str += message;
    ...
    static bool HandleGMNotifyCommand(ChatHandler* handler, Tail message)
    {
        ...
        std::string str = handler->GetSyphrenaString(LANG_GM_NOTIFY);
        str += message;
    ...
    static bool HandleWhispersCommand(ChatHandler* handler, Optional<Variant<bool, EXACT_SEQUENCE("remove")>> operationArg, Optional<std::string> playerNameArg)
    {
        if (!operationArg)
        {
            handler->PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, handler->GetSession()->GetPlayer()->isAcceptWhispers() ?  handler->GetSyphrenaString(LANG_ON) : handler->GetSyphrenaString(LANG_OFF));
            return true;
        }
