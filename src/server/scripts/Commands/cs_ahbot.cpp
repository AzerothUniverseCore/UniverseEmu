...
public:
    ...
    static bool HandleAHBotItemsAmountCommand(ChatHandler* handler, std::array<uint32, MAX_AUCTION_QUALITY> items)
    {
        ...
        for (AuctionQuality quality : EnumUtils::Iterate<AuctionQuality>())
            handler->PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, handler->GetSyphrenaString(ahbotQualityLangIds.at(quality)), sAuctionBotConfig->GetConfigItemQualityAmount(quality));
    ...
    static bool HandleAHBotItemsAmountQualityCommand(ChatHandler* handler, uint32 amount)
    {
        ...
        handler->PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, handler->GetSyphrenaString(ahbotQualityLangIds.at(Q)),
    ...
    static bool HandleAHBotStatusCommand(ChatHandler* handler, Optional<EXACT_SEQUENCE("all")> all)
    {
        ...
        handler->PSendSysMessage(fmtId, handler->GetSyphrenaString(LANG_AHBOT_STATUS_ITEM_COUNT),
        ...
        if (all)
        {
            handler->PSendSysMessage(fmtId, handler->GetSyphrenaString(LANG_AHBOT_STATUS_ITEM_RATIO),
            ...
            for (AuctionQuality quality : EnumUtils::Iterate<AuctionQuality>())
                handler->PSendSysMessage(fmtId, handler->GetSyphrenaString(ahbotQualityLangIds.at(quality)),
