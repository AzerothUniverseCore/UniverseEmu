...
public:
    ...
    static bool HandleRBACPermListCommand(ChatHandler* handler, Optional<AccountIdentifier> account)
    {
        ...
        if (granted.empty())
            handler->PSendSysMessage("%s", handler->GetSyphrenaString(LANG_RBAC_LIST_EMPTY));
        else
        {
    ...
    static bool HandleRBACListPermissionsCommand(ChatHandler* handler, Optional<uint32> permId)
    {
        if (!permId)
        {
            ...
            handler->PSendSysMessage("%s", handler->GetSyphrenaString(LANG_RBAC_LIST_PERMISSIONS_HEADER));
        ...
        else
        {
            ...
            handler->PSendSysMessage("%s", handler->GetSyphrenaString(LANG_RBAC_LIST_PERMISSIONS_HEADER));
            handler->PSendSysMessage(LANG_RBAC_LIST_ELEMENT, permission->GetId(), permission->GetName());
            handler->PSendSysMessage("%s", handler->GetSyphrenaString(LANG_RBAC_LIST_PERMS_LINKED_HEADER));
