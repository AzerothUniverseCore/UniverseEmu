...
public:
    ...
    static bool HandleAccount2FASetupCommand(ChatHandler* handler, Optional<uint32> token)
    {
        ...
        { // check if 2FA already enabled
            ...
            if (!result)
            {
                SC_LOG_ERROR("misc", "Account {} not found in login database when processing .account 2fa setup command.", accountId);
        ...
        static std::unordered_map<uint32, Syphrena::Crypto::TOTP::Secret> suggestions;
        auto pair = suggestions.emplace(std::piecewise_construct, std::make_tuple(accountId), std::make_tuple(Syphrena::Crypto::TOTP::RECOMMENDED_SECRET_LENGTH)); // std::vector 1-argument size_t constructor invokes resize
        if (pair.second) // no suggestion yet, generate random secret
            Syphrena::Crypto::GetRandomBytes(pair.first->second);

        if (!pair.second && token) // suggestion already existed and token specified - validate
        {
            if (Syphrena::Crypto::TOTP::ValidateToken(pair.first->second, *token))
            {
                if (masterKey)
                    Syphrena::Crypto::AEEncryptWithRandomIV<Syphrena::Crypto::AES>(pair.first->second, *masterKey);
        ...
        handler->PSendSysMessage(LANG_2FA_SECRET_SUGGESTION, Syphrena::Encoding::Base32::Encode(pair.first->second));
        handler->SetSentErrorMessage(true);
        return false;
    }
    ...
    static bool HandleAccount2FARemoveCommand(ChatHandler* handler, Optional<uint32> token)
    {
        ...
        uint32 const accountId = handler->GetSession()->GetAccountId();
        Syphrena::Crypto::TOTP::Secret secret;
        ...
        if (token)
        {
            if (masterKey)
            {
                bool success = Syphrena::Crypto::AEDecrypt<Syphrena::Crypto::AES>(secret, *masterKey);
                if (!success)
                {
                    SC_LOG_ERROR("misc", "Account {} has invalid ciphertext in TOTP token.", accountId);
                    handler->SendSysMessage(LANG_UNKNOWN_ERROR);
                    handler->SetSentErrorMessage(true);
                    return false;
                }
            ...
            if (Syphrena::Crypto::TOTP::ValidateToken(secret, *token))
            {
                LoginDatabasePreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_ACCOUNT_TOTP_SECRET);
                stmt->setNull(0);
                stmt->setUInt32(1, accountId);
    ...
    static bool HandleAccountCreateCommand(ChatHandler* handler, std::string const& accountName, std::string const& password, Optional<std::string> const& email)
    {
        ...
        switch (sAccountMgr->CreateAccount(accountName, password, email.value_or("")))
        {
            case AccountOpResult::AOR_OK:
                ...
                {
                    SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Character:[{}] {}) created Account {} (Email: '{}')",
    ...
    static bool HandleAccountEmailCommand(ChatHandler* handler, std::string const& oldEmail, std::string const& password, std::string const& email, std::string const& emailConfirm)
    {
        if (!AccountMgr::CheckEmail(handler->GetSession()->GetAccountId(), oldEmail))
        {
            ...
            handler->SetSentErrorMessage(true);
            SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Character:[{}] {} Tried to change email, but the provided email [{}] is not equal to registration email [{}].",
                handler->GetSession()->GetAccountId(), handler->GetSession()->GetRemoteAddress(),
                handler->GetSession()->GetPlayer()->GetName(), handler->GetSession()->GetPlayer()->GetGUID().ToString(),
                email, oldEmail);
            return false;
        ...
        if (!AccountMgr::CheckPassword(handler->GetSession()->GetAccountId(), password))
        {
            ...
            sScriptMgr->OnFailedEmailChange(handler->GetSession()->GetAccountId());
            handler->SetSentErrorMessage(true);
            SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Character:[{}] {} Tried to change email, but the provided password is wrong.",
                handler->GetSession()->GetAccountId(), handler->GetSession()->GetRemoteAddress(),
                handler->GetSession()->GetPlayer()->GetName(), handler->GetSession()->GetPlayer()->GetGUID().ToString());
            return false;
        }
        ...
        if (email != emailConfirm)
        {
            ...
            sScriptMgr->OnFailedEmailChange(handler->GetSession()->GetAccountId());
            handler->SetSentErrorMessage(true);
            SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Character:[{}] {} Tried to change email, but the confirm email does not match.",
                handler->GetSession()->GetAccountId(), handler->GetSession()->GetRemoteAddress(),
                handler->GetSession()->GetPlayer()->GetName(), handler->GetSession()->GetPlayer()->GetGUID().ToString());
            return false;
        }
        ...
        switch (result)
        {
            case AccountOpResult::AOR_OK:
                ...
                SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Character:[{}] {} Changed Email from [{}] to [{}].",
                    handler->GetSession()->GetAccountId(), handler->GetSession()->GetRemoteAddress(),
                    handler->GetSession()->GetPlayer()->GetName(), handler->GetSession()->GetPlayer()->GetGUID().ToString(),
                    oldEmail, email);
                break;
    ...
    static bool HandleAccountPasswordCommand(ChatHandler* handler, std::string const& oldPassword, std::string const& newPassword, std::string const& confirmPassword, Optional<std::string> const& confirmEmail)
    {
        ...
        if (!AccountMgr::CheckPassword(handler->GetSession()->GetAccountId(), oldPassword))
        {
            ...
            handler->SetSentErrorMessage(true);
            SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Character:[{}] {} Tried to change password, but the provided old password is wrong.",
                handler->GetSession()->GetAccountId(), handler->GetSession()->GetRemoteAddress(),
                handler->GetSession()->GetPlayer()->GetName(), handler->GetSession()->GetPlayer()->GetGUID().ToString());
            return false;
        }
            ...
            && !AccountMgr::CheckEmail(handler->GetSession()->GetAccountId(), confirmEmail.value_or(""))) // ... and returns false if the comparison fails.
        {
            ...
            sScriptMgr->OnFailedPasswordChange(handler->GetSession()->GetAccountId());
            handler->SetSentErrorMessage(true);
            SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Character:[{}] {} Tried to change password, but the entered email [{}] is wrong.",
                handler->GetSession()->GetAccountId(), handler->GetSession()->GetRemoteAddress(),
                handler->GetSession()->GetPlayer()->GetName(), handler->GetSession()->GetPlayer()->GetGUID().ToString(),
        ...
        switch (result)
        {
            case AccountOpResult::AOR_OK:
                ...
                SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Character:[{}] {} changed password.",
                    handler->GetSession()->GetAccountId(), handler->GetSession()->GetRemoteAddress(),
                    handler->GetSession()->GetPlayer()->GetName(), handler->GetSession()->GetPlayer()->GetGUID().ToString());
                break;
            case AccountOpResult::AOR_PASS_TOO_LONG:
    ...
    static bool HandleAccountSet2FACommand(ChatHandler* handler, std::string accountName, std::string secret)
    {
        ...
        Optional<std::vector<uint8>> decoded = Syphrena::Encoding::Base32::Decode(secret);
        if (!decoded)
        {
        ...
        if (128 < (decoded->size() + Syphrena::Crypto::AES::IV_SIZE_BYTES + Syphrena::Crypto::AES::TAG_SIZE_BYTES))
        {
        ...
        if (masterKey)
            Syphrena::Crypto::AEEncryptWithRandomIV<Syphrena::Crypto::AES>(*decoded, *masterKey);
    ...
    static bool HandleAccountSetEmailCommand(ChatHandler* handler, std::string accountName, std::string const& email, std::string const& confirmEmail)
    {
        ...
        switch (result)
        {
            case AccountOpResult::AOR_OK:
                handler->SendSysMessage(LANG_COMMAND_EMAIL);
                SC_LOG_INFO("entities.player.character", "ChangeEmail: Account {} [Id: {}] had it's email changed to {}.",
    ...
    static bool HandleAccountSetRegEmailCommand(ChatHandler* handler, std::string accountName, std::string const& email, std::string const& confirmEmail)
    {
        ...
        switch (result)
        {
            case AccountOpResult::AOR_OK:
                handler->SendSysMessage(LANG_COMMAND_EMAIL);
                SC_LOG_INFO("entities.player.character", "ChangeRegEmail: Account {} [Id: {}] had it's Registration Email changed to {}.",
                    accountName, targetAccountId, email);
                break;
            case AccountOpResult::AOR_NAME_NOT_EXIST:
                handler->PSendSysMessage(LANG_ACCOUNT_NOT_EXIST, accountName.c_str());
