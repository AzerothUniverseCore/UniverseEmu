...
public:
    ...
    static bool HandleGPSCommand(ChatHandler* handler, char const* args)
    {
        ...
        CellCoord cellCoord = Syphrena::ComputeCellCoord(object->GetPositionX(), object->GetPositionY());
        Cell cell(cellCoord);
        ...
        GridCoord gridCoord = Syphrena::ComputeGridCoord(object->GetPositionX(), object->GetPositionY());
        ...
        char const* unknown = handler->GetSyphrenaString(LANG_UNKNOWN);
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), handler->GetSyphrenaString(LANG_OFFLINE));
    ...
    {
        Syphrena::ChatCommands::SendCommandHelpFor(*handler, "");
        return true;
    }
    ...
    {
        Syphrena::ChatCommands::SendCommandHelpFor(*handler, cmd);
        if (cmd.empty())
            Syphrena::ChatCommands::SendCommandHelpFor(*handler, "help");
        return true;
    }
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, Optional<SpellInfo const*> spellArg)
    {
        ...
        else
        {
            if (!*spellArg)
            {
                handler->PSendSysMessage(LANG_UNKNOWN_SPELL, owner == handler->GetSession()->GetPlayer() ? handler->GetSyphrenaString(LANG_YOU) : nameLink.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }

            target->GetSpellHistory()->ResetCooldown((*spellArg)->Id, true);
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, (*spellArg)->Id, owner == handler->GetSession()->GetPlayer() ? handler->GetSyphrenaString(LANG_YOU) : nameLink.c_str());
        }
        return true;
    }
    ...
    static bool HandleKickPlayerCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string kickReasonStr = handler->GetSyphrenaString(LANG_NO_REASON);
        if (*args != '\0')
        {
    ...
    static bool HandleNearGraveCommand(ChatHandler* handler, Optional<std::string> teamArg)
    {
        ...
        if (graveyard)
        {
            ...
            std::string team_name = handler->GetSyphrenaString(LANG_COMMAND_GRAVEYARD_NOTEAM);

            if (team == 0)
                team_name = handler->GetSyphrenaString(LANG_COMMAND_GRAVEYARD_ANY);
            else if (team == HORDE)
                team_name = handler->GetSyphrenaString(LANG_COMMAND_GRAVEYARD_HORDE);
            else if (team == ALLIANCE)
                team_name = handler->GetSyphrenaString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        else                                                    // item_id or [name] Shift-click form |color|Hitem:item_id:0:0:0|h[name]|h|r
        {
            ...
            itemId = Syphrena::StringTo<uint32>(id).value_or(0);
        }
    ...
    **/
    static bool HandlePInfoCommand(ChatHandler* handler, Optional<PlayerIdentifier> arg)
    {
        ...
        std::string userName          = handler->GetSyphrenaString(LANG_ERROR);
        uint32 accId                  = 0;
        ObjectGuid::LowType lowguid   = targetGuid.GetCounter();
        std::string eMail             = handler->GetSyphrenaString(LANG_ERROR);
        std::string regMail           = handler->GetSyphrenaString(LANG_ERROR);
        uint32 security               = 0;
        std::string lastIp            = handler->GetSyphrenaString(LANG_ERROR);
        uint8 locked                  = 0;
        std::string lastLogin         = handler->GetSyphrenaString(LANG_ERROR);
        uint32 failedLogins           = 0;
        uint32 latency                = 0;
        std::string OS                = handler->GetSyphrenaString(LANG_UNKNOWN);

        // Mute data print variables
        int64 muteTime                = -1;
        std::string muteReason        = handler->GetSyphrenaString(LANG_NO_REASON);
        std::string muteBy            = handler->GetSyphrenaString(LANG_UNKNOWN);

        // Ban data print variables
        int64 banTime                 = -1;
        std::string banType           = handler->GetSyphrenaString(LANG_UNKNOWN);
        std::string banReason         = handler->GetSyphrenaString(LANG_NO_REASON);
        std::string bannedBy          = handler->GetSyphrenaString(LANG_UNKNOWN);

        // Character data print variables
        uint8 raceid, classid           = 0; //RACE_NONE, CLASS_NONE
        std::string raceStr, classStr   = handler->GetSyphrenaString(LANG_UNKNOWN);
        uint8 gender                    = 0;
        int8 locale                     = handler->GetSessionDbcLocale();
        uint32 totalPlayerTime          = 0;
        uint8 level                     = 0;
        std::string alive               = handler->GetSyphrenaString(LANG_ERROR);
        uint32 money                    = 0;
        uint32 xp                       = 0;
        uint32 xptotal                  = 0;
        ...
        if (target)
        {
            ...
            alive             = target->IsAlive() ? handler->GetSyphrenaString(LANG_YES) : handler->GetSyphrenaString(LANG_NO);
        ...
        else
        {
            ...
            if (!health || playerFlags & PLAYER_FLAGS_GHOST)
                alive = handler->GetSyphrenaString(LANG_NO);
            else
                alive = handler->GetSyphrenaString(LANG_YES);
        }
        ...
        if (result)
        {
            ...
            {
                eMail     = handler->GetSyphrenaString(LANG_UNAUTHORIZED);
                regMail   = handler->GetSyphrenaString(LANG_UNAUTHORIZED);
                lastIp    = handler->GetSyphrenaString(LANG_UNAUTHORIZED);
                lastLogin = handler->GetSyphrenaString(LANG_UNAUTHORIZED);
            }
        ...
        if (!result2)
        {
            banType = handler->GetSyphrenaString(LANG_CHARACTER);
            stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_PINFO_BANS);
            stmt->setUInt32(0, lowguid);
            result2 = CharacterDatabase.Query(stmt);
        }
        else
            banType = handler->GetSyphrenaString(LANG_ACCOUNT);
        ...
        handler->PSendSysMessage(LANG_PINFO_PLAYER, target ? "" : handler->GetSyphrenaString(LANG_OFFLINE), nameLink.c_str(), lowguid);
        ...
        if (banTime >= 0)
            handler->PSendSysMessage(LANG_PINFO_BANNED, banType.c_str(), banReason.c_str(), banTime > 0 ? secsToTimeString(banTime - GameTime::GetGameTime(), TimeFormat::ShortText).c_str() : handler->GetSyphrenaString(LANG_PERMANENTLY), bannedBy.c_str());
        ...
        handler->PSendSysMessage(LANG_PINFO_ACC_IP, lastIp.c_str(), locked ? handler->GetSyphrenaString(LANG_YES) : handler->GetSyphrenaString(LANG_NO));
        ...
        handler->PSendSysMessage(LANG_PINFO_CHR_RACE, (gender == 0 ? handler->GetSyphrenaString(LANG_CHARACTER_GENDER_MALE) : handler->GetSyphrenaString(LANG_CHARACTER_GENDER_FEMALE)), raceStr.c_str(), classStr.c_str());
        ...
        if (!zoneName)
            zoneName = handler->GetSyphrenaString(LANG_UNKNOWN);
    ...
    static bool HandleRespawnCommand(ChatHandler* handler)
    {
        ...
        Syphrena::RespawnDo u_do;
        Syphrena::WorldObjectWorker<Syphrena::RespawnDo> worker(player, u_do);
        ...
        if (!data.empty())
        {
            uint32 const gridId = Syphrena::ComputeGridCoord(player->GetPositionX(), player->GetPositionY()).GetId();
    ...
    static bool HandleMuteCommand(ChatHandler* handler, Optional<PlayerIdentifier> player, uint32 muteTime, Tail muteReason)
    {
        ...
        if (muteReason.empty())
            muteReasonStr = handler->GetSyphrenaString(LANG_NO_REASON);
        ...
        if (Player* gmPlayer = handler->GetPlayer())
            ...
            muteBy = handler->GetSyphrenaString(LANG_CONSOLE);
