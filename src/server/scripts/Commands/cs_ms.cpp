...
class ms_commandscript : public CommandScript
{
public:
    ms_commandscript() : CommandScript("ms_commandscript") { }
    ...
    static bool HandleGMVisibleCommand(ChatHandler* handler, Optional<bool> visibleArg)
    {
        ...
        if (!visibleArg)
        {
            handler->PSendSysMessage(LANG_YOU_ARE, _player->isGMVisible() ? handler->GetSyphrenaString(LANG_VISIBLE) : handler->GetSyphrenaString(LANG_INVISIBLE));
            return true;
        }
...
void AddSC_ms_commandscript()
{
    new ms_commandscript();
}
