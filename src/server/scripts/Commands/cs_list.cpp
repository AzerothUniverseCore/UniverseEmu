...
public:
    ...
    static bool ListAurasCommand(ChatHandler* handler, Optional<uint32> spellId, std::wstring namePart)
    {
        ...
        char const* talentStr = handler->GetSyphrenaString(LANG_TALENT);
        char const* passiveStr = handler->GetSyphrenaString(LANG_PASSIVE);
    ...
    static bool HandleListRespawnsCommand(ChatHandler* handler, Optional<uint32> range)
    {
        ...
        char const* stringOverdue = sObjectMgr->GetSyphrenaString(LANG_LIST_RESPAWNS_OVERDUE, locale);
