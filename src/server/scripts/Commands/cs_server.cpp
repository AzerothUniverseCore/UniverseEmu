...
public:
    ...
    static bool HandleServerDebugCommand(ChatHandler* handler, char const* /*args*/)
    {
...

        {
            ...
            if (dbPort)
                dbPortOutput = Syphrena::StringFormat("Realmlist (Realm Id: {}) configured in port {}", realm.Id.Realm, dbPort);
            else
                dbPortOutput = Syphrena::StringFormat("Realm Id: {} not found in `realmlist` table. Please check your setup", realm.Id.Realm);
        }
