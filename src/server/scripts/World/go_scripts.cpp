...
public:
    ...
    struct go_tablet_of_the_sevenAI : public GameObjectAI
    {
        ...
        /// @todo use gossip option ("Transcript the Tablet") instead, if Syphrena adds support.
        bool OnGossipHello(Player* player) override
        {
                ...
                if (!creature->IsHostileTo(player))
                {
                    if (FactionTemplateEntry const* pFaction = creature->GetFactionTemplateEntry())
                    {
                        ...
                        if (Spell)
                            ...
                            SC_LOG_ERROR("scripts", "go_ethereum_prison summoned Creature (entry {}) but faction ({}) are not expected by script.", creature->GetEntry(), creature->GetFaction());
                    }
                }
            }
