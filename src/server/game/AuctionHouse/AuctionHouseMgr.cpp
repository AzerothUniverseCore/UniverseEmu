...
uint32 AuctionHouseMgr::GetAuctionDeposit(AuctionHouseEntry const* entry, uint32 time, Item* pItem, uint32 count)
{
    ...
    SC_LOG_DEBUG("auctionHouse", "MSV:        {}", MSV);
    SC_LOG_DEBUG("auctionHouse", "Items:      {}", count);
    SC_LOG_DEBUG("auctionHouse", "Multiplier: {}", multiplier);
    SC_LOG_DEBUG("auctionHouse", "Deposit:    {}", deposit);
    SC_LOG_DEBUG("auctionHouse", "Deposit rm: {}", remainderbase * count);
...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry* auction, CharacterDatabaseTransaction trans)
{
    ...
    else
    {
        ...
        if (logGmTrade && !sCharacterCache->GetCharacterNameByGuid(bidderGuid, bidderName))
            bidderName = sObjectMgr->GetSyphrenaStringForDBCLocale(LANG_UNKNOWN);
    }
    ...
    if (logGmTrade)
    {
        ...
        if (!sCharacterCache->GetCharacterNameByGuid(ownerGuid, ownerName))
            ownerName = sObjectMgr->GetSyphrenaStringForDBCLocale(LANG_UNKNOWN);
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 auction items. DB table `auctionhouse` or `item_instance` is empty!");
    ...
    do
    {
        ...
        if (!proto)
        {
            SC_LOG_ERROR("misc", "AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: {} item entry: #{}) in auction, skipped.", item_guid, itemEntry);
            continue;
        }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} auction items in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    if (!resultAuctions)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 auctions. DB table `auctionhouse` is empty.");
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} auctions with {} bidders in {} ms", countAuctions, countBidders, GetMSTimeDiffToNow(oldMSTime));
}
...
void AuctionHouseMgr::UpdatePendingAuctions()
{
    for (auto itr = pendingAuctionMap.begin(); itr != pendingAuctionMap.end();)
    {
        ...
        else
        {
            ...
            SC_LOG_WARN("auctionHouse", "Player {} was offline, unable to retrieve deposit!", playerGUID.ToString());
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data, Item* sourceItem) const
{
    ...
    if (!item)
    {
        SC_LOG_ERROR("misc", "AuctionEntry::BuildAuctionInfo: Auction {} has a non-existent item: {}", Id, itemGUIDLow);
        return false;
    }
    data << uint32(Id);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    {
        SC_LOG_ERROR("misc", "Auction {} has invalid house id {}", Id, houseId);
        return false;
    }
    ...
    {
        SC_LOG_ERROR("misc", "Auction {} has not a existing item : {}", Id, itemGUIDLow);
        return false;
    }
...
std::string AuctionEntry::BuildAuctionMailSubject(MailAuctionAnswers response) const
{
    ...
    return Syphrena::StringFormat("{}:{}:{}:{}:{}", itemEntry, item ? item->GetItemRandomPropertyId() : 0, response, Id, itemCount);
}

std::string AuctionEntry::BuildAuctionWonMailBody(ObjectGuid guid, uint32 bid, uint32 buyout)
{
    return Syphrena::StringFormat("{:X}:{}:{}", guid.GetRawValue(), bid, buyout);
}
...
{
    return Syphrena::StringFormat("{:X}:{}:{}:{}:{}", guid.GetRawValue(), bid, buyout, deposit, consignment);
}
...
{
    return Syphrena::StringFormat("{:X}:{}:{}:{}:{}:{}:{}", guid.GetRawValue(), bid, buyout, deposit, consignment, moneyDelay, eta);
}
