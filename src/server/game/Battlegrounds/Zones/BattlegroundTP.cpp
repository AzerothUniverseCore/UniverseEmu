...
//npcbot
void BattlegroundTP::AddBot(Creature* bot)
{
    bool const isInBattleground = IsPlayerInBattleground(bot->GetGUID());
    Battleground::AddBot(bot);
    if (!isInBattleground)
        BotScores[bot->GetEntry()] = new BattlegroundTPScore(bot->GetGUID());
}
//end npcbot
...
void BattlegroundTP::HandleFlagRoomCapturePoint(int32 team)
{
    ...
    if (GetFlagPickerGUID(team).IsCreature())
    {
        ...
        uint32 areaBotTrigger = team == TEAM_ALLIANCE ? 5905 : 5904;
    ...
    uint32 areaTrigger = team == TEAM_ALLIANCE ? 5905 : 5904;
...
//npcbot
void BattlegroundTP::EventBotCapturedFlag(Creature* bot)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
    ...
    if (GetBotTeam(bot->GetGUID()) == ALLIANCE)
    {
        ...
        bot->RemoveAurasDueToSpell(BG_TP_SPELL_HORDE_FLAG);
        if (_flagDebuffState == 1)
            bot->RemoveAurasDueToSpell(TP_SPELL_FOCUSED_ASSAULT);
        else if (_flagDebuffState == 2)
            bot->RemoveAurasDueToSpell(TP_SPELL_BRUTAL_ASSAULT);
    ...
    else
    {
        ...
        bot->RemoveAurasDueToSpell(BG_TP_SPELL_ALLIANCE_FLAG);
        if (_flagDebuffState == 1)
            bot->RemoveAurasDueToSpell(TP_SPELL_FOCUSED_ASSAULT);
        else if (_flagDebuffState == 2)
            bot->RemoveAurasDueToSpell(TP_SPELL_BRUTAL_ASSAULT);
    ...
    if (GetBotTeam(bot->GetGUID()) == ALLIANCE)
        SendBroadcastText(BG_TP_TEXT_CAPTURED_HORDE_FLAG, CHAT_MSG_BG_SYSTEM_ALLIANCE, bot);
    else
        SendBroadcastText(BG_TP_TEXT_CAPTURED_ALLIANCE_FLAG, CHAT_MSG_BG_SYSTEM_HORDE, bot);
    ...
    {
        _flagsTimer[GetTeamIndexByTeamId(GetBotTeam(bot->GetGUID())) ? 0 : 1] = BG_TP_FLAG_RESPAWN_TIME;
    }
}

void BattlegroundTP::EventBotDroppedFlag(Creature* bot)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
    {
    ...
    if (GetBotTeam(bot->GetGUID()) == ALLIANCE)
    {
        ...
        if (GetFlagPickerGUID(TEAM_HORDE) == bot->GetGUID())
        {
            ...
            bot->CastSpell(bot, BG_TP_SPELL_HORDE_FLAG_DROPPED, true);
            set = true;
        }
    }
    else
    {
        ...
        if (GetFlagPickerGUID(TEAM_ALLIANCE) == bot->GetGUID())
        {
            ...
            bot->CastSpell(bot, BG_TP_SPELL_ALLIANCE_FLAG_DROPPED, true);
            set = true;
        }
    }
    ...
    if (set)
    {
        ...
        if (GetBotTeam(bot->GetGUID()) == ALLIANCE)
        {
            SendBroadcastText(BG_TP_TEXT_HORDE_FLAG_DROPPED, CHAT_MSG_BG_SYSTEM_HORDE, bot);
            UpdateWorldState(BG_TP_FLAG_UNK_HORDE, uint32(-1));
        }
        else
        {
            SendBroadcastText(BG_TP_TEXT_ALLIANCE_FLAG_DROPPED, CHAT_MSG_BG_SYSTEM_ALLIANCE, bot);
            UpdateWorldState(BG_TP_FLAG_UNK_ALLIANCE, uint32(-1));
        }

        _flagsDropTimer[GetTeamIndexByTeamId(GetBotTeam(bot->GetGUID())) ? 0 : 1] = BG_TP_FLAG_DROP_TIME;
    }
}

void BattlegroundTP::EventBotClickedOnFlag(Creature* bot, GameObject* target_obj)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;

    //alliance flag picked up from base
    if (GetBotTeam(bot->GetGUID()) == HORDE && GetFlagState(ALLIANCE) == BG_TP_FLAG_STATE_ON_BASE
        && BgObjects[BG_TP_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
        SendBroadcastText(BG_TP_TEXT_ALLIANCE_FLAG_PICKED_UP, CHAT_MSG_BG_SYSTEM_HORDE, bot);
        ...
        bot->CastSpell(bot, BG_TP_SPELL_ALLIANCE_FLAG, true);
        if (_flagState[1] == BG_TP_FLAG_STATE_ON_PLAYER)
            _bothFlagsKept = true;

        if (_flagDebuffState == 1)
            bot->CastSpell(bot, TP_SPELL_FOCUSED_ASSAULT, true);
        else if (_flagDebuffState == 2)
            bot->CastSpell(bot, TP_SPELL_BRUTAL_ASSAULT, true);
    }

    //horde flag picked up from base
    if (GetBotTeam(bot->GetGUID()) == ALLIANCE && GetFlagState(HORDE) == BG_TP_FLAG_STATE_ON_BASE
        && BgObjects[BG_TP_OBJECT_H_FLAG] == target_obj->GetGUID())
    {
        SendBroadcastText(BG_TP_TEXT_HORDE_FLAG_PICKED_UP, CHAT_MSG_BG_SYSTEM_ALLIANCE, bot);
        ...
        bot->CastSpell(bot, BG_TP_SPELL_HORDE_FLAG, true);
    ...
    if (GetFlagState(ALLIANCE) == BG_TP_FLAG_STATE_ON_GROUND && bot->IsWithinDistInMap(target_obj, 10)
        && target_obj->GetGOInfo()->entry == BG_OBJECT_A_FLAG_GROUND_TP_ENTRY)
    {
        if (GetBotTeam(bot->GetGUID()) == ALLIANCE)
        {
            SendBroadcastText(BG_TP_TEXT_ALLIANCE_FLAG_RETURNED, CHAT_MSG_BG_SYSTEM_ALLIANCE, bot);
    ...
    if (GetFlagState(HORDE) == BG_TP_FLAG_STATE_ON_GROUND && bot->IsWithinDistInMap(target_obj, 10)
        && target_obj->GetGOInfo()->entry == BG_OBJECT_H_FLAG_GROUND_TP_ENTRY)
    {
        if (GetBotTeam(bot->GetGUID()) == HORDE)
        {
            SendBroadcastText(BG_TP_TEXT_HORDE_FLAG_RETURNED, CHAT_MSG_BG_SYSTEM_HORDE, bot);
...
void BattlegroundTP::RemoveBot(ObjectGuid guid)
{
    ...
    if (IsAllianceFlagPickedup() && m_FlagKeepers[TEAM_ALLIANCE] == guid)
    {
        ...
        if (!bot)
        {
            SC_LOG_ERROR("bg.battleground", "BattlegroundWS: Removing offline bot {} who has the FLAG!!", guid.GetEntry());
            SetAllianceFlagPicker(ObjectGuid::Empty);
            RespawnFlag(ALLIANCE, false);
        }
        else
...
bool BattlegroundTP::SetupBattleground()
{
    ...
    {
        SC_LOG_ERROR("misc", "BatteGroundTP: Failed to spawn some objects. Battleground not created!");
        return false;
    }
    ...
    for (uint32 i = TP_GRAVEYARD_START_ALLIANCE; i < TP_MAX_GRAVEYARDS; ++i)
    {
        ...
        if (grave)
        {
            ...
            {
                SC_LOG_ERROR("misc", "BatteGroundTP: Failed to spawn spirit guide id: %u. Battleground not created!", grave->ID);
                return false;
            }
        }
        else
        {
            SC_LOG_ERROR("misc", "BatteGroundTP: Failed to find grave %u. Battleground not created!", BG_TP_GraveyardIds[i]);
            return false;
        }
    }
...
//start npcbot
void BattlegroundTP::HandleBotAreaTrigger(Creature* bot, uint32 trigger)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
//npcbot
void BattlegroundTP::HandleBotKillPlayer(Creature* killer, Player* victim)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
void BattlegroundTP::HandleBotKillBot(Creature* killer, Creature* victim)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
void BattlegroundTP::HandlePlayerKillBot(Creature* victim, Player* killer)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
//npcbot
bool BattlegroundTP::UpdateBotScore(Creature const* bot, uint32 type, uint32 value, bool doAddHonor)
{
...
//npcbot
WorldSafeLocsEntry const* BattlegroundTP::GetClosestGraveyardForBot(WorldLocation const& curPos, uint32 team) const
{
    ...
    for (uint8 i = TP_GRAVEYARD_START_ALLIANCE + static_cast<uint8>(teamIndex); i < TP_MAX_GRAVEYARDS; i += 2)
    {
        WorldSafeLocsEntry const* entry = sWorldSafeLocsStore.LookupEntry(BG_TP_GraveyardIds[i]);
    ...
    if (!good_entry)
        good_entry = sWorldSafeLocsStore.LookupEntry(BG_TP_GraveyardIds[teamIndex + 2]);
...
void BattlegroundTP::RemovePlayer(Player* player, ObjectGuid guid, uint32 /* team */)
{
    ...
    if (IsAllianceFlagPickedup() && m_FlagKeepers[TEAM_ALLIANCE] == guid)
    {
        if (!player)
        {
            SC_LOG_ERROR("bg.battleground", "BattlegroundTP: Removing offline player who has the FLAG!!");
            SetAllianceFlagPicker(ObjectGuid::Empty);
            RespawnFlag(ALLIANCE, false);
        }
        else
...
void BattlegroundTP::RespawnFlag(uint32 Team, bool captured)
{
    if (Team == ALLIANCE)
    {
        SC_LOG_DEBUG("bg.battleground", "Respawn Alliance flag");
        _flagState[TEAM_ALLIANCE] = BG_TP_FLAG_STATE_ON_BASE;
    }
    else
    {
        SC_LOG_DEBUG("bg.battleground", "Respawn Horde flag");
        _flagState[TEAM_HORDE] = BG_TP_FLAG_STATE_ON_BASE;
    }
...
void BattlegroundTP::RespawnFlagAfterDrop(uint32 team)
{
    ...
    if (GameObject* obj = GetBgMap()->GetGameObject(GetDroppedFlagGUID(team)))
        ...
        SC_LOG_ERROR("bg.battleground", "unknown dropped flag (%s)", GetDroppedFlagGUID(team).ToString().c_str());
