...
//npcbot
void BattlegroundBFG::AddBot(Creature* bot)
{
    bool const isInBattleground = IsPlayerInBattleground(bot->GetGUID());
    Battleground::AddBot(bot);
    if (!isInBattleground)
        BotScores[bot->GetEntry()] = new BattlegroundBFGScore(bot->GetGUID());
}
//end npcbot
...
void BattlegroundBFG::_NodeOccupied(uint8 node, Team team)
{
    if (!AddSpiritGuide(node, BG_BFG_SpiritGuidePos[node], GetTeamIndexByTeamId(team)))
        SC_LOG_ERROR("bg.battleground", "Failed to spawn spirit guide! point: %u, team: %u, ", node, team);
...
//npcbot
void BattlegroundBFG::EventBotClickedOnFlag(Creature* bot, GameObject* /*target_obj*/)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
    ...
    uint8 node = BG_BFG_NODE_LIGHTHOUSE;
    GameObject* obj = GetBgMap()->GetGameObject(BgObjects[node * 8 + 7]);
    while ((node < BG_BFG_DYNAMIC_NODES_COUNT) && ((!obj) || (!bot->IsWithinDistInMap(obj, 10))))
    {
        ++node;
        obj = GetBgMap()->GetGameObject(BgObjects[node * 8 + BG_BFG_OBJECT_AURA_CONTESTED]);
    }
    ...
    if (m_Nodes[node] == BG_BFG_NODE_TYPE_NEUTRAL)
    {
        ...
        if (teamIndex == TEAM_ALLIANCE)
            SendBroadcastText(BFGNodes[node].TextAllianceClaims, CHAT_MSG_BG_SYSTEM_ALLIANCE, bot);
        else
            SendBroadcastText(BFGNodes[node].TextHordeClaims, CHAT_MSG_BG_SYSTEM_HORDE, bot);
    ...
    else if ((m_Nodes[node] == BG_BFG_NODE_STATUS_ALLY_CONTESTED) || (m_Nodes[node] == BG_BFG_NODE_STATUS_HORDE_CONTESTED))
    {
        ...
        if (m_prevNodes[node] < BG_BFG_NODE_TYPE_OCCUPIED)
        {
            ...
            if (teamIndex == TEAM_ALLIANCE)
                SendBroadcastText(BFGNodes[node].TextAllianceAssaulted, CHAT_MSG_BG_SYSTEM_ALLIANCE, bot);
            else
                SendBroadcastText(BFGNodes[node].TextHordeAssaulted, CHAT_MSG_BG_SYSTEM_HORDE, bot);
        }
        // If contested, change back to occupied
        else
        {
            ...
            if (teamIndex == TEAM_ALLIANCE)
                SendBroadcastText(BFGNodes[node].TextAllianceDefended, CHAT_MSG_BG_SYSTEM_ALLIANCE, bot);
            else
                SendBroadcastText(BFGNodes[node].TextHordeDefended, CHAT_MSG_BG_SYSTEM_HORDE, bot);
        }
        sound = (teamIndex == TEAM_ALLIANCE) ? BG_BFG_SOUND_NODE_ASSAULTED_ALLIANCE : BG_BFG_SOUND_NODE_ASSAULTED_HORDE;
    }
    // If node is occupied, change to enemy-contested
...
bool BattlegroundBFG::IsNodeOccupied(uint8 node, TeamId teamId) const
{
    if (node < BG_BFG_DYNAMIC_NODES_COUNT)
    {
        switch (teamId)
        {
        case TEAM_ALLIANCE:
            return m_Nodes[node] == BG_BFG_NODE_STATUS_ALLY_OCCUPIED;
        case TEAM_HORDE:
            return m_Nodes[node] == BG_BFG_NODE_STATUS_HORDE_OCCUPIED;
        default:
            break;
        }
    }
...
bool BattlegroundBFG::IsNodeContested(uint8 node, TeamId teamId) const
{
    if (node < BG_BFG_DYNAMIC_NODES_COUNT)
    {
        switch (teamId)
        {
        case TEAM_ALLIANCE:
            return m_Nodes[node] == BG_BFG_NODE_STATUS_ALLY_CONTESTED;
        case TEAM_HORDE:
            return m_Nodes[node] == BG_BFG_NODE_STATUS_HORDE_CONTESTED;
        default:
            break;
        }
    }
...
bool BattlegroundBFG::SetupBattleground()
{
    for (int i = 0; i < BG_BFG_DYNAMIC_NODES_COUNT; ++i)
    {
        ...
        {
            SC_LOG_ERROR("bg.battleground", "BatteGroundBG: Failed to spawn some objects, Battleground not created!");
            return false;
        }
    }
    ...
    {
        SC_LOG_ERROR("bg.battleground", "BatteGroundBG: Failed to spawn door object, Battleground not created!");
        return false;
    }
    //buffs
    for (int i = 0; i < BG_BFG_DYNAMIC_NODES_COUNT; ++i)
    {
        if (!AddObject(BG_BFG_OBJECT_SPEEDBUFF_LIGHTHOUSE + 3 * i, Buff_Entries[0], BG_BFG_BuffPositions[i][0], BG_BFG_BuffPositions[i][1], BG_BFG_BuffPositions[i][2], BG_BFG_BuffPositions[i][3], 0, 0, sin(BG_BFG_BuffPositions[i][3] / 2), cos(BG_BFG_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY)
            ...
            SC_LOG_ERROR("bg.battleground", "BatteGroundBG: Failed to spawn buff object!");
    }
...
//npcbot
WorldSafeLocsEntry const* BattlegroundBFG::GetClosestGraveyardForBot(WorldLocation const& curPos, uint32 team) const
{
...
void BattlegroundBFG::RewardKillScore(TeamId teamId, uint32 amount)
{
    // Score feature
    m_TeamScores[teamId] += amount;

    if (m_TeamScores[teamId] > BG_BFG_MAX_TEAM_SCORE)
        m_TeamScores[teamId] = BG_BFG_MAX_TEAM_SCORE;

    UpdateWorldState(teamId == TEAM_ALLIANCE ? BG_BFG_OP_RESOURCES_ALLY : BG_BFG_OP_RESOURCES_HORDE, m_TeamScores[teamId]);

    if (m_TeamScores[teamId] >= BG_BFG_MAX_TEAM_SCORE)
        EndBattleground(teamId);
}

void BattlegroundBFG::HandleBotKillPlayer(Creature* killer, Player* victim)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;

    Battleground::HandleBotKillPlayer(killer, victim);
    //RewardKillScore(GetBotTeamId(killer->GetGUID()), BG_BFG_TickPoints[1]);
}
void BattlegroundBFG::HandleBotKillBot(Creature* killer, Creature* victim)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
void BattlegroundBFG::HandlePlayerKillBot(Creature* victim, Player* killer)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
