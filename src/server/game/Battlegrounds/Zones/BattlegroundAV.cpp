...
{
    SC_LOG_DEBUG("bg.battleground", "bg_av HandleKillUnit {}", unit->GetEntry());
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
    ...
    */
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN])
    {
        if (!m_CaptainAlive[0])
        {
            SC_LOG_ERROR("bg.battleground", "Killed a Captain twice, please report this bug, if you haven't done \".respawn\"");
            return;
        }
...
{
    SC_LOG_DEBUG("bg.battleground", "bg_av HandleBotKillUnit {}", victim->GetEntry());
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player* player)
{
    ...
    SC_LOG_DEBUG("bg.battleground", "BG_AV Quest {} completed", questid);
    switch (questid)
    {
        ...
        case AV_QUEST_H_SCRAPS2:
            ...
            {
                SC_LOG_DEBUG("bg.battleground", "BG_AV Quest {} completed starting with unit upgrading..", questid);
        ...
        case AV_QUEST_H_COMMANDER1:
            ...
            if (m_Team_QuestStatus[team][1] == 30)
                SC_LOG_DEBUG("bg.battleground", "BG_AV Quest {} completed (need to implement some events here", questid);
            break;
        ...
        case AV_QUEST_H_NEAR_MINE:
            ...
            if (m_Team_QuestStatus[team][5] == 28)
            {
                ...
                if (m_Team_QuestStatus[team][6] == 7)
                    SC_LOG_DEBUG("bg.battleground", "BG_AV Quest {} completed (need to implement some events here - ground assault ready", questid);
            }
            break;
        ...
        case AV_QUEST_H_RIDER_HIDE:
            ...
            if (m_Team_QuestStatus[team][7] == 25)
            {
                ...
                if (m_Team_QuestStatus[team][8] == 25)
                    SC_LOG_DEBUG("bg.battleground", "BG_AV Quest {} completed (need to implement some events here - rider assault ready", questid);
            }
            break;
        ...
        case AV_QUEST_H_RIDER_TAME:
            ...
            break;
        default:
            SC_LOG_DEBUG("bg.battleground", "BG_AV Quest {} completed but is not interesting at all", questid);
...
{
    SC_LOG_DEBUG("bg.battleground", "BG_AV: start spawning mine stuff");
...
void BattlegroundAV::RemovePlayer(Player* player, ObjectGuid /*guid*/, uint32 /*team*/)
{
   if (!player)
    {
        SC_LOG_ERROR("bg.battleground", "bg_AV no player at remove");
        return;
    }
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    SC_LOG_DEBUG("bg.battleground", "bg_av: player destroyed point node {} object {}", node, object);
    ...
    if (IsTower(node))
    {
        ...
        if (BgCreatures[AV_CPLACE_A_MARSHAL_SOUTH + tmp])
            ...
            SC_LOG_ERROR("bg.battleground", "BG_AV: playerdestroyedpoint: marshal {} doesn't exist", AV_CPLACE_A_MARSHAL_SOUTH + tmp);
        //spawn destroyed aura
        for (uint8 i=0; i <= 9; i++)
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, uint32 team, bool initial)
{
    ...
    if (!initial)
    {
        SC_LOG_DEBUG("bg.battleground", "bg_av depopulating mine {} (0=north, 1=south)", mine);
    ...
    SC_LOG_DEBUG("bg.battleground", "bg_av populating mine {} (0=north, 1=south)", mine);
    uint16 miner;
    ...
    else
    {
       ...
       //vermin
        SC_LOG_DEBUG("bg.battleground", "spawning vermin");
        if (team == ALLIANCE)
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node)
{
    ...
    if (m_Team_QuestStatus[team2][0] < 500)
        ...
        if (!AddSpiritGuide(node, BG_AV_CreaturePos[node], GetTeamIndexByTeamId(owner)))
            SC_LOG_ERROR("bg.battleground", "AV: couldn't spawn spiritguide at node {}", node);
    }
    for (uint8 i=0; i<4; i++)
...
{
    SC_LOG_DEBUG("bg.battleground", "bg_AV getnodethroughobject {}", object);
    ...
    SC_LOG_ERROR("bg.battleground", "BattlegroundAV: ERROR! GetPlace got a wrong object :(");
    ABORT();
...
{ //this function is the counterpart to GetNodeThroughObject()
    ...
    if (log)
    //end npcbot
    SC_LOG_DEBUG("bg.battleground", "bg_AV GetObjectThroughNode {}", node);
    ...
    SC_LOG_ERROR("bg.battleground", "BattlegroundAV: Error! GetPlaceNode couldn't resolve node {}", node);
    ABORT();
    return 0;
}
...
void BattlegroundAV::EventPlayerClickedOnFlag(Player* source, GameObject* target_obj)
{
    ...
    SC_LOG_DEBUG("bg.battleground", "BG_AV using gameobject {} with type {}", target_obj->GetEntry(), object);
    if (object < 0)
        return;
...
void BattlegroundAV::EventBotClickedOnFlag(Creature* bot, GameObject* target_obj)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
    int32 object = GetObjectType(target_obj->GetGUID());
    SC_LOG_DEBUG("bg.battleground", "BG_AV bot is using gameobject {} with type {}", target_obj->GetEntry(), object);
    if (object < 0)
        return;
    switch (target_obj->GetEntry())
    {
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    SC_LOG_DEBUG("bg.battleground", "player defends point object: {} node: {}", object, node);
    if (m_Nodes[node].PrevOwner != team)
    {
        SC_LOG_ERROR("bg.battleground", "BG_AV: player defends point which doesn't belong to his team {}", node);
        return;
    }
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    uint32 team  = player->GetTeam();
    SC_LOG_DEBUG("bg.battleground", "bg_av: player assaults point object {} node {}", object, node);
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    {
        SC_LOG_ERROR("sql.sql", "BatteGroundAV: Failed to spawn some object Battleground not created!1");
        return false;
    }
        ...
        if (i <= BG_AV_NODES_FROSTWOLF_HUT)
        {
            if (!AddObject(i, BG_AV_OBJECTID_BANNER_A_B,
                ...
                || !AddObject(BG_AV_OBJECT_AURA_H_FIRSTAID_STATION+i*3, BG_AV_OBJECTID_AURA_H,
                              ...
                              0, 0, std::sin(BG_AV_ObjectPos[i].GetOrientation()/2), std::cos(BG_AV_ObjectPos[i].GetOrientation()/2), RESPAWN_ONE_DAY))
            {
                SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!2");
                return false;
            }
        }
        else //towers
        {
            if (i <= BG_AV_NODES_STONEHEART_BUNKER) //alliance towers
            {
                if (!AddObject(i, BG_AV_OBJECTID_BANNER_A,
                    ...
                    || !AddObject(BG_AV_OBJECT_TFLAG_H_DUNBALDAR_SOUTH+(2*(i-BG_AV_NODES_DUNBALDAR_SOUTH)), BG_AV_OBJECTID_TOWER_BANNER_PH,
                                  ...
                                  0, 0, std::sin(BG_AV_ObjectPos[i+8].GetOrientation()/2), std::cos(BG_AV_ObjectPos[i+8].GetOrientation()/2), RESPAWN_ONE_DAY))
                {
                    SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!3");
                    return false;
                }
            }
            else //horde towers
            {
                if (!AddObject(i+7, BG_AV_OBJECTID_BANNER_CONT_A,
                    ...
                    || !AddObject(BG_AV_OBJECT_TFLAG_H_DUNBALDAR_SOUTH+(2*(i-BG_AV_NODES_DUNBALDAR_SOUTH)), BG_AV_OBJECTID_TOWER_BANNER_H,
                                  ...
                                  0, 0, std::sin(BG_AV_ObjectPos[i+8].GetOrientation()/2), std::cos(BG_AV_ObjectPos[i+8].GetOrientation()/2), RESPAWN_ONE_DAY))
                {
                    SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!4");
                    return false;
                }
            }
            for (uint8 j=0; j <= 9; j++) //burning aura
            {
                if (!AddObject(BG_AV_OBJECT_BURN_DUNBALDAR_SOUTH+((i-BG_AV_NODES_DUNBALDAR_SOUTH)*10)+j,
                               ...
                               RESPAWN_ONE_DAY))
                {
                    SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!5.{}", i);
                    return false;
                }
            }
        }
        ...
        for (uint8 j=0; j <= 9; j++)
        {
            if (j<5)
            {
                if (!AddObject(BG_AV_OBJECT_BURN_BUILDING_ALLIANCE+(i*10)+j,
                               ...
                               RESPAWN_ONE_DAY))
                {
                    SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!6.{}", i);
                    return false;
                }
            }
            else
            {
                if (!AddObject(BG_AV_OBJECT_BURN_BUILDING_ALLIANCE+(i*10)+j,
                               ...
                               RESPAWN_ONE_DAY))
                {
                    SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!7.{}", i);
                    return false;
                }
            }
        }
    ...
    for (uint16 i= 0; i <= (BG_AV_OBJECT_MINE_SUPPLY_N_MAX-BG_AV_OBJECT_MINE_SUPPLY_N_MIN); i++)
    {
        if (!AddObject(BG_AV_OBJECT_MINE_SUPPLY_N_MIN+i,
                       ...
                       RESPAWN_ONE_DAY))
        {
            SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some mine supplies Battleground not created!7.5.{}", i);
            return false;
        }
    }
    for (uint16 i= 0; i <= (BG_AV_OBJECT_MINE_SUPPLY_S_MAX-BG_AV_OBJECT_MINE_SUPPLY_S_MIN); i++)
    {
        if (!AddObject(BG_AV_OBJECT_MINE_SUPPLY_S_MIN+i,
                       ...
                       RESPAWN_ONE_DAY))
        {
            SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some mine supplies Battleground not created!7.6.{}", i);
            return false;
        }
    }
    ...
    if (!AddObject(BG_AV_OBJECT_FLAG_N_SNOWFALL_GRAVE,
                   ...
                   RESPAWN_ONE_DAY))
    {
        SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!8");
        return false;
    }
    for (uint8 i = 0; i < 4; i++)
    {
        if (!AddObject(BG_AV_OBJECT_SNOW_EYECANDY_A+i, BG_AV_OBJECTID_SNOWFALL_CANDY_A,
            ...
            || !AddObject(BG_AV_OBJECT_SNOW_EYECANDY_PH+i, BG_AV_OBJECTID_SNOWFALL_CANDY_PH,
                          ...
                          0, 0, std::sin(BG_AV_ObjectPos[AV_OPLACE_SNOW_1+i].GetOrientation()/2), std::cos(BG_AV_ObjectPos[AV_OPLACE_SNOW_1+i].GetOrientation()/2), RESPAWN_ONE_DAY))
        {
            SC_LOG_ERROR("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!9.{}", i);
            return false;
        }
    }

    uint16 i;
    SC_LOG_DEBUG("bg.battleground", "Alterac Valley: entering state STATUS_WAIT_JOIN ...");
    // Initial Nodes
    ...
    //creatures
    SC_LOG_DEBUG("bg.battleground", "BG_AV start poputlating nodes");
    ...
    SC_LOG_DEBUG("bg.battleground", "BG_AV: start spawning static creatures");
    for (i = 0; i < AV_STATICCPLACE_MAX; i++)
        AddAVCreature(0, i + AV_CPLACE_MAX);
    //mainspiritguides:
    SC_LOG_DEBUG("bg.battleground", "BG_AV: start spawning spiritguides creatures");
    ...
    SC_LOG_DEBUG("bg.battleground", "BG_AV: start spawning marshal creatures");
...
void BattlegroundAV::AssaultNode(BG_AV_Nodes node, uint16 team)
{
    ...
    {
        SC_LOG_FATAL("bg.battleground", "Assaulting team is TotalOwner of node");
        ABORT();
    }
    if (m_Nodes[node].Owner == team)
    {
        SC_LOG_FATAL("bg.battleground", "Assaulting team is owner of node");
        ABORT();
    }
    if (m_Nodes[node].State == POINT_DESTROYED)
    {
        SC_LOG_FATAL("bg.battleground", "Destroyed node is being assaulted");
        ABORT();
    }
    if (m_Nodes[node].State == POINT_ASSAULTED && m_Nodes[node].TotalOwner) //only assault an assaulted node if no totalowner exists
    {
        SC_LOG_FATAL("bg.battleground", "Assault on an not assaulted node with total owner");
        ABORT();
    }
