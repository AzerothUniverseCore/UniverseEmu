...
void BattlegroundWS::RespawnFlagAfterDrop(uint32 team)
{
    ...
    if (GameObject* obj = GetBgMap()->GetGameObject(GetDroppedFlagGUID(team)))
        ...
        SC_LOG_ERROR("bg.battleground", "unknown dropped flag ({})", GetDroppedFlagGUID(team).ToString());
...
void BattlegroundWS::RemovePlayer(Player* player, ObjectGuid guid, uint32 /*team*/)
{
    ...
    if (IsAllianceFlagPickedup() && m_FlagKeepers[TEAM_ALLIANCE] == guid)
    {
        if (!player)
        {
            SC_LOG_ERROR("bg.battleground", "BattlegroundWS: Removing offline player who has the FLAG!!");
            SetAllianceFlagPicker(ObjectGuid::Empty);
            RespawnFlag(ALLIANCE, false);
        }
        else
...
bool BattlegroundWS::SetupBattleground()
{
    ...
    {
        SC_LOG_ERROR("sql.sql", "BatteGroundWS: Failed to spawn some object Battleground not created!");
        return false;
    }
    ...
    {
        SC_LOG_ERROR("sql.sql", "BatteGroundWS: Failed to spawn Alliance spirit guide! Battleground not created!");
        return false;
    }
    ...
    {
        SC_LOG_ERROR("sql.sql", "BatteGroundWS: Failed to spawn Horde spirit guide! Battleground not created!");
        return false;
    }

    SC_LOG_DEBUG("bg.battleground", "BatteGroundWS: BG objects and spirit guides spawned");
