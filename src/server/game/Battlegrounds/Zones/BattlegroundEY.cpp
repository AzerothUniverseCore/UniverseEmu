    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[EY_POINTS_MAX].size())
            {
                ...
                if (!player)
                {
                    SC_LOG_ERROR("bg.battleground", "BattlegroundEY:CheckSomeoneJoinedPoint: Player ({}) could not be found!", m_PlayersNearPoint[EY_POINTS_MAX][j].ToString());
                    ++j;
                    continue;
                }
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[i].size())
            {
                ...
                if (!player)
                {
                    SC_LOG_ERROR("bg.battleground", "BattlegroundEY:CheckSomeoneLeftPoint Player ({}) could not be found!", m_PlayersNearPoint[i][j].ToString());
...
bool BattlegroundEY::SetupBattleground()
{
    ...
    {
        SC_LOG_ERROR("sql.sql", "BatteGroundEY: Failed to spawn some objects. The battleground was not created.");
        return false;
    }
    ...
    for (int i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (!at)
        {
            SC_LOG_ERROR("bg.battleground", "BattlegroundEY: Unknown trigger: {}", m_Points_Trigger[i]);
            continue;
        }
        if (!AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3, Buff_Entries[0], at->Pos.X, at->Pos.Y, at->Pos.Z, 0.907571f, 0, 0, 0.438371f, 0.898794f, RESPAWN_ONE_DAY)
            ...
            SC_LOG_ERROR("bg.battleground", "BattlegroundEY: Could not spawn Speedbuff Fel Reaver.");
    }
    ...
    {
        SC_LOG_ERROR("sql.sql", "BatteGroundEY: Failed to spawn spirit guide. The battleground was not created.");
        return false;
    }
...
void BattlegroundEY::RespawnFlagAfterDrop()
{
    ...
    if (obj)
        ...
        SC_LOG_ERROR("bg.battleground", "BattlegroundEY: Unknown dropped flag ({}).", GetDroppedFlagGUID().ToString());
...
void BattlegroundEY::EventTeamCapturedPoint(Player* player, uint32 Point)
{
    ...
    if (!sg || !AddSpiritGuide(Point, sg->Loc.X, sg->Loc.Y, sg->Loc.Z, 3.124139f, GetTeamIndexByTeamId(Team)))
        SC_LOG_ERROR("bg.battleground", "BatteGroundEY: Failed to spawn spirit guide. point: {}, team: {}, graveyard_id: {}",
...
WorldSafeLocsEntry const* BattlegroundEY::GetClosestGraveyard(Player* player)
{
    ...
    if (!entry)
    {
        SC_LOG_ERROR("bg.battleground", "BattlegroundEY: The main team graveyard could not be found. The graveyard system will not be operational!");
        return nullptr;
    }
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        if (m_PointOwnedByTeam[i] == player->GetTeam() && m_PointState[i] == EY_POINT_UNDER_CONTROL)
        {
            ...
            if (!entry)
                SC_LOG_ERROR("bg.battleground", "BattlegroundEY: Graveyard {} could not be found.", m_CapturingPointTypes[i].GraveyardId);
            else
            {
                distance = (entry->Loc.X - plr_x)*(entry->Loc.X - plr_x) + (entry->Loc.Y - plr_y)*(entry->Loc.Y - plr_y) + (entry->Loc.Z - plr_z)*(entry->Loc.Z - plr_z);
