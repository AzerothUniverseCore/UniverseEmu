...
bool ArenaTeam::Create(ObjectGuid captainGuid, uint8 type, std::string const& teamName, uint32 backgroundColor, uint8 emblemStyle, uint32 emblemColor, uint8 borderStyle, uint32 borderColor)
{
    ...
    SC_LOG_DEBUG("bg.arena", "New ArenaTeam created [Id: {}, Name: {}] [Type: {}] [Captain low GUID: {}]", GetId(), GetName(), GetType(), captainLowGuid);
    return true;
}
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid)
{
    ...
    {
        SC_LOG_DEBUG("bg.arena", "Arena: {} {} already has an arena team of type {}", playerGuid.ToString(), playerName, GetType());
        return false;
    }
    ...
    SC_LOG_DEBUG("bg.arena", "Player: {} [{}] joined arena team type: {} [Id: {}, Name: {}].", playerName, playerGuid.ToString(), GetType(), GetId(), GetName());
...
bool ArenaTeam::LoadMembersFromDB(QueryResult result)
{
    ...
    do
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "ArenaTeam {} has member with empty name - probably {} doesn't exist, deleting him from memberlist!", arenaTeamId, newMember.Guid.ToString());
    ...
    if (Empty() || !captainPresentInTeam)
    {
        ...
        SC_LOG_DEBUG("bg.arena", "ArenaTeam {} does not have any members or its captain is not in team, disbanding it...", TeamId);
        return false;
    }
...
void ArenaTeam::SetCaptain(ObjectGuid guid)
{
    ...
    if (Player* newCaptain = ObjectAccessor::FindPlayer(guid))
    {
        ...
        if (oldCaptain)
        {
            SC_LOG_DEBUG("bg.arena", "Player: {} {} promoted player: {} {} to leader of arena team [Id: {}, Name: {}] [Type: {}].",
...
void ArenaTeam::DelMember(ObjectGuid guid, bool cleanDb)
{
    ...
    if (player)
    {
        ...
        SC_LOG_DEBUG("bg.arena", "Player: {} {} left arena team type: {} [Id: {}, Name: {}].", player->GetName(), player->GetGUID().ToString(), GetType(), GetId(), GetName());
    }
...
void ArenaTeam::Disband(WorldSession* session)
{
    ...
    if (session)
    {
        ...
        if (Player* player = session->GetPlayer())
            SC_LOG_DEBUG("bg.arena", "Player: {} {} disbanded arena team type: {} [Id: {}, Name: {}].", player->GetName(), player->GetGUID().ToString(), GetType(), GetId(), GetName());
    }
...
void ArenaTeam::Roster(WorldSession* session)
{
    ...
    SC_LOG_DEBUG("network", "WORLD: Sent SMSG_ARENA_TEAM_ROSTER");
}
...
void ArenaTeam::Query(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    SC_LOG_DEBUG("network", "WORLD: Sent SMSG_ARENA_TEAM_QUERY_RESPONSE");
}
...
void ArenaTeam::BroadcastEvent(ArenaTeamEvents event, ObjectGuid guid, uint8 strCount, std::string const& str1, std::string const& str2, std::string const& str3)
{
    ...
    switch (strCount)
    {
        ...
        case 3:
            ...
            break;
        default:
            SC_LOG_ERROR("bg.arena", "Unhandled strCount {} in ArenaTeam::BroadcastEvent", strCount);
            return;
    }
    ...
    SC_LOG_DEBUG("network", "WORLD: Sent SMSG_ARENA_TEAM_EVENT");
}
...
uint8 ArenaTeam::GetSlotByType(uint32 type)
{
    ...
    SC_LOG_ERROR("bg.arena", "FATAL: Unknown arena team type {} for some arena team", type);
    return 0xFF;
}
