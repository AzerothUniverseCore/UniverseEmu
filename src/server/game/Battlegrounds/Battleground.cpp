    ...
    if (m_ValidStartPositionTimer >= CHECK_PLAYER_POSITION_INVERVAL)
    {
        ...
        for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
        {
            if (Player* player = ObjectAccessor::FindPlayer(itr->first))
            {
                ...
                {
                    SC_LOG_DEBUG("bg.battleground", "BATTLEGROUND: Sending {} back to start location (map: {}) (possible exploit)", player->GetName(), GetMapId());
...
inline void Battleground::_ProcessJoin(uint32 diff)
{
    ...
    if (!(m_Events & BG_STARTING_EVENT_1))
    {
        ...
        if (!FindBgMap())
        {
            SC_LOG_ERROR("bg.battleground", "Battleground::_ProcessJoin: map (map id: {}, instance id: {}) is not created!", m_MapId, m_InstanceID);
            EndNow();
            return;
        }
...
Player* Battleground::_GetPlayer(ObjectGuid guid, bool offlineRemove, char const* context) const
{
    ...
    if (!offlineRemove)
    {
        ...
        if (!player)
            SC_LOG_ERROR("bg.battleground", "Battleground::{}: player ({}) not found for BG (map: {}, instance id: {})!",
...
void Battleground::SendBroadcastText(uint32 id, ChatMsg msgType, WorldObject const* target)
{
    ...
    {
        SC_LOG_ERROR("bg.battleground", "Battleground::SendBroadcastText: `broadcast_text` (ID: {}) was not found", id);
        return;
    }

    Syphrena::BroadcastTextBuilder builder(nullptr, msgType, id, GENDER_MALE, target);
...
uint32 Battleground::GetBonusHonorFromKill(uint32 kills) const
{
    ...
    return Syphrena::Honor::hk_honor_at_level(maxLevel, float(kills));
}
...
void Battleground::RemovePlayerAtLeave(ObjectGuid guid, bool Transport, bool SendPacket)
{
    ...
    if (player)
    {
        ...
        SC_LOG_DEBUG("bg.battleground", "Removed player {} from Battleground.", player->GetName());
    }
...
void Battleground::Reset()
{
    ...
    if (m_InvitedAlliance > 0 || m_InvitedHorde > 0)
        SC_LOG_ERROR("bg.battleground", "Battleground::Reset: one of the counters is not 0 (alliance: {}, horde: {}) for BG (map: {}, instance id: {})!",
...
void Battleground::StartBattleground()
{
    ...
    if (m_IsRated)
        SC_LOG_DEBUG("bg.arena", "Arena match type: {} for Team1Id: {} - Team2Id: {} started.", m_ArenaType, m_ArenaTeamIds[TEAM_ALLIANCE], m_ArenaTeamIds[TEAM_HORDE]);
}
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/, GOState goState)
{
    ...
    {
        SC_LOG_DEBUG("bg.battleground", "Battleground::AddObject: gameoobject [entry: {}, object type: {}] for BG (map: {}) has zeroed rotation fields, "
    ...
    {
        SC_LOG_ERROR("bg.battleground", "Battleground::AddObject: cannot create gameobject (entry: {}) for BG (map: {}, instance id: {})!",
...
void Battleground::DoorClose(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        SC_LOG_ERROR("bg.battleground", "Battleground::DoorClose: door gameobject (type: {}, {}) not found for BG (map: {}, instance id: {})!",
...
void Battleground::DoorOpen(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        SC_LOG_ERROR("bg.battleground", "Battleground::DoorOpen: door gameobject (type: {}, {}) not found for BG (map: {}, instance id: {})!",
            type, BgObjects[type].ToString(), m_MapId, m_InstanceID);
}
...
GameObject* Battleground::GetBGObject(uint32 type, bool logError)
{
    ...
    {
        if (logError)
            SC_LOG_ERROR("bg.battleground", "Battleground::GetBGObject: gameobject (type: {}, {}) not found for BG (map: {}, instance id: {})!",
                type, BgObjects[type].ToString(), m_MapId, m_InstanceID);
        else
            SC_LOG_INFO("bg.battleground", "Battleground::GetBGObject: gameobject (type: {}, {}) not found for BG (map: {}, instance id: {})!",
                type, BgObjects[type].ToString(), m_MapId, m_InstanceID);
    }
    return obj;
}
...
Creature* Battleground::GetBGCreature(uint32 type, bool logError)
{
    ...
    {
        if (logError)
            SC_LOG_ERROR("bg.battleground", "Battleground::GetBGCreature: creature (type: {}, {}) not found for BG (map: {}, instance id: {})!",
                type, BgCreatures[type].ToString(), m_MapId, m_InstanceID);
        else
            SC_LOG_INFO("bg.battleground", "Battleground::GetBGCreature: creature (type: {}, {}) not found for BG (map: {}, instance id: {})!",
                type, BgCreatures[type].ToString(), m_MapId, m_InstanceID);
    }
    return creature;
}
...
Creature* Battleground::AddCreature(uint32 entry, uint32 type, float x, float y, float z, float o, TeamId /*teamId = TEAM_NEUTRAL*/, uint32 respawntime /*= 0*/, Transport* transport)
{
    ...
    {
        SC_LOG_ERROR("bg.battleground", "Battleground::AddCreature: cannot create creature (entry: {}) for BG (map: {}, instance id: {})!",
            entry, m_MapId, m_InstanceID);
        delete creature;
        return nullptr;
    }
    ...
    if (!cinfo)
    {
        SC_LOG_ERROR("bg.battleground", "Battleground::AddCreature: creature template (entry: {}) does not exist for BG (map: {}, instance id: {})!",
            entry, m_MapId, m_InstanceID);
        delete creature;
        return nullptr;
    }
...
bool Battleground::DelCreature(uint32 type)
{
    ...
    SC_LOG_ERROR("bg.battleground", "Battleground::DelCreature: creature (type: {}, {}) not found for BG (map: {}, instance id: {})!",
        type, BgCreatures[type].ToString(), m_MapId, m_InstanceID);
    BgCreatures[type].Clear();
    return false;
}
...
bool Battleground::DelObject(uint32 type)
{
    ...
    SC_LOG_ERROR("bg.battleground", "Battleground::DelObject: gameobject (type: {}, {}) not found for BG (map: {}, instance id: {})!",
        type, BgObjects[type].ToString(), m_MapId, m_InstanceID);
    BgObjects[type].Clear();
    return false;
}
...
bool Battleground::RemoveObjectFromWorld(uint32 type)
{
    ...
    SC_LOG_INFO("bg.battleground", "Battleground::RemoveObjectFromWorld: gameobject (type: {}, {}) not found for BG (map: {}, instance id: {})!",
        type, BgObjects[type].ToString(), m_MapId, m_InstanceID);
    return false;
}
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z, float o, TeamId teamId /*= TEAM_NEUTRAL*/)
{
    ...
    SC_LOG_ERROR("bg.battleground", "Battleground::AddSpiritGuide: cannot create spirit guide (type: {}, entry: {}) for BG (map: {}, instance id: {})!",
...
void Battleground::SendMessageToAll(uint32 entry, ChatMsg msgType, Player const* source)
{
    ...
    Syphrena::SyphrenaStringChatBuilder builder(nullptr, msgType, entry, source);
    Syphrena::LocalizedPacketDo<Syphrena::SyphrenaStringChatBuilder> localizer(builder);
    BroadcastWorker(localizer);
}
...
void Battleground::PSendMessageToAll(uint32 entry, ChatMsg msgType, Player const* source, ...)
{
    ...
    Syphrena::SyphrenaStringChatBuilder builder(nullptr, msgType, entry, source, &ap);
    Syphrena::LocalizedPacketDo<Syphrena::SyphrenaStringChatBuilder> localizer(builder);
    BroadcastWorker(localizer);
...
void Battleground::HandleTriggerBuff(ObjectGuid go_guid)
{
    if (!FindBgMap())
    {
        SC_LOG_ERROR("bg.battleground", "Battleground::HandleTriggerBuff called with null bg map, {}", go_guid.ToString());
        return;
    }
    ...
    if (index < 0)
    {
        SC_LOG_ERROR("bg.battleground", "Battleground::HandleTriggerBuff: cannot find buff gameobject ({}, entry: {}, type: {}) in internal data for BG (map: {}, instance id: {})!",
...
int32 Battleground::GetObjectType(ObjectGuid guid)
{
    for (uint32 i = 0; i < BgObjects.size(); ++i)
        if (BgObjects[i] == guid)
            return i;
    SC_LOG_ERROR("bg.battleground", "Battleground::GetObjectType: player used gameobject ({}) which is not in internal data for BG (map: {}, instance id: {}), cheating?",
...
uint32 Battleground::GetTeamScore(uint32 teamId) const
{
    ...
    SC_LOG_ERROR("bg.battleground", "GetTeamScore with wrong Team {} for BG {}", teamId, GetTypeID());
    return 0;
}

void Battleground::HandleAreaTrigger(Player* player, uint32 trigger)
{
    SC_LOG_DEBUG("bg.battleground", "Unhandled AreaTrigger {} in Battleground {}. Player coords (x: {}, y: {}, z: {})",
...
{
    SC_LOG_ERROR("bg.battleground", "Battleground::CheckAchievementCriteriaMeet: No implementation for criteria {}", criteriaId);
    return false;
}
