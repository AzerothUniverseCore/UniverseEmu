...
namespace LuaGlobalFunctions
{
     ...
     *
     * The result will be either `MaNGOS`, `cMaNGOS`, or `SyphrenaCore`.
     *
     ...
     * - for SyphrenaCore returns the realmID as it is in the conf file.
     ...
     *
     * - For SyphrenaCore returns the date of the last revision, e.g. `2015-08-26 22:53:12 +0300`
    ...
    int GetPlayersInWorld(lua_State* L)
    {
        ...
        for (HashMapHolder<Player>::MapType::const_iterator it = m.begin(); it != m.end(); ++it)
        {
            if (Player* player = it->second)
            {
                ...
                if ((static_cast<uint32>(team) == TEAM_NEUTRAL || static_cast<uint32>(player->GetTeamId()) == team) && (!onlyGM || player->IsGameMaster()))
                {
                    Eluna::Push(L, player);
                    lua_rawseti(L, tbl, ++i);
                }
     ...
     * [Player] and [Creature] for example can have the same low GUID but not GUID.
     * 
     * On SyphrenaCore all low GUIDs are different for all objects of the same type.
     ...
     * </pre>
     *
     * @param uint32 event : event ID, refer to TicketEvents above
     * @param function function : function to register
     * @param uint32 shots = 0 : the number of times the function will be called, 0 means "always call this function"
     */
    ...
    /**
     * Registers a [Creature] event handler. It uses an AllCreatureScript-style hook so it doesn't need a creature entry as a key.
     *
     * <pre>
     ...
     *     ALL_CREATURE_EVENT_ON_MODIFY_PERIODIC_DAMAGE_AURAS_TICK = 9, // (event, creature, target, damage, spellInfo) - Can return new damage amount - NOT YET WIRED, see note in Hooks.h
    ...
    */
     ...
     * @proto (entry)
     * @proto (entry, event_type)
     * @param uint32 entry : the ID of a [Spell]
