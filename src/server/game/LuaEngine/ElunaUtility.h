...
#if defined(SYPHRENA) || defined(AZEROTHCORE)
...
#define ELUNA_LOG_SC_FMT(SC_LOG_MACRO, ...) \
    try { \
        std::string message = fmt::sprintf(__VA_ARGS__); \
        SC_LOG_MACRO("eluna", "{}", message); \
    } catch (const std::exception& e) { \
        SC_LOG_MACRO("eluna", "Failed to format log message: {}", e.what()); \
    }
#define ELUNA_LOG_INFO(...)     ELUNA_LOG_SC_FMT(SC_LOG_INFO, __VA_ARGS__);
#define ELUNA_LOG_ERROR(...)    ELUNA_LOG_SC_FMT(SC_LOG_ERROR, __VA_ARGS__);
#define ELUNA_LOG_DEBUG(...)    ELUNA_LOG_SC_FMT(SC_LOG_DEBUG, __VA_ARGS__);
#else
#define ELUNA_LOG_INFO(...)     SC_LOG_INFO("eluna", __VA_ARGS__);
#define ELUNA_LOG_ERROR(...)    SC_LOG_ERROR("eluna", __VA_ARGS__);
#define ELUNA_LOG_DEBUG(...)    SC_LOG_DEBUG("eluna", __VA_ARGS__);
#endif
...
#if defined(SYPHRENA) || defined(AZEROTHCORE) || defined(MANGOS) || defined(CMANGOS) || defined(VMANGOS)
