...
void LFGPlayerScript::OnLogin(Player* player, bool /*loginFirst*/)
{
    ...
    if (Group const* group = player->GetGroup())
    {
        ...
        if (gguid != gguid2)
        {
            SC_LOG_ERROR("lfg", "{} on group {} but LFG has group {} saved... Fixing.",
...
void LFGPlayerScript::OnMapChanged(Player* player)
{
    ...
    if (sLFGMgr->inLfgDungeonMap(player->GetGUID(), map->GetId(), map->GetDifficulty()))
    {
        ...
        if (!group)
        {
            ...
            SC_LOG_ERROR("lfg", "LFGPlayerScript::OnMapChanged, Player {} {} is in LFG dungeon map but does not have a valid group! "
    ...
    else
    {
        ...
        //end npcbot
        {
            ...
            group->Disband();
            SC_LOG_DEBUG("lfg", "LFGPlayerScript::OnMapChanged, Player {}({}) is last in the lfggroup so we disband the group.",
...
void LFGGroupScript::OnAddMember(Group* group, ObjectGuid guid)
{
    ...
    if (leader == guid)
    {
        SC_LOG_DEBUG("lfg", "LFGScripts::OnAddMember [{}]: added [{}] leader [{}]", gguid.ToString(), guid.ToString(), leader.ToString());
    ...
    else
    {
        ...
        SC_LOG_DEBUG("lfg", "LFGScripts::OnAddMember [{}]: added [{}] leader [{}] gstate: {}, state: {}", gguid.ToString(), guid.ToString(), leader.ToString(), gstate, state);
...
void LFGGroupScript::OnRemoveMember(Group* group, ObjectGuid guid, RemoveMethod method, ObjectGuid kicker, char const* reason)
{
    ...
    ObjectGuid gguid = group->GetGUID();
    SC_LOG_DEBUG("lfg", "LFGScripts::OnRemoveMember [{}]: remove [{}] Method: {} Kicker: [{}] Reason: {}",
...
void LFGGroupScript::OnDisband(Group* group)
{
    ...
    ObjectGuid gguid = group->GetGUID();
    SC_LOG_DEBUG("lfg", "LFGScripts::OnDisband [{}]", gguid.ToString());
...
void LFGGroupScript::OnChangeLeader(Group* group, ObjectGuid newLeaderGuid, ObjectGuid oldLeaderGuid)
{
    ...
    SC_LOG_DEBUG("lfg", "LFGScripts::OnChangeLeader [{}]: old [{}] new [{}]",
...
void LFGGroupScript::OnInviteMember(Group* group, ObjectGuid guid)
{
    ...
    ObjectGuid gguid = group->GetGUID();
    ObjectGuid leader = group->GetLeaderGUID();
    SC_LOG_DEBUG("lfg", "LFGScripts::OnInviteMember [{}]: invite [{}] leader [{}]",
