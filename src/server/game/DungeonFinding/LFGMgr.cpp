...
void LFGMgr::LoadRewards()
{
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 lfg dungeon rewards. DB table `lfg_dungeon_rewards` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "Dungeon {} specified in table `lfg_dungeon_rewards` does not exist!", dungeonId);
            continue;
        }

        if (!maxLevel || maxLevel > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
        {
            SC_LOG_ERROR("sql.sql", "Level {} specified for dungeon {} in table `lfg_dungeon_rewards` can never be reached!", maxLevel, dungeonId);
            maxLevel = sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL);
        }

        if (!firstQuestId || !sObjectMgr->GetQuestTemplate(firstQuestId))
        {
            SC_LOG_ERROR("sql.sql", "First quest {} specified for dungeon {} in table `lfg_dungeon_rewards` does not exist!", firstQuestId, dungeonId);
            continue;
        }

        if (otherQuestId && !sObjectMgr->GetQuestTemplate(otherQuestId))
        {
            SC_LOG_ERROR("sql.sql", "Other quest {} specified for dungeon {} in table `lfg_dungeon_rewards` does not exist!", otherQuestId, dungeonId);
            otherQuestId = 0;
        }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} lfg dungeon rewards in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void LFGMgr::LoadLFGDungeons(bool reload /* = false */)
{
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 lfg entrance positions. DB table `lfg_dungeon_template` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "table `lfg_dungeon_template` contains coordinates for wrong dungeon {}", dungeonId);
            continue;
        }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} lfg entrance positions in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    for (LFGDungeonContainer::iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
    {
        ...
        if (dungeon.type != LFG_TYPE_RANDOM && dungeon.x == 0.0f && dungeon.y == 0.0f && dungeon.z == 0.0f)
        {
            ...
            if (!at)
            {
                SC_LOG_ERROR("sql.sql", "Failed to load dungeon {}, cant find areatrigger for map {}", dungeon.name, dungeon.map);
                continue;
            }
...
void LFGMgr::Update(uint32 diff)
{
    ...
    for (LfgQueueContainer::iterator it = QueuesStore.begin(); it != QueuesStore.end(); ++it)
        if (uint8 newProposals = it->second.FindGroups())
            SC_LOG_DEBUG("lfg.update", "Found {} new groups in queue {}", newProposals, it->first);
...
void LFGMgr::JoinLfg(Player* player, uint8 roles, LfgDungeonSet& dungeons, const std::string& comment)
{
    ...
    if (joinData.result != LFG_JOIN_OK)
    {
        SC_LOG_DEBUG("lfg.join", "{} joining with {} members. Result: {}, Dungeons: {}",
    ...
    if (isRaid)
    {
        SC_LOG_DEBUG("lfg.join", "{} trying to join raid browser and it's disabled.", guid.ToString());
        return;
    }
    ...
    SC_LOG_DEBUG("lfg.join", "{} joined ({}), Members: {}. Dungeons ({}): {}", guid.ToString(),
...
void LFGMgr::LeaveLfg(ObjectGuid guid, bool disconnected)
{
    ...
    SC_LOG_DEBUG("lfg.leave", "{} left ({})", guid.ToString(), guid == gguid ? "group" : "player");
...
void LFGMgr::UpdateProposal(uint32 proposalId, ObjectGuid guid, bool accept)
{
    ...
    SC_LOG_DEBUG("lfg.proposal.update", "{}, Proposal {}, Selection: {}", guid.ToString(), proposalId, accept);
    if (!accept)
    {
...
void LFGMgr::RemoveProposal(LfgProposalContainer::iterator itProposal, LfgUpdateType type)
{
    ...
    SC_LOG_DEBUG("lfg.proposal.remove", "Proposal {}, state FAILED, UpdateType {}", itProposal->first, type);
    ...
    for (LfgProposalPlayerContainer::const_iterator it = proposal.players.begin(); it != proposal.players.end(); ++it)
    {
        ...
        if (toRemove.find(gguid) != toRemove.end())         // Didn't accept or in same group that someone that didn't accept
        {
            ...
            if (it->second.accept == LFG_ANSWER_DENY)
            {
                ...
                SC_LOG_DEBUG("lfg.proposal.remove", "{} didn't accept. Removing from queue and compatible cache", guid.ToString());
            }
            else
            {
                updateData.updateType = LFG_UPDATETYPE_REMOVED_FROM_QUEUE;
                SC_LOG_DEBUG("lfg.proposal.remove", "{} in same group that someone that didn't accept. Removing from queue and compatible cache", guid.ToString());
            }
        ...
        {
            SC_LOG_DEBUG("lfg.proposal.remove", "Readding {} to queue.", guid.ToString());
...
void LFGMgr::TeleportPlayer(Player* player, bool out, bool fromOpcode /*= false*/)
{
    ...
    if (!dungeon)
    {
        SC_LOG_DEBUG("lfg.teleport", "Player {} not in group/lfggroup or dungeon not found!",
            player->GetName());
    ...
    if (out)
    {
        SC_LOG_DEBUG("lfg.teleport", "Player {} is being teleported out. Current Map {} - Expected Map {}",
    ...
    SC_LOG_DEBUG("lfg.teleport", "Player {} is being teleported in to map {} "
...
void LFGMgr::FinishDungeon(ObjectGuid gguid, const uint32 dungeonId, Map const* currMap)
{
    ...
    {
        SC_LOG_DEBUG("lfg.dungeon.finish", "Group {} finished dungeon {} but queued for {}", gguid.ToString(), dungeonId, gDungeonId);
        return;
    }

    if (GetState(gguid) == LFG_STATE_FINISHED_DUNGEON) // Shouldn't happen. Do not reward multiple times
    {
        SC_LOG_DEBUG("lfg.dungeon.finish", "Group: {} already rewarded", gguid.ToString());
        return;
    }
    ...
    for (GuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        {
            SC_LOG_DEBUG("lfg.dungeon.finish", "Group: {}, Player: {} already rewarded", gguid.ToString(), guid.ToString());
            continue;
        }
        ...
        {
            SC_LOG_DEBUG("lfg.dungeon.finish", "Group: {}, Player: {} dungeon {} is not random or seasonal", gguid.ToString(), guid.ToString(), rDungeonId);
            continue;
        }
        ...
        if (!player)
        {
            SC_LOG_DEBUG("lfg.dungeon.finish", "Group: {}, Player: {} not found in world", gguid.ToString(), guid.ToString());
            continue;
        }

        if (player->FindMap() != currMap)
        {
            SC_LOG_DEBUG("lfg.dungeon.finish", "Group: {}, Player: {} is in a different map", gguid.ToString(), guid.ToString());
            continue;
        }
        ...
        {
            SC_LOG_DEBUG("lfg.dungeon.finish", "Group: {}, Player: {} is in map {} and should be in {} to get reward", gguid.ToString(), guid.ToString(), player->GetMapId(), mapId);
            continue;
        }
        ...
        // Give rewards
        SC_LOG_DEBUG("lfg.dungeon.finish", "Group: {}, Player: {} done dungeon {}, {} previously done.", gguid.ToString(), guid.ToString(), GetDungeon(gguid), done ? " " : " not");
...
LfgState LFGMgr::GetState(ObjectGuid guid)
{
    ...
    if (guid.IsGroup())
    {
        ...
        SC_LOG_TRACE("lfg.data.group.state.get", "Group: {}, State: {}", guid.ToString(), GetStateString(state));
    }
    else
    {
        state = PlayersStore[guid].GetState();
        SC_LOG_TRACE("lfg.data.player.state.get", "Player: {}, State: {}", guid.ToString(), GetStateString(state));
    }
...
LfgState LFGMgr::GetOldState(ObjectGuid guid)
{
    ...
    if (guid.IsGroup())
    {
        ...
        SC_LOG_TRACE("lfg.data.group.oldstate.get", "Group: {}, Old state: {}", guid.ToString(), state);
    }
    else
    {
        state = PlayersStore[guid].GetOldState();
        SC_LOG_TRACE("lfg.data.player.oldstate.get", "Player: {}, Old state: {}", guid.ToString(), state);
    }
...
bool LFGMgr::IsVoteKickActive(ObjectGuid gguid)
{
    ...
    SC_LOG_TRACE("lfg.data.group.votekick.get", "Group: {}, Active: {}", gguid.ToString(), active);
...
uint32 LFGMgr::GetDungeon(ObjectGuid guid, bool asId /*= true */)
{
    ...
    SC_LOG_TRACE("lfg.data.group.dungeon.get", "Group: {}, asId: {}, Dungeon: {}", guid.ToString(), asId, dungeon);
    return dungeon;
}
...
uint32 LFGMgr::GetDungeonMapId(ObjectGuid guid)
{
    ...
    SC_LOG_TRACE("lfg.data.group.dungeon.map", "Group: {}, MapId: {} (DungeonId: {})", guid.ToString(), mapId, dungeonId);
...
uint8 LFGMgr::GetRoles(ObjectGuid guid)
{
    ...
    SC_LOG_TRACE("lfg.data.player.role.get", "Player: {}, Role: {}", guid.ToString(), roles);
    return roles;
}

const std::string& LFGMgr::GetComment(ObjectGuid guid)
{
    SC_LOG_TRACE("lfg.data.player.comment.get", "Player: {}, Comment: {}", guid.ToString(), PlayersStore[guid].GetComment());
    return PlayersStore[guid].GetComment();
}

LfgDungeonSet const& LFGMgr::GetSelectedDungeons(ObjectGuid guid)
{
    SC_LOG_TRACE("lfg.data.player.dungeons.selected.get", "Player: {}, Selected Dungeons: {}", guid.ToString(), ConcatenateDungeons(PlayersStore[guid].GetSelectedDungeons()));
    return PlayersStore[guid].GetSelectedDungeons();
}

LfgLockMap const LFGMgr::GetLockedDungeons(ObjectGuid guid)
{
    SC_LOG_TRACE("lfg.data.player.dungeons.locked.get", "Player: {}, LockedDungeons.", guid.ToString());
    LfgLockMap lock;
    Player* player = ObjectAccessor::FindConnectedPlayer(guid);
    if (!player)
    {
        SC_LOG_WARN("lfg.data.player.dungeons.locked.get", "Player: {} not ingame while retrieving his LockedDungeons.", guid.ToString());
        return lock;
    }
...
uint8 LFGMgr::GetKicksLeft(ObjectGuid guid)
{
    ...
    SC_LOG_TRACE("lfg.data.group.kickleft.get", "Group: {}, Kicks left: {}", guid.ToString(), kicks);
    return kicks;
}
...
void LFGMgr::RestoreState(ObjectGuid guid, char const* debugMsg)
{
    if (guid.IsGroup())
    {
        ...
        SC_LOG_TRACE("lfg.data.group.state.restore", "Group: {} ({}), State: {}, Old state: {}",
    ...
    else
    {
        ...
        SC_LOG_TRACE("lfg.data.player.state.restore", "Player: {} ({}), State: {}, Old state: {}",
            guid.ToString(), debugMsg, GetStateString(data.GetState()),
            GetStateString(data.GetOldState()));
...
void LFGMgr::SetState(ObjectGuid guid, LfgState state)
{
    ...
    {
        LfgGroupData& data = GroupsStore[guid];
        SC_LOG_TRACE("lfg.data.group.state.set", "Group: {}, New state: {}, Previous: {}, Old state: {}",
    ...
    {
        LfgPlayerData& data = PlayersStore[guid];
        SC_LOG_TRACE("lfg.data.player.state.set", "Player: {}, New state: {}, Previous: {}, OldState: {}",
            guid.ToString(), GetStateString(state), GetStateString(data.GetState()),
            GetStateString(data.GetOldState()));
...
void LFGMgr::SetVoteKick(ObjectGuid gguid, bool active)
{
    ...
    SC_LOG_TRACE("lfg.data.group.votekick.set", "Group: {}, New state: {}, Previous: {}",
...
{
    SC_LOG_TRACE("lfg.data.group.dungeon.set", "Group: {}, Dungeon: {}", guid.ToString(), dungeon);
    GroupsStore[guid].SetDungeon(dungeon);
}

void LFGMgr::SetRoles(ObjectGuid guid, uint8 roles)
{
    SC_LOG_TRACE("lfg.data.player.role.set", "Player: {}, Roles: {}", guid.ToString(), roles);
    PlayersStore[guid].SetRoles(roles);
}

void LFGMgr::SetComment(ObjectGuid guid, std::string const& comment)
{
    SC_LOG_TRACE("lfg.data.player.comment.set", "Player: {}, Comment: {}", guid.ToString(), comment);
    PlayersStore[guid].SetComment(comment);
}

void LFGMgr::SetSelectedDungeons(ObjectGuid guid, LfgDungeonSet const& dungeons)
{
    SC_LOG_TRACE("lfg.data.player.dungeon.selected.set", "Player: {}, Dungeons: {}", guid.ToString(), ConcatenateDungeons(dungeons));
...
void LFGMgr::DecreaseKicksLeft(ObjectGuid guid)
{
    ...
    SC_LOG_TRACE("lfg.data.group.kicksleft.decrease", "Group: {}, Kicks: {}", guid.ToString(), GroupsStore[guid].GetKicksLeft());
}

void LFGMgr::RemovePlayerData(ObjectGuid guid)
{
    SC_LOG_TRACE("lfg.data.player.remove", "Player: {}", guid.ToString());
...
{
    SC_LOG_TRACE("lfg.data.group.remove", "Group: {}", guid.ToString());
...
uint8 LFGMgr::GetTeam(ObjectGuid guid)
{
    ...
    SC_LOG_TRACE("lfg.data.player.team.get", "Player: {}, Team: {}", guid.ToString(), team);
    return team;
}
...
bool LFGMgr::AllQueued(GuidList const& check)
{
    ...
    for (GuidList::const_iterator it = check.begin(); it != check.end(); ++it)
    {
        ...
        {
            if (state != LFG_STATE_PROPOSAL)
                SC_LOG_DEBUG("lfg.allqueued", "Unexpected state found while trying to form new group. Guid: {}, State: {}", (*it).ToString(), GetStateString(state));
