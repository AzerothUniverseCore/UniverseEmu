...
Object::~Object()
{
    if (IsInWorld())
    {
        SC_LOG_FATAL("misc", "Object::~Object {} deleted but still in world!!", GetGUID().ToString());
        if (Item* item = ToItem())
            SC_LOG_FATAL("misc", "Item slot {}", item->GetSlot());
        ABORT();
    }

    if (m_objectUpdated)
    {
        SC_LOG_FATAL("misc", "Object::~Object {} deleted but still in update list!!", GetGUID().ToString());
        ABORT();
    }
...
void Object::BuildCreateUpdateBlockForPlayer(UpdateData* data, Player* target) const
{
    ...
    //SC_LOG_DEBUG("BuildCreateUpdate: update-type: {}, object-type: {} got flags: {:X}, flags2: {:X}", updateType, m_objectTypeId, flags, flags2);
...
bool Object::_LoadIntoDataField(std::string const& data, uint32 startOffset, uint32 count)
{
    ...
    {
        Optional<uint32> val = Syphrena::StringTo<uint32>(tokens[index]);
        if (!val)
            return false;
...
{
    SC_LOG_ERROR("misc", "Attempt to {} non-existing value field: {} (count: {}) for object typeid: {} type mask: {}", (set ? "set value to" : "get value from"), index, m_valuesCount, GetTypeId(), m_objectType);
...
{
    SC_LOG_DEBUG("misc", "MOVEMENT INFO");
    SC_LOG_DEBUG("misc", "{}", guid.ToString());
    SC_LOG_DEBUG("misc", "flags {}", flags);
    SC_LOG_DEBUG("misc", "flags2 {}", flags2);
    SC_LOG_DEBUG("misc", "time {} current time {}", flags2, uint64(::time(nullptr)));
    SC_LOG_DEBUG("misc", "position: `{}`", pos.ToString());
    if (flags & MOVEMENTFLAG_ONTRANSPORT)
    {
        SC_LOG_DEBUG("misc", "TRANSPORT:");
        SC_LOG_DEBUG("misc", "{}", transport.guid.ToString());
        SC_LOG_DEBUG("misc", "position: `{}`", transport.pos.ToString());
        SC_LOG_DEBUG("misc", "seat: {}", transport.seat);
        SC_LOG_DEBUG("misc", "time: {}", transport.time);
        if (flags2 & MOVEMENTFLAG2_INTERPOLATED_MOVEMENT)
            SC_LOG_DEBUG("misc", "time2: {}", transport.time2);
    }

    if ((flags & (MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_FLYING)) || (flags2 & MOVEMENTFLAG2_ALWAYS_ALLOW_PITCHING))
        SC_LOG_DEBUG("misc", "pitch: {}", pitch);

    SC_LOG_DEBUG("misc", "fallTime: {}", fallTime);
    if (flags & MOVEMENTFLAG_FALLING)
        SC_LOG_DEBUG("misc", "j_zspeed: {} j_sinAngle: {} j_cosAngle: {} j_xyspeed: {}", jump.zspeed, jump.sinAngle, jump.cosAngle, jump.xyspeed);

    if (flags & MOVEMENTFLAG_SPLINE_ELEVATION)
        SC_LOG_DEBUG("misc", "splineElevation: {}", splineElevation);
}
...
WorldObject::~WorldObject()
{
    ...
    if (IsStoredInWorldObjectGridContainer() && m_currMap)
    {
        if (GetTypeId() == TYPEID_CORPSE)
        {
            SC_LOG_FATAL("misc", "WorldObject::~WorldObject Corpse Type: {} ({}) deleted but still in map!!",
...
void WorldObject::GetRandomPoint(Position const& pos, float distance, float& rand_x, float& rand_y, float& rand_z) const
{
    ...
    Syphrena::NormalizeMapCoord(rand_x);
    Syphrena::NormalizeMapCoord(rand_y);
...
void WorldObject::SetMap(Map* map)
{
    ...
    if (m_currMap)
    {
        SC_LOG_FATAL("misc", "WorldObject::SetMap: obj {} new map {} {}, old map {} {}", (uint32)GetTypeId(), map->GetId(), map->GetInstanceId(), m_currMap->GetId(), m_currMap->GetInstanceId());
        ABORT();
    }
    m_currMap = map;
    m_mapId = map->GetId();
...
void WorldObject::AddObjectToRemoveList()
{
    ...
    if (!map)
    {
        SC_LOG_ERROR("misc", "Object {} at attempt add to move list not have valid map (Id: {}).", GetGUID().ToString(), GetMapId());
        return;
    }
...
TempSummon* Map::SummonCreature(uint32 entry, Position const& pos, SummonPropertiesEntry const* properties /*= nullptr*/, uint32 duration /*= 0*/, WorldObject* summoner /*= nullptr*/, uint32 spellId /*= 0*/, uint32 vehId /*= 0*/, bool visibleBySummonerOnly /*= false*/)
{
    ...
    Syphrena::AIRelocationNotifier notifier(*summon);
...
GameObject* WorldObject::SummonGameObject(uint32 entry, Position const& pos, QuaternionData const& rot, Seconds respawnTime, GOSummonType summonType)
{
    ...
    if (!goinfo)
    {
        SC_LOG_ERROR("sql.sql", "Gameobject template {} not found in database!", entry);
        return nullptr;
    }
...
void WorldObject::SummonCreatureGroup(uint8 group, std::list<TempSummon*>* list /*= nullptr*/)
{
    ...
    if (!data)
    {
        SC_LOG_WARN("scripts", "{} ({}) tried to summon non-existing summon group {}.", GetName(), GetGUID().ToString(), group);
        return;
    }
...
Creature* WorldObject::FindNearestCreature(uint32 entry, float range, bool alive) const
{
    ...
    Syphrena::NearestCreatureEntryWithLiveStateInObjectRangeCheck checker(*this, entry, alive, range);
    Syphrena::CreatureLastSearcher<Syphrena::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(this, creature, checker);
...
GameObject* WorldObject::FindNearestGameObject(uint32 entry, float range, bool spawnedOnly) const
{
    ...
    Syphrena::NearestGameObjectEntryInObjectRangeCheck checker(*this, entry, range, spawnedOnly);
    Syphrena::GameObjectLastSearcher<Syphrena::NearestGameObjectEntryInObjectRangeCheck> searcher(this, go, checker);
...
{
    GameObject* go = nullptr;
    Syphrena::NearestUnspawnedGameObjectEntryInObjectRangeCheck checker(*this, entry, range);
    Syphrena::GameObjectLastSearcher<Syphrena::NearestUnspawnedGameObjectEntryInObjectRangeCheck> searcher(this, go, checker);
    Cell::VisitGridObjects(this, searcher, range);
    return go;
}
...
{
    GameObject* go = nullptr;
    Syphrena::NearestGameObjectTypeInObjectRangeCheck checker(*this, type, range);
    Syphrena::GameObjectLastSearcher<Syphrena::NearestGameObjectTypeInObjectRangeCheck> searcher(this, go, checker);
    Cell::VisitGridObjects(this, searcher, range);
    return go;
}
...
Player* WorldObject::SelectNearestPlayer(float distance) const
{
    ...
    Syphrena::NearestPlayerInObjectRangeCheck checker(this, distance);
    Syphrena::PlayerLastSearcher<Syphrena::NearestPlayerInObjectRangeCheck> searcher(this, target, checker);
...
int32 WorldObject::CalcSpellDuration(SpellInfo const* spellInfo) const
{
        ...
        spellInfo->GetDuration() != spellInfo->GetMaxDuration())
    {
        ...
        if (bot && bot->IsNPCBot())
        {
            ...
            //SC_LOG_ERROR("scripts", "CalcSpellDuration bot {} veh spell {} cp {}",
...
FactionTemplateEntry const* WorldObject::GetFactionTemplateEntry() const
{
    ...
    if (!entry)
    {
        ...
        {
            case TYPEID_PLAYER:
                SC_LOG_ERROR("entities.unit", "Player {} has invalid faction (faction template id) #{}", ToPlayer()->GetName(), factionId);
                break;
            case TYPEID_UNIT:
                SC_LOG_ERROR("entities.unit", "Creature (template id: {}) has invalid faction (faction template Id) #{}", ToCreature()->GetCreatureTemplate()->Entry, factionId);
                break;
            case TYPEID_GAMEOBJECT:
                if (factionId) // Gameobjects may have faction template id = 0
                    SC_LOG_ERROR("entities.faction", "GameObject (template id: {}) has invalid faction (faction template Id) #{}", ToGameObject()->GetGOInfo()->entry, factionId);
                break;
            default:
                SC_LOG_ERROR("entities.unit", "Object (name={}, type={}) has invalid faction (faction template Id) #{}", GetName(), uint32(GetTypeId()), factionId);
                break;
        }
    }
...
SpellCastResult WorldObject::CastSpell(CastSpellTargetArg const& targets, uint32 spellId, CastSpellExtraArgs const& args /*= { }*/)
{
    ...
    if (!info)
    {
        SC_LOG_ERROR("entities.unit", "CastSpell: unknown spell {} by caster {}", spellId, GetGUID().ToString());
    ...
    if (!targets.Targets)
    {
        SC_LOG_ERROR("entities.unit", "CastSpell: Invalid target passed to spell cast {} by {}", spellId, GetGUID().ToString());
...
{
    Syphrena::AllGameObjectsWithEntryInRange check(this, entry, maxSearchRange);
    Syphrena::GameObjectListSearcher<Syphrena::AllGameObjectsWithEntryInRange> searcher(this, gameObjectContainer, check);
...
{
    Syphrena::AllCreaturesOfEntryInRange check(this, entry, maxSearchRange);
    Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher(this, creatureContainer, check);
    Cell::VisitGridObjects(this, searcher, maxSearchRange);
}
...
{
    Syphrena::AllDeadCreaturesInRange check(this, maxSearchRange, alive);
    Syphrena::CreatureListSearcher<Syphrena::AllDeadCreaturesInRange> searcher(this, creaturedeadContainer, check);
    Cell::VisitGridObjects(this, searcher, maxSearchRange);
}
...
{
    Syphrena::AnyPlayerInObjectRangeCheck checker(this, maxSearchRange, alive);
    Syphrena::PlayerListSearcher<Syphrena::AnyPlayerInObjectRangeCheck> searcher(this, playerContainer, checker);
...
void WorldObject::GetNearPoint2D(WorldObject const* searcher, float& x, float& y, float distance2d, float absAngle) const
{
    ...
    Syphrena::NormalizeMapCoord(x);
    Syphrena::NormalizeMapCoord(y);
}
...
void WorldObject::MovePosition(Position &pos, float dist, float angle)
{
    ...
    if (!Syphrena::IsValidMapCoord(destx, desty, pos.m_positionZ))
    {
        SC_LOG_FATAL("misc", "WorldObject::MovePosition: Object {} has invalid coordinates X: {} and Y: {} were passed!",
...
void WorldObject::MovePositionToFirstCollision(Position &pos, float dist, float angle) const
{
    ...
    // Prevent invalid coordinates here, position is unchanged
    if (!Syphrena::IsValidMapCoord(destx, desty))
    {
        SC_LOG_FATAL("misc", "WorldObject::MovePositionToFirstCollision invalid coordinates X: {} and Y: {} were passed!", destx, desty);
        return;
    }
...
void WorldObject::DestroyForNearbyPlayers()
{
    ...
    Syphrena::AnyPlayerInObjectRangeCheck check(this, GetVisibilityRange(), false);
    Syphrena::PlayerListSearcher<Syphrena::AnyPlayerInObjectRangeCheck> searcher(this, targets, check);
...
void WorldObject::UpdateObjectVisibility(bool /*forced*/)
{
    ...
    Syphrena::VisibleChangesNotifier notifier(*this);
    Cell::VisitWorldObjects(this, notifier, GetVisibilityRange());
}
...
template SC_GAME_API void WorldObject::GetGameObjectListWithEntryInGrid(std::list<GameObject*>&, uint32, float) const;
template SC_GAME_API void WorldObject::GetGameObjectListWithEntryInGrid(std::deque<GameObject*>&, uint32, float) const;
template SC_GAME_API void WorldObject::GetGameObjectListWithEntryInGrid(std::vector<GameObject*>&, uint32, float) const;

template SC_GAME_API void WorldObject::GetCreatureListWithEntryInGrid(std::list<Creature*>&, uint32, float) const;
template SC_GAME_API void WorldObject::GetCreatureListWithEntryInGrid(std::deque<Creature*>&, uint32, float) const;
template SC_GAME_API void WorldObject::GetCreatureListWithEntryInGrid(std::vector<Creature*>&, uint32, float) const;

template SC_GAME_API void WorldObject::GetDeadCreatureListInGrid(std::list<Creature*>&, float, bool) const;
template SC_GAME_API void WorldObject::GetDeadCreatureListInGrid(std::deque<Creature*>&, float, bool) const;
template SC_GAME_API void WorldObject::GetDeadCreatureListInGrid(std::vector<Creature*>&, float, bool) const;

template SC_GAME_API void WorldObject::GetPlayerListInGrid(std::list<Player*>&, float, bool) const;
template SC_GAME_API void WorldObject::GetPlayerListInGrid(std::deque<Player*>&, float, bool) const;
template SC_GAME_API void WorldObject::GetPlayerListInGrid(std::vector<Player*>&, float, bool) const;
