...
void AddItemsSetItem(Player* player, Item* item)
{
    ...
    if (!set)
    {
        SC_LOG_ERROR("sql.sql", "Item set {} for item (id {}) not found, mods not applied.", setid, proto->ItemId);
        return;
    }
    ...
    for (uint32 x = 0; x < MAX_ITEM_SET_SPELLS; ++x)
    {
        ...
        for (uint32 y = 0; y < MAX_ITEM_SET_SPELLS; ++y)
        {
            if (!eff->spells[y])                             // free slot
            {
                ...
                if (!spellInfo)
                {
                    SC_LOG_ERROR("entities.player.items", "WORLD: unknown spell id {} in items set {} effects", set->SetSpellID[x], setid);
                    break;
                }
...
void RemoveItemsSetItem(Player*player, ItemTemplate const* proto)
{
    ...
    if (!set)
    {
        SC_LOG_ERROR("sql.sql", "Item set #{} for item #{} not found, mods not removed.", setid, proto->ItemId);
        return;
    }
...
void Item::UpdateDuration(Player* owner, uint32 diff)
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Item::UpdateDuration Item (Entry: {} Duration {} Diff {})", GetEntry(), GetUInt32Value(ITEM_FIELD_DURATION), diff);
...
bool Item::LoadFromDB(ObjectGuid::LowType guid, ObjectGuid owner_guid, Field* fields, uint32 entry)
{
    ...
    if (!proto)
    {
        SC_LOG_ERROR("entities.item", "Invalid entry {} for item %s. Refusing to load.", GetEntry(), GetGUID().ToString());
        return false;
    }
    ...
    std::vector<std::string_view> tokens = Syphrena::Tokenize(fields[4].GetStringView(), ' ', false);
    if (tokens.size() == MAX_ITEM_PROTO_SPELLS)
    {
        for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
        {
            if (Optional<int32> charges = Syphrena::StringTo<int32>(tokens[i]))
                SetSpellCharges(i, *charges);
            else
                SC_LOG_ERROR("entities.item", "Invalid charge info '%s' for item %s, charge data not loaded.", std::string(tokens[i]), GetGUID().ToString());
        }
    }
    ...
    if (!_LoadIntoDataField(fields[6].GetString(), ITEM_FIELD_ENCHANTMENT_1_1, MAX_ENCHANTMENT_SLOT * MAX_ENCHANTMENT_OFFSET))
        SC_LOG_WARN("entities.item", "Invalid enchantment data '%s' for item %s. Forcing partial load.", fields[6].GetString(), GetGUID().ToString());
    ...
    switch (proto->Class)
    {
        case ITEM_CLASS_WEAPON:
            switch (proto->SubClass)
            {
                ...
                case ITEM_SUBCLASS_WEAPON_WARGLAIVES:     return  208;
        ...
        case ITEM_CLASS_ARMOR:
            switch (proto->SubClass)
            {
                ...
                case ITEM_SUBCLASS_ARMOR_RUBIS:    return  752;
...
void AddItemToUpdateQueueOf(Item* item, Player* player)
{
    ...
    {
        SC_LOG_DEBUG("entities.player.items", "AddItemToUpdateQueueOf - Owner's guid (%s) and player's guid (%s) don't match!",
...
void RemoveItemFromUpdateQueueOf(Item* item, Player* player)
{
    ...
    if (player->GetGUID() != item->GetOwnerGUID())
    {
        SC_LOG_DEBUG("entities.player.items", "RemoveItemFromUpdateQueueOf - Owner's guid (%s) and player's guid (%s) don't match!",
            item->GetOwnerGUID().ToString(), player->GetGUID().ToString());
        return;
    }
