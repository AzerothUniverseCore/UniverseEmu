...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::PresetTransmog");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::LoadPlayerSets");
    ...
    do
    {
        ...
        while (SetData.good())
        {
            ...
            if (slot >= EQUIPMENT_SLOT_END)
            {
                SC_LOG_ERROR("custom.transmog", "Item entry (FakeEntry: {}, playerGUID: {}, slot: {}, presetId: {}) has invalid slot, ignoring.", entry, player->GetGUID().GetCounter(), uint32(slot), uint32(PresetID));
                continue;
            }
            if (sObjectMgr->GetItemTemplate(entry))
            {
                ...
                SC_LOG_ERROR("custom.transmog", "Item entry (FakeEntry: {}, playerGUID: {}, slot: {}, presetId: {}) does not exist, ignoring.", entry, player->GetGUID().GetCounter(), uint32(slot), uint32(PresetID));
        }
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::GetSlotName");

    switch (slot)
    {
        case EQUIPMENT_SLOT_HEAD: return  "Head";// session->GetSyphrenaString(LANG_SLOT_NAME_HEAD);
        case EQUIPMENT_SLOT_SHOULDERS: return  "Shoulders";// session->GetSyphrenaString(LANG_SLOT_NAME_SHOULDERS);
        case EQUIPMENT_SLOT_BODY: return  "Shirt";// session->GetSyphrenaString(LANG_SLOT_NAME_BODY);
        case EQUIPMENT_SLOT_CHEST: return  "Chest";// session->GetSyphrenaString(LANG_SLOT_NAME_CHEST);
        case EQUIPMENT_SLOT_WAIST: return  "Waist";// session->GetSyphrenaString(LANG_SLOT_NAME_WAIST);
        case EQUIPMENT_SLOT_LEGS: return  "Legs";// session->GetSyphrenaString(LANG_SLOT_NAME_LEGS);
        case EQUIPMENT_SLOT_FEET: return  "Feet";// session->GetSyphrenaString(LANG_SLOT_NAME_FEET);
        case EQUIPMENT_SLOT_WRISTS: return  "Wrists";// session->GetSyphrenaString(LANG_SLOT_NAME_WRISTS);
        case EQUIPMENT_SLOT_HANDS: return  "Hands";// session->GetSyphrenaString(LANG_SLOT_NAME_HANDS);
        case EQUIPMENT_SLOT_BACK: return  "Back";// session->GetSyphrenaString(LANG_SLOT_NAME_BACK);
        case EQUIPMENT_SLOT_MAINHAND: return  "Main hand";// session->GetSyphrenaString(LANG_SLOT_NAME_MAINHAND);
        case EQUIPMENT_SLOT_OFFHAND: return  "Off hand";// session->GetSyphrenaString(LANG_SLOT_NAME_OFFHAND);
        case EQUIPMENT_SLOT_RANGED: return  "Ranged";// session->GetSyphrenaString(LANG_SLOT_NAME_RANGED);
        case EQUIPMENT_SLOT_TABARD: return  "Tabard";// session->GetSyphrenaString(LANG_SLOT_NAME_TABARD);
        default: return NULL;
    }
}
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::GetItemIcon");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::GetSlotIcon");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::GetItemLink");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::UpdateItem");
...
TransmogSyphrenaStrings Transmogrification::Transmogrify(Player* player, ObjectGuid itemGUID, uint8 slot, bool no_cost)
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::Transmogrify");

    // slot of the transmogrified item
    if (slot >= EQUIPMENT_SLOT_END)
    {
        SC_LOG_DEBUG("custom.transmog", "Transmogrification::Transmogrify - %s (%s) tried to transmogrify an %s with a wrong slot ({}) when transmogrifying items.", player->GetName().c_str(), player->GetGUID().ToString().c_str(), itemGUID.ToString().c_str(), slot);
    ...
    if (!itemGUID.IsEmpty())
    {
        ...
        {
            SC_LOG_DEBUG("custom.transmog", "Transmogrification::Transmogrify - %s (%s) tried to transmogrify with an invalid %s.", player->GetName().c_str(), player->GetGUID().ToString().c_str(), itemGUID.ToString().c_str());
    ...
    {
        SC_LOG_DEBUG("custom.transmog", "Transmogrification::Transmogrify - %s (%s) tried to transmogrify an invalid item in a valid slot (slot: {}).", player->GetName().c_str(), player->GetGUID().ToString().c_str(), slot);
    ...
    else
    {
        ...
        {
            SC_LOG_DEBUG("custom.transmog", "Transmogrification::Transmogrify - %s (%s) failed CanTransmogrifyItemWithItem ({} with {}).", player->GetName().c_str(), player->GetGUID().ToString().c_str(), itemTransmogrified->GetEntry(), itemTransmogrifier->GetEntry());
        ...
        if (!no_cost)
        {
            ...
            {
                if (cost < 0)
                    SC_LOG_DEBUG("custom.transmog", "Transmogrification::Transmogrify - %s (%s) transmogrification invalid cost (non negative, amount %i). Transmogrified {} with {}", player->GetName().c_str(), player->GetGUID().ToString().c_str(), -cost, itemTransmogrified->GetEntry(), itemTransmogrifier->GetEntry());
                else
                {
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::CanTransmogrifyItemWithItem");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::SuitableForTransmogrification");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::GetSpecialPrice");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::IsRangedWeapon");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::IsAllowed");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::IsNotAllowed");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::IsAllowedQuality");
...
{
    SC_LOG_DEBUG("custom.transmog", "Transmogrification::LoadConfig");
    ...
    {
        SC_LOG_INFO("custom.transmog", "Transmogrification.TokenEntry ({}) does not exist. Using default ({}).", TokenEntry, 49426);
        TokenEntry = 49426;
    }
}
