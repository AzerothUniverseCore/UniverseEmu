...
void GameObject::RemoveFromOwner()
{
    ...
    SC_LOG_DEBUG("misc", "Removed GameObject ({} Entry: {} SpellId: {} LinkedGO: {}) that just lost any reference to the owner ({}) GO list",
...
void GameObject::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (m_spawnId)
            Syphrena::Containers::MultimapErasePair(GetMap()->GetGameObjectBySpawnIdStore(), m_spawnId, this);
...
bool GameObject::Create(ObjectGuid::LowType guidlow, uint32 name_id, Map* map, uint32 phaseMask, Position const& pos, QuaternionData const& rotation, uint32 animprogress, GOState go_state, uint32 artKit /*= 0*/, bool dynamic, ObjectGuid::LowType spawnid, float size)
{
    ...
    if (!IsPositionValid())
    {
        SC_LOG_ERROR("misc", "Gameobject (GUID: {} Entry: {}) not created. Suggested coordinates isn't valid (X: {} Y: {})", guidlow, name_id, pos.GetPositionX(), pos.GetPositionY());
        return false;
    }
    ...
    if (!goinfo)
    {
        SC_LOG_ERROR("sql.sql", "Gameobject (GUID: {} Entry: {}) not created: non-existing entry in `gameobject_template`. Map: {} (X: {} Y: {} Z: {})", guidlow, name_id, map->GetId(), pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
        return false;
    }

    if (goinfo->type == GAMEOBJECT_TYPE_MO_TRANSPORT)
    {
        SC_LOG_ERROR("sql.sql", "Gameobject (GUID: {} Entry: {}) not created: gameobject type GAMEOBJECT_TYPE_MO_TRANSPORT cannot be manually created.", guidlow, name_id);
        return false;
    }
    ...
    {
        SC_LOG_ERROR("sql.sql", "Gameobject (GUID: {} Entry: {}) not created: non-existing GO type '{}' in `gameobject_template`. It will crash client if created.", guidlow, name_id, goinfo->type);
        return false;
    }
...
void GameObject::Update(uint32 diff)
{
    ...
    if (AI())
        ...
        SC_LOG_ERROR("misc", "Could not initialize GameObjectAI");
                ...
                case GAMEOBJECT_TYPE_TRANSPORT:
                {
                    ...
                    if (GetGoState() == GO_STATE_READY)
                    {
                        ...
                        if (node && m_goValue.Transport.CurrentSeg != node->TimeSeg)
                        {
                            ...
                            SC_LOG_DEBUG("misc", "Src: {} Dest: {}", src.toString(), pos.toString());
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (GetOwner())
                    {
                        ...
                        if (SpellInfo const* trapSpell = sSpellMgr->GetSpellInfo(goInfo->trap.spellId))
                        {
                            ...
                            Syphrena::WorldObjectSpellNearbyTargetCheck checker(radius, this, trapSpell, m_goValue.Trap.TargetSearcherCheckType, nullptr);
                            Syphrena::WorldObjectLastSearcher searcher(this, worldObjectTarget, checker, GRID_MAP_TYPE_MASK_CREATURE | GRID_MAP_TYPE_MASK_PLAYER);
                            Cell::VisitAllObjects(this, searcher, radius);
                            target = Object::ToUnit(worldObjectTarget);
                        }
                        else
                        {
                            Syphrena::NearestAttackableNoTotemUnitInObjectRangeCheck checker(this, radius);
                            Syphrena::UnitLastSearcher<Syphrena::NearestAttackableNoTotemUnitInObjectRangeCheck> searcher(this, target, checker);
                            Cell::VisitAllObjects(this, searcher, radius);
                        }
                    }
                    else
                    {
                        ...
                        Syphrena::AnyPlayerInObjectRangeCheck checker(this, radius);
                        Syphrena::PlayerSearcher<Syphrena::AnyPlayerInObjectRangeCheck> searcher(this, player, checker);
...
void GameObject::SaveToDB()
{
    ...
    if (!data)
    {
        SC_LOG_ERROR("misc", "GameObject::SaveToDB failed, cannot get gameobject data!");
        return;
    }
...
bool GameObject::LoadFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap, bool)
{
    ...
    if (!data)
    {
        SC_LOG_ERROR("sql.sql", "Gameobject (GUID: {}) not found in table `gameobject`, can't load. ", spawnId);
        return false;
    }
    ...
    else
    {
        if (!m_respawnCompatibilityMode)
        {
            SC_LOG_WARN("sql.sql", "GameObject {} (SpawnID {}) is not spawned by default, but tries to use a non-hack spawn system. This will not work. Defaulting to compatibility mode.", entry, spawnId);
...
/*static*/ bool GameObject::DeleteFromDB(ObjectGuid::LowType spawnId)
{
    ...
    sMapMgr->DoForAllMapsWithMapId(data->mapId,
        [spawnId, charTrans](Map* map) -> void
        {
            ...
            for (auto const& pair : Syphrena::Containers::MapEqualRange(map->GetGameObjectBySpawnIdStore(), spawnId))
...
void GameObject::SaveRespawnTime(uint32 forceDelay)
{
    if (m_goData && (forceDelay || m_respawnTime > GameTime::GetGameTime()) && m_spawnedByDefault)
    {
        ...
        GetMap()->SaveRespawnTime(SPAWN_TYPE_GAMEOBJECT, m_spawnId, GetEntry(), thisRespawnTime, Syphrena::ComputeGridCoord(GetPositionX(), GetPositionY()).GetId());
    }
}
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    Syphrena::NearestGameObjectFishingHole u_check(*this, range);
    Syphrena::GameObjectSearcher<Syphrena::NearestGameObjectFishingHole> checker(this, ok, u_check);
...
void GameObject::ActivateObject(GameObjectActions action, WorldObject* spellCaster, uint32 spellId, int32 effectIndex)
{
    ...
    switch (action)
    {
        case GameObjectActions::None:
            SC_LOG_FATAL("spell", "Spell {} has action type NONE in effect {}", spellId, effectIndex);
            break;
        ...
        case GameObjectActions::UseArtKit3:
        {
            ...
            if (artKitValue == 0)
                SC_LOG_ERROR("sql.sql", "GameObject {} hit by spell {} needs `artkit{}` in `gameobject_template_addon`", GetEntry(), spellId, artKitIndex);
            else
        ...
        case GameObjectActions::SetTapList:
            ...
            break;
        default:
            SC_LOG_ERROR("spell", "Spell {} has unhandled action {} in effect {}", spellId, int32(action), effectIndex);
            break;
    }
}
...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            if (Player* player = user->ToPlayer())
            {
                ...
                {
                    SC_LOG_DEBUG("maps.script", "Goober ScriptStart id {} for GO entry {} (GUID {}).", info->goober.eventId, GetEntry(), GetSpawnId());
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:                   //17
        {
            ...
            switch (getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    if (!zone_skill)
                        SC_LOG_ERROR("sql.sql", "Fishable areaId {} are not properly defined in `skill_fishing_base_level`.", subzone);
                    ...
                    SC_LOG_DEBUG("misc", "Fishing check (skill: {} zone min skill: {} chance {} roll: {}", skill, zone_skill, chance, roll);
            ...
            if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
                        if (Player* target = ObjectAccessor::GetPlayer(*this, Syphrena::Containers::SelectRandomContainerElement(m_unique_users)))
                    ...
                    if (GameObjectTemplate const* bgoinfo = GetGOInfo())
                    {
                        switch (bgoinfo->entry)
                        {
                            ...
                            case 208209:                        // Twin Peaks Horde Flag
                                if (botbg->GetTypeID(true) == BATTLEGROUND_TP)
                                    botbg->EventBotClickedOnFlag(bot, this);
                                break;
                        }
                    }
        ...
        case GAMEOBJECT_TYPE_BARBER_CHAIR:                  //32
        {
            ...
            if (player->GetRace() == RACE_GOBLIN_ || player->GetRace() == RACE_WORGEN)
            if (player->GetRace() == RACE_PANDAREN_HORDE || player->GetRace() == RACE_PANDAREN_ALLIANCE)
            if (player->GetRace() == RACE_BLOODELF_ILLIDARI || player->GetRace() == RACE_NIGHTELF_ILLIDARI)
            if (player->GetRace() == RACE_EREDAR || player->GetRace() == RACE_VOIDELF)
            if (player->GetRace() == RACE_VULPERA || player->GetRace() == RACE_NIGHTBORNE)
            if (player->GetRace() == RACE_LIGHTFORGED_DRAENEI || player->GetRace() == RACE_ZANDALARI_TROLL)
            if (player->GetRace() == RACE_DARK_IRON_DWARF || player->GetRace() == RACE_DARK_IRON_DWARF_H)
            if (player->GetRace() == RACE_HIGHELF || player->GetRace() == RACE_HIGHMOUNTAINTAUREN)
            if (player->GetRace() == RACE_VULPERA_A || player->GetRace() == RACE_DRACTHYR_H)
            if (player->GetRace() == RACE_DRACTHYR_A || player->GetRace() == RACE_MAGHAR)
            if (player->GetRace() == RACE_KULTIRAN)
            {
                player->GetSession()->SendNotification("Votre race ne peut pas utiliser le salon de coiffure. (En developpement)");
                return;
            }
        ...
        default:
            if (GetGoType() >= MAX_GAMEOBJECT_TYPE)
                SC_LOG_ERROR("misc", "GameObject::Use(): unit ({}, name: {}) tries to use object ({}, name: {}) of unknown type ({})",
    ...
    if (!sSpellMgr->GetSpellInfo(spellId))
    {
        if (user->GetTypeId() != TYPEID_PLAYER || !sOutdoorPvPMgr->HandleCustomSpell(user->ToPlayer(), spellId, this))
            SC_LOG_ERROR("misc", "WORLD: unknown spell id {} at use action for gameobject (Entry: {} GoType: {})", spellId, GetEntry(), GetGoType());
        else
            SC_LOG_DEBUG("outdoorpvp", "WORLD: {} non-dbc spell was handled by OutdoorPvP", spellId);
        return;
    }
