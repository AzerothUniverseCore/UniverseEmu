...
void Creature::AddToWorld()
{
    ...
    if (!IsInWorld())
    {
        ...
        SC_LOG_DEBUG("entities.unit", "Adding creature {} with DBGUID {} to world in map {}", GetGUID().ToString(), m_spawnId, GetMap()->GetId());
...
void Creature::RemoveFromWorld()
{
    if (IsInWorld())
    {
        ...
        if (m_spawnId)
            Syphrena::Containers::MultimapErasePair(GetMap()->GetCreatureBySpawnIdStore(), m_spawnId, this);

        SC_LOG_DEBUG("entities.unit", "Removing creature {} with DBGUID {} to world in map {}", GetGUID().ToString(), m_spawnId, GetMap()->GetId());
...
bool Creature::InitEntry(uint32 entry, CreatureData const* data /*= nullptr*/)
{
    ...
    if (!normalInfo)
    {
        SC_LOG_ERROR("sql.sql", "Creature::InitEntry creature entry {} does not exist.", entry);
        return false;
    }
    ...
    {
        SC_LOG_ERROR("sql.sql", "Creature (Entry: {}) has no model defined in table `creature_template`, can't load. ", entry);
        return false;
    }
    ...
    {
        SC_LOG_ERROR("sql.sql", "Creature (Entry: {}) has invalid model {} defined in table `creature_template`, can't load.", entry, displayID);
        return false;
    }
...
void Creature::Update(uint32 diff)
{
    ...
    switch (m_deathState)
    {
        case JUST_RESPAWNED:
            ...
            SC_LOG_ERROR("entities.unit", "Creature {} in wrong state: JUST_RESPAWNED (4)", GetGUID().ToString());
            break;
        case JUST_DIED:
            // Must not be called, see Creature::setDeathState JUST_DIED -> CORPSE promoting.
            SC_LOG_ERROR("entities.unit", "Creature {} in wrong state: JUST_DIED (1)", GetGUID().ToString());
            break;
        case DEAD:
        {
            //npcbot
            ...
            if (!m_respawnCompatibilityMode)
            {
                SC_LOG_ERROR("entities.unit", "Creature {} in wrong state: DEAD (3)", GetGUID().ToString());
                break;
            }
        ...
        case CORPSE:
        {
            ...
            else if (m_corpseRemoveTime <= GameTime::GetGameTime())
            {
                ...
                //end npcbot
                RemoveCorpse(false);
                SC_LOG_DEBUG("entities.unit", "Removing corpse... {} ", GetEntry());
            }
            break;
        }
        case ALIVE:
            ...
            if (m_regenTimer == 0)
            {
                if (!IsInEvadeMode())
                {
                    ...
                    else if (CanNotReachTarget())
                    {
                        ...
                        {
                            RegenerateHealth();
                            SC_LOG_DEBUG("entities.unit.chase", "RegenerateHealth() enabled because Creature cannot reach the target. Detail: {}", GetDebugInfo());
                        }
                        else
                            SC_LOG_DEBUG("entities.unit.chase", "RegenerateHealth() disabled even if the Creature cannot reach the target. Detail: {}", GetDebugInfo());
                    }
                }
...
void Creature::DoFleeToGetAssistance()
{
    ...
    if (radius >0)
    {
        ...
        Syphrena::NearestAssistCreatureInCreatureRangeCheck u_check(this, GetVictim(), radius);
        Syphrena::CreatureLastSearcher<Syphrena::NearestAssistCreatureInCreatureRangeCheck> searcher(this, creature, u_check);
...
bool Creature::Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 entry, Position const& pos, CreatureData const* data /*= nullptr*/, uint32 vehId /*= 0*/, bool dynamic)
{
    ...
    if (!cinfo)
    {
        SC_LOG_ERROR("sql.sql", "Creature::Create(): creature template (guidlow: {}, entry: {}) does not exist.", guidlow, entry);
        return false;
    }
    ...
    if (!IsPositionValid())
    {
        SC_LOG_ERROR("entities.unit", "Creature::Create(): given coordinates for creature (guidlow {}, entry {}) are not valid (X: {}, Y: {}, Z: {}, O: {})", guidlow, entry, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation());
        return false;
    }
    {
...
void Creature::SaveToDB()
{
    ...
    if (!data)
    {
        SC_LOG_ERROR("entities.unit", "Creature::SaveToDB failed, cannot get creature data!");
        return;
    }
...
bool Creature::CreateFromProto(ObjectGuid::LowType guidlow, uint32 entry, CreatureData const* data /*= nullptr*/, uint32 vehId /*= 0*/)
{
    ...
    if (!cinfo)
    {
        SC_LOG_ERROR("sql.sql", "Creature::CreateFromProto(): creature template (guidlow: {}, entry: {}) does not exist.", guidlow, entry);
        return false;
    }
...
bool Creature::LoadFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap, bool allowDuplicate)
{
    if (!allowDuplicate)
    {
        ...
        if (creatureBounds.first != creatureBounds.second)
        {
            for (auto itr = creatureBounds.first; itr != creatureBounds.second; ++itr)
            {
                ...
                {
                    SC_LOG_DEBUG("maps", "Would have spawned {} but {} already exists", spawnId, creatureBounds.first->second->GetGUID().ToString());
                    return false;
                }
                else
                {
                    despawnList.push_back(itr->second);
                    SC_LOG_DEBUG("maps", "Despawned dead instance of spawn {} ({})", spawnId, itr->second->GetGUID().ToString());
                }
            }
    ...
    if (!data)
    {
        SC_LOG_ERROR("sql.sql", "Creature (SpawnID {}) not found in table `creature`, can't load. ", spawnId);
        return false;
    }
    ...
    if (!m_respawnTime && !map->IsSpawnGroupActive(data->spawnGroupData->groupId))
    {
        if (!m_respawnCompatibilityMode)
        {
            ...
            {
                SC_LOG_ERROR("entities.unit", "Creature (SpawnID {}) trying to load in inactive spawn group '{}':\n{}", spawnId, data->spawnGroupData->name, GetDebugInfo());
                return false;
            }
        }
    ...
    if (m_respawnTime)
    {
        if (!m_respawnCompatibilityMode)
        {
            ...
            if (!sPoolMgr->IsPartOfAPool<Creature>(spawnId))
            {
                SC_LOG_ERROR("entities.unit", "Creature (SpawnID {}) trying to load despite a respawn timer in progress:\n{}", spawnId, GetDebugInfo());
                return false;
            }
        }
        else
        ...
        if (CanFly())
        {
            ...
            if (data->spawnPoint.GetPositionZ() - tz > 0.1f && Syphrena::IsValidMapCoord(tz))
    ...
    if (IsNPCBot())
    {
        ...
        SC_LOG_INFO("entities.unit", "Creature: loading npcbot {} (id: {})", GetName(), GetEntry());
        ASSERT(!IsInWorld());
...
/*static*/ bool Creature::DeleteFromDB(ObjectGuid::LowType spawnId)
{
    ...
    sMapMgr->DoForAllMapsWithMapId(data->mapId,
        [spawnId, charTrans](Map* map) -> void
        {
            ...
            for (auto const& pair : Syphrena::Containers::MapEqualRange(map->GetCreatureBySpawnIdStore(), spawnId))
...
{
    if (Syphrena::XP::GetColorCode(playerLevel, creatureLevel) != XP_GRAY)
        return false;
...
void Creature::Respawn(bool force)
{
    ...
    if (m_respawnCompatibilityMode)
    {
        ...
        {
            SC_LOG_DEBUG("entities.unit", "Respawning creature {} ({})", GetName(), GetGUID().ToString());
            m_respawnTime = 0;
    ...
    SC_LOG_DEBUG("entities.unit", "Respawning creature {} ({})",
...
Unit* Creature::SelectNearestTarget(float dist, bool playerOnly /* = false */) const
{
    ...
    Unit* target = nullptr;
    Syphrena::NearestHostileUnitCheck u_check(this, dist, playerOnly);
    Syphrena::UnitLastSearcher<Syphrena::NearestHostileUnitCheck> searcher(this, target, u_check);
...
Unit* Creature::SelectNearestTargetInAttackDistance(float dist) const
{
    ...
    {
        SC_LOG_ERROR("entities.unit", "Creature {} SelectNearestTargetInAttackDistance called with dist > MAX_VISIBILITY_DISTANCE. Distance set to ATTACK_DISTANCE.", GetGUID().ToString());
        dist = ATTACK_DISTANCE;
    }

    Unit* target = nullptr;
    Syphrena::NearestHostileUnitInAttackDistanceCheck u_check(this, dist);
    Syphrena::UnitLastSearcher<Syphrena::NearestHostileUnitInAttackDistanceCheck> searcher(this, target, u_check);
...
void Creature::SendAIReaction(AiReaction reactionType)
{
    ...
    SC_LOG_DEBUG("network", "WORLD: Sent SMSG_AI_REACTION, type {}.", reactionType);
}
...
void Creature::CallAssistance()
{
    if (!m_AlreadyCallAssistance && GetVictim() && !IsPet() && !IsCharmed())
    {
        ...
        if (radius > 0)
        {
            ...
            Syphrena::AnyAssistCreatureInRangeCheck u_check(this, GetVictim(), radius);
            Syphrena::CreatureListSearcher<Syphrena::AnyAssistCreatureInRangeCheck> searcher(this, assistList, u_check);
            Cell::VisitGridObjects(this, searcher, radius);
...
void Creature::CallForHelp(float radius)
{
    ...
    if (!target)
    {
        SC_LOG_ERROR("entities.unit", "Creature {} ({}) trying to call for help without being in combat.", GetEntry(), GetName());
        return;
    }

    Syphrena::CallOfHelpCreatureInRangeDo u_do(this, target, radius);
    Syphrena::CreatureWorker<Syphrena::CallOfHelpCreatureInRangeDo> worker(this, u_do);
...
void Creature::SaveRespawnTime(uint32 forceDelay)
{
    ...
    GetMap()->SaveRespawnTime(SPAWN_TYPE_CREATURE, m_spawnId, GetEntry(), thisRespawnTime, Syphrena::ComputeGridCoord(GetHomePosition().GetPositionX(), GetHomePosition().GetPositionY()).GetId());
}
...
bool Creature::LoadCreaturesAddon()
{
    ...
    if (!creatureAddon->auras.empty())
    {
        for (std::vector<uint32>::const_iterator itr = creatureAddon->auras.begin(); itr != creatureAddon->auras.end(); ++itr)
        {
            ...
            {
                SC_LOG_ERROR("sql.sql", "Creature {} has wrong spell {} defined in `auras` field.", GetGUID().ToString(), *itr);
                continue;
            }
            ...
            AddAura(*itr, this);
            SC_LOG_DEBUG("entities.unit", "Spell: {} added to creature {}", *itr, GetGUID().ToString());
        }
    }
...
void Creature::SetCannotReachTarget(bool cannotReach)
{
    ...
    if (cannotReach)
        SC_LOG_DEBUG("entities.unit.chase", "Creature::SetCannotReachTarget() called with true. Details: {}", GetDebugInfo());
}
...
Unit* Creature::SelectNearestHostileUnitInAggroRange(bool useLOS, bool ignoreCivilians) const
{
    ...
    Syphrena::NearestHostileUnitInAggroRangeCheck u_check(this, useLOS, ignoreCivilians);
    Syphrena::UnitSearcher<Syphrena::NearestHostileUnitInAggroRangeCheck> searcher(this, target, u_check);
...
bool Creature::HasSpellFocus(Spell const* focusSpell) const
{
    if (isDead()) // dead creatures cannot focus
    {
        ...
        {
            SC_LOG_WARN("entities.unit", "Creature '{}' (entry {}) has spell focus (spell id {}, delay {}ms) despite being dead.",
...
void Creature::ReacquireSpellFocusTarget()
{
    if (!HasSpellFocus())
    {
        SC_LOG_ERROR("entities.unit", "Creature::ReacquireSpellFocusTarget() being called with HasSpellFocus() returning false. {}", GetDebugInfo());
        return;
    }
...
void Creature::SetTextRepeatId(uint8 textGroup, uint8 id)
{
    ...
    if (std::find(repeats.begin(), repeats.end(), id) == repeats.end())
        ...
        SC_LOG_ERROR("sql.sql", "CreatureTextMgr: TextGroup {} for Creature({}) {}, id {} already added", uint32(textGroup), GetName(), GetGUID().ToString(), uint32(id));
}
...
bool Creature::LoadBotCreatureFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap, bool generated, uint32 entry, Position const* pos)
{
    ...
    if (!data)
    {
        if (!generated)
        {
            SC_LOG_ERROR("sql.sql", "Bot creature (GUID: {}) not found in table `creature`, can't load. ", spawnId);
            return false;
        }
        else
        {
    ...
    SC_LOG_INFO("entities.unit", "Creature: loading npcbot {} (id: {}, gen: {})", GetName(), GetEntry(), uint32(generated));
    ASSERT(!IsInWorld());
