...
void FormationMgr::AddCreatureToGroup(ObjectGuid::LowType leaderSpawnId, Creature* creature)
{
    ...
    if (itr != map->CreatureGroupHolder.end())
    {
        ...
        SC_LOG_DEBUG("entities.unit", "Group found: {}, inserting creature {}, Group InstanceID {}", leaderSpawnId, creature->GetGUID().ToString(), creature->GetInstanceId());
        ...
        auto bounds = Syphrena::Containers::MapEqualRange(map->GetCreatureBySpawnIdStore(), creature->GetSpawnId());
    ...
    {
        //Create new group
        SC_LOG_DEBUG("entities.unit", "Group not found: {}. Creating new group.", leaderSpawnId);
...
{
    SC_LOG_DEBUG("entities.unit", "Deleting member pointer to GUID: {} from group {}", group->GetLeaderSpawnId(), member->GetSpawnId());
    ...
    if (group->IsEmpty())
    {
        ...
        SC_LOG_DEBUG("entities.unit", "Deleting group with InstanceID {}", member->GetInstanceId());
...
void FormationMgr::LoadCreatureFormations()
{
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">>  Loaded 0 creatures in formations. DB table `creature_formations` is empty!");
        return;
    }
    ...
    do
    {
        ...
        // check data correctness
        {
            ...
            {
                SC_LOG_ERROR("sql.sql", "creature_formations table leader guid {} incorrect (not exist)", member.LeaderSpawnId);
                continue;
            }

            if (!sObjectMgr->GetCreatureData(memberSpawnId))
            {
                SC_LOG_ERROR("sql.sql", "creature_formations table member guid {} incorrect (not exist)", memberSpawnId);
                continue;
            }
    ...
    for (ObjectGuid::LowType leaderSpawnId : leaderSpawnIds)
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "creature_formation contains leader spawn {} which is not included on its formation, removing", leaderSpawnId);
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} creatures in formations in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}

FormationInfo* FormationMgr::GetFormationInfo(ObjectGuid::LowType spawnId)
{
    return Syphrena::Containers::MapGetValuePtr(_creatureGroupMap, spawnId);
}
...
{
    SC_LOG_DEBUG("entities.unit", "CreatureGroup::AddMember: Adding unit {}.", member->GetGUID().ToString());

    //Check if it is a leader
    if (member->GetSpawnId() == _leaderSpawnId)
    {
        SC_LOG_DEBUG("entities.unit", "Unit {} is formation leader. Adding group.", member->GetGUID().ToString());
        _leader = member;
    }
...
void CreatureGroup::FormationReset(bool /*dismiss*/)
{
    for (auto const& pair : _members)
    {
        if (pair.first != _leader && pair.first->IsAlive())
        {
            ...
            // SC_LOG_DEBUG("entities.unit", "CreatureGroup::FormationReset: Set {} movement for member {}", dismiss ? "default" : "idle", pair.first->GetGUID().ToString());
        }
    }
