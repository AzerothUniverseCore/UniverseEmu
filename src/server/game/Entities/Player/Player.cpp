...
static uint32 corpseReclaimDelay[MAX_DEATH_COUNT] = { 5, 10, 15 };
...
bool Player::Create(ObjectGuid::LowType guidlow, CharacterCreateInfo* createInfo)
{
    ...
    if (!info)
    {
        SC_LOG_ERROR("entities.player.cheat", "Player::Create: Possible hacking attempt: Account {} tried to create a character named '{}' with an invalid race/class pair ({}/{}) - refusing to do so.",
    ...
    if (!cEntry)
    {
        SC_LOG_ERROR("entities.player.cheat", "Player::Create: Possible hacking attempt: Account {} tried to create a character named '{}' with an invalid character class ({}) - refusing to do so (wrong DBC-files?)",
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "Player::Create: Possible hacking attempt: Account {} tried to create a character named '{}' with an invalid gender ({}) - refusing to do so",
...
{
    SC_LOG_DEBUG("entities.player.items", "Player::StoreNewItemInBestSlots: Player '{}' ({}) creates initial item (ItemID: {}, Count: {})",
    ...
    // item can't be added
    SC_LOG_ERROR("entities.player.items", "Player::StoreNewItemInBestSlots: Player '{}' ({}) can't equip or store initial item (ItemID: {}, Race: {}, Class: {}, InventoryResult: {})",
...
uint32 Player::EnvironmentalDamage(EnviromentalDamage type, uint32 damage)
{
    ...
    if (!IsAlive())
    {
        ...
        {
            SC_LOG_DEBUG("entities.player", "Player::EnvironmentalDamage: Player '{}' ({}) fall to death, losing {} durability",
...
void Player::Update(uint32 p_time)
{
    ...
    if (m_nextSave > 0)
    {
        if (p_time >= m_nextSave)
        {
            ...
            SaveToDB();
            SC_LOG_DEBUG("entities.player", "Player::Update: Player '{}' ({}) saved", GetName(), GetGUID().ToString());

            if (sWorld->getBoolConfig(CONFIG_WHO_BOTS_ENABLE))
                CharacterDatabase.PExecute("UPDATE characters_bots SET zone = zone WHERE online > 0");
        }
        else
            m_nextSave -= p_time;
    }
    ...
    if ((GetClass() == CLASS_PALADIN ||
        ...
        GetClass() == CLASS_TAMER ||
        GetClass() == CLASS_EVOKER ||
        GetClass() == CLASS_NECROMANCER ||
        GetClass() == CLASS_VENOMANCER ||
        GetClass() == CLASS_PYROMANCER ||
        GetClass() == CLASS_CHRONOMANCER ||
        GetClass() == CLASS_GEOMANCER) &&
        GetLevel() >= 84)
    {
        float maxMana = GetMaxPower(POWER_MANA);

        float regenAmount = maxMana * 0.00025f;

        ModifyPower(POWER_MANA, regenAmount);

        if (GetPower(POWER_MANA) > maxMana)
        {
            SetPower(POWER_MANA, maxMana);
        }
    }
}
...
void Player::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        if (!cur)
        {
            SC_LOG_ERROR("entities.player", "Player::setDeathState: Attempt to kill a dead player '{}' ({})", GetName(), GetGUID().ToString());
            return;
        }
...
bool Player::BuildEnumData(PreparedQueryResult result, WorldPacket* data)
{
    ...
    if (!info)
    {
        SC_LOG_ERROR("entities.player.loading", "Player {} has incorrect race/class pair. Don't build enum.", guid);
        return false;
    }
    else if (!IsValidGender(gender))
    {
        SC_LOG_ERROR("entities.player.loading", "Player ({}) has incorrect gender ({}), don't build enum.", guid, gender);
        return false;
    }
    ...
    *data << uint32(petFamily);
    ...
    std::vector<std::string_view> equipment = Syphrena::Tokenize(fields[22].GetStringView(), ' ', false);
    for (uint8 slot = 0; slot < INVENTORY_SLOT_BAG_END; ++slot)
    {
        uint32 const visualBase = slot * 2;
        Optional<uint32> itemId;
        if (visualBase < equipment.size())
            itemId = Syphrena::StringTo<uint32>(equipment[visualBase]);
        ...
        if (!proto)
        {
            if (!itemId || *itemId)
            {
                SC_LOG_WARN("entities.player.loading", "Player {} has invalid equipment '{}' in `equipmentcache` at index {}. Skipped.",
        ...
        if ((visualBase + 1) < equipment.size())
            enchants = Syphrena::StringTo<uint32>(equipment[visualBase + 1]);
        if (!enchants)
        {
            SC_LOG_WARN("entities.player.loading", "Player {} has invalid enchantment info '{}' in `equipmentcache` at index {}. Skipped.",
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "Player::TeleportTo: Player '{}' ({}) tried to enter a forbidden map (MapID: {})", GetGUID().ToString(), GetName(), mapid);
    ...
    {
        SC_LOG_DEBUG("maps", "Player '{}' ({}) using client without required expansion tried teleport to non accessible map (MapID: {})",
        ...
        SC_LOG_DEBUG("maps", "Player {} ({}) is being teleported to map (MapID: {})", GetName(), GetGUID().ToString(), mapid);
...
void Player::RemoveFromWorld()
{
    ...
    if (m_uint32Values)
    {
        ...
        {
            SC_LOG_ERROR("entities.player", "Player::RemoveFromWorld: Player '{}' ({}) has viewpoint (Entry:{}, Type: {}) when removed from world",
...
bool Player::AddTalent(uint32 spellId, uint8 spec, bool learning)
{
    ...
    if (!spellInfo)
    {
        ...
        {
            SC_LOG_ERROR("spells", "Player::AddTalent: Spell (ID: {}) does not exist. Deleting for all characters in `character_spell` and `character_talent`.", spellId);

            DeleteSpellFromAllPlayers(spellId);
        }
        else
            SC_LOG_ERROR("spells", "Player::AddTalent: Spell (ID: {}) does not exist", spellId);
    ...
    if (!SpellMgr::IsSpellValid(spellInfo, this, false))
    {
        ...
        if (!IsInWorld() && !learning)                       // spell load case
        {
            SC_LOG_ERROR("spells", "Player::AddTalent: Spell (ID: {}) is invalid. Deleting for all characters in `character_spell` and `character_talent`.", spellId);

            DeleteSpellFromAllPlayers(spellId);
        }
        else
            SC_LOG_ERROR("spells", "Player::AddTalent: Spell (ID: {}) is invalid", spellId);
...
bool Player::AddSpell(uint32 spellId, bool active, bool learning, bool dependent, bool disabled, bool loading /*= false*/, uint32 fromSkill /*= 0*/)
{
    ...
    if (!spellInfo)
    {
        ...
        if (!IsInWorld() && !learning)                       // spell load case
        {
            SC_LOG_ERROR("spells", "Player::AddSpell: Spell (ID: {}) does not exist. deleting for all characters in `character_spell` and `character_talent`.", spellId);

            DeleteSpellFromAllPlayers(spellId);
        }
        else
            SC_LOG_ERROR("spells", "Player::AddSpell: Spell (ID: {}) does not exist", spellId);
    ...
    if (!SpellMgr::IsSpellValid(spellInfo, this, false))
    {
        ...
        if (!IsInWorld() && !learning)                       // spell load case
        {
            SC_LOG_ERROR("spells", "Player::AddSpell: Spell (ID: {}) is invalid. deleting for all characters in `character_spell` and `character_talent`.", spellId);

            DeleteSpellFromAllPlayers(spellId);
        }
        else
            SC_LOG_ERROR("spells", "Player::AddSpell: Spell (ID: {}) is invalid", spellId);
...
void Player::DeleteFromDB(ObjectGuid playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    default:
        SC_LOG_ERROR("entities.player.cheat", "Player::DeleteFromDB: Tried to delete player ({}) with unsupported delete method ({}).",
...
{
    SC_LOG_INFO("entities.player", "Player::DeleteOldCharacters: Deleting all characters which have been deleted {} days before...", keepDays);
    ...
    if (result)
    {
        SC_LOG_DEBUG("entities.player", "Player::DeleteOldCharacters: Found " UI64FMTD " character(s) to delete", result->GetRowCount());
        do
        {
            Field* fields = result->Fetch();
...
void Player::SetMovement(PlayerMovementType pType)
{
    ...
    default:
        SC_LOG_ERROR("entities.player", "Player::SetMovement: Unsupported move type ({}), data not sent to client.", pType);
        return;
    }
    data << GetPackGUID();
    data << uint32(0);
...
void Player::BuildPlayerRepop()
{
    ...
    {
        SC_LOG_ERROR("entities.player", "Player::BuildPlayerRepop: Player '{}' ({}) already has a corpse", GetName(), GetGUID().ToString());
        return;
    }
    ...
    if (!corpse)
    {
        SC_LOG_ERROR("entities.player", "Player::BuildPlayerRepop: Error creating corpse for player '{}' ({})", GetName(), GetGUID().ToString());
        return;
    }
...
void Player::DurabilityRepair(uint16 pos, bool takeCost, float discountMod)
{
    ...
    if (takeCost)
    {
        ...
        {
            SC_LOG_DEBUG("entities.player.items", "Player::DurabilityRepair: Player '{}' ({}) has not enough money to repair item",
...
void Player::CleanupChannels()
{
    ...
    SC_LOG_DEBUG("chat.system", "Player::CleanupChannels: Channels of player '{}' ({}) cleaned up.", GetName(), GetGUID().ToString());
}
...
void Player::HandleBaseModFlatValue(BaseModGroup modGroup, float amount, bool apply)
{
    ...
    {
        SC_LOG_ERROR("spells", "Player::HandleBaseModValue: Invalid BaseModGroup/BaseModType ({}/{}) for player '{}' ({})",
...
float Player::GetBaseModValue(BaseModGroup modGroup, BaseModType modType) const
{
    ...
    {
        SC_LOG_ERROR("spells", "Player::GetBaseModValue: Invalid BaseModGroup/BaseModType ({}/{}) for player '{}' ({})",
...
float Player::GetTotalBaseModValue(BaseModGroup modGroup) const
{
    if (modGroup >= BASEMOD_END)
    {
        SC_LOG_ERROR("spells", "Player::GetTotalBaseModValue: Invalid BaseModGroup ({}) for player '{}' ({})",
...
void Player::GetDodgeFromAgility(float& diminishing, float& nondiminishing) const
{
    ...
    const float dodge_base[MAX_CLASSES] =
    {
        ...
        -0.040873f, // Hunter
         ...
         0.036587f, // Mage
         0.024211f, // Warlock
         0.034943f, // Blood Battlemage
         0.050957f, // Druid
		 0.036640f, // Knight
		 0.020957f, // Demon Hunter
		 0.020957f, // Monk
		 0.024211f, // Tamer
         0.036640f, // Hero
         0.036587f, // Evoker
         0.036587f  // Chronomancer
    };
    ...
    const float crit_to_dodge[MAX_CLASSES] =
    {
         ...
         1.00f / 1.15f,    // Mage
         0.97f / 1.15f,    // Warlock (?)
         1.00f / 1.15f,    // Blood Battlemage
         1.60f / 1.15f,    // Druid
		 0.85f / 1.15f,    // Knight
		 1.60f / 1.15f,    // Demon Hunter
		 1.60f / 1.15f,    // Monk
		 0.97f / 1.15f,    // Tamer
         0.85f / 1.15f,    // Hero
         1.00f / 1.15f,    // Evoker
         1.00f / 1.15f     // Chronomancer
    };
    ...
    diminishing = 20.0f * bonus_agility * dodgeRatio->Data * crit_to_dodge[pclass - 1];
    nondiminishing = 20.0f * (dodge_base[pclass - 1] + base_agility * dodgeRatio->Data * crit_to_dodge[pclass - 1]);
}
...
{
    SC_LOG_DEBUG("entities.player.skills", "Player::UpdateCraftSkill: Player '{}' ({}), SpellID: {}",
...
{
    SC_LOG_DEBUG("entities.player.skills", "Player::UpdateGatherSkill: Player '{}' ({}), SkillID: {}, SkillLevel: {},  RedLevel: {})",
...
{
    SC_LOG_DEBUG("entities.player.skills", "Player::UpdateFishingSkill: Player '{}' ({})", GetName(), GetGUID().ToString());
...
{
    SC_LOG_DEBUG("entities.player.skills", "Player::UpdateSkillPro: Player '{}' ({}), SkillID: {}, Chance: {:3.1f}%)",
    ...
    {
        SC_LOG_DEBUG("entities.player.skills", "Player::UpdateSkillPro: Player '{}' ({}), SkillID: {}, Chance: {:3.1f}% missed",
            GetName(), GetGUID().ToString(), SkillId, Chance / 10.0f);
        return false;
    }
    ...
    if (Roll <= Chance)
    {
        ...
        SC_LOG_DEBUG("entities.player.skills", "Player::UpdateSkillPro: Player '{}' ({}), SkillID: {}, Chance: {:3.1f}% taken",
            GetName(), GetGUID().ToString(), SkillId, Chance / 10.0f);
        return true;
    }
...
void Player::UpdateCombatSkills(Unit* victim, WeaponAttackType attType, bool defense)
{
    ...
    int32 greylevel = Syphrena::XP::GetGrayLevel(plevel);
...
void Player::SetSkill(uint32 id, uint16 step, uint32 newVal, uint32 maxVal)
{
    if (!id)
        return;
    ...
    else if (newVal)                                        //add
    {
        ...
        for (int i = 0; i < PLAYER_MAX_SKILLS; ++i)
        {
            if (!GetUInt32Value(PLAYER_SKILL_INDEX(i)))
            {
                ...
                if (!pSkill)
                {
                    SC_LOG_ERROR("misc", "Player::SetSkill: Skill (SkillID: {}) not found in SkillLineStore for player '{}' ({})",
...
uint32 Player::GetSkillStep(uint32 skill) const
{
    if (!skill)
        return 0;
...
uint32 Player::GetSkillValue(uint32 skill) const
{
    if (!skill)
        return 0;
...
uint32 Player::GetMaxSkillValue(uint32 skill) const
{
    if (!skill)
        return 0;
...
uint32 Player::GetPureMaxSkillValue(uint32 skill) const
{
    if (!skill)
        return 0;
...
uint32 Player::GetBaseSkillValue(uint32 skill) const
{
    if (!skill)
        return 0;
...
uint32 Player::GetPureSkillValue(uint32 skill) const
{
    if (!skill)
        return 0;
...
int32 Player::GetSkillPermBonusValue(uint32 skill) const
{
    if (!skill)
        return 0;
...
int32 Player::GetSkillTempBonusValue(uint32 skill) const
{
    if (!skill)
        return 0;
...
void Player::SendActionButtons(uint32 state) const
{
    ...
    /*
        state can be 0, 1, 2
        0 - Looks to be sent when initial action buttons get sent, however on Syphrena we use 1 since 0 had some difficulties
        1 - Used in any SMSG_ACTION_BUTTONS packet with button data on Syphrena. Only used after spec swaps on retail.
...
bool Player::IsActionButtonDataValid(uint8 button, uint32 action, uint8 type) const
{
    ...
    {
        SC_LOG_ERROR("entities.player", "Player::IsActionButtonDataValid: Action {} not added into button {} for player {} ({}): button must be < {}",
    ...
    {
        SC_LOG_ERROR("entities.player", "Player::IsActionButtonDataValid: Action {} not added into button {} for player {} ({}): action must be < {}",
    ...
    case ACTION_BUTTON_SPELL:
        ...
        {
            SC_LOG_DEBUG("entities.player", "Player::IsActionButtonDataValid: Spell action {} not added into button {} for player {} ({}): spell does not exist. This can be due to a character imported from a different expansion",
        ...
        if (!HasSpell(action))
        {
            SC_LOG_DEBUG("entities.player", "Player::IsActionButtonDataValid: Spell action {} not added into button {} for player {} ({}): player does not known this spell, this can be due to a player changing their talents",
                action, button, GetName(), GetGUID().ToString());
            return false;
        }
        break;
    case ACTION_BUTTON_ITEM:
        ...
        {
            SC_LOG_ERROR("entities.player", "Player::IsActionButtonDataValid: Item action {} not added into button {} for player {} ({}): item not exist",
                action, button, GetName(), GetGUID().ToString());
            return false;
        }
        break;
    ...
    case ACTION_BUTTON_EQSET:
        break;
    default:
        SC_LOG_ERROR("entities.player", "Player::IsActionButtonDataValid: Unknown action type {}", type);
...
ActionButton* Player::addActionButton(uint8 button, uint32 action, uint8 type)
{
    ...
    SC_LOG_DEBUG("entities.player", "Player::AddActionButton: Player '{}' ({}) added action '{}' (type {}) to button '{}'",
...
void Player::removeActionButton(uint8 button)
{
    ...
    SC_LOG_DEBUG("entities.player", "Player::RemoveActionButton: Player '{}' ({}) removed action button '{}'",
...
void Player::SendMessageToSetInRange(WorldPacket const* data, float dist, bool self) const
{
    ...
    Syphrena::MessageDistDeliverer notifier(this, data, dist);
...
void Player::SendMessageToSetInRange(WorldPacket const* data, float dist, bool self, bool own_team_only) const
{
    ...
    Syphrena::MessageDistDeliverer notifier(this, data, dist, own_team_only);
    Cell::VisitWorldObjects(this, notifier, dist);
}
...
void Player::SendMessageToSet(WorldPacket const* data, Player const* skipped_rcvr) const
{
    ...
    Syphrena::MessageDistDeliverer notifier(this, data, GetVisibilityRange(), false, skipped_rcvr);
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (!areaEntry)
    {
        SC_LOG_ERROR("entities.player", "Player '{}' ({}) discovered unknown area (x: {} y: {} z: {} map: {})",
    ...
    {
        SC_LOG_ERROR("entities.player", "Player::CheckAreaExploreAndOutdoor: Wrong area flag {} in map data for (X: {} Y: {}) point to field PLAYER_EXPLORED_ZONES_1 + {} ( {} must be < {} ).",
    ...
    if (!(currFields & val))
    {
        ...
        if (areaEntry->ExplorationLevel > 0)
        {
            ...
            SC_LOG_DEBUG("entities.player", "Player '{}' ({}) discovered a new area: {}", GetName(), GetGUID().ToString(), areaId);
        }
    }
}
...
uint32 Player::TeamForRace(uint8 race)
{
    if (ChrRacesEntry const* rEntry = sChrRacesStore.LookupEntry(race))
    {
        ...
        SC_LOG_ERROR("entities.player", "Race ({}) has wrong teamid ({}) in DBC: wrong DBC files?", uint32(race), rEntry->BaseLanguage);
    }
    else
        SC_LOG_ERROR("entities.player", "Race ({}) not found in DBC: wrong DBC files?", uint32(race));
...
int32 Player::CalculateReputationGain(ReputationSource source, uint32 creatureOrQuestLevel, int32 rep, int32 faction, bool noQuestBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= Syphrena::XP::GetGrayLevel(GetLevel()))
        percent *= rate;
...
bool Player::RewardHonor(Unit* victim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (honor_f <= 0)
    {
        ...
        if (Player* plrVictim = victim->ToPlayer())
        {
            ...
            uint8 k_grey = Syphrena::XP::GetGrayLevel(k_level);
            ...
            honor_f = std::ceil(Syphrena::Honor::hk_honor_at_level_f(k_level) * (v_level - k_grey) / (k_level - k_grey));
        ...
        else if (victim->IsNPCBot() && !victim->ToCreature()->IsTempBot())
        {
            ...
            honor_f = ceil(Syphrena::Honor::hk_honor_at_level_f(k_level) * (v_level - k_grey) / (k_level - k_grey));
...
void Player::DuelComplete(DuelCompleteType type)
{
    ...
    SC_LOG_DEBUG("entities.unit", "Player::DuelComplete: Player '{}' ({}), Opponent: '{}' ({})",
...
void Player::_ApplyItemMods(Item* item, uint8 slot, bool apply, bool updateItemAuras /*= true*/)
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::_ApplyItemMods: Applying mods for item {}", item->GetGUID().ToString());
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::_ApplyItemMods: completed");
}
...
void Player::_ApplyItemBonuses(ItemTemplate const* proto, uint8 slot, bool apply, bool only_level_scale /*= false*/)
{
    ...
    if (armor)
    {
        ...
        if (proto->Class == ITEM_CLASS_ARMOR)
        {
            ...
            case ITEM_SUBCLASS_ARMOR_RUBIS:
...
void Player::ApplyEquipSpell(SpellInfo const* spellInfo, Item* item, bool apply, bool form_change)
{
    if (apply)
    {
        ...
        SC_LOG_DEBUG("entities.player", "Player::ApplyEquipSpell: Player '{}' ({}) cast {} equip spell (ID: {})",
...
void Player::CastItemCombatSpell(DamageInfo const& damageInfo, Item* item, ItemTemplate const* proto)
{
    ...
    if (canTrigger)
    {
        for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
        {
            ...
            if (!spellInfo)
            {
                SC_LOG_ERROR("entities.player.items", "Player::CastItemCombatSpell: Player '{}' ({}) cast unknown item spell (ID: {})",
    ...
    for (uint8 e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (uint8 s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (!spellInfo)
            {
                SC_LOG_ERROR("entities.player.items", "Player::CastItemCombatSpell: Player '{}' ({}) cast unknown spell (EnchantID: {}, SpellID: {}), ignoring",
...
void Player::CastItemUseSpell(Item* item, SpellCastTargets const& targets, uint8 cast_count, uint32 glyphIndex)
{
    ...
    if (proto->Spells[0].SpellId == 483 || proto->Spells[0].SpellId == 55884)
    {
        ...
        if (!spellInfo)
        {
            SC_LOG_ERROR("entities.player", "Player::CastItemUseSpell: Item (Entry: {}) has wrong spell id {}, ignoring.", proto->ItemId, learn_spell_id);
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        if (!spellInfo)
        {
            SC_LOG_ERROR("entities.player", "Player::CastItemUseSpell: Item (Entry: {}) has wrong spell id {}, ignoring", proto->ItemId, spellData.SpellId);
            continue;
        }
    ...
    for (uint8 e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (uint8 s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (!spellInfo)
            {
                SC_LOG_ERROR("entities.player", "Player::CastItemUseSpell: Enchant {}, cast unknown spell {}", pEnchant->ID, pEnchant->EffectArg[s]);
                continue;
            }
...
{
    SC_LOG_DEBUG("entities.player.items", "_RemoveAllItemMods start.");
    ...
    SC_LOG_DEBUG("entities.player.items", "_RemoveAllItemMods complete.");
}

void Player::_ApplyAllItemMods()
{
    SC_LOG_DEBUG("entities.player.items", "_ApplyAllItemMods start.");
    ...
    SC_LOG_DEBUG("entities.player.items", "_ApplyAllItemMods complete.");
}
...
void Player::SendLoot(ObjectGuid guid, LootType loot_type)
{
    ...
    SC_LOG_DEBUG("loot", "Player::SendLoot: Player: '{}' ({}), Loot: {}",
...
void Player::SendInitWorldStates(uint32 zoneId, uint32 areaId)
{
    ...
    SC_LOG_DEBUG("network", "Player::SendInitWorldStates: Sending SMSG_INIT_WORLD_STATES for Map: {}, Zone: {}", mapId, zoneId);
...
uint32 Player::GetXPRestBonus(uint32 xp)
{
    ...
    SC_LOG_DEBUG("entities.player", "Player::GetXPRestBonus: Player '{}' ({}) gain {} xp (+{} Rested Bonus). Rested points={}", GetGUID().ToString(), GetName(), xp + rested_bonus, rested_bonus, GetRestBonus());
    return rested_bonus;
}
...
void Player::ResetPetTalents()
{
    ...
    if (!charmInfo)
    {
        SC_LOG_ERROR("entities.player", "Object {} is considered pet-like, but doesn't have charm info!", pet->GetGUID().ToString());
        return;
    }
    pet->resetTalents();
...
{
    SC_LOG_DEBUG("entities.player.items", "Player::CanStoreItem: Bag: {}, Slot: {}, Item: {}, Count: {}", bag, slot, entry, count);
...
InventoryResult Player::CanStoreItems(Item** items, int count, uint32* itemLimitCategory) const
{
    ...
    for (int k = 0; k < count; ++k)
    {
        ...
        SC_LOG_DEBUG("entities.player.items", "Player::CanStoreItems: Player '{}' ({}), Index: {} ItemID: {}, Count: {}",
...
InventoryResult Player::CanEquipItem(uint8 slot, uint16& dest, Item* pItem, bool swap, bool not_loading) const
{
    ...
    if (pItem)
    {
        SC_LOG_DEBUG("entities.player.items", "Player::CanEquipItem: Player '{}' ({}), Slot: {}, Item: {}, Count: {}",
...
InventoryResult Player::CanUnequipItem(uint16 pos, bool swap) const
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::CanUnequipItem: Player '{}' ({}), Slot: {}, Item: {}, Count: {}",
...
InventoryResult Player::CanBankItem(uint8 bag, uint8 slot, ItemPosCountVec& dest, Item* pItem, bool swap, bool not_loading) const
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::CanBankItem: Player '{}' ({}), Bag: {}, Slot: {}, Item: {}, Count: {}",
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "Possible hacking attempt: Player {} ({}) tried to move token [{} entry: {}] out of the currency bag!",
...
InventoryResult Player::CanUseItem(Item* pItem, bool not_loading) const
{
    if (pItem)
    {
        SC_LOG_DEBUG("entities.player.items", "Player::CanUseItem: Player '{}' ({}),  Item: {}",
...
InventoryResult Player::CanRollForItemInLFG(ItemTemplate const* proto, WorldObject const* lootedObject) const
{
    ...
    {
        if (_class == CLASS_WARRIOR || _class == CLASS_PALADIN || _class == CLASS_DEATH_KNIGHT || _class == CLASS_BLOODMAGE || _class == CLASS_KNIGHT || _class == CLASS_HERO)
        {
            if (GetLevel() < 40)
            {
        ...
        if (_class == CLASS_MAGE || _class == CLASS_PRIEST || _class == CLASS_WARLOCK || _class == CLASS_TAMER)
...
{
    SC_LOG_DEBUG("entities.player.items", "STORAGE: CanUseAmmo item = {}", item);
    if (!IsAlive())
        return EQUIP_ERR_YOU_ARE_DEAD;
    //if (isStunned())
    //    return EQUIP_ERR_YOU_ARE_STUNNED;
...
Item* Player::_StoreItem(uint16 pos, Item* pItem, uint32 count, bool clone, bool update)
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::_StoreItem: Player '{}' ({}), Bag: {}, Slot: {}, Item: {} ({}), Count: {}",
    ...
    if (!pItem2)
    {
        ...
        if (IsAlive())
        {
            ...
            if (pProto && IsInCombat() && (pProto->Class == ITEM_CLASS_WEAPON || pProto->InventoryType == INVTYPE_RELIC) && m_weaponChangeTimer == 0)
            {
                ...
                if (!spellProto)
                    SC_LOG_ERROR("entities.player", "Player::EquipItem: Weapon switch cooldown spell {} for player '{}' ({}) couldn't be found in Spell.dbc",
...
void Player::VisualizeItem(uint8 slot, Item* pItem)
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::SetVisibleItemSlot: Player '{}' ({}), Slot: {}, Item: {}",
...
void Player::RemoveItem(uint8 bag, uint8 slot, bool update)
{
    ...
    if (pItem)
    {
        SC_LOG_DEBUG("entities.player.items", "Player::RemoveItem: Player '{}' ({}), Bag: {}, Slot: {}, Item: {}",
...
void Player::DestroyItem(uint8 bag, uint8 slot, bool update)
{
    ...
    if (pItem)
    {
        SC_LOG_DEBUG("entities.player.items", "Player::DestroyItem: Player '{}' ({}), Bag: {}, Slot: {}, Item: {}",
            GetName(), GetGUID().ToString(), bag, slot, pItem->GetEntry());
...
{
    SC_LOG_DEBUG("entities.player.items", "Player::DestroyItemCount: Player '{}' ({}), Item: {}, Count: {}",
...
{
    SC_LOG_DEBUG("entities.player.items", "Player::DestroyZoneLimitedItem: In map {} and area {} for player '{}' ({})",
...
void Player::DestroyConjuredItems(bool update)
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::DestroyConjuredItems: Player '{}' ({})",
        GetName(), GetGUID().ToString());
...
void Player::DestroyItemCount(Item* pItem, uint32& count, bool update)
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::DestroyItemCount: Player '{}' ({}), Item ({}, Entry: {}), Count: {}",
...
void Player::SplitItem(uint16 src, uint16 dst, uint32 count)
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::SplitItem: Player '{}' ({}), Bag: {}, Slot: {}, Item: {}, Count: {}",
...
void Player::SwapItem(uint16 src, uint16 dst)
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::SwapItem: Player '{}' ({}), Bag: {}, Slot: {}, Item: {}",
...
void Player::AddItemToBuyBackSlot(Item* pItem)
{
    if (pItem)
    {
        ...
        SC_LOG_DEBUG("entities.player.items", "Player::AddItemToBuyBackSlot: Player '{}' ({}), Item: {}, Slot: {}",
...
{
    SC_LOG_DEBUG("entities.player.items", "Player::GetItemFromBuyBackSlot: Player '{}' ({}), Slot: {}",
...
{
    SC_LOG_DEBUG("entities.player.items", "Player::RemoveItemFromBuyBackSlot: Player '{}' ({}), Slot: {}",
        GetName(), GetGUID().ToString(), slot);
    if (slot >= BUYBACK_SLOT_START && slot < BUYBACK_SLOT_END)
    {
...
void Player::UpdateItemDuration(uint32 time, bool realtimeonly)
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::UpdateItemDuration: Player '{}' ({}), Time: {}, RealTimeOnly: {}",
    ...
    if (!item->IsBroken())
    {
        for (uint8 s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            case ITEM_ENCHANTMENT_TYPE_STAT:
            {
                ...
                SC_LOG_DEBUG("entities.player.items", "Adding {} to stat nb {}", enchant_amount, enchant_spell_id);
                switch (enchant_spell_id)
                {
                case ITEM_MOD_MANA:
                    SC_LOG_DEBUG("entities.player.items", "+ {} MANA", enchant_amount);
                    HandleStatFlatModifier(UNIT_MOD_MANA, BASE_VALUE, float(enchant_amount), apply);
                    break;
                case ITEM_MOD_HEALTH:
                    SC_LOG_DEBUG("entities.player.items", "+ {} HEALTH", enchant_amount);
                    HandleStatFlatModifier(UNIT_MOD_HEALTH, BASE_VALUE, float(enchant_amount), apply);
                    break;
                case ITEM_MOD_AGILITY:
                    SC_LOG_DEBUG("entities.player.items", "+ {} AGILITY", enchant_amount);
                    HandleStatFlatModifier(UNIT_MOD_STAT_AGILITY, TOTAL_VALUE, float(enchant_amount), apply);
                    UpdateStatBuffMod(STAT_AGILITY);
                    break;
                case ITEM_MOD_STRENGTH:
                    SC_LOG_DEBUG("entities.player.items", "+ {} STRENGTH", enchant_amount);
                    HandleStatFlatModifier(UNIT_MOD_STAT_STRENGTH, TOTAL_VALUE, float(enchant_amount), apply);
                    UpdateStatBuffMod(STAT_STRENGTH);
                    break;
                case ITEM_MOD_INTELLECT:
                    SC_LOG_DEBUG("entities.player.items", "+ {} INTELLECT", enchant_amount);
                    HandleStatFlatModifier(UNIT_MOD_STAT_INTELLECT, TOTAL_VALUE, float(enchant_amount), apply);
                    UpdateStatBuffMod(STAT_INTELLECT);
                    break;
                case ITEM_MOD_SPIRIT:
                    SC_LOG_DEBUG("entities.player.items", "+ {} SPIRIT", enchant_amount);
                    HandleStatFlatModifier(UNIT_MOD_STAT_SPIRIT, TOTAL_VALUE, float(enchant_amount), apply);
                    UpdateStatBuffMod(STAT_SPIRIT);
                    break;
                case ITEM_MOD_STAMINA:
                    SC_LOG_DEBUG("entities.player.items", "+ {} STAMINA", enchant_amount);
                    HandleStatFlatModifier(UNIT_MOD_STAT_STAMINA, TOTAL_VALUE, float(enchant_amount), apply);
                    UpdateStatBuffMod(STAT_STAMINA);
                    break;
                case ITEM_MOD_DEFENSE_SKILL_RATING:
                    ApplyRatingMod(CR_DEFENSE_SKILL, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} DEFENSE", enchant_amount);
                    break;
                case  ITEM_MOD_DODGE_RATING:
                    ApplyRatingMod(CR_DODGE, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} DODGE", enchant_amount);
                    break;
                case ITEM_MOD_PARRY_RATING:
                    ApplyRatingMod(CR_PARRY, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} PARRY", enchant_amount);
                    break;
                case ITEM_MOD_BLOCK_RATING:
                    ApplyRatingMod(CR_BLOCK, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} SHIELD_BLOCK", enchant_amount);
                    break;
                case ITEM_MOD_HIT_MELEE_RATING:
                    ApplyRatingMod(CR_HIT_MELEE, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} MELEE_HIT", enchant_amount);
                    break;
                case ITEM_MOD_HIT_RANGED_RATING:
                    ApplyRatingMod(CR_HIT_RANGED, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} RANGED_HIT", enchant_amount);
                    break;
                case ITEM_MOD_HIT_SPELL_RATING:
                    ApplyRatingMod(CR_HIT_SPELL, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} SPELL_HIT", enchant_amount);
                    break;
                case ITEM_MOD_CRIT_MELEE_RATING:
                    ApplyRatingMod(CR_CRIT_MELEE, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} MELEE_CRIT", enchant_amount);
                    break;
                case ITEM_MOD_CRIT_RANGED_RATING:
                    ApplyRatingMod(CR_CRIT_RANGED, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} RANGED_CRIT", enchant_amount);
                    break;
                case ITEM_MOD_CRIT_SPELL_RATING:
                    ApplyRatingMod(CR_CRIT_SPELL, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} SPELL_CRIT", enchant_amount);
                    break;
                ...
                case ITEM_MOD_HIT_RATING:
                    ...
                    ApplyRatingMod(CR_HIT_RANGED, enchant_amount, apply);
                    ApplyRatingMod(CR_HIT_SPELL, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} HIT", enchant_amount);
                    break;
                case ITEM_MOD_CRIT_RATING:
                    ApplyRatingMod(CR_CRIT_MELEE, enchant_amount, apply);
                    ApplyRatingMod(CR_CRIT_RANGED, enchant_amount, apply);
                    ApplyRatingMod(CR_CRIT_SPELL, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} CRITICAL", enchant_amount);
                    break;
                ...
                case ITEM_MOD_RESILIENCE_RATING:
                    ...
                    ApplyRatingMod(CR_CRIT_TAKEN_RANGED, enchant_amount, apply);
                    ApplyRatingMod(CR_CRIT_TAKEN_SPELL, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} RESILIENCE", enchant_amount);
                    break;
                case ITEM_MOD_HASTE_RATING:
                    ApplyRatingMod(CR_HASTE_MELEE, enchant_amount, apply);
                    ApplyRatingMod(CR_HASTE_RANGED, enchant_amount, apply);
                    ApplyRatingMod(CR_HASTE_SPELL, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} HASTE", enchant_amount);
                    break;
                case ITEM_MOD_EXPERTISE_RATING:
                    ApplyRatingMod(CR_EXPERTISE, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} EXPERTISE", enchant_amount);
                    break;
                case ITEM_MOD_ATTACK_POWER:
                    ...
                    SC_LOG_DEBUG("entities.player.items", "+ {} ATTACK_POWER", enchant_amount);
                    break;
                case ITEM_MOD_RANGED_ATTACK_POWER:
                    HandleStatFlatModifier(UNIT_MOD_ATTACK_POWER_RANGED, TOTAL_VALUE, float(enchant_amount), apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} RANGED_ATTACK_POWER", enchant_amount);
                    break;
                    //                        case ITEM_MOD_FERAL_ATTACK_POWER:
                    //                            ApplyFeralAPBonus(enchant_amount, apply);
                    //                            SC_LOG_DEBUG("entities.player.items", "+ {} FERAL_ATTACK_POWER", enchant_amount);
                    //                            break;
                case ITEM_MOD_MANA_REGENERATION:
                    ApplyManaRegenBonus(enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} MANA_REGENERATION", enchant_amount);
                    break;
                case ITEM_MOD_ARMOR_PENETRATION_RATING:
                    ApplyRatingMod(CR_ARMOR_PENETRATION, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} ARMOR PENETRATION", enchant_amount);
                    break;
                case ITEM_MOD_SPELL_POWER:
                    ApplySpellPowerBonus(enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} SPELL_POWER", enchant_amount);
                    break;
                case ITEM_MOD_HEALTH_REGEN:
                    ApplyHealthRegenBonus(enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} HEALTH_REGENERATION", enchant_amount);
                    break;
                case ITEM_MOD_SPELL_PENETRATION:
                    ApplySpellPenetrationBonus(enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} SPELL_PENETRATION", enchant_amount);
                    break;
                case ITEM_MOD_BLOCK_VALUE:
                    HandleBaseModFlatValue(SHIELD_BLOCK_VALUE, float(enchant_amount), apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} BLOCK_VALUE", enchant_amount);
                    break;
                case ITEM_MOD_MASTERY_RATING:
                    ApplyRatingMod(CR_MASTERY, enchant_amount, apply);
                    SC_LOG_DEBUG("entities.player.items", "+ {} MASTERY", enchant_amount);
                    break;
            ...
            case ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET:
                ...
                break;
            default:
                SC_LOG_ERROR("entities.player", "Player::ApplyEnchantment: Unknown item enchantment (ID: {}, DisplayType: {}) for player '{}' ({})",
    ...
    for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
    {
        ...
        if (Creature* creature = source->ToCreature())
        {
            ...
            case GOSSIP_OPTION_VENDOR:
            {
                ...
                {
                    SC_LOG_ERROR("sql.sql", "Creature {} ({} DB GUID: {}) has UNIT_NPC_FLAG_VENDOR set, but has an empty trading item list.", creature->GetName(), creature->GetGUID().ToString(), creature->GetSpawnId());
                    canTalk = false;
                }
                break;
            }
            ...
            case GOSSIP_OPTION_TRAINER:
            {
                ...
                {
                    SC_LOG_ERROR("sql.sql", "GOSSIP_OPTION_TRAINER:: Player {} {} requested wrong gossip menu: {} at Creature: {} (Entry: {})",
            ...
            case GOSSIP_OPTION_OUTDOORPVP:
                ...
                break;
            default:
                SC_LOG_ERROR("sql.sql", "Creature entry {} has unknown gossip option {} for menu {}.", creature->GetEntry(), itr->second.OptionType, itr->second.MenuID);
                canTalk = false;
                break;
            }
        }
...
void Player::OnGossipSelect(WorldObject* source, uint32 gossipListId, uint32 menuId)
{
    ...
    if (source->GetTypeId() == TYPEID_GAMEOBJECT)
    {
        ...
        {
            SC_LOG_ERROR("entities.player", "Player '{}' ({}) requests invalid gossip option for GameObject (Entry: {})",
    ...
    case GOSSIP_OPTION_BATTLEFIELD:
    {
        ...
        {
            SC_LOG_ERROR("entities.player", "Player '{}' ({}) requested battlegroundlist from an invalid creature ({})",
...
bool Player::SatisfyQuestSkill(Quest const* qInfo, bool msg) const
{
    ...
    if (GetSkillValue(skill) < qInfo->GetRequiredSkillValue())
    {
        if (msg)
        {
            ...
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestSkill: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) doesn't have the required skill value.",
...
bool Player::SatisfyQuestLevel(Quest const* qInfo, bool msg) const
{
    if (GetLevel() < qInfo->GetMinLevel())
    {
        if (msg)
        {
            ...
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestLevel: Sent INVALIDREASON_QUEST_FAILED_LOW_LEVEL (QuestID: {}) because player '{}' ({}) doesn't have the required (min) level.",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
        }
        return false;
    }
    ...
    if (qInfo->GetMaxLevel() > 0 && GetLevel() > qInfo->GetMaxLevel())
    {
        if (msg)
        {
            ...
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestLevel: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) doesn't have the required (max) level.",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
        }
        return false;
    }
...
bool Player::SatisfyQuestPreviousQuest(Quest const* qInfo, bool msg) const
{
    ...
    {
        SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
        SC_LOG_DEBUG("misc", "Player::SatisfyQuestPreviousQuest: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) doesn't have required quest {}.",
...
bool Player::SatisfyQuestDependentPreviousQuests(Quest const* qInfo, bool msg) const
{
        ...
        if (IsQuestRewarded(prevId))
        {
            ...
            for (auto itr = bounds.first; itr != bounds.second; ++itr)
            {
                ...
                if (!IsQuestRewarded(exclusiveQuestId))
                {
                    ...
                    {
                        SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
                        SC_LOG_DEBUG("misc", "Player::SatisfyQuestDependentPreviousQuests: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) doesn't have the required quest (1).",
                            qInfo->GetQuestId(), GetName(), GetGUID().ToString());
                    }
    ...
    {
        SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
        SC_LOG_DEBUG("misc", "Player::SatisfyQuestDependentPreviousQuests: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) doesn't have required quest (2).",
            qInfo->GetQuestId(), GetName(), GetGUID().ToString());
    }
...
bool Player::SatisfyQuestBreadcrumbQuest(Quest const* qInfo, bool msg) const
{
    ...
    if (!CanTakeQuest(sObjectMgr->GetQuestTemplate(breadcrumbTargetQuestId), false))
    {
        ...
        {
            SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestBreadcrumbQuest: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because target quest (QuestID: {}) is not available to player '{}' ({}).",
...
bool Player::SatisfyQuestDependentBreadcrumbQuests(Quest const* qInfo, bool msg) const
{
    for (uint32 breadcrumbQuestId : qInfo->DependentBreadcrumbQuests)
    {
        ...
        if (status == QUEST_STATUS_INCOMPLETE || status == QUEST_STATUS_COMPLETE || status == QUEST_STATUS_FAILED)
        {
            ...
            {
                SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
                SC_LOG_DEBUG("misc", "Player::SatisfyQuestDependentBreadcrumbQuests: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) has a breadcrumb quest towards this quest in the quest log.",
                    qInfo->GetQuestId(), GetName(), GetGUID().ToString());
            }
...
bool Player::SatisfyQuestClass(Quest const* qInfo, bool msg) const
{
    ...
    if ((reqClass & GetClassMask()) == 0)
    {
        ...
        {
            SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestClass: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) doesn't have required class.",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
        }
...
bool Player::SatisfyQuestRace(Quest const* qInfo, bool msg) const
{
    ...
    if ((reqraces & GetRaceMask()) == 0)
    {
        if (msg)
        {
            ...
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestRace: Sent INVALIDREASON_QUEST_FAILED_WRONG_RACE (QuestID: {}) because player '{}' ({}) doesn't have required race.",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
...
bool Player::SatisfyQuestReputation(Quest const* qInfo, bool msg) const
{
    ...
    if (fIdMin && GetReputationMgr().GetReputation(fIdMin) < qInfo->GetRequiredMinRepValue())
    {
        ...
        {
            SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestReputation: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) doesn't required reputation (min).",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
        }
        return false;
    }
    ...
    if (fIdMax && GetReputationMgr().GetReputation(fIdMax) >= qInfo->GetRequiredMaxRepValue())
    {
        ...
        {
            SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
            SC_LOG_DEBUG("misc", "SatisfyQuestReputation: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) doesn't required reputation (max).",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
        }
        return false;
    }
    ...
    if (fIdObj && GetReputationMgr().GetReputation(fIdObj) >= qInfo->GetRepObjectiveValue2())
    {
        ...
        {
            SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
            SC_LOG_DEBUG("misc", "SatisfyQuestReputation: Sent INVALIDREASON_DONT_HAVE_REQ (questId: {}) because player does not have the required reputation (ReputationObjective2).", qInfo->GetQuestId());
        }
        return false;
    }
...
bool Player::SatisfyQuestStatus(Quest const* qInfo, bool msg) const
{
    if (GetQuestStatus(qInfo->GetQuestId()) == QUEST_STATUS_REWARDED)
    {
        if (msg)
        {
            ...
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestStatus: Sent QUEST_STATUS_REWARDED (QuestID: {}) because player '{}' ({}) quest status is already REWARDED.",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
        }
        return false;
    }
    ...
    if (GetQuestStatus(qInfo->GetQuestId()) != QUEST_STATUS_NONE)
    {
        if (msg)
        {
            ...
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestStatus: Sent INVALIDREASON_QUEST_ALREADY_ON (QuestID: {}) because player '{}' ({}) quest status is not NONE.",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
        }
        return false;
    }
...
bool Player::SatisfyQuestConditions(Quest const* qInfo, bool msg) const
{
    if (!sConditionMgr->IsObjectMeetingNotGroupedConditions(CONDITION_SOURCE_TYPE_QUEST_AVAILABLE, qInfo->GetQuestId(), const_cast<Player*>(this)))
    {
        ...
        {
            SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestConditions: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) doesn't meet conditions.",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
        }
        SC_LOG_DEBUG("condition", "Player::SatisfyQuestConditions: conditions not met for quest {}", qInfo->GetQuestId());
        return false;
    }
    return true;
}
...
bool Player::SatisfyQuestTimed(Quest const* qInfo, bool msg) const
{
    if (!m_timedquests.empty() && qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAGS_TIMED))
    {
        if (msg)
        {
            ...
            SC_LOG_DEBUG("misc", "Player::SatisfyQuestTimed: Sent INVALIDREASON_QUEST_ONLY_ONE_TIMED (QuestID: {}) because player '{}' ({}) is already on a timed quest.",
                qInfo->GetQuestId(), GetName(), GetGUID().ToString());
        }
        return false;
    }
...
bool Player::SatisfyQuestExclusiveGroup(Quest const* qInfo, bool msg) const
{
    ...
    for (auto itr = bounds.first; itr != bounds.second; ++itr)
    {
        ...
        if (!SatisfyQuestDay(Nquest, false) || !SatisfyQuestWeek(Nquest, false) || !SatisfyQuestSeasonal(Nquest, false))
        {
            ...
            {
                SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
                SC_LOG_DEBUG("misc", "Player::SatisfyQuestExclusiveGroup: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) already did daily quests in exclusive group.",
                    qInfo->GetQuestId(), GetName(), GetGUID().ToString());
            }
        ...
        if (GetQuestStatus(exclude_Id) != QUEST_STATUS_NONE || (!(qInfo->IsRepeatable() && Nquest->IsRepeatable()) && GetQuestRewardStatus(exclude_Id)))
        {
            ...
            {
                SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
                SC_LOG_DEBUG("misc", "Player::SatisfyQuestExclusiveGroup: Sent INVALIDREASON_DONT_HAVE_REQ (QuestID: {}) because player '{}' ({}) already did quest in exclusive group.",
                    qInfo->GetQuestId(), GetName(), GetGUID().ToString());
            }
            return false;
        }
...
bool Player::SatisfyQuestDay(Quest const* qInfo, bool msg) const
{
    ...
    if (!have_slot)
    {
        if (msg)
        {
            ...
            SC_LOG_DEBUG("misc", "SatisfyQuestDay: Sent INVALIDREASON_DAILY_QUESTS_REMAINING (questId: {}) because player already did all possible quests today.", qInfo->GetQuestId());
        }
        return false;
    }
...
QuestGiverStatus Player::GetQuestDialogStatus(Object* questgiver)
{
    ...
    default:
        ...
        SC_LOG_ERROR("entities.player.quest", "GetQuestDialogStatus called for unexpected type {}", questgiver->GetTypeId());
...
// not used in Syphrena, but used in scripting code
...
//not used in Syphrenad, function for external script library
...
void Player::SendQuestTimerFailed(uint32 quest_id) const
{
    if (quest_id)
    {
        ...
        data << uint32(quest_id);
        SendDirectMessage(&data);
        SC_LOG_DEBUG("network", "WORLD: Sent SMSG_QUESTUPDATE_FAILEDTIMER");
    }
}
...
void Player::SendCanTakeQuestResponse(QuestFailedReason msg) const
{
    ...
    data << uint32(msg);
    SendDirectMessage(&data);
    SC_LOG_DEBUG("network", "WORLD: Sent SMSG_QUESTGIVER_QUEST_INVALID");
}
...
void Player::SendQuestConfirmAccept(Quest const* quest, Player* pReceiver) const
{
    if (pReceiver)
    {
        ...
        SC_LOG_DEBUG("network", "WORLD: Sent SMSG_QUEST_CONFIRM_ACCEPT");
    }
}
...
void Player::SendPushToPartyResponse(Player const* player, QuestShareMessages msg) const
{
    if (player)
    {
        ...
        SendDirectMessage(&data);
        SC_LOG_DEBUG("network", "WORLD: Sent MSG_QUEST_PUSH_RESULT");
    }
}
...
void Player::SendQuestUpdateAddItem(Quest const* /*quest*/, uint32 /*item_idx*/, uint16 /*count*/) const
{
    ...
    SC_LOG_DEBUG("network", "WORLD: Sent SMSG_QUESTUPDATE_ADD_ITEM");
...
void Player::SendQuestUpdateAddCreatureOrGo(Quest const* quest, ObjectGuid guid, uint32 creatureOrGO_idx, uint16 old_count, uint16 add_count)
{
    ...
    SC_LOG_DEBUG("network", "WORLD: Sent SMSG_QUESTUPDATE_ADD_KILL");
    data << uint32(quest->GetQuestId());
    data << uint32(entry);
...
void Player::_LoadArenaTeamInfo(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            if (!arenaTeam)
            {
                SC_LOG_ERROR("entities.player.loading", "Player::_LoadArenaTeamInfo: couldn't load arenateam {}", arenaTeamId);
                continue;
            }
...
bool Player::LoadFromDB(ObjectGuid guid, CharacterDatabaseQueryHolder const& holder)
{
    ...
    if (!result)
    {
        ...
        SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player '{}' ({}) not found in table `characters`, can't load. ", name.c_str(), guid.ToString());
        return false;
    }
    ...
    {
        SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) loading from wrong account (is: {}, should be: {})", guid.ToString(), GetSession()->GetAccountId(), dbAccountId);
        return false;
    }

    if (holder.GetPreparedResult(PLAYER_LOGIN_QUERY_LOAD_BANNED))
    {
        SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) is banned, can't load.", guid.ToString());
        return false;
    }
    ...
    {
        SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) has wrong gender ({}), can't load.", guid.ToString(), uint32(gender));
        return false;
    }
    ...
    if (!info)
    {
        SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) has wrong race/class ({}/{}), can't load.", guid.ToString(), GetRace(), GetClass());
        return false;
    }
    ...
    if (!_LoadIntoDataField(fields[66].GetString(), PLAYER_EXPLORED_ZONES_1, PLAYER_EXPLORED_ZONES_SIZE))
        SC_LOG_WARN("entities.player.loading", "Player::LoadFromDB: Player ({}) has invalid exploredzones data ({}). Forcing partial load.", guid.ToString(), fields[66].GetCString());

    if (!_LoadIntoDataField(fields[69].GetString(), PLAYER__FIELD_KNOWN_TITLES, KNOWN_TITLES_SIZE * 2))
        SC_LOG_WARN("entities.player.loading", "Player::LoadFromDB: Player ({}) has invalid knowntitles mask ({}). Forcing partial load.", guid.ToString(), fields[69].GetCString());
    ...
    SC_LOG_DEBUG("entities.player.loading", "Player::LoadFromDB: Load Basic value of player '{}' is: ", m_name.c_str());
    outDebugValues();
    ...
    {
        SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) has invalid coordinates (MapId: {} X: {} Y: {} Z: {} O: {}). Teleport to default race/class locations.",
    ...
    else if (mapEntry->IsBattlegroundOrArena())
    {
        ...
        else
        {
            ...
            {
                SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) was in BG in database, but BG was not found and entry point was invalid! Teleport to default race/class locations.",
                    guid.ToString());
                RelocateToHomebind();
            }
            else
    ...
    else if (transLowGUID)
    {
        ...
        if (transport)
        {
            ...
            if (!Syphrena::IsValidMapCoord(x, y, z, o) ||
            ...
            {
                SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) has invalid transport coordinates (X: {} Y: {} Z: {} O: {}). Teleport to bind location.",
        ...
        {
            SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) has problems with transport guid ({}). Teleport to bind location.",
    ...
    else if (!taxi_nodes.empty())
    {
        ...
        else if (!m_taxi.LoadTaxiDestinationsFromString(taxi_nodes, GetTeam()))
        {
            ...
            {
                SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) has wrong data in taxi destination list ({}), teleport to homebind.", GetGUID().ToString(), taxi_nodes.c_str());
                RelocateToHomebind();
            }
            else                                                // has start node, teleport to it
            {
                SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player ({}) has too short taxi destination list ({}), teleport to original node.", GetGUID().ToString(), taxi_nodes.c_str());
    ...
    if (mapEntry)
    {
        ...
        {
            SC_LOG_DEBUG("entities.player.loading", "Player::LoadFromDB: Player '{}' ({}) using client without required expansion tried login at non accessible map {}",
                GetName(), GetGUID().ToString(), mapId);
            RelocateToHomebind();
        }
    ...
    else if (map->IsDungeon()) // if map is dungeon...
    {
        if (Map::EnterState denyReason = ((InstanceMap*)map)->CannotEnter(this)) // ... and can't enter map, then look for entry point.
        {
            ...
            case Map::CANNOT_ENTER_INSTANCE_BIND_MISMATCH:
                ChatHandler(GetSession()).PSendSysMessage(GetSession()->GetSyphrenaString(LANG_INSTANCE_BIND_MISMATCH), map->GetMapName());
                break;
    ...
    if (check) // in case of special event when creating map...
    {
        ...
        {
            SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player '{}' ({}) Map: {}, X: {}, Y: {}, Z: {}, O: {}. Areatrigger not found.",
                m_name.c_str(), guid.ToString(), mapId, GetPositionX(), GetPositionY(), GetPositionZ(), GetOrientation());
            RelocateToHomebind();
            map = nullptr;
        }
    }
    ...
    if (!map)
    {
        ...
        if (!map)
        {
            SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player '{}' ({}) Map: {}, X: {}, Y: {}, Z: {}, O: {}. Invalid default map coordinates or instance couldn't be created.",
                m_name.c_str(), guid.ToString(), mapId, GetPositionX(), GetPositionY(), GetPositionZ(), GetOrientation());
            return false;
        }
    }
    ...
    if (!m_taxi.LoadTaxiMask(fields[22].GetString()))                // must be before InitTaxiNodesForLevel
        SC_LOG_WARN("entities.player.loading", "Player::LoadFromDB: Player ({}) has invalid taximask ({}) in DB. Forced partial load.", GetGUID().ToString(), fields[22].GetString());
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "Player::LoadFromDB: Player ({}) tried to login while forced to rename, can't load.'", GetGUID().ToString());
        return false;
    }
    ...
    {
        SC_LOG_ERROR("entities.player.loading", "Player::LoadFromDB: Player {} ({}) has invalid SpecCount = {} and/or invalid ActiveSpec = {}.",
    ...
    SC_LOG_DEBUG("entities.player.loading", "Player::LoadFromDB: The value of player '{}' after load item and aura is: ", m_name.c_str());
    outDebugValues();
...
void Player::_LoadActions(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                SC_LOG_DEBUG("entities.player", "Player::_LoadActions: Player '{}' ({}) has an invalid action button (Button: {}, Action: {}, Type: {}). It will be deleted at next save. This can be due to a player changing their talents.",
...
{
    SC_LOG_DEBUG("entities.player.loading", "Player::_LoadAuras: Loading auras for {}", GetGUID().ToString());
    ...
    */
    ...
    if (result)
    {
        do
        {
            ...
            if (!spellInfo)
            {
                SC_LOG_ERROR("entities.player", "Player::_LoadAuras: Player '{}' ({}) has an invalid aura (SpellID: {}), ignoring.",
                    GetName(), GetGUID().ToString(), spellid);
                continue;
            }
            ...
            if (Aura* aura = Aura::TryCreate(createInfo))
            {
                ...
                SC_LOG_DEBUG("entities.player", "Player::_LoadAuras: Added aura (SpellID: {}, EffectMask: {}) to player '{} ({})",
            ...
            if (GlyphPropertiesEntry const* gp = sGlyphPropertiesStore.LookupEntry(glyph))
            {
                if (GlyphSlotEntry const* gs = sGlyphSlotStore.LookupEntry(GetGlyphSlot(i)))
                {
                    if (gp->GlyphSlotFlags == gs->Type)
                    {
                        ...
                        SC_LOG_ERROR("entities.player", "Player::_LoadGlyphAuras: Player '{}' ({}) has glyph with typeflags {} in slot with typeflags {}, removing.", GetName(), GetGUID().ToString(), gp->GlyphSlotFlags, gs->Type);
                }
                else
                    SC_LOG_ERROR("entities.player", "Player::_LoadGlyphAuras: Player '{}' ({}) has not existing glyph slot entry {} on index {}", GetName(), GetGUID().ToString(), GetGlyphSlot(i), i);
            }
            else
                SC_LOG_ERROR("entities.player", "Player::_LoadGlyphAuras: Player '{}' ({}) has not existing glyph entry {} on index {}", GetName(), GetGUID().ToString(), glyph, i);
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (Item* item = _LoadItem(trans, zoneId, timeDiff, fields))
            {
                ...
                else
                {
                    ...
                    {
                        SC_LOG_ERROR("entities.player", "Player::_LoadInventory: Player '{}' ({}) has item ({}, entry: {}) which doesnt have a valid bag (Bag {}, slot: {}). Possible cheat?",
                ...
                {
                    SC_LOG_ERROR("entities.player", "Player::_LoadInventory: Player '{}' ({}) has item ({}, entry: {}) which can't be loaded into inventory (Bag {}, slot: {}) by reason {}. Item will be sent by mail.",
        ...
        {
            std::string subject = GetSession()->GetSyphrenaString(LANG_NOT_EQUIPPED_ITEM);
...
Item* Player::_LoadItem(CharacterDatabaseTransaction trans, uint32 zoneId, uint32 timeDiff, Field* fields)
{
    ...
    if (ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemEntry))
    {
        ...
        if (item->LoadFromDB(itemGuid, GetGUID(), fields, itemEntry))
        {
            ...
            {
                SC_LOG_DEBUG("entities.player.loading", "Player::_LoadInventory: player ({}, name: '{}', map: {}) has item ({}) limited to another map ({}). Deleting item.",
            ...
            {
                SC_LOG_DEBUG("entities.player.loading", "Player::_LoadInventory: player ({}, name: '{}', diff: {}) has conjured item ({}) with expired lifetime (15 minutes). Deleting item.",
            ...
            else if (item->IsRefundable())
            {
                ...
                {
                    SC_LOG_DEBUG("entities.player.loading", "Player::_LoadInventory: player ({}, name: '{}') has item ({}) with expired refund time ({}). Deleting refund data and removing refundable flag.",
                ...
                else
                {
                    ...
                    {
                        SC_LOG_WARN("entities.player.loading", "Player::_LoadInventory: player ({}, name: '{}') has item ({}) with refundable flags, but without data in item_refund_instance. Removing flag.",
            ...
            else if (item->IsBOPTradeable())
            {
                ...
                {
                    GuidSet looters;
                    for (std::string_view guidStr : Syphrena::Tokenize((*result)[0].GetStringView(), ' ', false))
                    {
                        if (Optional<ObjectGuid::LowType> guid = Syphrena::StringTo<ObjectGuid::LowType>(guidStr))
                            looters.insert(ObjectGuid::Create<HighGuid::Player>(*guid));
                        else
                            SC_LOG_WARN("entities.player.loading", "Player::_LoadInventory: invalid item_soulbound_trade_data GUID '{}' for item {}. Skipped.", std::string(guidStr).c_str(), item->GetGUID().ToString());
                    }
                ...
                {
                    SC_LOG_WARN("entities.player.loading", "Player::_LoadInventory: player ({}, name: '{}') has item ({}) with ITEM_FLAG_BOP_TRADEABLE flag, but without data in item_soulbound_trade_data. Removing flag.",
                        GetGUID().ToString(), GetName(), item->GetGUID().ToString());
        ...
        {
            SC_LOG_ERROR("entities.player", "Player::_LoadInventory: player ({}, name: '{}') has a broken item (GUID: {}, entry: {}) in inventory. Deleting item.",
    ...
    {
        SC_LOG_ERROR("entities.player", "Player::_LoadInventory: player ({}, name: '{}') has an unknown item (entry: {}) in inventory. Deleting item.",
...
Item* Player::_LoadMailedItem(ObjectGuid const& playerGuid, Player* player, uint32 mailId, Mail* mail, Field* fields)
{
    ...
    if (!proto)
    {
        SC_LOG_ERROR("entities.player", "Player '{}' ({}) has unknown item in mailed items (GUID: {}, Entry: {}) in mail ({}), deleted.",
    ...
    {
        SC_LOG_ERROR("entities.player", "Player::_LoadMailedItems: Item (GUID: {}) in mail ({}) doesn't exist, deleted from mail.", itemGuid, mailId);
...
void Player::_LoadMail(PreparedQueryResult mailsResult, PreparedQueryResult mailItemsResult)
{
    ...
    if (mailsResult)
    {
        do
        {
            ...
            {
                SC_LOG_ERROR("entities.player", "Player::_LoadMail: Mail ({}) has nonexistent MailTemplateId ({}), remove at load", m->messageID, m->mailTemplateId);
                m->mailTemplateId = 0;
            }
    ...
    if (result)
    {
        do
        {
            ...
            if (quest)
            {
                ...
                {
                    questStatusData.Status = QUEST_STATUS_INCOMPLETE;
                    SC_LOG_ERROR("entities.player", "Player::_LoadQuestStatus: Player '{}' ({}) has invalid quest {} status ({}), replaced by QUEST_STATUS_INCOMPLETE(3).",
                ...
                SC_LOG_DEBUG("entities.player.loading", "Player::_LoadQuestStatus: Quest status is {{}} for quest {{}} for player ({})", questStatusData.Status, quest_id, GetGUID().ToString());
            }
        } while (result->NextRow());
    }
...
void Player::_LoadDailyQuestStatus(PreparedQueryResult result)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                SC_LOG_ERROR("entities.player", "Player {} has more than 25 daily quest records in `charcter_queststatus_daily`", GetGUID().ToString());
                break;
            }
            ...
            SC_LOG_DEBUG("entities.player.loading", "Player::_LoadDailyQuestStatus: Loaded daily quest cooldown (QuestID: {}) for player '{}' ({})",
...
void Player::_LoadWeeklyQuestStatus(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            SC_LOG_DEBUG("entities.player.loading", "Player::_LoadWeeklyQuestStatus: Loaded weekly quest cooldown (QuestID: {}) for player '{}' ({})",
                quest_id, GetName(), GetGUID().ToString());
        } while (result->NextRow());
    }
...
void Player::_LoadSeasonalQuestStatus(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            SC_LOG_DEBUG("entities.player.loading", "Player::_LoadSeasonalQuestStatus: Loaded seasonal quest cooldown (QuestID: {}) for player '{}' ({})",
                quest_id, GetName(), GetGUID().ToString());
        } while (result->NextRow());
    }
...
void Player::_LoadMonthlyQuestStatus(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            SC_LOG_DEBUG("entities.player.loading", "Player::_LoadMonthlyQuestStatus: Loaded monthly quest cooldown (QuestID: {}) for player '{}' ({})",
                quest_id, GetName(), GetGUID().ToString());
        } while (result->NextRow());
    }
...
void Player::_LoadBoundInstances(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                SC_LOG_ERROR("entities.player", "Player::_LoadBoundInstances: Player '{}' ({}) has bind to not existed or not dungeon map {} ({})",
                    GetName(), GetGUID().ToString(), mapId, mapname.c_str());
                deleteInstance = true;
            }
            else if (difficulty >= MAX_DIFFICULTY)
            {
                SC_LOG_ERROR("entities.player", "Player::_LoadBoundInstances: player '{}' ({}) has bind to not existed difficulty {} instance for map {} ({})",
                    GetName(), GetGUID().ToString(), difficulty, mapId, mapname.c_str());
                deleteInstance = true;
            }
            else
            {
                ...
                {
                    SC_LOG_ERROR("entities.player", "Player::_LoadBoundInstances: player '{}' ({}) is in group {} but has a non-permanent character bind to map {} ({}), {}, {}",
                        GetName(), GetGUID().ToString(), group->GetGUID().ToString(), mapId, mapname.c_str(), instanceId, difficulty);
                    deleteInstance = true;
                }
            }
...
InstancePlayerBind* Player::BindToInstance(InstanceSave* save, bool permanent, BindExtensionState extendState, bool load)
{
    if (save)
    {
        ...
        if (!load)
            SC_LOG_DEBUG("maps", "Player::BindToInstance: Player '{}' ({}) is now bound to map (ID: {}, Instance: {}, Difficulty: {})",
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!IsGameMaster() && ar)
    {
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetSyphrenaString(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetSyphrenaString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, ASSERT_NOTNULL(sObjectMgr->GetItemTemplate(missingItem))->Name1.c_str());
                else if (LevelMin)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetSyphrenaString(LANG_LEVEL_MINREQUIRED), LevelMin);
...
bool Player::_LoadHomeBind(PreparedQueryResult result)
{
    ...
    if (!info)
    {
        SC_LOG_ERROR("entities.player", "Player::_LoadHomeBind: Player '{}' ({}) has incorrect race/class ({}/{}) pair. Can't load.",
    ...
    SC_LOG_DEBUG("entities.player", "Player::_LoadHomeBind: Setting home position (MapID: {}, AreaID: {}, X: {}, Y: {}, Z: {}) of player '{}' ({})",
...
void Player::SaveToDB(CharacterDatabaseTransaction trans, bool create /* = false */)
{
    ...
    SC_LOG_DEBUG("entities.unit", "Player::SaveToDB: The value of player {} at save: ", m_name.c_str());
    outDebugValues();
...
void Player::_SaveInventory(CharacterDatabaseTransaction trans)
{
    ...
    for (GuidSet::iterator itr = m_refundableItems.begin(); itr != m_refundableItems.end(); itr = i_next)
    {
        ...
        {
            SC_LOG_ERROR("entities.player", "Player::_SaveInventory: Can't find item ({}) in refundable storage for player '{}' ({}), removing.",
    ...
    for (size_t i = 0; i < m_itemUpdateQueue.size(); ++i)
    {
        ...
        if (item->GetState() != ITEM_REMOVED)
        {
            ...
            if (test == nullptr)
            {
                ...
                SC_LOG_ERROR("entities.player", "Player::_SaveInventory: Player '{}' ({}) has incorrect values (Bag: {}, Slot: {}) for the item ({}, State: {}). The player doesn't have an item at that position.",
            ...
            {
                SC_LOG_ERROR("entities.player", "Player::_SaveInventory: Player '{}' ({}) has incorrect values (Bag: {}, Slot: {}) for the item ({}). {} is there instead!",
...
void Player::outDebugValues() const
{
    ...
    SC_LOG_DEBUG("entities.unit", "HP is: \t\t\t{}\t\tMP is: \t\t\t{}", GetMaxHealth(), GetMaxPower(POWER_MANA));
    SC_LOG_DEBUG("entities.unit", "AGILITY is: \t\t{}\t\tSTRENGTH is: \t\t{}", GetStat(STAT_AGILITY), GetStat(STAT_STRENGTH));
    SC_LOG_DEBUG("entities.unit", "INTELLECT is: \t\t{}\t\tSPIRIT is: \t\t{}", GetStat(STAT_INTELLECT), GetStat(STAT_SPIRIT));
    SC_LOG_DEBUG("entities.unit", "STAMINA is: \t\t{}", GetStat(STAT_STAMINA));
    SC_LOG_DEBUG("entities.unit", "Armor is: \t\t{}\t\tBlock is: \t\t{}", GetArmor(), GetFloatValue(PLAYER_BLOCK_PERCENTAGE));
    SC_LOG_DEBUG("entities.unit", "HolyRes is: \t\t{}\t\tFireRes is: \t\t{}", GetResistance(SPELL_SCHOOL_HOLY), GetResistance(SPELL_SCHOOL_FIRE));
    SC_LOG_DEBUG("entities.unit", "NatureRes is: \t\t{}\t\tFrostRes is: \t\t{}", GetResistance(SPELL_SCHOOL_NATURE), GetResistance(SPELL_SCHOOL_FROST));
    SC_LOG_DEBUG("entities.unit", "ShadowRes is: \t\t{}\t\tArcaneRes is: \t\t{}", GetResistance(SPELL_SCHOOL_SHADOW), GetResistance(SPELL_SCHOOL_ARCANE));
    SC_LOG_DEBUG("entities.unit", "MIN_DAMAGE is: \t\t{}\tMAX_DAMAGE is: \t\t{}", GetFloatValue(UNIT_FIELD_MINDAMAGE), GetFloatValue(UNIT_FIELD_MAXDAMAGE));
    SC_LOG_DEBUG("entities.unit", "MIN_OFFHAND_DAMAGE is: \t{}\tMAX_OFFHAND_DAMAGE is: \t{}", GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE), GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE));
    SC_LOG_DEBUG("entities.unit", "MIN_RANGED_DAMAGE is: \t{}\tMAX_RANGED_DAMAGE is: \t{}", GetFloatValue(UNIT_FIELD_MINRANGEDDAMAGE), GetFloatValue(UNIT_FIELD_MAXRANGEDDAMAGE));
    SC_LOG_DEBUG("entities.unit", "ATTACK_TIME is: \t{}\t\tRANGE_ATTACK_TIME is: \t{}", GetAttackTime(BASE_ATTACK), GetAttackTime(RANGED_ATTACK));
}
...
void Player::SetContestedPvP(Player* attackedPlayer)
{
    ...
    if (!HasUnitState(UNIT_STATE_ATTACK_PLAYER))
    {
        ...
        Syphrena::AIRelocationNotifier notifier(*this);
        Cell::VisitWorldObjects(this, notifier, GetVisibilityRange());
    }
    for (Unit* unit : m_Controlled)
    {
        if (!unit->HasUnitState(UNIT_STATE_ATTACK_PLAYER))
        {
            ...
            Syphrena::AIRelocationNotifier notifier(*unit);
            Cell::VisitWorldObjects(this, notifier, GetVisibilityRange());
        }
    }
}
...
Pet* Player::GetPet() const
{
    if (ObjectGuid pet_guid = GetPetGUID())
    {
        ...
        //SC_LOG_ERROR("entities.player", "Player::GetPet: Pet {} does not exist.", GUID_LOPART(pet_guid));
...
void Player::RemovePet(Pet* pet, PetSaveMode mode, bool returnreagent)
{
    ...
    if (pet)
    {
        SC_LOG_DEBUG("entities.pet", "Player::RemovePet: Player '{}' ({}), Pet (Entry: {}, Mode: {}, ReturnReagent: {})",
...
void Player::StopCastingCharm()
{
    ...
    if (charm->GetTypeId() == TYPEID_UNIT)
    {
        ...
        else if (charm->IsVehicle())
        {
            ...
            // Temporary for issue https://github.com/SyphrenaCore/SyphrenaCore/issues/24876
            if (!GetCharmedGUID().IsEmpty() && !charm->HasAuraTypeWithCaster(SPELL_AURA_CONTROL_VEHICLE, GetGUID()))
            {
                SC_LOG_FATAL("entities.player", "Player::StopCastingCharm Player '{}' ({}) is not able to uncharm vehicle ({}) because of missing SPELL_AURA_CONTROL_VEHICLE",
    ...
    if (GetCharmedGUID())
    {
        SC_LOG_FATAL("entities.player", "Player::StopCastingCharm: Player '{}' ({}) is not able to uncharm unit ({})", GetName(), GetGUID().ToString(), GetCharmedGUID().ToString());
        if (!charm->GetCharmerGUID().IsEmpty())
        {
            SC_LOG_FATAL("entities.player", "Player::StopCastingCharm: Charmed unit has charmer {}\nPlayer debug info: {}\nCharm debug info: {}",
...
void Player::Whisper(uint32 textId, Player* target, bool /*isBossWhisper = false*/)
{
    ...
    if (!bct)
    {
        SC_LOG_ERROR("entities.unit", "WorldObject::MonsterWhisper: `broadcast_text` was not {} found", textId);
        return;
    }
...
void Player::PetSpellInitialize()
{
    ...
    SC_LOG_DEBUG("entities.pet", "Player::PetspellInitialize: Creating spellgroups for summoned pet");
...
void Player::PossessSpellInitialize()
{
    ...
    if (!charmInfo)
    {
        SC_LOG_ERROR("entities.player", "Player::PossessSpellInitialize: charm ({}) has no charminfo!", charm->GetGUID().ToString());
        return;
    }
...
void Player::VehicleSpellInitialize()
{
    ...
    for (uint32 i = 0; i < MAX_CREATURE_SPELLS; ++i)
    {
        ...
        {
            SC_LOG_DEBUG("condition", "Player::VehicleSpellInitialize: Player '{}' ({}) doesn't meet conditions for vehicle (Entry: {}, Spell: {})",
...
void Player::CharmSpellInitialize()
{
    ...
    if (!charmInfo)
    {
        SC_LOG_ERROR("entities.player", "Player::CharmSpellInitialize(): Player '{}' ({}) has a charm ({}) but no no charminfo!",
...
template SC_GAME_API void Player::ApplySpellMod(uint32 spellId, SpellModOp op, int32& basevalue, Spell* spell) const;
template SC_GAME_API void Player::ApplySpellMod(uint32 spellId, SpellModOp op, uint32& basevalue, Spell* spell) const;
template SC_GAME_API void Player::ApplySpellMod(uint32 spellId, SpellModOp op, float& basevalue, Spell* spell) const;

void Player::AddSpellMod(SpellModifier* mod, bool apply)
{
    SC_LOG_DEBUG("spells", "Player::AddSpellMod: Player '{}' ({}), SpellID: {}", GetName(), GetGUID().ToString(), mod->spellId);
...
void Player::ContinueTaxiFlight() const
{
    ...
    SC_LOG_DEBUG("entities.unit", "Player::ContinueTaxiFlight: Restart {} taxi flight", GetGUID().ToString());
...
void Player::InitDisplayIds()
{
    ...
    if (!info)
    {
        SC_LOG_ERROR("entities.player", "Player::InitDisplayIds: Player '{}' ({}) has incorrect race/class pair. Can't init display ids.", GetName(), GetGUID().ToString());
        return;
    }
    ...
    case GENDER_MALE:
        ...
        break;
    default:
        SC_LOG_ERROR("entities.player", "Player::InitDisplayIds: Player '{}' ({}) has invalid gender {}", GetName(), GetGUID().ToString(), gender);
    }
}
...
bool Player::BuyItemFromVendorSlot(ObjectGuid vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    if (!creature)
    {
        SC_LOG_DEBUG("network", "Player::BuyItemFromVendorSlot: Vendor ({}) not found or player '{}' ({}) can't interact with him.",
    ...
    {
        SC_LOG_DEBUG("condition", "Player::BuyItemFromVendorSlot: Player '{}' ({}) doesn't meed conditions for creature (Entry: {}, Item: {})",
    ...
    if (crItem->ExtendedCost)
    {
        ...
        if (!iece)
        {
            SC_LOG_ERROR("entities.player", "Player::BuyItemFromVendorSlot: Item {} has wrong ExtendedCost field value {}", pProto->ItemId, crItem->ExtendedCost);
            return false;
        }
    ...
    if (crItem->IsGoldRequired(pProto) && pProto->BuyPrice > 0) //Assume price cannot be negative (do not know why it is int32)
    {
        ...
        {
            SC_LOG_ERROR("entities.player.cheat", "Player::BuyItemFromVendorSlot: Player '{}' ({}) tried to buy item (ItemID: {}, Count: {}), causing overflow",
...
void Player::UpdateHomebindTime(uint32 time)
{
    ...
    else
    {
        ...
        data << uint32(1);
        SendDirectMessage(&data);
        SC_LOG_DEBUG("maps", "Player::UpdateHomebindTime: Player '{}' ({}) will be teleported to homebind in 60 seconds",
            GetName(), GetGUID().ToString());
    }
}
...
bool Player::EnchantmentFitsRequirements(uint32 enchantmentcondition, int8 slot) const
{
    ...
    SC_LOG_DEBUG("entities.player.items", "Player::EnchantmentFitsRequirements: Checking Condition {}, there are {} Meta Gems, {} Red Gems, {} Yellow Gems and {} Blue Gems, Activate:{}",
    ...
    else
    {
        ...
        if (GetMap()->IsDungeon())
        {
            if (WorldSafeLocsEntry const* entry = sObjectMgr->GetClosestGraveyard(GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId(), GetTeam()))
                ...
                SC_LOG_ERROR("entities.player", "Player::SetBattlegroundEntryPoint: Dungeon (MapID: {}) has no linked graveyard, setting home location as entry point.", GetMapId());
        }
...
#ifdef SYPHRENA_DEBUG
            SC_LOG_DEBUG("maps", "Object {} out of range for player {}. Distance = {}", target->GetGUID().ToString(), GetGUID().ToString(), GetDistance(target));
#endif
        }
    }
    else
...
#ifdef SYPHRENA_DEBUG
            SC_LOG_DEBUG("maps", "Object {} is visible now for player {}. Distance = {}", target->GetGUID().ToString(), GetGUID().ToString(), GetDistance(target));
#endif
...
#ifdef SYPHRENA_DEBUG
            SC_LOG_DEBUG("maps", "Object {} is out of range for player {}. Distance = {}", target->GetGUID().ToString(), GetGUID().ToString(), GetDistance(target));
#endif
        }
    }
...
void Player::UpdateVisibilityForPlayer()
{
    ...
    Syphrena::VisibleNotifier notifier(*this);
...
void Player::SendInitialPacketsAfterAddToMap()
{
    ...
    {
        setCompoundState << uint8(2 + GetPackGUID().size() + 4);
        setCompoundState << uint16(SMSG_MOVE_ENABLE_DOUBLE_JUMP);
        setCompoundState << GetPackGUID();
        setCompoundState << uint32(0);
    }*/
...
void Player::LearnCustomSpells()
{
    ...
    {
        uint32 tspell = *itr;
        SC_LOG_DEBUG("entities.player.loading", "Player::LearnCustomSpells: Player '{}' ({}, Class: {} Race: {}): Adding initial spell (SpellID: {})",
...
void Player::LearnDefaultSkill(uint32 skillId, uint16 rank)
{
    ...
    SC_LOG_DEBUG("entities.player.loading", "PLAYER (Class: {} Race: {}): Adding initial skill, id = {}", uint32(GetClass()), uint32(GetRace()), skillId);
...
bool Player::HasItemFitToSpellRequirements(SpellInfo const* spellInfo, Item const* ignoreItem) const
{
    ...
    default:
        SC_LOG_ERROR("entities.player", "Player::HasItemFitToSpellRequirements: Not handled spell requirement for item class {}", spellInfo->EquippedItemClass);
        break;
    }
...
uint32 Player::GetResurrectionSpellId()
{
    ...
    for (AuraEffectList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
    {
        ...
        if (prio < 2 && (*itr)->GetSpellInfo()->SpellVisual[0] == 99 && (*itr)->GetSpellInfo()->SpellIconID == 92)
        {
            ...
            default:
                SC_LOG_ERROR("entities.player", "Unhandled spell {}: S.Resurrection", (*itr)->GetId());
                continue;
            }
...
{
    uint8 v_level = victim->GetLevel();
    uint8 k_grey = Syphrena::XP::GetGrayLevel(GetLevel());
...
void Player::SetClientControl(Unit* target, bool allowMove)
{
    ...
    {
        SC_LOG_ERROR("entities.player", "Player '{}' attempt to client control '{}', which is charmed by GUID {}",
...
void Player::SetViewpoint(WorldObject* target, bool apply)
{
    if (apply)
    {
        SC_LOG_DEBUG("maps", "Player::CreateViewpoint: Player '{}' ({}) creates seer (Entry: {}, TypeId: {}).",
            GetName(), GetGUID().ToString(), target->GetEntry(), target->GetTypeId());

        if (!AddGuidValue(PLAYER_FARSIGHT, target->GetGUID()))
        {
            SC_LOG_FATAL("entities.player", "Player::CreateViewpoint: Player '{}' ({}) cannot add new viewpoint!", GetName(), GetGUID().ToString());
            return;
        }
    ...
    {
        SC_LOG_DEBUG("maps", "Player::CreateViewpoint: Player {} removed seer", GetName());

        if (!RemoveGuidValue(PLAYER_FARSIGHT, target->GetGUID()))
        {
            SC_LOG_FATAL("entities.player", "Player::CreateViewpoint: Player '{}' ({}) cannot remove current viewpoint!", GetName(), GetGUID().ToString());
            return;
        }
...
void Player::RestoreBaseRune(uint8 index)
{
    ...
    Syphrena::Containers::EraseIf(auras, criteria);
...
void Player::_LoadSkills(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            if (!rcEntry)
            {
                SC_LOG_ERROR("entities.player", "Player::_LoadSkills: Player '{}' ({}, Race: {}, Class: {}) has forbidden skill {} for his race/class combination",
            ...
            if (value == 0)
            {
                SC_LOG_ERROR("entities.player", "Player::_LoadSkills: Player '{}' ({}) has skill {} with value 0, deleted.",
            ...
            {
                SC_LOG_ERROR("entities.player", "Player::_LoadSkills: Player '{}' ({}) has more than {} skills.",
...
void Player::LearnTalent(uint32 talentId, uint32 talentRank)
{
    ...
    if (spellid == 0)
    {
        SC_LOG_ERROR("entities.player", "Player::LearnTalent: Talent.dbc has no spellInfo for talent: {} (spell id = 0)", talentId);
        return;
    }
    ...
    SC_LOG_DEBUG("misc", "Player::LearnTalent: TalentID: {} Spell: {} Group: {}\n", talentId, spellid, uint32(m_activeSpec));
...
void Player::LearnPetTalent(ObjectGuid petGuid, uint32 talentId, uint32 talentRank)
{
    ...
    if (spellid == 0)
    {
        SC_LOG_ERROR("entities.player", "Talent.dbc contains talent: {} Rank: {} spell id = 0", talentId, talentRank);
        return;
    }
    ...
    SC_LOG_DEBUG("entities.player", "PetTalentID: {} Rank: {} Spell: {}\n", talentId, talentRank, spellid);
...
void Player::SetEquipmentSet(EquipmentSetInfo::EquipmentSetData const& eqSet)
{
    if (eqSet.Guid != 0)
    {
        ...
        {
            SC_LOG_ERROR("entities.player", "Player::SetEquipmentSet: Player '{}' ({}) tried to save nonexistent equipment set " UI64FMTD " (index: {})",
...
void Player::SendRefundInfo(Item* item)
{
    ...
    {
        SC_LOG_DEBUG("entities.player.items", "Item refund: item not refundable!");
        return;
    }

    if (GetGUID().GetCounter() != item->GetRefundRecipient()) // Formerly refundable item got traded
    {
        SC_LOG_DEBUG("entities.player.items", "Item refund: item was traded!");
    ...
    if (!iece)
    {
        SC_LOG_DEBUG("entities.player.items", "Item refund: cannot find extendedcost data.");
        return;
    }
...
void Player::_LoadPetStable(uint8 petStableSlots, PreparedQueryResult result)
{
    ...
    {
        SC_LOG_ERROR("entities.player", "Player::LoadFromDB: Player ({}) can't have more stable slots than {}, but has {} in DB",
...
Pet* Player::SummonPet(uint32 entry, float x, float y, float z, float ang, PetType petType, uint32 duration)
{
    ...
    {
        SC_LOG_ERROR("misc", "Player::SummonPet: Pet ({}, Entry: {}) not summoned. Suggested coordinates aren't valid (X: {} Y: {})", pet->GetGUID().ToString(), pet->GetEntry(), pet->GetPositionX(), pet->GetPositionY());
        delete pet;
        return nullptr;
    }
    ...
    {
        SC_LOG_ERROR("misc", "Player::SummonPet: No such creature entry {}", entry);
        delete pet;
        return nullptr;
    }
