...
// 2^n values, Player::m_isunderwater is a bitmask. These are Syphrena internal values, they are never send to any client
...
enum TeleportToOptions
{
    ...
    TELE_TO_BYPASS_CONTINENT_LOCKDOWN = 0x80
};
...
SC_GAME_API extern uint64 const MAX_MONEY_AMOUNT;
...
class SC_GAME_API Player : public Unit, public GridObject<Player>
{
...
public:
    ...
    /*Increased gold limit int32 to int64 or uint32 to uint64*/
    ...
    void SetSkill(uint32 id, uint16 step, uint32 newVal, uint32 maxVal);
    uint32 GetMaxSkillValue(uint32 skill) const;        // max + perm. bonus + temp bonus
    uint32 GetPureMaxSkillValue(uint32 skill) const;    // max
    uint32 GetSkillValue(uint32 skill) const;           // skill value + perm. bonus + temp bonus
    uint32 GetBaseSkillValue(uint32 skill) const;       // skill value + perm. bonus
    uint32 GetPureSkillValue(uint32 skill) const;       // skill value
    int32 GetSkillPermBonusValue(uint32 skill) const;
    int32 GetSkillTempBonusValue(uint32 skill) const;
    uint32 GetSkillStep(uint32 skill) const;            // 0...6
...
SC_GAME_API void AddItemsSetItem(Player* player, Item* item);
SC_GAME_API void RemoveItemsSetItem(Player* player, ItemTemplate const* proto);
