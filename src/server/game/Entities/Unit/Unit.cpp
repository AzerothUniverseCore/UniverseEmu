...
//Spell Regulator
#include "../../../scripts/Universe/Custom/SpellRegulator/SpellRegulator.h"
//
...
void Unit::DealSpellDamage(SpellNonMeleeDamage const* damageInfo, bool durabilityLoss)
{
    ...
    {
        SC_LOG_DEBUG("entities.unit", "Unit::DealSpellDamage has wrong damageInfo->SpellID: {}", damageInfo->SpellID);
        return;
    }
    ...
    // Appliquer la régulation du Spell Regulator
    uint32 regulatedDamage = damageInfo->damage;
    sSpellRegulator->Regulate(regulatedDamage, spellProto->Id);

    // Call default DealDamage avec les dégâts régulés
    CleanDamage cleanDamage(damageInfo->cleanDamage, damageInfo->absorb, BASE_ATTACK, MELEE_HIT_NORMAL);
    Unit::DealDamage(this, victim, regulatedDamage, &cleanDamage, SPELL_DIRECT_DAMAGE, SpellSchoolMask(damageInfo->schoolMask), spellProto, durabilityLoss);
}
...
void Unit::CalculateMeleeDamage(Unit* victim, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    switch (damageInfo->HitOutCome)
    {
        ...
        case MELEE_HIT_GLANCING:
        {
            ...
            if (leveldif < 0)
            {
                SC_LOG_DEBUG("entities.unit", "Unit::CalculateMeleeDamage: (Player) {} attacked {}. Glancing should never happen against lower level target", GetGUID().ToString(), victim->GetGUID().ToString());
                break;
            }
            if (leveldif == 0)
                leveldif = 1;
...
/*static*/ void Unit::CalcAbsorbResist(DamageInfo& damageInfo, Spell* spell /*= nullptr*/)
{
    ...
    vSchoolAbsorbCopy.sort(Syphrena::AbsorbAuraOrderPred());
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            SC_LOG_DEBUG("entities.unit", "AttackerStateUpdate: (Player) {} attacked {} for {} dmg, absorbed {}, blocked {}, resisted {}.",
                GetGUID().ToString(), victim->GetGUID().ToString(), dmgInfo.GetDamage(), dmgInfo.GetAbsorb(), dmgInfo.GetBlock(), dmgInfo.GetResist());
        else
            SC_LOG_DEBUG("entities.unit", "AttackerStateUpdate: (NPC)    {} attacked {} for {} dmg, absorbed {}, blocked {}, resisted {}.",
                GetGUID().ToString(), victim->GetGUID().ToString(), dmgInfo.GetDamage(), dmgInfo.GetAbsorb(), dmgInfo.GetBlock(), dmgInfo.GetResist());
    }
}
...
void Unit::SendMeleeAttackStop(Unit* victim)
{
    ...
    if (victim)
        SC_LOG_DEBUG("entities.unit", "{} stopped attacking {}", GetGUID().ToString(), victim->GetGUID().ToString());
    else
        SC_LOG_DEBUG("entities.unit", "{} stopped attacking", GetGUID().ToString());
}
    ...
    for (AuraEffect const* aurEff : ignore)
    {
        ...
        switch (aurEff->GetMiscValue())
        {
            ...
            case MELEE_HIT_PARRY:
                ...
                break;
            default:
                SC_LOG_DEBUG("entities.unit", "Spell {} SPELL_AURA_IGNORE_COMBAT_RESULT has unhandled state {}", aurEff->GetId(), aurEff->GetMiscValue());
                break;
        }
    }
...
{
    //SC_LOG_DEBUG("entities.unit", "Interrupt spell for unit {}.", GetEntry());
...
AuraApplication* Unit::_CreateAuraApplication(Aura* aura, uint8 effMask)
{
    ...
    if (aura->IsRemoved())
    {
        SC_LOG_ERROR("spells", "Unit::_CreateAuraApplication() called with a removed aura. Check if OnEffectHitTarget() is triggering any spell with apply aura effect (that's not allowed!)\nUnit: {}\nAura: {}", GetDebugInfo(), aura->GetDebugInfo());
        return nullptr;
    }
...
void Unit::RemoveOwnedAura(Aura* aura, AuraRemoveMode removeMode)
{
    ...
    {
        SC_LOG_ERROR("spells", "Unit::RemoveOwnedAura() called with unallowed removeMode AURA_REMOVE_NONE, spellId {}", aura->GetId());
        return;
    }
...
void Unit::RemoveAllAuras()
{
    ...
    for (int counter = 0; !m_appliedAuras.empty() || !m_ownedAuras.empty(); counter++)
    {
        ...
        if (counter >= maxIteration)
        {
            ...
            SC_LOG_ERROR("entities.unit", "{}", sstr.str());
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
    ...
    switch (aura->GetAuraType())
    {
        ...
        case SPELL_AURA_PERIODIC_MANA_LEECH:
            ...
            break;
        default:
            SC_LOG_ERROR("entities.unit", "Unit::SendPeriodicAuraLog: unknown aura {}", uint32(aura->GetAuraType()));
            return;
    }
...
void Unit::RemoveAllAttackers()
{
    while (!m_attackers.empty())
    {
        ...
        if (!(*iter)->AttackStop())
        {
            SC_LOG_ERROR("entities.unit", "WORLD: Unit has an attacker that isn't attacking it!");
...
Minion* Unit::GetFirstMinion() const
{
    if (ObjectGuid pet_guid = GetMinionGUID())
    {
        ...
        SC_LOG_ERROR("entities.unit", "Unit::GetFirstMinion: Minion {} not exist.", pet_guid.ToString());
...
Guardian* Unit::GetGuardianPet() const
{
    if (ObjectGuid pet_guid = GetPetGUID())
    {
        ...
        SC_LOG_FATAL("entities.unit", "Unit::GetGuardianPet: Guardian {} not exist.", pet_guid.ToString());
...
{
    SC_LOG_DEBUG("entities.unit", "SetMinion {} for {}, apply {}", minion->GetEntry(), GetEntry(), apply);

    if (apply)
    {
        if (minion->GetOwnerGUID())
        {
            SC_LOG_FATAL("entities.unit", "SetMinion: Minion {} is not the minion of owner {}", minion->GetEntry(), GetEntry());
            return;
        }

        if (!IsInWorld())
        {
            SC_LOG_FATAL("entities.unit", "SetMinion: Minion being added to owner not in world. Minion: {}, Owner: {}", minion->GetGUID().ToString(), GetDebugInfo());
            return;
        }
...
void Unit::RemoveAllControlled()
{
    ...
    while (!m_Controlled.empty())
    {
        ...
        if (target->GetCharmerGUID() == GetGUID())
            ...
            SC_LOG_ERROR("entities.unit", "Unit {} is trying to release unit {} which is neither charmed nor owned by it", GetEntry(), target->GetEntry());
    }
    if (GetPetGUID())
        SC_LOG_FATAL("entities.unit", "Unit {} is not able to release its pet {}", GetEntry(), GetPetGUID().ToString());
    if (GetMinionGUID())
        SC_LOG_FATAL("entities.unit", "Unit {} is not able to release its minion {}", GetEntry(), GetMinionGUID().ToString());
    if (GetCharmedGUID())
        SC_LOG_FATAL("entities.unit", "Unit {} is not able to release its charm {}", GetEntry(), GetCharmedGUID().ToString());
...
bool Unit::IsImmunedToSpell(SpellInfo const* spellInfo, WorldObject const* caster, bool requireImmunityPurgesEffectAttribute /*= false*/) const
{
    ...
    {
        Syphrena::IteratorPair<SpellImmuneContainer::const_iterator> range = Syphrena::Containers::MapEqualRange(container, key);
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry)
{
    ...
    if (IsNPCBot())
    {
        if (VehicleId)
        {
            SC_LOG_ERROR("scripts", "NPCBot::Mount mounting {}, vehicle {} ({})", mount, VehicleId, creatureEntry);
...
void Unit::Dismount()
{
    ...
    {
        //SC_LOG_ERROR("scripts", "NPCBot::Dismount dismounting vehicle {} (base {}, cre {})",
...
void Unit::UpdateSpeed(UnitMoveType mtype)
{
    ...
    switch (mtype)
    {
        ...
        default:
            SC_LOG_ERROR("entities.unit", "Unit::UpdateSpeed: Unsupported move type ({})", mtype);
            return;
    }
...
void Unit::HandleStatFlatModifier(UnitMods unitMod, UnitModifierFlatType modifierType, float amount, bool apply)
{
    ...
    {
        SC_LOG_ERROR("entities.unit", "ERROR in HandleStatFlatModifier(): non-existing UnitMods or wrong UnitModifierType!");
        return;
    }
...
void Unit::ApplyStatPctModifier(UnitMods unitMod, UnitModifierPctType modifierType, float pct)
{
    ...
    {
        SC_LOG_ERROR("entities.unit", "ERROR in ApplyStatPctModifier(): non-existing UnitMods or wrong UnitModifierType!");
        return;
    }
...
float Unit::GetFlatModifierValue(UnitMods unitMod, UnitModifierFlatType modifierType) const
{
    if (unitMod >= UNIT_MOD_END || modifierType >= MODIFIER_TYPE_FLAT_END)
    {
        SC_LOG_ERROR("entities.unit", "attempt to access non-existing modifier value from UnitMods!");
        return 0.0f;
    }
...
float Unit::GetTotalAuraModValue(UnitMods unitMod) const
{
    ...
    {
        SC_LOG_ERROR("entities.unit", "attempt to access non-existing UnitMods in GetTotalAuraModValue()!");
        return 0.0f;
    }
...
void Unit::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (Unit* owner = GetOwner())
        {
            ...
            {
                SC_LOG_FATAL("entities.unit", "Unit {} is in controlled list of {} when removed from world", GetEntry(), owner->GetEntry());
                ABORT();
            }
        }
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            if (Unit* charmer = GetCharmer())
            {
                ...
                if (Creature* creatureCharmer = charmer->ToCreature())
                {
                    ...
                    SC_LOG_ERROR("entities.unit.ai", "Attempt to assign charm AI to player {} who is charmed by non-creature {}.", GetGUID().ToString(), GetCharmerGUID().ToString());
            }
...
void CharmInfo::LoadPetActionBar(const std::string& data)
{
    ...
    std::vector<std::string_view> tokens = Syphrena::Tokenize(data, ' ', false);
    ...
    {
        Optional<uint8> type = Syphrena::StringTo<uint8>(*(iter++));
        Optional<uint32> action = Syphrena::StringTo<uint32>(*(iter++));
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist) const
{
    ...
    Syphrena::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, dist);
    Syphrena::UnitListSearcher<Syphrena::AnyUnfriendlyUnitInObjectRangeCheck> searcher(this, targets, u_check);
    ...
    // select random
    return Syphrena::Containers::SelectRandomContainerElement(targets);
}
...
bool Unit::InitTamedPet(Pet* pet, uint8 level, uint32 spell_id)
{
    ...
    {
        SC_LOG_ERROR("entities.unit", "Pet::InitStatsForLevel() failed for creature (Entry: {})!", pet->GetEntry());
        return false;
    }
...
/*static*/ void Unit::Kill(Unit* attacker, Unit* victim, bool durabilityLoss /*= true*/)
{
    ...
    {
        SC_LOG_DEBUG("entities.unit", "SET JUST_DIED");
    ...
    if (player)
    {
        ...
        if (pet && pet->IsAlive() && pet->isControlled())
        {
            if (pet->IsAIEnabled())
                ...
                SC_LOG_ERROR("entities.unit", "Pet doesn't have any AI in Unit::Kill(). {}", pet->GetDebugInfo());
        }
    }
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        {
            SC_LOG_DEBUG("entities.unit", "We are dead, losing {} percent durability", sWorld->getRate(RATE_DURABILITY_LOSS_ON_DEATH));
    ...
    {
        SC_LOG_DEBUG("entities.unit", "DealDamageNotPlayer");
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    SC_LOG_DEBUG("entities.unit", "SetCharmedBy: charmer {}, charmed {}, type {}.", charmer->GetGUID().ToString(), GetGUID().ToString(), uint32(type));

    if (this == charmer)
    {
        SC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: Unit {} is trying to charm itself!", GetGUID().ToString());
        return false;
    }
    ...
    {
        SC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: {} is trying to charm Player {} on transport", charmer->GetGUID().ToString(), GetGUID().ToString());
        return false;
    }
    ...
    if (GetCharmerGUID())
    {
        SC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: {} has already been charmed but {} is trying to charm it!", GetGUID().ToString(), charmer->GetGUID().ToString());
        return false;
    }
    ...
    if (!IsInWorld())
    {
        SC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: {} is not in world but {} is trying to charm it!", GetGUID().ToString(), charmer->GetGUID().ToString());
        return false;
    }
                ...
                if (GetTypeId() == TYPEID_UNIT && charmer->GetClass() == CLASS_WARLOCK)
                {
                    ...
                    if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
                    {
                        ...
                        if (GetCharmInfo())
                            ...
                            SC_LOG_ERROR("entities.unit", "Aura::HandleModCharm: {} has a charm aura but no charm info!", GetGUID().ToString());
                    }
                }
                break;
            case CHARM_TYPE_CONVERT:
...
bool Unit::CreateVehicleKit(uint32 id, uint32 creatureEntry)
{
    ...
    m_vehicleKit = Syphrena::make_unique_trackable<Vehicle>(this, vehInfo, creatureEntry);
...
void Unit::HandleSpellClick(Unit* clicker, int8 seatId /*= -1*/)
{
    ...
    for (auto const& clickPair : clickBounds)
    {
        ...
        if (seatId > -1)
        {
            ...
            if (!valid)
            {
                SC_LOG_ERROR("sql.sql", "Spell {} specified in npc_spellclick_spells is not a valid vehicle enter aura!", clickPair.second.spellId);
                continue;
            }
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
    ...
    if (m_vehicle)
    {
        ...
        {
            SC_LOG_DEBUG("entities.vehicle", "EnterVehicle: {} exit {} and enter {}.", GetEntry(), m_vehicle->GetBase()->GetEntry(), vehicle->GetBase()->GetEntry());
            ExitVehicle();
        }
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
    ...
    if (!vehicle)
    {
        SC_LOG_ERROR("entities.vehicle", "RemovePassenger() couldn't remove current unit from vehicle. Debug info: {}", GetDebugInfo());
        return;
    }
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    if (!Syphrena::IsValidMapCoord(x, y, z, orientation))
    {
        SC_LOG_DEBUG("entities.unit", "Unit::UpdatePosition({}, {}, {}) .. bad coordinates!", x, y, z);
        return false;
    }
...
void Unit::CheckPendingMovementAcks()
{
    ...
    if (GameTime::GetGameTimeMS() > oldestChangeToAck.time + sWorld->getIntConfig(CONFIG_PENDING_MOVE_CHANGES_TIMEOUT))
    {
        ...
        */
        ...
        SC_LOG_INFO("cheat", "Unit::CheckPendingMovementAcks: Player GUID: {} took too long to acknowledge a movement change. He was therefore kicked.", controller->GetBasePlayer()->GetGUID().ToString());
    }
}
...
{
    SC_LOG_ERROR("entities.unit", "Unit::OutDebugInfo");
    SC_LOG_DEBUG("entities.unit", "{} name {}", GetGUID().ToString(), GetName());
    SC_LOG_DEBUG("entities.unit", "Owner {}, Minion {}, Charmer {}, Charmed {}", GetOwnerGUID().ToString(), GetMinionGUID().ToString(), GetCharmerGUID().ToString(), GetCharmedGUID().ToString());
    SC_LOG_DEBUG("entities.unit", "In world {}, unit type mask {}", (uint32)(IsInWorld() ? 1 : 0), m_unitTypeMask);
    if (IsInWorld())
        SC_LOG_DEBUG("entities.unit", "Mapid {}", GetMapId());
    ...
    SC_LOG_DEBUG("entities.unit", "{}", o.str());
    o.str("");
    ...
    if (GetVehicle())
        SC_LOG_DEBUG("entities.unit", "On vehicle {}.", GetVehicleBase()->GetEntry());
}
...
{
    Syphrena::CustomChatTextBuilder builder(this, msgType, text, language, target);
    Syphrena::LocalizedPacketDo<Syphrena::CustomChatTextBuilder> localizer(builder);
    Syphrena::PlayerDistWorker<Syphrena::LocalizedPacketDo<Syphrena::CustomChatTextBuilder> > worker(this, textRange, localizer);
...
void Unit::Talk(uint32 textId, ChatMsg msgType, float textRange, WorldObject const* target)
{
    ...
    {
        SC_LOG_ERROR("entities.unit", "WorldObject::MonsterText: `broadcast_text` (ID: {}) was not found", textId);
        return;
    }

    Syphrena::BroadcastTextBuilder builder(this, msgType, textId, GetGender(), target);
    Syphrena::LocalizedPacketDo<Syphrena::BroadcastTextBuilder> localizer(builder);
    Syphrena::PlayerDistWorker<Syphrena::LocalizedPacketDo<Syphrena::BroadcastTextBuilder> > worker(this, textRange, localizer);
    Cell::VisitWorldObjects(this, worker, textRange);
}
