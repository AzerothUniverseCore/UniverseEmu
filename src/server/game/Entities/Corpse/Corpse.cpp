...
bool Corpse::Create(ObjectGuid::LowType guidlow, Player* owner)
{
    ...
    if (!IsPositionValid())
    {
        SC_LOG_ERROR("entities.player", "Corpse (guidlow {}, owner {}) not created. Suggested coordinates isn't valid (X: {} Y: {})",
    ...
    _cellCoord = Syphrena::ComputeCellCoord(GetPositionX(), GetPositionY());
...
bool Corpse::LoadCorpseFromDB(ObjectGuid::LowType guid, Field* fields)
{
    ...
    {
        SC_LOG_ERROR("entities.player", "Corpse ({}, owner: {}) is not created, given equipment info is not valid ('{}')",
    ...
    if (!IsPositionValid())
    {
        SC_LOG_ERROR("entities.player", "Corpse ({}, owner: {}) is not created, given coordinates are not valid (X: {}, Y: {}, Z: {})",
