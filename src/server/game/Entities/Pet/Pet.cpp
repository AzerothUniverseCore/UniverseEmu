...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current)
{
    ...
    if (IsCritter())
    {
        ...
        if (!IsPositionValid())
        {
            SC_LOG_ERROR("entities.pet", "Pet{} not loaded. Suggested coordinates isn't valid (X: {} Y: {})",
    ...
    switch (getPetType())
    {
        ...
        default:
            if (!IsPetGhoul())
                SC_LOG_ERROR("entities.pet", "Pet have incorrect type ({}) for pet loading.", getPetType());
            break;
    }
        ...
        .AfterComplete([this, owner, session = owner->GetSession(), isTemporarySummon, current, lastSaveTime = petInfo->LastSaveTime](SQLQueryHolderBase const& holder)
    {
        ...
        SC_LOG_DEBUG("entities.pet", "New Pet has {}", GetGUID().ToString());
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            if (isControlled())
            {
                ...
                {
                    SC_LOG_ERROR("entities.pet", "Pet {} is not pet of owner {}, removed", GetEntry(), GetOwner()->GetName());
...
bool Pet::CreateBaseAtCreature(Creature* creature)
{
    ...
    if (!IsPositionValid())
    {
        SC_LOG_ERROR("entities.pet", "Pet {} not created base at creature. Suggested coordinates isn't valid (X: {} Y: {})",
            GetGUID().ToString(), GetPositionX(), GetPositionY());
        return false;
    }
    ...
    if (!cinfo)
    {
        SC_LOG_ERROR("entities.pet", "CreateBaseAtCreature() failed, creatureInfo is missing!");
        return false;
    }
...
{
    SC_LOG_DEBUG("entities.pet", "Pet::CreateBaseForTamed");
    ObjectGuid::LowType guid = map->GenerateLowGuid<HighGuid::Pet>();
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (IsPet() && GetOwner()->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            SC_LOG_ERROR("entities.pet", "Unknown type pet {} is summoned by player class {}",
...
{
    SC_LOG_DEBUG("entities.pet", "Loading auras for pet {}", GetGUID().ToString());
    ...
    if (result)
    {
        do
        {
            ...
            if (!spellInfo)
            {
                SC_LOG_ERROR("entities.pet", "Unknown aura (spellid {}), ignore.", spellid);
                continue;
            }
            ...
            if (Aura* aura = Aura::TryCreate(createInfo))
            {
                ...
                SC_LOG_DEBUG("entities.pet", "Added aura spellid {}, effectmask {}", spellInfo->Id, effmask);
            }
        }
        while (result->NextRow());
    }
...
bool Pet::addSpell(uint32 spellId, ActiveStates active /*= ACT_DECIDE*/, PetSpellState state /*= PETSPELL_NEW*/, PetSpellType type /*= PETSPELL_NORMAL*/)
{
    ...
    if (!spellInfo)
    {
        ...
        {
            SC_LOG_ERROR("entities.pet", "Pet::addSpell: Non-existed in SpellStore spell #{} request, deleting for all pets in `pet_spell`.", spellId);
            ...
            SC_LOG_ERROR("entities.pet", "Pet::addSpell: Non-existed in SpellStore spell #{} request.", spellId);
