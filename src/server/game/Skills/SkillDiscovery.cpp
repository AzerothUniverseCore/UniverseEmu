...
void LoadSkillDiscoveryTable()
{
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 skill discovery definitions. DB table `skill_discovery_template` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (reqSkillOrSpell > 0)                            // spell case
        {
            ...
            if (!reqSpellInfo)
            {
                ...
                {
                    SC_LOG_ERROR("sql.sql", "Spell (ID: {}) has a non-existing spell (ID: {}) in `reqSpell` field in the `skill_discovery_template` table.", spellId, reqSkillOrSpell);
                ...
                !reqSpellInfo->IsExplicitDiscovery())
            {
                if (reportedReqSpells.find(absReqSkillOrSpell) == reportedReqSpells.end())
                {
                    SC_LOG_ERROR("sql.sql", "Spell (ID: {}) does not have any MECHANIC_DISCOVERY (28) value in the Mechanic field in spell.dbc"
        ...
        else if (reqSkillOrSpell == 0)                      // skill case
        {
            ...
            {
                SC_LOG_ERROR("sql.sql", "Spell (ID: {}) is not listed in `SkillLineAbility.dbc`, but listed with `reqSpell`= 0 in the `skill_discovery_template` table.", spellId);
                continue;
            }
        ...
        {
            SC_LOG_ERROR("sql.sql", "Spell (ID: {}) has a negative value in `reqSpell` field in the `skill_discovery_template` table.", spellId);
            continue;
        }
    ...
    if (!ssNonDiscoverableEntries.str().empty())
        SC_LOG_ERROR("sql.sql", "Some items can't be successfully discovered, their chance field value is < 0.000001 in the `skill_discovery_template` DB table. List:\n{}", ssNonDiscoverableEntries.str());
    ...
    for (uint32 spell_id = 1; spell_id < sSpellMgr->GetSpellInfoStoreSize(); ++spell_id)
    {
        ...
        if (SkillDiscoveryStore.find(int32(spell_id)) == SkillDiscoveryStore.end())
            SC_LOG_ERROR("sql.sql", "Spell (ID: {}) has got 100%% chance random discovery ability, but does not have data in the `skill_discovery_template` table.", spell_id);
    }

    SC_LOG_INFO("server.loading", ">> Loaded {} skill discovery definitions in {} ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
