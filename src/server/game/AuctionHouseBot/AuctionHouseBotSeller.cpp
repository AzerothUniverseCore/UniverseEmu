...
bool AuctionBotSeller::Initialize()
{
    ...
    SC_LOG_DEBUG("ahbot", "AHBot seller filters:");
    ...
    SC_LOG_DEBUG("ahbot", "Forced Inclusion {} items", (uint32)includeItems.size());
    SC_LOG_DEBUG("ahbot", "Forced Exclusion {} items", (uint32)excludeItems.size());

    SC_LOG_DEBUG("ahbot", "Loading npc vendor items for filter..");
    ...
    SC_LOG_DEBUG("ahbot", "Npc vendor filter has {} items", (uint32)npcItems.size());

    SC_LOG_DEBUG("ahbot", "Loading loot items for filter..");
    ...
    SC_LOG_DEBUG("ahbot", "Loot filter has {} items", (uint32)lootItems.size());
    SC_LOG_DEBUG("ahbot", "Sorting and cleaning items for AHBot seller...");
    ...
    if (!itemsAdded)
    {
        SC_LOG_ERROR("ahbot", "AuctionHouseBot seller not have items, disabled.");
    ...
    SC_LOG_DEBUG("ahbot", "AuctionHouseBot seller will use {} items to fill auction house (according your config choices)", itemsAdded);

    LoadConfig();

    SC_LOG_DEBUG("ahbot", "Items loaded \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    ...
    SC_LOG_DEBUG("ahbot", "AHBot seller configuration data loaded and initialized");
    return true;
}
...
uint32 AuctionBotSeller::SetStat(SellerConfiguration& config)
{
    ...
    SC_LOG_DEBUG("ahbot", "AHBot: Missed Item       \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
    {
        SC_LOG_DEBUG("ahbot", "AHBot: \t\t{}\t{}\t{}\t{}\t{}\t{}\t{}",
...
void AuctionBotSeller::AddNewAuctions(SellerConfiguration& config)
{
    ...
    if (config.LastMissedItem > sAuctionBotConfig->GetItemPerCycleBoost())
    {
        ...
        SC_LOG_DEBUG("ahbot", "AHBot: Boost value used to fill AH! (if this happens often adjust both ItemsPerCycle in worldserver.conf)");
    }
    else
    ...
    while (GetItemsToSell(config, itemsToSell, allItems) && items > 0)
    {
        ...
        ItemToSell const& sellItem = Syphrena::Containers::SelectRandomContainerElement(itemsToSell);

        // Set itemId with random item ID for selected categories and color, from _itemPool table
        uint32 itemId = Syphrena::Containers::SelectRandomContainerElement(_itemPool[sellItem.Color][sellItem.Itemclass]);
        ...
        if (!itemId)
        {
            SC_LOG_DEBUG("ahbot", "AHBot: Item entry 0 auction creating attempt.");
            continue;
        }
        ...
        if (!prototype)
        {
            SC_LOG_DEBUG("ahbot", "AHBot: Unknown item {} auction creating attempt.", itemId);
            continue;
        }
        ...
        if (!item)
        {
            SC_LOG_ERROR("ahbot", "AHBot: Item::CreateItem() returned NULL for item {} (stack: {})", itemId, stackCount);
            return;
        }
    ...
    SC_LOG_DEBUG("ahbot", "AHBot: Added {} items to auction", count);
}
...
bool AuctionBotSeller::Update(AuctionHouseType houseType)
{
    ...
    {
        SC_LOG_DEBUG("ahbot", "AHBot: {} selling ...", AuctionBotConfig::GetHouseTypeName(houseType));
