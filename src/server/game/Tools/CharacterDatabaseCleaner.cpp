...
void CharacterDatabaseCleaner::CleanDatabase()
{
    ...
    SC_LOG_INFO("misc", "Cleaning character database...");
    ...
    SC_LOG_INFO("server.loading", ">> Cleaned character database in {} ms", GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDatabaseCleaner::CheckUnique(char const* column, char const* table, bool (*check)(uint32))
{
    ...
    if (!result)
    {
        SC_LOG_INFO("misc", "Table {} is empty.", table);
        return;
    }
