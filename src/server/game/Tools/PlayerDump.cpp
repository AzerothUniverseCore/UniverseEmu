...
inline void MarkDependentColumn(TableStruct& tableStruct, std::string const& columnName, GuidType dependentType)
{
    ...
    {
        SC_LOG_FATAL("server.loading", "Column `{}` declared in table `{}` marked as dependent but doesn't exist, PlayerDump will not work properly, please update table definitions",
    ...
    {
        SC_LOG_FATAL("server.loading", "Attempt to mark column `{}` in table `{}` as dependent column but already marked! please check your code.",
            columnName, tableStruct.TableName);
        ABORT();
        return;
    }
...
inline void MarkWhereField(TableStruct& tableStruct, std::string const& whereField)
{
    ...
    {
        SC_LOG_FATAL("server.loading", "Column name `{}` set as 'WHERE' column for table `{}` doesn't exist. PlayerDump won't work properly",
...
void PlayerDump::InitializeTables()
{
    ...
    for (DumpTable const& dumpTable : DumpTables)
    {
        ...
        switch (dumpTable.Type)
        {
            ...
            case DTT_EQSET_TABLE:
                ...
                {
                    std::string itColumn = Syphrena::StringFormat("item{}", j);
            ...
            case DTT_PET_TABLE:
                ...
                break;
            default:
                SC_LOG_FATAL("server.loading", "Wrong dump table type {}, probably added a new table type without updating code", uint32(dumpTable.Type));
                ABORT();
                return;
        }
    ...
    for (TableStruct const& tableStruct : CharacterTables)
    {
        ...
        {
            SC_LOG_FATAL("server.loading", "Table `{}` defined in player dump doesn't have a WHERE query field", tableStruct.TableName);
            ABORT();
        }
    }
    ...
    SC_LOG_INFO("server.loading", ">> Initialized tables for PlayerDump in {} ms.", GetMSTimeDiffToNow(oldMSTime));
}
...
inline bool ValidateFields(TableStruct const& ts, std::string const& str, size_t lineNumber)
{
    ...
    if (s == std::string::npos)
    {
        SC_LOG_ERROR("misc", "LoadPlayerDump: (line {}) dump format not recognized.", lineNumber);
        return false;
    }
    s += 4;
    ...
    {
        SC_LOG_ERROR("misc", "LoadPlayerDump: (line {}) unexpected end of line", lineNumber);
        return false;
    }
    ...
    do
    {
        ...
        if (columnIndex == -1)
        {
            SC_LOG_ERROR("misc", "LoadPlayerDump: (line {}) unknown column name `{}` for table `{}`, aborting due to incompatible DB structure.", lineNumber, column, ts.TableName);
            return false;
        }
...
{
    T oldGuid = Syphrena::StringTo<T>(GetColumn(ts, str, column)).template value_or<T>(0);
...
DumpReturn PlayerDumpReader::LoadDump(std::istream& input, uint32 account, std::string name, ObjectGuid::LowType guid)
{
    ...
    while (std::getline(input, line))
    {
        ...
        if (tn.empty())
        {
            SC_LOG_ERROR("misc", "LoadPlayerDump: (line {}) Can't extract table name!", lineNumber);
        ...
        {
            SC_LOG_ERROR("misc", "LoadPlayerDump: (line {}) Unknown table: `{}`!", lineNumber, tn);
            return DUMP_FILE_BROKEN;
        }
        ...
        switch (type)
        {
            case DTT_CHARACTER:
            {
                race = Syphrena::StringTo<uint8>(GetColumn(ts, line, "race")).value_or<uint8>(0);
                playerClass = Syphrena::StringTo<uint8>(GetColumn(ts, line, "class")).value_or<uint8>(0);
                gender = Syphrena::StringTo<uint8>(GetColumn(ts, line, "gender")).value_or<uint8>(0);
                level = Syphrena::StringTo<uint8>(GetColumn(ts, line, "level")).value_or<uint8>(0);
                if (name.empty())
                {
                    // generate a temporary name
                    std::string guidPart = Syphrena::StringFormat("{:X}", guid);
