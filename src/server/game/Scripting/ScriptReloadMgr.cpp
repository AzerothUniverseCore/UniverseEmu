...
#ifndef SYPHRENA_API_USE_DYNAMIC_LINKING
...
#if SYPHRENA_PLATFORM == SYPHRENA_PLATFORM_WINDOWS
    return "dll";
#elif SYPHRENA_PLATFORM == SYPHRENA_PLATFORM_APPLE
    return "dylib";
#else // Posix
    return "so";
#endif
...
public:
    ...
    void operator() (HandleType handle) const
    {
        ...
        if (!success)
        {
            SC_LOG_ERROR("scripts.hotswap", "Failed to unload (syscall) the shared library \"{}\".",
        ...
        if (cache_path_)
        {
            ...
            {
                SC_LOG_ERROR("scripts.hotswap", "Failed to delete the cached shared library \"{}\" ({}).",
                    cache_path_->generic_string(), error.message());

                return;
            }

            SC_LOG_TRACE("scripts.hotswap", "Lazy unloaded the shared library \"{}\" "
    ...
    ScriptModule::CreateFromPath(fs::path const& path, Optional<fs::path> cache_path)
{
    ...
    if (!handle)
    {
        if (cache_path)
        {
            SC_LOG_ERROR("scripts.hotswap", "Could not dynamic load the shared library \"{}\" "
    ...
    {
        SC_LOG_ERROR("scripts.hotswap", "Could not extract all required functions from the shared library \"{}\"!",
            path.generic_string());
...
static bool HasValidScriptModuleName(std::string const& name)
{
    ...
    static Syphrena::regex const regex(
        Syphrena::StringFormat("^{}[sS]cripts_[a-zA-Z0-9_]+\\.{}$",
            GetSharedLibraryPrefix(),
            GetSharedLibraryExtension()));

    return Syphrena::regex_match(name, regex);
}
...
static int InvokeCMakeCommand(T&&... args)
{
    ...
    return Syphrena::StartProcess(executable, {
        std::forward<T>(args)...
    }, "scripts.hotswap");
}
...
template<typename... T>
static std::shared_ptr<Syphrena::AsyncProcessResult> InvokeAsyncCMakeCommand(T&&... args)
{
    auto const executable = BuiltInConfig::GetCMakeCommand();
    return Syphrena::StartAsyncProcess(executable, {
        std::forward<T>(args)...
    }, "scripts.hotswap");
}
    ...
    : public ScriptReloadMgr
{
    ...
    class BuildJob
    {
        ...
        std::shared_ptr<Syphrena::AsyncProcessResult> async_result_;
    ...
    public:
        ...
        std::shared_ptr<Syphrena::AsyncProcessResult> const& GetProcess() const
        {
        ...
        void UpdateCurrentJob(BuildJobType type,
                              std::shared_ptr<Syphrena::AsyncProcessResult> async_result)
        {
...
public:
    ...
    void Initialize() final override
    {
        ...
        {
            SC_LOG_ERROR("scripts.hotswap", "Your build directory path \"{}\" "
...

        {
            ...
            {
                SC_LOG_ERROR("scripts.hotswap", "Library directory \"{}\" doesn't exist!.",
        ...
        {
            SC_LOG_ERROR("scripts.hotswap", "Couldn't create the cache directory at \"{}\".",
...
private:
    ...
    void InitializeDefaultLibraries()
    {
        ...
        for (fs::directory_iterator dir_itr(libraryDirectory); dir_itr != dir_end ; ++dir_itr)
            ...
            {
                SC_LOG_INFO("scripts.hotswap", "Loading script module \"{}\"...",
        ...
        SC_LOG_INFO("scripts.hotswap", ">> Loaded {} script modules.", count);
    }
    ...
    void InitializeFileWatchers()
    {
        ...
        if (_libraryWatcher >= 0)
        {
            SC_LOG_INFO("scripts.hotswap", ">> Library reloader is listening on \"{}\".",
                GetLibraryDirectory().generic_string());
        }
        else
        {
            SC_LOG_ERROR("scripts.hotswap", "Failed to initialize the library reloader on \"{}\".",
                GetLibraryDirectory().generic_string());
        }
    ...
    static fs::path CalculateTemporaryCachePath()
    {
        ...
        path /= Syphrena::StringFormat("SC_script_cache_{}_{}",
            GitRevision::GetBranch(),
            ByteArrayToHexStr(Syphrena::Crypto::SHA1::GetDigestOf(sConfigMgr->GetFilename())));
    ...
    fs::path GenerateUniquePathForLibraryInCache(fs::path path)
    {
        ...
        cache_path /= Syphrena::StringFormat("{}.{}{}",
    ...
    void ProcessLoadScriptModule(fs::path const& path, bool swap_context = true)
    {
...

        {
            ...
            if (code)
            {
                SC_LOG_FATAL("scripts.hotswap", ">> Failed to create cache entry for module "
                    "\"{}\" at \"{}\" with reason (\"{}\")!",
            ...
            SC_LOG_TRACE("scripts.hotswap", ">> Copied the shared library \"{}\" to \"{}\" for caching.",
                path.filename().generic_string(), cache_path.generic_string());
        }
        ...
        if (!module)
        {
            SC_LOG_FATAL("scripts.hotswap", ">> Failed to load script module \"{}\"!",
        ...
        SC_LOG_INFO("scripts.hotswap", ">> Loaded script module \"{}\" (\"{}\" - {}).",
            path.filename().generic_string(), module_name, module_revision);

        if (module_revision.empty())
        {
            SC_LOG_WARN("scripts.hotswap", ">> Script module \"{}\" has an empty revision hash!",
                path.filename().generic_string());
        }
        else
        {
            ...
            {
                SC_LOG_WARN("scripts.hotswap", ">> Script module \"{}\" has a different revision hash! "
...

        {
            ...
            {
                SC_LOG_ERROR("scripts.hotswap", ">> Attempt to load a module twice \"{}\" (loaded module is at {})!",
        ...
        (*module)->AddScripts();
        SC_LOG_TRACE("scripts.hotswap", ">> Registered all scripts of module {}.", module_name);
    ...
    void ProcessUnloadScriptModule(fs::path const& path, bool finish = true)
    {
        ...
        SC_LOG_INFO("scripts.hotswap", "Released script module \"{}\" (\"{}\")...",
    ...
    void DispatchRunningBuildJobs()
    {
        if (_build_job)
        {
            ...
            if (sWorld->getBoolConfig(CONFIG_HOTSWAP_EARLY_TERMINATION_ENABLED))
            {
                if (!terminate_early && _sources_changed.find(_build_job->GetModuleName()) != _sources_changed.end())
                {
                    ...
                    SC_LOG_INFO("scripts.hotswap", "Terminating the running build of module \"{}\"...",
        ...
        if (IsDebuggerBlockingRebuild())
        {
                ...
                (GetMSTimeDiffToNow(_last_time_user_informed) > 7500))
            {
                ...
                SC_LOG_INFO("scripts.hotswap", "Your attached debugger is blocking the SyphrenaCore "
        ...
        auto module_name = [&]
        {
            ...
            if (sLog->ShouldLog("scripts.hotswap", LogLevel::LOG_LEVEL_TRACE))
                ...
                {
                    SC_LOG_TRACE("scripts.hotswap", "Source file {} was {}.",
        ...
        SC_LOG_INFO("scripts.hotswap", "Recompiling Module \"{}\"...",
            module_name);
    ...
    void ProcessReadyBuildJob()
    {
        ...
        switch (_build_job->GetType())
        {
            case BuildJobType::BUILD_JOB_RERUN_CMAKE:
            {
                if (!error)
                {
                    SC_LOG_INFO("scripts.hotswap", ">> Successfully updated the build files!");
                }
                else
                {
                    SC_LOG_INFO("scripts.hotswap", ">> Failed to update the build files at \"{}\", "
            ...
            case BuildJobType::BUILD_JOB_INSTALL:
            {
                if (!error)
                {
                    ...
                    SC_LOG_INFO("scripts.hotswap", ">> Successfully installed module {} in {}s",
                        _build_job->GetModuleName(),
    ...
    void DoRerunCMake()
    {
        ...
        SC_LOG_INFO("scripts.hotswap", "Rerunning CMake because there were sources added or removed...");
    ...
    void DoCompileCurrentProcessedModule()
    {
        ...
        SC_LOG_INFO("scripts.hotswap", "Starting asynchronous build job for module {}...",
                    _build_job->GetModuleName());
    ...
    void DoInstallCurrentProcessedModule()
    {
        ...
        SC_LOG_INFO("scripts.hotswap", "Starting asynchronous install job for module {}...",
                    _build_job->GetModuleName());
    ...
    {
        SC_LOG_INFO("scripts.hotswap", "Correcting your CMAKE_INSTALL_PREFIX in \"{}\"...",
                    BuiltInConfig::GetBuildDirectory());
        ...
        if (![&]
        {
            ...
            {
                SC_LOG_ERROR("scripts.hotswap", ">> CMake cache \"{}\" doesn't exist, "
        ...
        SC_LOG_TRACE("scripts.hotswap", "Checking CMake cache (\"{}\") "
        ...
        std::string cmake_cache_content;
        {
            ...
            if (!in.is_open())
            {
                SC_LOG_ERROR("scripts.hotswap", ">> Failed to read the CMake cache at \"{}\"!",
                    cmake_cache_path.generic_string());
        ...
        if (begin != std::string::npos)
        {
            ...
            if (end != std::string::npos)
            {
                ...
                if (value != current_path)
                {
                    ...
                    SC_LOG_INFO("scripts.hotswap", ">> Found outdated CMAKE_INSTALL_PREFIX (\"{}\"), "
                        "worldserver is currently installed at {}",
                        value.generic_string(), current_path.generic_string());
                }
                else
                {
                    SC_LOG_INFO("scripts.hotswap", ">> CMAKE_INSTALL_PREFIX is equal to the current path of execution.");
                    return;
                }
            }
        }

        SC_LOG_INFO("scripts.hotswap", "Invoking CMake cache correction...");
        ...
        if (error)
        {
            SC_LOG_ERROR("scripts.hotswap", ">> Failed to update the CMAKE_INSTALL_PREFIX! "
                "This could lead to unexpected behaviour!");
        }
        else
        {
            SC_LOG_ERROR("scripts.hotswap", ">> Successfully corrected your CMAKE_INSTALL_PREFIX variable"
    ...
    // The path to the SC_scripts temporary cache
...
{
    // SC_LOG_TRACE("scripts.hotswap", "Library listener detected change on possible module \"{}\ ({})".", filename, ActionToString(action));
    ...
    sScriptReloadMgr->QueueMessage([=](HotSwapScriptReloadMgr* reloader) mutable
    {
        ...
        switch (action)
        {
            case efsw::Actions::Add:
                SC_LOG_TRACE("scripts.hotswap", ">> Loading \"{}\" ({})...",
                    path.generic_string(), ActionToString(action));
                reloader->QueueSharedLibraryChanged(path);
                break;
            case efsw::Actions::Delete:
                SC_LOG_TRACE("scripts.hotswap", ">> Unloading \"{}\" ({})...",
                    path.generic_string(), ActionToString(action));
                reloader->QueueSharedLibraryChanged(path);
                break;
            case efsw::Actions::Modified:
                SC_LOG_TRACE("scripts.hotswap", ">> Reloading \"{}\" ({})...",
                    path.generic_string(), ActionToString(action));
                reloader->QueueSharedLibraryChanged(path);
                break;
            default:
...
{
    static Syphrena::regex const regex("^\\.(h|hpp|c|cc|cpp)$");
    return Syphrena::regex_match(path.extension().generic_string(), regex);
}
      ...
      watcher_id_(sScriptReloadMgr->_fileWatcher.addWatch(path_.generic_string(), this, true))
{
    if (watcher_id_ >= 0)
    {
        SC_LOG_TRACE("scripts.hotswap", ">> Attached the source recompiler to \"{}\".",
            path_.generic_string());
    }
    else
    {
        SC_LOG_ERROR("scripts.hotswap", "Failed to initialize thesource recompiler on \"{}\".",
            path_.generic_string());
    }
}
...
SourceUpdateListener::~SourceUpdateListener()
{
    if (watcher_id_ >= 0)
    {
        ...
        SC_LOG_TRACE("scripts.hotswap", ">> Detached the source recompiler from \"{}\".",
            path_.generic_string());
    }
}
...
void SourceUpdateListener::handleFileAction(efsw::WatchID watchid, std::string const& dir,
    std::string const& filename, efsw::Action action, std::string oldFilename)
{
    // SC_LOG_TRACE("scripts.hotswap", "Source listener detected change on possible file \"{}/{}\" ({}).", dir, filename, ActionToString(action));
    ...
    {
        SC_LOG_TRACE("scripts.hotswap", "Detected source change on module \"{}\", "
            "queued for recompilation...", script_module_name_);

        switch (action)
        {
            case efsw::Actions::Add:
                SC_LOG_TRACE("scripts.hotswap", "Source file {} of module {} was added.",
                    path.generic_string(), script_module_name_);
                reloader->QueueAddSourceFile(script_module_name_, path);
                break;
            case efsw::Actions::Delete:
                SC_LOG_TRACE("scripts.hotswap", "Source file {} of module {} was deleted.",
                    path.generic_string(), script_module_name_);
                reloader->QueueRemoveSourceFile(script_module_name_, path);
                break;
            case efsw::Actions::Modified:
                SC_LOG_TRACE("scripts.hotswap", "Source file {} of module {} was modified.",
                    path.generic_string(), script_module_name_);
