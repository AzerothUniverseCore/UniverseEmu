...
public:
    ...
    void LogDuplicatedScriptPointerError(ScriptType const* first, ScriptType const* second)
    {
        ...
        SC_LOG_ERROR("scripts", "Script '{}' has same memory pointer as '{}'.",
...
public:
    ...
    void AddScript(ScriptType* script)
    {
        ...
        else
        {
            ...
            SC_LOG_ERROR("sql.sql", "Script '{}' exists in the core, but the database does not assign it to any creature.",
                script->GetName());
...
public:
    void BeforeReleaseContext(std::string const& /*context*/) final override
    {
        Syphrena::ChatCommands::InvalidateCommandMap();
    }
...
void ScriptMgr::Initialize()
{
    ...
    SC_LOG_INFO("server.loading", "Loading C++ scripts");
    ...
    for (std::string const& scriptName : unusedScriptNames)
    {
        ...
        SC_LOG_ERROR("sql.sql", "Script '{}' is referenced by the database, but does not exist in the core!", scriptName);
    }

    SC_LOG_INFO("server.loading", ">> Loaded {} C++ scripts in {} ms",
...
Syphrena::ChatCommands::ChatCommandTable ScriptMgr::GetChatCommands()
{
    Syphrena::ChatCommands::ChatCommandTable table;

    FOR_SCRIPTS(CommandScript, itr, end)
    {
        Syphrena::ChatCommands::ChatCommandTable cmds = itr->second->GetCommands();
...
template class SC_GAME_API MapScript<Map>;
template class SC_GAME_API MapScript<InstanceMap>;
template class SC_GAME_API MapScript<BattlegroundMap>;

WorldMapScript::WorldMapScript(char const* name, uint32 mapId)
    : ScriptObject(name), MapScript(sMapStore.LookupEntry(mapId))
{
    if (!GetEntry())
        SC_LOG_ERROR("scripts", "Invalid WorldMapScript for {}; no such map ID.", mapId);

    if (GetEntry() && !GetEntry()->IsWorldMap())
        SC_LOG_ERROR("scripts", "WorldMapScript for map {} is invalid.", mapId);
...
{
    if (!GetEntry())
        SC_LOG_ERROR("scripts", "Invalid InstanceMapScript for {}; no such map ID.", mapId);

    if (GetEntry() && !GetEntry()->IsDungeon())
        SC_LOG_ERROR("scripts", "InstanceMapScript for map {} is invalid.", mapId);
...
{
    if (!GetEntry())
        SC_LOG_ERROR("scripts", "Invalid BattlegroundMapScript for {}; no such map ID.", mapId);

    if (GetEntry() && !GetEntry()->IsBattleground())
        SC_LOG_ERROR("scripts", "BattlegroundMapScript for map {} is invalid.", mapId);
...
template class SC_GAME_API ScriptRegistry<SpellScriptLoader>;
template class SC_GAME_API ScriptRegistry<ServerScript>;
template class SC_GAME_API ScriptRegistry<WorldScript>;
template class SC_GAME_API ScriptRegistry<FormulaScript>;
template class SC_GAME_API ScriptRegistry<WorldMapScript>;
template class SC_GAME_API ScriptRegistry<InstanceMapScript>;
template class SC_GAME_API ScriptRegistry<BattlegroundMapScript>;
template class SC_GAME_API ScriptRegistry<ItemScript>;
template class SC_GAME_API ScriptRegistry<CreatureScript>;
template class SC_GAME_API ScriptRegistry<GameObjectScript>;
template class SC_GAME_API ScriptRegistry<AreaTriggerScript>;
template class SC_GAME_API ScriptRegistry<BattlefieldScript>;
template class SC_GAME_API ScriptRegistry<BattlegroundScript>;
template class SC_GAME_API ScriptRegistry<OutdoorPvPScript>;
template class SC_GAME_API ScriptRegistry<CommandScript>;
template class SC_GAME_API ScriptRegistry<WeatherScript>;
template class SC_GAME_API ScriptRegistry<AuctionHouseScript>;
template class SC_GAME_API ScriptRegistry<ConditionScript>;
template class SC_GAME_API ScriptRegistry<VehicleScript>;
template class SC_GAME_API ScriptRegistry<DynamicObjectScript>;
template class SC_GAME_API ScriptRegistry<TransportScript>;
template class SC_GAME_API ScriptRegistry<AchievementCriteriaScript>;
template class SC_GAME_API ScriptRegistry<PlayerScript>;
template class SC_GAME_API ScriptRegistry<GuildScript>;
template class SC_GAME_API ScriptRegistry<GroupScript>;
template class SC_GAME_API ScriptRegistry<UnitScript>;
template class SC_GAME_API ScriptRegistry<AccountScript>;
template class SC_GAME_API ScriptRegistry<AllMapScript>;
template class SC_GAME_API ScriptRegistry<AllCreatureScript>;
