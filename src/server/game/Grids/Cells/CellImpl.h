...
#ifndef SYPHRENA_CELLIMPL_H
#define SYPHRENA_CELLIMPL_H
...
{
    CellCoord p = Syphrena::ComputeCellCoord(x, y);
    data.Part.grid_x = p.x_coord / MAX_NUMBER_OF_CELLS;
    data.Part.grid_y = p.y_coord / MAX_NUMBER_OF_CELLS;
    data.Part.cell_x = p.x_coord % MAX_NUMBER_OF_CELLS;
    data.Part.cell_y = p.y_coord % MAX_NUMBER_OF_CELLS;
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = Syphrena::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = Syphrena::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = Syphrena::ComputeCellCoord(x + radius, y + radius).normalize();
...
{
    CellCoord p(Syphrena::ComputeCellCoord(center_obj->GetPositionX(), center_obj->GetPositionY()));
    Cell cell(p);
    if (dont_load)
        cell.SetNoCreate();
