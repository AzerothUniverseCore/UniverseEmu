...
void CalendarMgr::LoadFromDB()
{
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} calendar events in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    count = 0;
    oldMSTime = getMSTime();
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} calendar invites in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
...
CalendarEvent* CalendarMgr::GetEvent(uint64 eventId) const
{
    ...
    SC_LOG_DEBUG("calendar", "CalendarMgr::GetEvent: [{}] not found!", eventId);
    return nullptr;
}
...
CalendarInvite* CalendarMgr::GetInvite(uint64 inviteId) const
{
    ...
    SC_LOG_DEBUG("calendar", "CalendarMgr::GetInvite: [{}] not found!", inviteId);
    return nullptr;
}
