...
uint32 GroupMgr::GenerateNewGroupDbStoreId()
{
    ...
    {
        SC_LOG_ERROR("misc", "Group storage ID overflow!! Can't continue, shutting down server. ");
...
ObjectGuid::LowType GroupMgr::GenerateGroupId()
{
    ...
    {
        SC_LOG_ERROR("misc", "Group guid overflow!! Can't continue, shutting down server. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
    return NextGroupId++;
}
...
{
    {
        ...
        */
        ...
        if (!result)
        {
            SC_LOG_INFO("server.loading", ">> Loaded 0 group definitions. DB table `groups` is empty!");
            return;
        }
        ...
        SC_LOG_INFO("server.loading", ">> Loaded {} group definitions in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    SC_LOG_INFO("server.loading", "Loading Group members...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            SC_LOG_INFO("server.loading", ">> Loaded 0 group members. DB table `group_member` is empty!");
            return;
        }
        ...
        do
        {
            ...
            if (group)
                ...
                SC_LOG_ERROR("misc", "GroupMgr::LoadGroups: Consistency failed, can't find group (storage id: {})", fields[0].GetUInt32());

            ++count;
        }
        while (result->NextRow());

        SC_LOG_INFO("server.loading", ">> Loaded {} group members in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    SC_LOG_INFO("server.loading", "Loading Group instance saves...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            SC_LOG_INFO("server.loading", ">> Loaded 0 group-instance saves. DB table `group_instance` is empty!");
            return;
        }
        ...
        do
        {
            ...
            {
                SC_LOG_ERROR("sql.sql", "Incorrect entry in group_instance table : no dungeon map {}", fields[1].GetUInt16());
                continue;
            }
            ...
            {
                SC_LOG_ERROR("sql.sql", "Wrong dungeon difficulty use in group_instance table: {}", diff + 1);
        ...
        SC_LOG_INFO("server.loading", ">> Loaded {} group-instance saves in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
}
