...
Group::~Group()
{
    if (m_bgGroup)
    {
        SC_LOG_DEBUG("bg.battleground", "Group::~Group: battleground group being deleted.");
        if (m_bgGroup->GetBgRaid(ALLIANCE) == this)
            ...
            SC_LOG_ERROR("misc", "Group::~Group: battleground group is not linked to the correct battleground.");
    }
    Rolls::iterator itr;
    while (!RollId.empty())
    {
...
bool Group::Create(Player* leader)
{
    ...
    //SC_LOG_ERROR("entities.player", "Group::Create(): new group with leader {}", leader->GetName());
    ...
    if (Map* playerMap = player->FindMap())
        if (!switchLeader && playerMap->IsNonRaidDungeon())
            if (InstanceSave* save = sInstanceSaveMgr->GetInstanceSave(playerMap->GetInstanceId()))
                ...
                {
                    SC_LOG_DEBUG("maps", "Group::ConvertLeaderInstancesToGroup: Group for player {} is taking over unbound instance map {} with Id {}", player->GetName(), playerMap->GetId(), playerMap->GetInstanceId());
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (!item)
        {
            //SC_LOG_DEBUG("misc", "Group::GroupLoot: missing item prototype for item with id: {}", i->itemid);
            continue;
        }
...
{
    SC_LOG_DEBUG("network", "Group::MasterLoot (SMSG_LOOT_MASTER_LIST)");
        ...
        if (isEmpty || method == INSTANCE_RESET_GROUP_DISBAND || method == INSTANCE_RESET_CHANGE_DIFFICULTY)
        {
            ...
            if (isEmpty && instanceSave->CanReset())
            {
                if (map && map->IsDungeon() && SendMsgTo)
                {
                    ...
                    if (!instanceEntrance)
                        SC_LOG_DEBUG("root", "Instance entrance not found for maps {}", map->GetId());
                    else
                    {
...
InstanceGroupBind* Group::BindToInstance(InstanceSave* save, bool permanent, bool load)
{
    ...
    bind.perm = permanent;
    if (!load)
        SC_LOG_DEBUG("maps", "Group::BindToInstance: {}, storage id: {} is now bound to map {}, instance {}, difficulty {}",
...
void Group::BroadcastGroupUpdate(void)
{
    ...
    for (member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        if (pp)
        {
            ...
            SC_LOG_DEBUG("misc", "-- Forced group value update for '{}'", pp->GetName());
        }
    }
}
