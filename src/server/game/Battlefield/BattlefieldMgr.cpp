...
void BattlefieldMgr::InitBattlefield()
{
    ...
    if (QueryResult result = WorldDatabase.Query("SELECT TypeId, ScriptName FROM battlefield_template"))
    {
        do
        {
            ...
            {
                SC_LOG_ERROR("sql.sql", "BattlefieldMgr::InitBattlefield: Invalid TypeId value {} in battlefield_template, skipped.", typeId);
                continue;
            }
            ...
            {
                SC_LOG_INFO("bg.battlefield", "Setting up battlefield with TypeId {} failed.", typeId);
                delete bf;
            }
            else
            {
                _battlefieldSet.push_back(bf);
                SC_LOG_INFO("bg.battlefield", "Setting up battlefield with TypeId {} succeeded.", typeId);
            }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} battlefields in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void BattlefieldMgr::HandlePlayerEnterZone(Player* player, uint32 zoneId)
{
    ...
    SC_LOG_DEBUG("bg.battlefield", "Player {} entered battlefield id {}", player->GetGUID().ToString(), bf->GetTypeId());
}
...
void BattlefieldMgr::HandlePlayerLeaveZone(Player* player, uint32 zoneId)
{
    ...
    SC_LOG_DEBUG("bg.battlefield", "Player {} left battlefield id {}", player->GetGUID().ToString(), itr->second->GetTypeId());
}
