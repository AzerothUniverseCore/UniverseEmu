...
void Battlefield::InitStalker(uint32 entry, Position const& pos)
{
    if (Creature* creature = SpawnCreature(entry, pos))
        ...
        SC_LOG_ERROR("bg.battlefield", "Battlefield::InitStalker: Could not spawn Stalker (Creature entry {}), zone messages will be unavailable!", entry);
}
...
BfGraveyard* Battlefield::GetGraveyardById(uint32 id) const
{
    if (id < m_GraveyardList.size())
    {
        if (BfGraveyard* graveyard = m_GraveyardList.at(id))
            ...
            SC_LOG_ERROR("bg.battlefield", "Battlefield::GetGraveyardById Id:{} does not exist.", id);
    }
    else
        SC_LOG_ERROR("bg.battlefield", "Battlefield::GetGraveyardById Id:{} could not be found.", id);
...
void BfGraveyard::SetSpirit(Creature* spirit, TeamId team)
{
    if (!spirit)
    {
        SC_LOG_ERROR("bg.battlefield", "BfGraveyard::SetSpirit: Invalid Spirit.");
        return;
    }
...
Creature* Battlefield::SpawnCreature(uint32 entry, Position const& pos)
{
    ...
    if (!map)
    {
        SC_LOG_ERROR("bg.battlefield", "Battlefield::SpawnCreature: Can't create creature entry: {}, map not found.", entry);
        return nullptr;
    }
    ...
    {
        SC_LOG_ERROR("bg.battlefield", "Battlefield::SpawnCreature: Can't create creature entry: {}", entry);
        delete creature;
        return nullptr;
    }
...
GameObject* Battlefield::SpawnGameObject(uint32 entry, Position const& pos, QuaternionData const& rot)
{
    ...
    {
        SC_LOG_ERROR("bg.battlefield", "Battlefield::SpawnGameObject: Gameobject template {} could not be found in the database! Battlefield has not been created!", entry);
        SC_LOG_ERROR("bg.battlefield", "Battlefield::SpawnGameObject: Could not create gameobject template {}! Battlefield has not been created!", entry);
        delete go;
        return nullptr;
    }
...
bool BfCapturePoint::SetCapturePointData(GameObject* capturePoint)
{
    ...
    SC_LOG_DEBUG("bg.battlefield", "Creating capture point {}", capturePoint->GetEntry());
    ...
    {
        SC_LOG_ERROR("misc", "OutdoorPvP: GO {} is not a capture point!", capturePoint->GetEntry());
        return false;
    }
...
bool BfCapturePoint::Update(uint32 diff)
{
    ...
    if (GameObject* capturePoint = m_Bf->GetGameObject(m_capturePointGUID))
    {
        ...
        Syphrena::AnyPlayerInObjectRangeCheck checker(capturePoint, radius);
        Syphrena::PlayerListSearcher<Syphrena::AnyPlayerInObjectRangeCheck> searcher(capturePoint, players, checker);
    ...
    {
        //SC_LOG_ERROR("bg.battlefield", "{}->{}", m_OldState, m_State);
        if (oldTeam != m_team)
            ChangeTeam(oldTeam);
        return true;
    }
