...
typedef std::array<std::vector<Syphrena::wregex>, TOTAL_LOCALES> NameValidationRegexContainer;
...
{
    SC_LOG_ERROR("misc", "Size of '{}' set by format string ({}) not equal size of C++ structure ({}).", filename, fsize, rsize);
...
#undef LOAD_DBC_EXT
    ...
    for (NamesProfanityEntry const* namesProfanity : sNamesProfanityStore)
    {
        ...
        if (namesProfanity->Language != -1)
            NamesProfaneValidators[namesProfanity->Language].emplace_back(wname, Syphrena::regex::icase | Syphrena::regex::optimize);
        else
            for (uint32 i = 0; i < TOTAL_LOCALES; ++i)
                NamesProfaneValidators[i].emplace_back(wname, Syphrena::regex::icase | Syphrena::regex::optimize);
    }
    ...
    for (NamesReservedEntry const* namesReserved : sNamesReservedStore)
    {
        ...
        if (namesReserved->Language != -1)
            NamesReservedValidators[namesReserved->Language].emplace_back(wname, Syphrena::regex::icase | Syphrena::regex::optimize);
        else
            for (uint32 i = 0; i < TOTAL_LOCALES; ++i)
                NamesReservedValidators[i].emplace_back(wname, Syphrena::regex::icase | Syphrena::regex::optimize);
    }
        ...
        for (uint8 x = 0; x < MAX_DIFFICULTY; ++x)
        {
            if (spellDiff->DifficultySpellID[x] <= 0 || !sSpellStore.LookupEntry(spellDiff->DifficultySpellID[x]))
            {
                if (spellDiff->DifficultySpellID[x] > 0)//don't show error if spell is <= 0, not all modes have spells and there are unknown negative values
                    SC_LOG_ERROR("sql.sql", "spelldifficulty_dbc: spell {} at field id:{} at spellid{} does not exist in SpellStore (spell.dbc), loaded as 0", spellDiff->DifficultySpellID[x], spellDiff->ID, x);
    ...
    {
        SC_LOG_ERROR("misc", "Incorrect DataDir value in worldserver.conf or ALL required *.dbc files ({}) not found by path: {}dbc", DBCFileCount, dataPath);
        exit(1);
    }
    else if (!bad_dbc_files.empty())
    {
        ...
        SC_LOG_ERROR("misc", "Some required *.dbc files ({} from {}) not found or not compatible:\n{}", (uint32)bad_dbc_files.size(), DBCFileCount, str);
        exit(1);
    }
    ...
    {
        SC_LOG_ERROR("misc", "You have _outdated_ DBC files. Please extract correct versions from current using client.");
        exit(1);
    }

    SC_LOG_INFO("server.loading", ">> Initialized {} data stores in {} ms", DBCFileCount, GetMSTimeDiffToNow(oldMSTime));
...
CharSectionsEntry const* GetCharSectionEntry(uint8 race, CharSectionType genType, uint8 gender, uint8 type, uint8 color)
{
    ...
    for (auto const& section : Syphrena::Containers::MapEqualRange(sCharSectionMap, key))
    {
...
{
    return Syphrena::Containers::MapGetValuePtr(SkillLineAbilitiesBySkill, skillLine);
}
...
ResponseCodes ValidateName(std::wstring const& name, LocaleConstant locale)
{
    ...
    for (Syphrena::wregex const& regex : NamesProfaneValidators[locale])
        if (Syphrena::regex_search(name, regex))
            return CHAR_NAME_PROFANE;

    // regexes at TOTAL_LOCALES are loaded from NamesReserved which is not locale specific
    for (Syphrena::wregex const& regex : NamesReservedValidators[locale])
        if (Syphrena::regex_search(name, regex))
