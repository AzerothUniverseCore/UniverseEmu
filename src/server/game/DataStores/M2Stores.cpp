...
void LoadM2Cameras(std::string const& dataPath)
{
    ...
    SC_LOG_INFO("server.loading", ">> Loading Cinematic Camera files");
    ...
    for (CinematicCameraEntry const* dbcentry : sCinematicCameraStore)
    {
        ...
        {
            SC_LOG_ERROR("server.loading", "Camera file {} is damaged. File is smaller than header size", filename.string());
            m2file.close();
            continue;
        }
        ...
        {
            SC_LOG_ERROR("server.loading", "Camera file {} is damaged. File identifier not found", filename.string());
            m2file.close();
            continue;
        }
        ...
        {
            SC_LOG_ERROR("server.loading", "Camera file {} is damaged. Camera references position beyond file end", filename.string());
            continue;
        }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} cinematic waypoint sets in {} ms", (uint32)sFlyByCameraStore.size(), GetMSTimeDiffToNow(oldMSTime));
}

std::vector<FlyByCamera> const* GetFlyByCameras(uint32 cinematicCameraId)
{
    return Syphrena::Containers::MapGetValuePtr(sFlyByCameraStore, cinematicCameraId);
}
