...
bool WorldSession::CanOpenMailBox(ObjectGuid guid)
{
    if (guid == _player->GetGUID())
    {
        ...
        {
            SC_LOG_WARN("cheat", "{} attempted to open mailbox by using a cheat.", _player->GetName());
            return false;
        }
    }
...
void WorldSession::HandleSendMail(WorldPackets::Mail::SendMail& sendMail)
{
    ...
    {
        SendNotification(GetSyphrenaString(LANG_MAIL_SENDER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    if (!receiverGuid)
    {
        SC_LOG_INFO("network", "Player {} is sending mail to {} (GUID: non-existing!) with subject {} "
            "and body {} includes {} items, {} copper and {} COD copper with StationeryID = %d, PackageID = %d",
    ...
    if (sendMail.Info.SendMoney < 0)
    {
        ...
        SC_LOG_WARN("cheat", "Player {} attempted to send mail to {} ({}) with negative money value (SendMoney: %d)",
    ...
    {
        GetPlayer()->SendMailResult(0, MAIL_SEND, MAIL_ERR_INTERNAL_ERROR);
        SC_LOG_WARN("cheat", "Player {} attempted to send mail to {} ({}) with negative COD value (Cod: %d)",
            GetPlayerInfo(), sendMail.Info.Target, receiverGuid.ToString(), sendMail.Info.Cod);
        return;
    }

    SC_LOG_INFO("network", "Player {} is sending mail to {} ({}) with subject {} and body {} "
        "including {} items, {} copper and {} COD copper with StationeryID = %d, PackageID = %d",
    ...
    auto mailCountCheckContinuation = [this, player = _player, receiverGuid, mailInfo = std::move(sendMail.Info), reqmoney, cost](uint32 receiverTeam, uint64 mailsCount, uint8 receiverLevel, uint32 receiverAccountId) mutable
    {
        ...
        {
            SendNotification(GetSyphrenaString(LANG_MAIL_RECEIVER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
            return;
        }
            ...
            if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
            {
                ...
                else
                {
                    ...
                    if (!sCharacterCache->GetCharacterNameByGuid(sender_guid, sender_name))
                        sender_name = sObjectMgr->GetSyphrenaStringForDBCLocale(LANG_UNKNOWN);
                }
...
void WorldSession::HandleMailCreateTextItem(WorldPackets::Mail::MailCreateTextItem& createTextItem)
{
    ...
    SC_LOG_INFO("network", "HandleMailCreateTextItem mailid={}", createTextItem.MailID);
