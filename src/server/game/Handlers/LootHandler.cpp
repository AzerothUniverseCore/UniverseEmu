...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_AUTOSTORE_LOOT_ITEM");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_LOOT_MONEY");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_LOOT");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_LOOT_RELEASE");
...
void WorldSession::HandleLootMasterGiveOpcode(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("network", "WorldSession::HandleLootMasterGiveOpcode (CMSG_LOOT_MASTER_GIVE, 0x02A3) Target = [{}].", target->GetName());
    ...
    if (!_player->IsInRaidWith(target) || !_player->IsInMap(target))
    {
        ...
        SC_LOG_INFO("entities.player.cheat", "MasterLootItem: Player {} tried to give an item to ineligible player {} !", GetPlayer()->GetName(), target->GetName());
        return;
    }
    ...
    {
        SC_LOG_DEBUG("loot", "MasterLootItem: Player {} might be using a hack! (slot {}, size {})",
