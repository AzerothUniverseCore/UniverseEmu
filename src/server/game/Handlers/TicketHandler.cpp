...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket& recvData)
{
    ...
    {
        SendNotification(GetSyphrenaString(LANG_TICKET_REQ), sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ));
        return;
    }
    ...
    if (!ticket || ticket->IsClosed())
    {
        ...
        if (count && decompressedSize && decompressedSize < 0xFFFF)
        {
            ...
            {
                SC_LOG_ERROR("network", "CMSG_GMTICKET_CREATE possibly corrupt. Uncompression failed.");
                recvData.rfinish();
                return;
            }
