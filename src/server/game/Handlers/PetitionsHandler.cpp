...
{
    SC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_BUY");
    ...
    SC_LOG_DEBUG("network", "Petitioner {} tried sell petition: name {}", guidNPC.ToString(), name);

    // prevent cheating
    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(guidNPC, UNIT_NPC_FLAG_PETITIONER);
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandlePetitionBuyOpcode - {} not found or you can't interact with him.", guidNPC.ToString());
        return;
    }
    ...
    else
    {
        ...
        switch (clientIndex)                                 // arenaSlot+1 as received from client (1 from 3 case)
        {
            ...
            case 3:
                ...
                break;
            default:
                SC_LOG_DEBUG("network", "unknown selection at buy arena petition: {}", clientIndex);
                return;
        }
    ...
    if (Petition const* petition = sPetitionMgr->GetPetitionByOwnerWithType(_player->GetGUID(), type))
    {
        ...
        SC_LOG_DEBUG("network", "Invalid petition {}", petition->PetitionGuid.ToString());
    }
...
{
    SC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SHOW_SIGNATURES");
    ...
    if (!petition)
    {
        SC_LOG_DEBUG("entities.player.items", "Petition {} is not found for player {} {}", petitionGuid.ToString(), GetPlayer()->GetGUID().ToString(), GetPlayer()->GetName());
        return;
    }
    ...
    SC_LOG_DEBUG("network", "CMSG_PETITION_SHOW_SIGNATURES petition {}", petitionGuid.ToString());
...
{
    SC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_QUERY");   // ok

    ObjectGuid::LowType guildguid;
    ObjectGuid petitionguid;
    recvData >> guildguid;                                 // in Syphrena always same as GUID_LOPART(petitionguid)
    recvData >> petitionguid;                              // petition guid
    SC_LOG_DEBUG("network", "CMSG_PETITION_QUERY Petition {} Guild GUID {}", petitionguid.ToString(), guildguid);
...
void WorldSession::SendPetitionQueryOpcode(ObjectGuid petitionguid)
{
    ...
    if (!petition)
    {
        SC_LOG_DEBUG("network", "CMSG_PETITION_QUERY failed for petition ({})", petitionguid.ToString());
        return;
    }

    WorldPacket data(SMSG_PETITION_QUERY_RESPONSE, (4+8+petition->PetitionName.size()+1+1+4*12+2+10));
    data << uint32(petitionguid.GetCounter());              // guild/team guid (in Syphrena always same as GUID_LOPART(petition guid)
...
{
    SC_LOG_DEBUG("network", "Received opcode MSG_PETITION_RENAME");   // ok
    ...
    SC_LOG_DEBUG("network", "Petition {} renamed to '{}'", petitionGuid.ToString(), newName);
...
{
    SC_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SIGN");    // ok
    ...
    if (!petition)
    {
        SC_LOG_ERROR("network", "Petition {} is not found for player {} {}", petitionGuid.ToString(), GetPlayer()->GetGUID().ToString(), GetPlayer()->GetName());
        return;
    }
    ...
    SC_LOG_DEBUG("network", "PETITION SIGN: {} by player: {} ({} Account: {})", petitionGuid.ToString(), _player->GetName(), playerGuid.ToString(), GetAccountId());
...
{
    SC_LOG_DEBUG("network", "Received opcode MSG_PETITION_DECLINE");  // ok

    ObjectGuid petitionguid;
    recvData >> petitionguid;                              // petition guid
    SC_LOG_DEBUG("network", "Petition {} declined by {}", petitionguid.ToString(), _player->GetGUID().ToString());
...
{
    SC_LOG_DEBUG("network", "Received opcode CMSG_OFFER_PETITION");   // ok
    ...
    SC_LOG_DEBUG("network", "OFFER PETITION: type {}, {}, to {}", static_cast<uint32>(type), petitionGuid.ToString(), offererGuid.ToString());
...
{
    SC_LOG_DEBUG("network", "Received opcode CMSG_TURN_IN_PETITION");
    ...
    SC_LOG_DEBUG("network", "Petition {} turned in by {}", petitionGuid.ToString(), _player->GetGUID().ToString());

    Petition const* petition = sPetitionMgr->GetPetition(petitionGuid);
    if (!petition)
    {
        SC_LOG_ERROR("entities.player.cheat", "Player {} {} tried to turn in petition ({}) that is not present in the database", _player->GetName(), _player->GetGUID().ToString(), petitionGuid.ToString());
        return;
    }
    ...
    else
    {
        ...
        SC_LOG_DEBUG("network", "PetitonsHandler: Arena team (guid: {}) added to ObjectMgr", arenaTeam->GetId());

        // Add members
        for (Signature const& signature : signatures)
        {
            SC_LOG_DEBUG("network", "PetitionsHandler: Adding arena team (guid: {}) member {}", arenaTeam->GetId(), signature.second.ToString());
    ...
    // created
    SC_LOG_DEBUG("network", "Player {} ({}) turning in petition {}", _player->GetName(), _player->GetGUID().ToString(), petitionGuid.ToString());
...
{
    SC_LOG_DEBUG("network", "Received CMSG_PETITION_SHOWLIST");
...
void WorldSession::SendPetitionShowList(ObjectGuid guid)
{
    ...
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandlePetitionShowListOpcode - {} not found or you can't interact with him.", guid.ToString());
        return;
    }
    ...
    SendPacket(&data);
    SC_LOG_DEBUG("network", "Sent SMSG_PETITION_SHOWLIST");
}
