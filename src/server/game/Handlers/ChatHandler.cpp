...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    {
        SC_LOG_ERROR("network", "CHAT: Wrong message type received: {}", type);
        recvData.rfinish();
        return;
    }
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "CMSG_MESSAGECHAT: Possible hacking-attempt: {} tried to send a message in universal language", GetPlayerInfo());
    ...
    //SC_LOG_DEBUG("CHAT: packet received. type {}, lang {}", type, lang);
    ...
    if (lang == LANG_ADDON)
    {
        ...
        switch (type)
        {
            ...
            case CHAT_MSG_WHISPER:
                ...
                break;
            default:
                SC_LOG_ERROR("network", "Player {}{} sent a chatmessage with an invalid language/message type combination",
    ...
    if (!CanSpeak())
    {
        ...
        SendNotification(GetSyphrenaString(LANG_WAIT_BEFORE_SPEAKING), timeStr);
    ...
    {
        SendNotification(GetSyphrenaString(LANG_GM_SILENCE), sender->GetName().c_str());
        recvData.rfinish();
        return;
    }
    ...
    if (lang != LANG_ADDON)
    {
        ...
        for (uint8 c : msg)
            if (isNasty(c))
            {
                SC_LOG_ERROR("network", "Player {} {} sent a message containing invalid character {} - blocked", GetPlayer()->GetName(),
    ...
    switch (type)
    {
        case CHAT_MSG_SAY:
        {
            ...
            {
                SendNotification(GetSyphrenaString(LANG_SAY_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_EMOTE:
        {
            ...
            {
                SendNotification(GetSyphrenaString(LANG_SAY_REQ), sWorld->getIntConfig(CONFIG_CHAT_EMOTE_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_YELL:
        {
            ...
            {
                SendNotification(GetSyphrenaString(LANG_SAY_REQ), sWorld->getIntConfig(CONFIG_CHAT_YELL_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            if (!receiver->IsInWhisperWhiteList(sender->GetGUID()) && !receiver->IsGameMasterAcceptingWhispers())
            {
                ...
                {
                    SendNotification(GetSyphrenaString(LANG_WHISPER_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
                    return;
                }
            ...
            {
                SendNotification(GetSyphrenaString(LANG_GM_SILENCE), GetPlayer()->GetName().c_str());
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            if (!HasPermission(rbac::RBAC_PERM_SKIP_CHECK_CHAT_CHANNEL_REQ))
            {
                ...
                {
                    SendNotification(GetSyphrenaString(LANG_CHANNEL_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                    return;
                }
            }
        ...
        case CHAT_MSG_AFK:
        {
            if (!sender->IsInCombat())
            {
                ...
                {
                    sender->autoReplyMsg = msg.empty() ? GetSyphrenaString(LANG_PLAYER_AFK_DEFAULT) : msg;
        ...
        case CHAT_MSG_DND:
        {
            ...
            {
                sender->autoReplyMsg = msg.empty() ? GetSyphrenaString(LANG_PLAYER_DND_DEFAULT) : msg;
        ...
        default:
            SC_LOG_ERROR("network", "CHAT: unknown message type {}, lang: {}", type, lang);
            break;
    }
}
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket& recvData)
{
    ...
    {
        std::string timeStr = secsToTimeString(m_muteTime - GameTime::GetGameTime());
        SendNotification(GetSyphrenaString(LANG_WAIT_BEFORE_SPEAKING), timeStr.c_str());
        return;
    }
    ...
    CellCoord p = Syphrena::ComputeCellCoord(GetPlayer()->GetPositionX(), GetPlayer()->GetPositionY());

    Cell cell(p);
    cell.SetNoCreate();

    Syphrena::EmoteChatBuilder emote_builder(*GetPlayer(), text_emote, emoteNum, unit);
    Syphrena::LocalizedPacketDo<Syphrena::EmoteChatBuilder > emote_do(emote_builder);
    Syphrena::PlayerDistWorker<Syphrena::LocalizedPacketDo<Syphrena::EmoteChatBuilder > > emote_worker(GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), emote_do);
    TypeContainerVisitor<Syphrena::PlayerDistWorker<Syphrena::LocalizedPacketDo<Syphrena::EmoteChatBuilder> >, WorldTypeMapContainer> message(emote_worker);
...
void WorldSession::HandleChatIgnoredOpcode(WorldPacket& recvData)
{
    ObjectGuid iguid;
    uint8 unk;
    //SC_LOG_DEBUG("network", "WORLD: Received CMSG_CHAT_IGNORED");
...
{
    SC_LOG_DEBUG("network", "Opcode {}", recvPacket.GetOpcode());
}
