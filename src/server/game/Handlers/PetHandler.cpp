...
void WorldSession::HandleDismissCritter(WorldPackets::Pet::DismissCritter& packet)
{
    ...
    if (!pet)
    {
        SC_LOG_DEBUG("entities.pet", "Vanitypet ({}) does not exist - player '{}' ({} / account: {}) attempted to dismiss it (possibly lagged out)",
...
void WorldSession::HandlePetAction(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("entities.pet", "HandlePetAction: {} - flag: {}, spellid: {}, target: {}.", guid1.ToString(), uint32(flag), spellid, guid2.ToString());

    if (!pet)
    {
        SC_LOG_DEBUG("entities.pet", "HandlePetAction: {} doesn't exist for {} {}", guid1.ToString(), GetPlayer()->GetGUID().ToString(), GetPlayer()->GetName());
        return;
    }

    if (pet != GetPlayer()->GetFirstControlled())
    {
        SC_LOG_DEBUG("entities.pet", "HandlePetAction: {} does not belong to {} {}", guid1.ToString(), GetPlayer()->GetGUID().ToString(), GetPlayer()->GetName());
        return;
    }
...
void WorldSession::HandlePetStopAttack(WorldPackets::Pet::PetStopAttack& packet)
{
    ...
    if (!pet)
    {
        SC_LOG_ERROR("entities.pet", "HandlePetStopAttack: {} does not exist", packet.PetGUID.ToString());
        return;
    }

    if (pet != GetPlayer()->GetPet() && pet != GetPlayer()->GetCharmed())
    {
        SC_LOG_ERROR("entities.pet", "HandlePetStopAttack: {} isn't a pet or charmed creature of player {}",
...
void WorldSession::HandlePetActionHelper(Unit* pet, ObjectGuid guid1, uint32 spellid, uint16 flag, ObjectGuid guid2)
{
    ...
    if (!charmInfo)
    {
        SC_LOG_DEBUG("entities.pet", "WorldSession::HandlePetAction(petGuid: {}, tagGuid: {}, spellId: {}, flag: {}): object {} is considered pet-like but doesn't have a charminfo!",
    ...
    switch (flag)
    {
        case ACT_COMMAND: // 0x07
            switch (spellid)
            {
                ...
                case COMMAND_ABANDON: // abandon (hunter pet) or dismiss (summoned pet)
                    ...
                    break;
                default:
                    SC_LOG_ERROR("entities.pet", "WORLD: unknown PET flag Action {} and spellid {}.", uint32(flag), spellid);
            }
            break;
        ...
        case ACT_ENABLED: // 0xC1 spell
        {
            ...
            if (!spellInfo)
            {
                SC_LOG_ERROR("spells.pet", "WORLD: unknown PET spell id {}", spellid);
                return;
            }
...
{
    SC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_NAME_QUERY");
...
bool WorldSession::CheckStableMaster(ObjectGuid guid)
{
    ...
    if (guid == GetPlayer()->GetGUID())
    {
        ...
        {
            SC_LOG_DEBUG("entities.player.cheat", "{} attempt open stable in cheating way.", guid.ToString());
            return false;
        }
    }
    // stable master case
    else
    {
        ...
        {
            SC_LOG_DEBUG("entities.player", "Stablemaster {} not found or you can't interact with him.", guid.ToString());
            return false;
        }
    }
    return true;
...
{
    SC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_SET_ACTION");
    ...
    {
        SC_LOG_ERROR("entities.pet", "HandlePetSetAction: Unknown {} or owner ({})", petguid.ToString(), _player->GetGUID().ToString());
        return;
    }
    ...
    if (!charmInfo)
    {
        SC_LOG_ERROR("entities.pet", "WorldSession::HandlePetSetAction: object {} is considered pet-like but doesn't have a charminfo!", pet->GetGUID().ToString());
        return;
    }
    ...
    for (Unit* petControlled : pets)
    {
        ...
        for (uint8 i = 0; i < count; ++i)
        {
            ...
            SC_LOG_DEBUG("entities.pet", "Player {} has changed pet spell action. Position: {}, Spell: {}, State: 0x{:X}",
...
{
    SC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_RENAME");
...
void WorldSession::HandlePetSpellAutocastOpcode(WorldPackets::Pet::PetSpellAutocast& packet)
{
    ...
    if (!pet)
    {
        SC_LOG_ERROR("entities.pet", "WorldSession::HandlePetSpellAutocastOpcode: Pet {} not found.", packet.PetGUID.ToString());
        return;
    }

    if (pet != _player->GetGuardianPet() && pet != _player->GetCharmed())
    {
        SC_LOG_ERROR("entities.pet", "WorldSession::HandlePetSpellAutocastOpcode: {} isn't pet of player {} ({}).",
    ...
    if (!spellInfo)
    {
        SC_LOG_ERROR("spells.pet", "WorldSession::HandlePetSpellAutocastOpcode: Unknown spell id {} used by {}.", packet.SpellID, packet.PetGUID.ToString());
        return;
    }
    ...
    for (Unit* petControlled : pets)
    {
        ...
        if (!charmInfo)
        {
            SC_LOG_ERROR("entities.pet", "WorldSession::HandlePetSpellAutocastOpcode: object {} is considered pet-like but doesn't have a charminfo!", petControlled->GetGUID().ToString());
            return;
        }
...
{
    SC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_CAST_SPELL");
    ...
    SC_LOG_DEBUG("entities.pet", "WORLD: CMSG_PET_CAST_SPELL, {}, castCount: {}, spellId {}, castFlags {}", guid.ToString(), castCount, spellId, castFlags);
    ...
    {
        SC_LOG_ERROR("entities.pet", "HandlePetCastSpellOpcode: {} isn't pet of player {} ({}).", guid.ToString(), GetPlayer()->GetName(), GetPlayer()->GetGUID().ToString());
        return;
    }
...
{
    SC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_PET_LEARN_TALENT");
...
{
    SC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_LEARN_PREVIEW_TALENTS_PET");
