...
{
    SC_LOG_DEBUG("network", "WORLD: got MSG_MOVE_WORLDPORT_ACK.");
...
void WorldSession::HandleMoveWorldportAck()
{
    ...
    if (player->IsInWorld())
    {
        SC_LOG_ERROR("network", "{} {} is still in world when teleported from map {} ({}) to new map {} ({})", player->GetGUID().ToString(), player->GetName(), oldMap->GetMapName(), oldMap->GetId(), newMap ? newMap->GetMapName() : "Unknown", loc.GetMapId());
    ...
    {
        SC_LOG_ERROR("network", "Map {} ({}) could not be created for player {} ({}), porting player to homebind", loc.GetMapId(), newMap ? newMap->GetMapName() : "Unknown", player->GetGUID().ToString(), player->GetName());
    ...
    {
        SC_LOG_ERROR("network", "WORLD: failed to teleport player {} {} to map {} ({}) because of unknown reason!",
...
{
    SC_LOG_DEBUG("network", "MSG_MOVE_TELEPORT_ACK");
    ObjectGuid guid;
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvData)
{
    ...
    if (movementInfo.HasMovementFlag(MOVEMENTFLAG_ONTRANSPORT))
    {
        ...
        if (!Syphrena::IsValidMapCoord(movementInfo.pos.GetPositionX() + movementInfo.transport.pos.GetPositionX(), movementInfo.pos.GetPositionY() + movementInfo.transport.pos.GetPositionY(),
...
void WorldSession::HandleForceSpeedChangeAck(WorldPacket &recvData)
{
    ...
    {
        recvData.rfinish();                     // prevent warnings spam
        SC_LOG_DEBUG("entities.unit", "Ignoring ACK. Bad or outdated movement data by Player {}", _player->GetName());
        return;
    }
    ...
    switch (recvData.GetOpcode())
    {
        ...
        default:
            SC_LOG_ERROR("network", "WorldSession::HandleForceSpeedChangeAck: Unknown move type opcode: {}", GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvData.GetOpcode())));
            return;
    }
    ...
    switch (changeType)
    {
        ...
        default:
            SC_LOG_INFO("cheat", "WorldSession::HandleForceSpeedChangeAck: Player {} from account id {} kicked for incorrect data returned in an ack. movementChangeType: {}",
    ...
    {
        SC_LOG_INFO("cheat", "WorldSession::HandleForceSpeedChangeAck: Player {} from account id {} kicked for incorrect data returned in an ack. pendingChange.movementCounter: {}, movementCounter: {}",
    ...
    {
        SC_LOG_INFO("cheat", "WorldSession::HandleForceSpeedChangeAck: Player {} from account id {} kicked for incorrect data returned in an ack. speedSent - speedReceived: {}",
    ...
    {
        SC_LOG_INFO("cheat", "WorldSession::HandleForceSpeedChangeAck: Player {} from account id {} kicked for incorrect data returned in an ack. moveTypeSent: {}, move_type: {}",
...
{
    SC_LOG_DEBUG("network", "WORLD: Recvd CMSG_SET_ACTIVE_MOVER");
    ...
    if (!client->IsAllowedToMove(guid))
    {
        ...
        SC_LOG_DEBUG("entities.unit", "set active mover FAILED for client of player {}. GUID {}.", _player->GetName(), guid.ToString());
        return;
    }

    // step 2:
    SC_LOG_DEBUG("entities.unit", "set active mover OK for client of player {}. GUID {}.", _player->GetName(), guid.ToString());
...
{
    SC_LOG_DEBUG("network", "WORLD: Recvd CMSG_MOVE_NOT_ACTIVE_MOVER");
    ...
    {
        SC_LOG_DEBUG("entities.unit", "unset active mover FAILED for client of player {}. GUID {}.", _player->GetName(), old_mover_guid.ToString());
        return;
    }

    SC_LOG_DEBUG("entities.unit", "unset active mover OK for client of player {}. GUID {}.", _player->GetName(), old_mover_guid.ToString());
...
{
    SC_LOG_DEBUG("network", "CMSG_MOVE_KNOCK_BACK_ACK");
...
{
    SC_LOG_DEBUG("network", "CMSG_MOVE_HOVER_ACK");
...
{
    SC_LOG_DEBUG("network", "CMSG_MOVE_WATER_WALK_ACK");
...
{
    SC_LOG_DEBUG("network", "CMSG_FORCE_MOVE_ROOT_ACK");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_MOVE_FEATHER_FALL_ACK");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_FORCE_MOVE_UNROOT_ACK");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_MOVE_SET_CAN_FLY_ACK");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_MOVE_SET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY_ACK");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_MOVE_GRAVITY_DISABLE_ACK");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_MOVE_GRAVITY_ENABLE_ACK");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_MOVE_SET_COLLISION_HGT_ACK");
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_MOVE_TIME_SKIPPED");
...
{
    SC_LOG_DEBUG("network", "CMSG_TIME_SYNC_RESP");
