...
{
    SC_LOG_DEBUG("network", "WORLD: Recvd CMSG_REPOP_REQUEST Message");
    ...
    {
        SC_LOG_DEBUG("network", "HandleRepopRequestOpcode: got request after player {} {} was killed and before he was updated",
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_GOSSIP_SELECT_OPTION");
    ...
    if (guid.IsCreatureOrVehicle())
    {
        ...
        if (!unit)
        {
            SC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - {} not found or you can't interact with him.", guid.ToString());
            return;
        }
    }
    else if (guid.IsGameObject())
    {
        ...
        if (!go)
        {
            SC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - {} not found or you can't interact with it.", guid.ToString());
            return;
        }
    }
    else if (guid.IsItem())
    {
        ...
        if (!item)
        {
            SC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - {} not found.", guid.ToString());
            return;
        }
    }
    ...
    {
        SC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - unsupported {}.", guid.ToString());
        return;
    }
    ...
    {
        SC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - Script reloaded while in use, ignoring and set new scipt id");
        if (unit)
...
{
    SC_LOG_DEBUG("network", "WORLD: Recvd CMSG_WHO Message");
    ...
    for (uint32 i = 0; i < zonesCount; ++i)
    {
        ...
        zoneids[i] = temp;
        SC_LOG_DEBUG("network", "Zone {}: {}", i, zoneids[i]);
    }
    ...
    SC_LOG_DEBUG("network", "Minlvl {}, maxlvl {}, name {}, guild {}, racemask {}, classmask {}, zones {}, strings {}", levelMin, levelMax, packetPlayerName, packetGuildName, racemask, classmask, zonesCount, strCount);
    ...
    for (uint32 i = 0; i < strCount; ++i)
    {
        ...
        SC_LOG_DEBUG("network", "String {}: {}", i, temp);
    }
    ...
    // client send in case not set max level value 100 but Syphrena supports 255 max level,
    ...
    if (sWorld->getBoolConfig(CONFIG_WHO_BOTS_ENABLE) && matchCount < 49)
    {
        QueryResult result = CharacterDatabase.Query("SELECT guid, name, race, class, level, zone, gender FROM characters_bots WHERE online > 0");
        if (result)
        {
            do
            {
    ...
    SendPacket(&data);
    SC_LOG_DEBUG("network", "WORLD: Send SMSG_WHO Message");
}
...
void WorldSession::HandleZoneUpdateOpcode(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("network", "WORLD: Recvd ZONE_UPDATE: {}", newZone);
...
void WorldSession::HandleBugOpcode(WorldPacket& recvData)
{
    ...
    if (suggestion == 0)
        SC_LOG_DEBUG("network", "WORLD: Received CMSG_BUG [Bug Report]");
    else
        SC_LOG_DEBUG("network", "WORLD: Received CMSG_BUG [Suggestion]");

    SC_LOG_DEBUG("network", "{}", type);
    SC_LOG_DEBUG("network", "{}", content);
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_RECLAIM_CORPSE");
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_RESURRECT_RESPONSE");
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("network", "CMSG_AREATRIGGER. Trigger ID: {}", triggerId);

    Player* player = GetPlayer();
    if (player->IsInFlight())
    {
        SC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '{}' {} in flight, ignore Area Trigger ID:{}",
    ...
    if (!atEntry)
    {
        SC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '{}' {} send unknown (by DBC) Area Trigger ID:{}",
            player->GetName(), player->GetGUID().ToString(), triggerId);
        return;
    }
    ...
    {
        SC_LOG_DEBUG("network", "HandleAreaTriggerOpcode: Player '{}' {} too far, ignore Area Trigger ID: {}",
            player->GetName(), player->GetGUID().ToString(), triggerId);
        return;
    }
        ...
        if (Map::EnterState denyReason = sMapMgr->PlayerCannotEnter(at->target_mapId, player, false))
        {
            ...
            switch (denyReason)
            {
                case Map::CANNOT_ENTER_NO_ENTRY:
                    SC_LOG_DEBUG("maps", "MAP: Player '{}' attempted to enter map with id {} which has no entry", player->GetName(), at->target_mapId);
                    break;
                case Map::CANNOT_ENTER_UNINSTANCED_DUNGEON:
                    SC_LOG_DEBUG("maps", "MAP: Player '{}' attempted to enter dungeon map {} but no instance template was found", player->GetName(), at->target_mapId);
                    break;
                case Map::CANNOT_ENTER_DIFFICULTY_UNAVAILABLE:
                    SC_LOG_DEBUG("maps", "MAP: Player '{}' attempted to enter instance map {} but the requested difficulty was not found", player->GetName(), at->target_mapId);
                ...
                case Map::CANNOT_ENTER_NOT_IN_RAID:
                {
                    ...
                    SC_LOG_DEBUG("maps", "MAP: Player '{}' must be in a raid group to enter instance map {}", player->GetName(), at->target_mapId);
                ...
                case Map::CANNOT_ENTER_CORPSE_IN_DIFFERENT_INSTANCE:
                {
                    ...
                    player->SendDirectMessage(&data);
                    SC_LOG_DEBUG("maps", "MAP: Player '{}' does not have a corpse in instance map {} and cannot enter", player->GetName(), at->target_mapId);
                    break;
                }
                case Map::CANNOT_ENTER_INSTANCE_BIND_MISMATCH:
                    if (MapEntry const* entry = sMapStore.LookupEntry(at->target_mapId))
                    {
                        ...
                        SC_LOG_DEBUG("maps", "MAP: Player '{}' cannot enter instance map '{}' because their permanent bind is incompatible with their group's", player->GetName(), mapName);
                        // is there a special opcode for this?
                        // @todo figure out how to get player localized difficulty string (e.g. "10 player", "Heroic" etc)
                        ChatHandler(player->GetSession()).PSendSysMessage(player->GetSession()->GetSyphrenaString(LANG_INSTANCE_BIND_MISMATCH), mapName);
                    }
                    reviveAtTrigger = true;
                    break;
                case Map::CANNOT_ENTER_TOO_MANY_INSTANCES:
                    player->SendTransferAborted(at->target_mapId, TRANSFER_ABORT_TOO_MANY_INSTANCES);
                    SC_LOG_DEBUG("maps", "MAP: Player '{}' cannot enter instance map {} because he has exceeded the maximum number of instances per hour.", player->GetName(), at->target_mapId);
                    reviveAtTrigger = true;
                    break;
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_UPDATE_ACCOUNT_DATA");

    uint32 type, timestamp, decompressedSize;
    recvData >> type >> timestamp >> decompressedSize;

    SC_LOG_DEBUG("network", "UAD: type {}, time {}, decompressedSize {}", type, timestamp, decompressedSize);
    ...
    if (decompressedSize > 0xFFFF)
    {
        ...
        SC_LOG_ERROR("network", "UAD: Account data packet too big, size {}", decompressedSize);
        return;
    }
    ...
    {
        recvData.rfinish();                   // unnneded warning spam in this case
        SC_LOG_ERROR("network", "UAD: Failed to decompress account data");
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_REQUEST_ACCOUNT_DATA");

    uint32 type;
    recvData >> type;

    SC_LOG_DEBUG("network", "RAD: type {}", type);
    ...
    {
        SC_LOG_DEBUG("network", "RAD: Failed to compress account data");
        return;
    }
...
void WorldSession::HandleSetActionButtonOpcode(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("network", "CMSG_SET_ACTION_BUTTON Button: {} Data: {}", button, packetData);
...
void WorldSession::HandleSetActionBarToggles(WorldPacket& recvData)
{
    ...
    {
        if (actionBar != 0)
            SC_LOG_ERROR("network", "WorldSession::HandleSetActionBarToggles in not logged state with value: {}, ignored", uint32(actionBar));
        return;
    }
...
void WorldSession::HandleInspectOpcode(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_INSPECT");

    Player* player = ObjectAccessor::GetPlayer(*_player, guid);
    if (!player)
    {
        SC_LOG_DEBUG("network", "CMSG_INSPECT: No player found from {}", guid.ToString());
        return;
    }
...
void WorldSession::HandleInspectHonorStatsOpcode(WorldPacket& recvData)
{
    ...
    if (!player)
    {
        SC_LOG_DEBUG("network", "MSG_INSPECT_HONOR_STATS: No player found from {}", guid.ToString());
        return;
    }
...
void WorldSession::HandleWorldTeleportOpcode(WorldPackets::Misc::WorldTeleport& worldTeleport)
{
    ...
    {
        SC_LOG_DEBUG("network", "Player '{}' ({}) in flight, ignore worldport command.",
    ...
    SC_LOG_DEBUG("network", "CMSG_WORLD_TELEPORT: Player = {}, time = {}, map = {}, pos = {}",
...
{
    SC_LOG_DEBUG("network", "Received opcode CMSG_WHOIS");
    std::string charname;
    recvData >> charname;
    ...
    SC_LOG_DEBUG("network", "Received whois command from player {} for character {}",
        GetPlayer()->GetName(), charname);
}

void WorldSession::HandleComplainOpcode(WorldPacket& recvData)
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_COMPLAIN");
    ...
    SC_LOG_DEBUG("network", "REPORT SPAM: type {}, {}, unk1 {}, unk2 {}, unk3 {}, unk4 {}, message {}",
        spam_type, spammer_guid.ToString(), unk1, unk2, unk3, unk4, description);
}

void WorldSession::HandleRealmSplitOpcode(WorldPacket& recvData)
{
    SC_LOG_DEBUG("network", "CMSG_REALM_SPLIT");
    ...
    data << split_date;
    SendPacket(&data);
    //SC_LOG_DEBUG("response sent {}", unk);
}

void WorldSession::HandleFarSightOpcode(WorldPacket& recvData)
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_FAR_SIGHT");
    ...
    if (apply)
    {
        SC_LOG_DEBUG("network", "Added FarSight {} to player {}", _player->GetGuidValue(PLAYER_FARSIGHT).ToString(), _player->GetGUID().ToString());
        if (WorldObject* target = _player->GetViewpoint())
            _player->SetSeer(target);
        else
            SC_LOG_DEBUG("network", "Player {} {} requests non-existing seer {}", _player->GetName(), _player->GetGUID().ToString(), _player->GetGuidValue(PLAYER_FARSIGHT).ToString());
    }
    else
    {
        SC_LOG_DEBUG("network", "Player {} set vision to self", _player->GetGUID().ToString());
...
{
    SC_LOG_DEBUG("network", "CMSG_SET_TITLE");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_RESET_INSTANCES");
...
{
    SC_LOG_DEBUG("network", "MSG_SET_DUNGEON_DIFFICULTY");
    ...
    {
        SC_LOG_DEBUG("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player {} sent an invalid instance mode {}!", _player->GetGUID().ToString(), mode);
        return;
    }
    ...
    {
        SC_LOG_DEBUG("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player (Name: {}, {}) tried to reset the instance while player is inside!",
            _player->GetName(), _player->GetGUID().ToString());
        return;
    }
    ...
    if (group)
    {
        if (group->IsLeader(_player->GetGUID()))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                ...
                {
                    SC_LOG_DEBUG("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player {} tried to reset the instance while group member (Name: {}, {}) is inside!",
...
{
    SC_LOG_DEBUG("network", "MSG_SET_RAID_DIFFICULTY");
    ...
    {
        SC_LOG_ERROR("network", "WorldSession::HandleSetRaidDifficultyOpcode: player {} sent an invalid instance mode {}!", _player->GetGUID().ToString(), mode);
        return;
    }
    ...
    if (map && map->IsDungeon())
    {
        SC_LOG_DEBUG("network", "WorldSession::HandleSetRaidDifficultyOpcode: player {} tried to reset the instance while inside!", _player->GetGUID().ToString());
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_SET_TAXI_BENCHMARK_MODE");
    ...
    SC_LOG_DEBUG("network", "Client used \"/timetest {}\" command", mode);
}
...
void WorldSession::HandleQueryInspectAchievements(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("network", "CMSG_QUERY_INSPECT_ACHIEVEMENTS [{}] Inspected Player [{}]", _player->GetGUID().ToString(), guid.ToString());
    Player* player = ObjectAccessor::GetPlayer(*_player, guid);
    if (!player)
        return;
...
{
    // empty opcode
    SC_LOG_DEBUG("network", "WORLD: CMSG_WORLD_STATE_UI_TIMER_UPDATE");
...
{
    // empty opcode
    SC_LOG_DEBUG("network", "WORLD: CMSG_READY_FOR_ACCOUNT_DATA_TIMES");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUERY");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUEUE");
...
void WorldSession::HandleInstanceLockResponse(WorldPacket& recvPacket)
{
    ...
    {
        SC_LOG_INFO("network", "InstanceLockResponse: Player {} {} tried to bind himself/teleport to graveyard without a pending bind!",
            _player->GetName(), _player->GetGUID().ToString());
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_UPDATE_MISSILE_TRAJECTORY");
