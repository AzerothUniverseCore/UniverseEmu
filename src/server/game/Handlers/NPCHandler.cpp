...
void WorldSession::HandleTabardVendorActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleTabardVendorActivateOpcode - {} not found or you can not interact with him.", guid.ToString());
        return;
    }
...
void WorldSession::HandleTrainerListOpcode(WorldPackets::NPC::Hello& packet)
{
    ...
    if (!npc)
    {
        SC_LOG_DEBUG("network", "WorldSession: SendTrainerList - {} not found or you can not interact with him.", packet.Unit.ToString());
        return;
    }
...
void WorldSession::SendTrainerList(Creature* npc, uint32 trainerEntry)
{
    ...
    if (!trainer)
    {
        SC_LOG_DEBUG("network", "WorldSession: SendTrainerList - trainer spells not found for {}", npc->GetGUID().ToString());
        return;
    }

    if (!trainer->IsTrainerValidForPlayer(_player))
    {
        SC_LOG_DEBUG("network", "WorldSession: SendTrainerList - trainer {} not valid for player {}", npc->GetGUID().ToString(), GetPlayerInfo());
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_TRAINER_BUY_SPELL {}, learn spell id is: {}", packet.TrainerGUID.ToString(), packet.SpellID);

    Creature* npc = GetPlayer()->GetNPCIfCanInteractWith(packet.TrainerGUID, UNIT_NPC_FLAG_TRAINER);
    if (!npc)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleTrainerBuySpellOpcode - {} not found or you can not interact with him.", packet.TrainerGUID.ToString());
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_GOSSIP_HELLO");
    ...
    if (!unit)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleGossipHelloOpcode - {} not found or you can not interact with him.", guid.ToString());
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_SPIRIT_HEALER_ACTIVATE");
    ...
    if (!unit)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleSpiritHealerActivateOpcode - {} not found or you can not interact with him.", guid.ToString());
        return;
    }
...
void WorldSession::HandleBinderActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleBinderActivateOpcode - {} not found or you can not interact with him.", npcGUID.ToString());
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: Recv MSG_LIST_STABLED_PETS");
    ObjectGuid npcGUID;
...
{
    SC_LOG_DEBUG("network", "WORLD: Recv MSG_LIST_STABLED_PETS Send.");
...
{
    SC_LOG_DEBUG("network", "WORLD: Recv CMSG_STABLE_PET");
    ObjectGuid npcGUID;
...
{
    SC_LOG_DEBUG("network", "WORLD: Recv CMSG_UNSTABLE_PET.");
    ObjectGuid npcGUID;
    uint32 petnumber;
...
{
    SC_LOG_DEBUG("network", "WORLD: Recv CMSG_BUY_STABLE_SLOT.");
    ObjectGuid npcGUID;
...
{
    SC_LOG_DEBUG("network", "HandleStableRevivePet: Not implemented");
}

void WorldSession::HandleStableSwapPet(WorldPacket& recvData)
{
    SC_LOG_DEBUG("network", "WORLD: Recv CMSG_STABLE_SWAP_PET.");
    ObjectGuid npcGUID;
    uint32 petId;
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_REPAIR_ITEM");
    ...
    if (!unit)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleRepairItemOpcode - {} not found or you can not interact with him.", npcGUID.ToString());
        return;
    }
    ...
    if (itemGUID)
    {
        SC_LOG_DEBUG("network", "ITEM: Repair {}, at {}", itemGUID.ToString(), npcGUID.ToString());
    ...
    {
        SC_LOG_DEBUG("network", "ITEM: Repair all items at {}", npcGUID.ToString());
