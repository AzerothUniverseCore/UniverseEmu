...
{
    SC_LOG_DEBUG("network", "CMSG_LEARN_PREVIEW_TALENTS");
...
{
    SC_LOG_DEBUG("network", "MSG_TALENT_WIPE_CONFIRM");

    Creature* trainer = GetPlayer()->GetNPCIfCanInteractWith(confirmRespecWipe.RespecMaster, UNIT_NPC_FLAG_TRAINER);
    if (!trainer)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleTalentWipeConfirmOpcode - {} not found or you can't interact with him.", confirmRespecWipe.RespecMaster);
        return;
    }
