...
void WorldSession::HandleAuctionHelloOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleAuctionHelloOpcode - Unit ({}) not found or you can't interact with him.", guid.ToString());
        return;
    }
...
void WorldSession::SendAuctionHello(ObjectGuid guid, Creature* unit)
{
    ...
    {
        SendNotification(GetSyphrenaString(LANG_AUCTION_REQ), sWorld->getIntConfig(CONFIG_AUCTION_LEVEL_REQ));
        return;
    }
...
void WorldSession::HandleAuctionSellItem(WorldPacket& recvData)
{
    ...
    {
        SC_LOG_DEBUG("network", "WORLD: HandleAuctionSellItem - Player {} {} attempted to sell item with higher price than max gold amount.", _player->GetName(), _player->GetGUID().ToString());
        SendAuctionCommandResult(0, AUCTION_SELL_ITEM, ERR_AUCTION_DATABASE_ERROR);
        return;
    }
    ...
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleAuctionSellItem - Unit ({}) not found or you can't interact with him.", auctioneer.ToString());
        return;
    }
    ...
    {
        SC_LOG_DEBUG("network", "WORLD: HandleAuctionSellItem - Unit ({}) has wrong faction.", auctioneer.ToString());
        return;
    }
    ...
    else
    {
        ...
        if (!auctioneerData)
        {
            SC_LOG_ERROR("misc", "Data for auctioneer not found ({})", auctioneer.ToString());
            SendAuctionCommandResult(0, AUCTION_SELL_ITEM, ERR_AUCTION_DATABASE_ERROR);
            delete AH;
            return;
        }
        ...
        if (!auctioneerInfo)
        {
            SC_LOG_ERROR("misc", "Non existing auctioneer ({})", auctioneer.ToString());
            SendAuctionCommandResult(0, AUCTION_SELL_ITEM, ERR_AUCTION_DATABASE_ERROR);
            delete AH;
            return;
        }
    ...
    if (itemsCount == 1 && item->GetCount() == count[0])
    {
        ...
        SC_LOG_INFO("network", "CMSG_AUCTION_SELL_ITEM: Player {} {} is selling item {} entry {} {} with count {} with initial bid {} with buyout {} and with time {} (in sec) in auctionhouse {}",
    ...
    else // Required stack size of auction does not match to current item stack size, clone item and set correct stack size
    {
        ...
        if (!newItem)
        {
            SC_LOG_ERROR("network", "CMSG_AUCTION_SELL_ITEM: Could not create clone of item {}", item->GetEntry());
            SendAuctionCommandResult(0, AUCTION_SELL_ITEM, ERR_AUCTION_DATABASE_ERROR);
            delete AH;
            return;
        }
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_AUCTION_PLACE_BID");
    ...
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleAuctionPlaceBid - {} not found or you can't interact with him.", auctioneer.ToString());
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_AUCTION_REMOVE_ITEM");

    ObjectGuid auctioneer;
    uint32 auctionId;
    recvData >> auctioneer;
    recvData >> auctionId;
    //SC_LOG_DEBUG("Cancel AUCTION AuctionID: {}", auctionId);

    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(auctioneer, UNIT_NPC_FLAG_AUCTIONEER);
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleAuctionRemoveItem - {} not found or you can't interact with him.", auctioneer.ToString());
        return;
    }
    ...
    if (auction && auction->owner == player->GetGUID().GetCounter())
    {
        ...
        {
            SC_LOG_ERROR("network", "Auction id: {} has non-existed item (item guid : {})!!!", auction->Id, auction->itemGUIDLow);
    ...
    else
    {
        ...
        SC_LOG_ERROR("entities.player.cheat", "CHEATER : {} tried to cancel auction (id: {}) of another player, or auction is NULL", player->GetGUID().ToString(), auctionId);
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_AUCTION_LIST_BIDDER_ITEMS");
    ...
    {
        SC_LOG_ERROR("network", "Client sent bad opcode!!! with count: {} and size : {} (must be: {})", outbiddedCount, (unsigned long)recvData.size(), (16 + outbiddedCount * 4));
        outbiddedCount = 0;
    }
    ...
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleAuctionListBidderItems - {} not found or you can't interact with him.", guid.ToString());
        recvData.rfinish();
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_AUCTION_LIST_OWNER_ITEMS");
    ...
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleAuctionListOwnerItems - {} not found or you can't interact with him.", guid.ToString());
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_AUCTION_LIST_ITEMS");
    ...
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleAuctionListItems - {} not found or you can't interact with him.", guid.ToString());
        return;
    }
    ...
    SC_LOG_DEBUG("auctionHouse", "Auctionhouse search ({}) list from: {}, searchedname: {}, levelmin: {}, levelmax: {}, auctionSlotID: {}, auctionMainCategory: {}, auctionSubCategory: {}, quality: {}, usable: {}",
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_AUCTION_LIST_PENDING_SALES");
