...
{
    //SC_LOG_DEBUG("network", "WORLD: CMSG_SPLIT_ITEM");
    uint8 srcbag, srcslot, dstbag, dstslot;
    uint32 count;

    recvData >> srcbag >> srcslot >> dstbag >> dstslot >> count;
    //SC_LOG_DEBUG("STORAGE: receive srcbag = {}, srcslot = {}, dstbag = {}, dstslot = {}, count = {}", srcbag, srcslot, dstbag, dstslot, count);
...
{
    //SC_LOG_DEBUG("network", "WORLD: CMSG_SWAP_INV_ITEM");
    uint8 srcslot, dstslot;

    recvData >> dstslot >> srcslot;
    //SC_LOG_DEBUG("STORAGE: receive srcslot = {}, dstslot = {}", srcslot, dstslot);
    ...
    {
        SC_LOG_DEBUG("network", "WORLD: HandleSwapInvItemOpcode - Unit ({}) not found or you can't interact with him.", m_currentBankerGUID.ToString());
        return;
    }
...
{
    //SC_LOG_DEBUG("network", "WORLD: CMSG_SWAP_ITEM");
    uint8 dstbag, dstslot, srcbag, srcslot;

    recvData >> dstbag >> dstslot >> srcbag >> srcslot;
    //SC_LOG_DEBUG("STORAGE: receive srcbag = {}, srcslot = {}, dstbag = {}, dstslot = {}", srcbag, srcslot, dstbag, dstslot);
    ...
    {
        SC_LOG_DEBUG("network", "WORLD: HandleSwapItem - Unit ({}) not found or you can't interact with him.", m_currentBankerGUID.ToString());
        return;
    }
...
{
    //SC_LOG_DEBUG("network", "WORLD: CMSG_AUTOEQUIP_ITEM");
    uint8 srcbag, srcslot;

    recvData >> srcbag >> srcslot;
    //SC_LOG_DEBUG("STORAGE: receive srcbag = {}, srcslot = {}", srcbag, srcslot);
...
{
    //SC_LOG_DEBUG("network", "WORLD: CMSG_DESTROYITEM");
    uint8 bag, slot, count, data1, data2, data3;

    recvData >> bag >> slot >> count >> data1 >> data2 >> data3;
    //SC_LOG_DEBUG("STORAGE: receive bag = {}, slot = {}, count = {}", bag, slot, count);
...
{
    SC_LOG_INFO("network", "STORAGE: Item Query = {}", query.ItemID);
    ...
    {
        SC_LOG_DEBUG("network", "WORLD: CMSG_ITEM_QUERY_SINGLE - NO item INFO! (ENTRY: {})", query.ItemID);
...
{
    //SC_LOG_DEBUG("network", "WORLD: CMSG_READ_ITEM");

    uint8 bag, slot;
    recvData >> bag >> slot;

    //SC_LOG_INFO("network", "STORAGE: Read bag = {}, slot = {}", bag, slot);
    Item* pItem = _player->GetItemByPos(bag, slot);
    ...
    if (pItem && pItem->GetTemplate()->PageText)
    {
        ...
        if (msg == EQUIP_ERR_OK)
        {
            ...
            SC_LOG_INFO("network", "STORAGE: Item page sent");
        }
        else
        {
            data.Initialize(SMSG_READ_ITEM_FAILED, 8);
            SC_LOG_INFO("network", "STORAGE: Unable to read item");
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_SELL_ITEM");
    ...
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleSellItemOpcode - {} not found or you can not interact with him.", vendorguid.ToString());
        ...
        if (pProto)
        {
            if (pProto->SellPrice > 0)
            {
                ...
                if (count < pItem->GetCount())               // need split items
                {
                    ...
                    if (!pNewItem)
                    {
                        SC_LOG_ERROR("network", "WORLD: HandleSellItemOpcode - could not create clone of item {}; count = {}", pItem->GetEntry(), count);
                        _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, creature, itemguid, 0);
                        return;
                    }
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_BUYBACK_ITEM");
    ...
    if (!creature)
    {
        SC_LOG_DEBUG("network", "WORLD: HandleBuybackItem - Unit ({}) not found or you can not interact with him.", vendorguid.ToString());
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_BUY_ITEM_IN_SLOT");
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_BUY_ITEM");
    ObjectGuid vendorguid;
    uint32 item, slot, count;
    uint8 unk1;
...
void WorldSession::HandleListInventoryOpcode(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("network", "WORLD: Recvd CMSG_LIST_INVENTORY");
...
void WorldSession::SendListInventory(ObjectGuid vendorGuid, uint32 vendorEntry)
{
    ...
    if (!vendor)
    {
        SC_LOG_DEBUG("network", "WORLD: SendListInventory - {} not found or you can not interact with him.", vendorGuid.ToString());
        _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, nullptr, ObjectGuid::Empty, 0);
        return;
    }
    ...
    for (uint8 slot = 0; slot < itemCount; ++slot)
    {
        if (VendorItem const* item = items->GetItem(slot))
        {
            if (ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(item->item))
            {
                ...
                {
                    SC_LOG_DEBUG("condition", "SendListInventory: conditions not met for creature entry {} item {}", vendor->GetEntry(), item->item);
                    continue;
                }
...
{
    //SC_LOG_DEBUG("network", "WORLD: CMSG_AUTOSTORE_BAG_ITEM");
    uint8 srcbag, srcslot, dstbag;

    recvData >> srcbag >> srcslot >> dstbag;
    //SC_LOG_DEBUG("STORAGE: receive srcbag = {}, srcslot = {}, dstbag = {}", srcbag, srcslot, dstbag);
...
void WorldSession::HandleSetAmmoOpcode(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("network", "WORLD: CMSG_SET_AMMO");
    uint32 item;
...
void WorldSession::HandleItemNameQueryOpcode(WorldPacket& recvData)
{
    ...
    SC_LOG_DEBUG("network", "WORLD: CMSG_ITEM_NAME_QUERY {}", itemid);
...
{
    SC_LOG_DEBUG("network", "Received opcode CMSG_WRAP_ITEM");
    ...
    SC_LOG_DEBUG("network", "WRAP: receive gift_bag = {}, gift_slot = {}, item_bag = {}, item_slot = {}", gift_bag, gift_slot, item_bag, item_slot);
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_SOCKET_GEMS");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_CANCEL_TEMP_ENCHANTMENT");
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_ITEM_REFUND_INFO");
    ...
    if (!item)
    {
        SC_LOG_DEBUG("network", "Item refund: item not found!");
        return;
    }
...
{
    SC_LOG_DEBUG("network", "WORLD: CMSG_ITEM_REFUND");
    ObjectGuid guid;
    recvData >> guid;                                      // item guid
...
void WorldSession::HandleItemTextQuery(WorldPacket& recvData )
{
    ...
    SC_LOG_DEBUG("network", "CMSG_ITEM_TEXT_QUERY {}", itemGuid.ToString());
