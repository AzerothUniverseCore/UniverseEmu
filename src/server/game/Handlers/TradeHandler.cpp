...
void WorldSession::moveItems(Item* myItems[], Item* hisItems[])
{
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            {
                // logging
                SC_LOG_DEBUG("network", "partner storing: {}", myItems[i]->GetGUID().ToString());
                if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
                {
                    sLog->OutCommand(_player->GetSession()->GetAccountId(), "GM {} (Account: {}) trade: {} (Entry: %d Count: {}) to player: {} (Account: {})",
            ...
            {
                // logging
                SC_LOG_DEBUG("network", "player storing: {}", hisItems[i]->GetGUID().ToString());
                if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
                {
                    sLog->OutCommand(trader->GetSession()->GetAccountId(), "GM {} (Account: {}) trade: {} (Entry: %d Count: {}) to player: {} (Account: {})",
                        trader->GetName().c_str(), trader->GetSession()->GetAccountId(),
        ...
        else
        {
            ...
            {
                if (!traderCanTrade)
                    SC_LOG_ERROR("network", "trader can't store item: {}", myItems[i]->GetGUID().ToString());
                if (_player->CanStoreItem(NULL_BAG, NULL_SLOT, playerDst, myItems[i], false) == EQUIP_ERR_OK)
                    _player->MoveItemToInventory(playerDst, myItems[i], true, true);
                else
                    SC_LOG_ERROR("network", "player can't take item back: {}", myItems[i]->GetGUID().ToString());
            }
            // return the already removed items to the original owner
            if (hisItems[i])
            {
                if (!playerCanTrade)
                    SC_LOG_ERROR("network", "player can't store item: {}", hisItems[i]->GetGUID().ToString());
                if (trader->CanStoreItem(NULL_BAG, NULL_SLOT, traderDst, hisItems[i], false) == EQUIP_ERR_OK)
                    trader->MoveItemToInventory(traderDst, hisItems[i], true, true);
                else
                    SC_LOG_ERROR("network", "trader can't take item back: {}", hisItems[i]->GetGUID().ToString());
            }
        }
    }
}
...
static void setAcceptTradeMode(TradeData* myTrade, TradeData* hisTrade, Item* *myItems, Item* *hisItems)
{
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        {
            SC_LOG_DEBUG("network", "player trade item {} bag: {} slot: {}", item->GetGUID().ToString(), item->GetBagSlot(), item->GetSlot());
            //Can return nullptr
            myItems[i] = item;
        ...
        {
            SC_LOG_DEBUG("network", "partner trade item {} bag: {} slot: {}", item->GetGUID().ToString(), item->GetBagSlot(), item->GetSlot());
            hisItems[i] = item;
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
    ...
    {
        SendNotification(GetSyphrenaString(LANG_TRADE_REQ), sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ));
        info.Status = TRADE_STATUS_CLOSE_WINDOW;
        SendTradeStatus(info);
        return;
    }
