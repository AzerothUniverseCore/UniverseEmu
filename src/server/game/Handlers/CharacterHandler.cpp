...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            SC_LOG_INFO("network", "Loading {} from account {}.", guid.ToString(), GetAccountId());
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    if (!classEntry)
    {
        SC_LOG_ERROR("network", "Class ({}) not found in DBC while creating new char for account (ID: {}): wrong DBC files or cheater?", createInfo->Class, GetAccountId());
    ...
    if (!raceEntry)
    {
        SC_LOG_ERROR("network", "Race ({}) not found in DBC while creating new char for account (ID: {}): wrong DBC files or cheater?", createInfo->Race, GetAccountId());
        SendCharCreate(CHAR_CREATE_FAILED);
        return;
    }
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "Expansion {} account:[%d] tried to Create character with expansion {} race ({})", Expansion(), GetAccountId(), raceEntry->RequiredExpansion, createInfo->Race);
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "Expansion {} account:[%d] tried to Create character with expansion {} class ({})", Expansion(), GetAccountId(), classEntry->RequiredExpansion, createInfo->Class);
    ...
    if (!HasPermission(rbac::RBAC_PERM_SKIP_CHECK_CHARACTER_CREATION_RACEMASK))
    {
        ...
        {
            SC_LOG_ERROR("network", "Race ({}) was not playable but requested while creating new char for account (ID: {}): wrong DBC files or cheater?", createInfo->Race, GetAccountId());
            SendCharCreate(CHAR_CREATE_DISABLED);
            return;
        }
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "Account:[%d] but tried to Create character with empty [name] ", GetAccountId());
        ...
        .WithChainingPreparedCallback([this, createInfo](QueryCallback& queryCallback, PreparedQueryResult result)
    {
        ...
        std::function<void(PreparedQueryResult)> finalizeCharacterCreation = [this, createInfo](PreparedQueryResult result)
        {
            ...
            AddTransactionCallback(CharacterDatabase.AsyncCommitTransaction(characterTransaction)).AfterComplete([this, newChar = std::move(newChar)](bool success)
            {
                if (success)
                {
                    SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Create Character: {} {}", GetAccountId(), GetRemoteAddress(), newChar->GetName(), newChar->GetGUID().ToString());
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    SC_LOG_INFO("entities.player.character", "Account: %d, IP: {} deleted character: {}, {}, Level: {}", accountId, GetRemoteAddress(), name, guid.ToString(), level);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recvData)
{
    ...
    {
        SC_LOG_ERROR("network", "Player tries to login again, AccountId = %d", GetAccountId());
    ...
    {
        SC_LOG_ERROR("network", "Account ({}) can't login with that character ({}).", GetAccountId(), playerGuid.ToString());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder const& holder)
{
    ...
    if (pCurrChar->GetGuildId() != 0)
    {
        ...
        else
        {
            ...
            SC_LOG_ERROR("network", "Player {} {} marked as member of not existing guild (id: {}), removing guild membership for player.", pCurrChar->GetName(), pCurrChar->GetGUID().ToString(), pCurrChar->GetGuildId());
            pCurrChar->SetInGuild(0);
        }
    }
    ...
    //SC_LOG_DEBUG("Player {} added to Map.", pCurrChar->GetName());
    ...
    SC_LOG_INFO("entities.player.character", "Account: %d (IP: {}) Login Character:[{}] {} Level: %d, XP: {}/{} ({} left)",
    ...
    SC_METRIC_EVENT("player_events", "Login", pCurrChar->GetName());
}
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_ATWAR");
...
{
    SC_LOG_ERROR("network", "WORLD SESSION: HandleSetFactionCheat, not expected call, please report.");
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_WATCHED_FACTION");
    uint32 fact;
    recvData >> fact;
...
{
    SC_LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_INACTIVE");
    uint32 replistid;
    uint8 inactive;
...
void WorldSession::HandleCharRenameCallBack(std::shared_ptr<CharacterRenameInfo> renameInfo, PreparedQueryResult result)
{
    ...
    SC_LOG_INFO("entities.player.character", "Account: %d (IP: {}) Character:[{}] ({}) Changed name to: {}", GetAccountId(), GetRemoteAddress(), oldName, renameInfo->Guid.ToString(), renameInfo->Name);
...
{
    SC_LOG_DEBUG("network", "CMSG_ALTER_APPEARANCE");
...
void WorldSession::HandleRemoveGlyph(WorldPacket& recvData)
{
    ...
    {
        SC_LOG_DEBUG("network", "Client sent wrong glyph slot number in opcode CMSG_REMOVE_GLYPH {}", slot);
        return;
    }
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "Account {}, IP: {} tried to customise {}, but it does not belong to their account!",
...
void WorldSession::HandleCharCustomizeCallback(std::shared_ptr<CharacterCustomizeInfo> customizeInfo, PreparedQueryResult result)
{
    ...
    SC_LOG_INFO("entities.player.character", "Account: %d (IP: {}), Character[{}] ({}) Customized to: {}",
        GetAccountId(), GetRemoteAddress(), oldName, customizeInfo->Guid.ToString(), customizeInfo->Name);
}

void WorldSession::HandleEquipmentSetSave(WorldPacket& recvData)
{
    SC_LOG_DEBUG("network", "CMSG_EQUIPMENT_SET_SAVE");
...
{
    SC_LOG_DEBUG("network", "CMSG_EQUIPMENT_SET_DELETE");
...
{
    SC_LOG_DEBUG("network", "CMSG_EQUIPMENT_SET_USE");
    ...
    for (uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i)
    {
        ...
        SC_LOG_DEBUG("entities.player.items", "{}: srcbag {}, srcslot {}", itemGuid.ToString(), srcbag, srcslot);
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    {
        SC_LOG_ERROR("entities.player.cheat", "Account {}, IP: {} tried to factionchange character {}, but it does not belong to their account!",
...
void WorldSession::HandleCharFactionOrRaceChangeCallback(std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo, PreparedQueryResult result)
{
    ...
    if (oldRace != factionChangeInfo->Race)
    {
        ...
        if (factionChangeInfo->Race != RACE_ORC && factionChangeInfo->Race != RACE_HUMAN && factionChangeInfo->Race != RACE_GOBLIN_ && factionChangeInfo->Race != RACE_WORGEN && factionChangeInfo->Race != RACE_PANDAREN_HORDE && factionChangeInfo->Race != RACE_PANDAREN_ALLIANCE && factionChangeInfo->Race != RACE_BLOODELF_ILLIDARI && factionChangeInfo->Race != RACE_NIGHTELF_ILLIDARI && factionChangeInfo->Race != RACE_EREDAR && factionChangeInfo->Race != RACE_VOIDELF && factionChangeInfo->Race != RACE_VULPERA && factionChangeInfo->Race != RACE_NIGHTBORNE && factionChangeInfo->Race != RACE_LIGHTFORGED_DRAENEI && factionChangeInfo->Race != RACE_ZANDALARI_TROLL && factionChangeInfo->Race != RACE_DARK_IRON_DWARF && factionChangeInfo->Race != RACE_DARK_IRON_DWARF_H && factionChangeInfo->Race != RACE_HIGHELF && factionChangeInfo->Race != RACE_HIGHMOUNTAINTAUREN && factionChangeInfo->Race != RACE_VULPERA_A && factionChangeInfo->Race != RACE_DRACTHYR_H && factionChangeInfo->Race != RACE_DRACTHYR_A && factionChangeInfo->Race != RACE_MAGHAR && factionChangeInfo->Race != RACE_KULTIRAN)
        {
            stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_SKILL_LANGUAGE);
            stmt->setUInt32(0, lowGuid);
        ...
        if (factionChangeInfo->FactionChange)
        {
            ...
            {
                std::vector<std::string_view> tokens = Syphrena::Tokenize(knownTitlesStr, ' ', false);
                ...
                for (uint32 index = 0; index < knownTitles.size(); ++index)
                {
                    ...
                    if (index < tokens.size())
                        thisMask = Syphrena::StringTo<uint32>(tokens[index]);

                    if (thisMask)
                        knownTitles[index] = *thisMask;
                    else
                    {
                        SC_LOG_WARN("entities.player", "{} has invalid title data '{}' at index {} - skipped, this may result in titles being lost",
    ...
    SC_LOG_DEBUG("entities.player", "{} (IP: {}) changed race from {} to {}", GetPlayerInfo(), GetRemoteAddress(), oldRace, factionChangeInfo->Race);
