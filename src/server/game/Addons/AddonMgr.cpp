...
void LoadFromDB()
{
    ...
    if (result)
    {
        ...
        SC_LOG_INFO("server.loading", ">> Loaded {} known addons in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
    else
        SC_LOG_INFO("server.loading", ">> Loaded 0 known addons. DB table `addons` is empty!");
    ...
    if (result)
    {
        ...
        do
        {
            ...
            addon.NameMD5 = Syphrena::Crypto::MD5::GetDigestOf(name);
            addon.VersionMD5 = Syphrena::Crypto::MD5::GetDigestOf(version);
        ...
        SC_LOG_INFO("server.loading", ">> Loaded {} banned addons in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
}
