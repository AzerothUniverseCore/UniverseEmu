...
void InstanceScript::LoadMinionData(MinionData const* data)
{
    ...
    SC_LOG_DEBUG("scripts", "InstanceScript::LoadMinionData: {} minions loaded.", uint64(minions.size()));
}
...
void InstanceScript::LoadDoorData(DoorData const* data)
{
    ...
    SC_LOG_DEBUG("scripts", "InstanceScript::LoadDoorData: {} doors loaded.", uint64(doors.size()));
}
...
void InstanceScript::LoadObjectData(ObjectData const* creatureData, ObjectData const* gameObjectData)
{
    ...
    SC_LOG_DEBUG("scripts", "InstanceScript::LoadObjectData: {} objects loaded.", _creatureInfo.size() + _gameObjectInfo.size());
}
...
bool InstanceScript::SetBossState(uint32 id, EncounterState state)
{
    if (id < bosses.size())
    {
        ...
        if (bossInfo->state == TO_BE_DECIDED) // loading
        {
            ...
            SC_LOG_DEBUG("scripts", "InstanceScript: Initialize boss {} state as {} (map {}, {}).", id, GetBossStateName(state), instance->GetId(), instance->GetInstanceId());
            return false;
        }
        else
        {
            ...
            {
                SC_LOG_ERROR("map", "InstanceScript: Tried to set instance boss {} state from {} back to {} for map {}, instance id {}. Blocked!", id, GetBossStateName(bossInfo->state), GetBossStateName(state), instance->GetId(), instance->GetInstanceId());
                return false;
            }
...
void InstanceScript::HandleGameObject(ObjectGuid guid, bool open, GameObject* go /*= nullptr*/)
{
    ...
    if (go)
        ...
        SC_LOG_DEBUG("scripts", "InstanceScript: HandleGameObject failed");
}
...
void InstanceScript::DoUseDoorOrButton(ObjectGuid guid, uint32 withRestoreTime /*= 0*/, bool useAlternativeState /*= false*/)
{
    ...
    if (GameObject* go = instance->GetGameObject(guid))
    {
        if (go->GetGoType() == GAMEOBJECT_TYPE_DOOR || go->GetGoType() == GAMEOBJECT_TYPE_BUTTON)
        {
            ...
            SC_LOG_ERROR("scripts", "InstanceScript: DoUseDoorOrButton can't use gameobject entry {}, because type is {}.", go->GetEntry(), go->GetGoType());
    }
    else
        SC_LOG_DEBUG("scripts", "InstanceScript: DoUseDoorOrButton failed");
}
...
void InstanceScript::DoCloseDoorOrButton(ObjectGuid guid)
{
    ...
    if (GameObject* go = instance->GetGameObject(guid))
    {
        if (go->GetGoType() == GAMEOBJECT_TYPE_DOOR || go->GetGoType() == GAMEOBJECT_TYPE_BUTTON)
        {
            ...
            SC_LOG_ERROR("scripts", "InstanceScript: DoCloseDoorOrButton can't use gameobject entry {}, because type is {}.", go->GetEntry(), go->GetGoType());
    }
    else
        SC_LOG_DEBUG("scripts", "InstanceScript: DoCloseDoorOrButton failed");
}
    ...
    if (GameObject* go = instance->GetGameObject(guid))
    {
        switch (go->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_FISHINGNODE:
                ...
                SC_LOG_ERROR("scripts", "InstanceScript: DoRespawnGameObject can't respawn gameobject entry {}, because type is {}.", go->GetEntry(), go->GetGoType());
                return;
            default:
                break;
        }
        ...
        SC_LOG_DEBUG("scripts", "InstanceScript: DoRespawnGameObject failed");
}
...
void InstanceScript::DoUpdateWorldState(uint32 uiStateId, uint32 uiStateData)
{
    ...
    if (!lPlayers.isEmpty())
    {
        ...
        SC_LOG_DEBUG("scripts", "DoUpdateWorldState attempt send data but no players in map.");
}
...
{
    SC_LOG_ERROR("misc", "Achievement system call InstanceScript::CheckAchievementCriteriaMeet but instance script for map {} not have implementation for achievement criteria {}",
    ...
    for (auto const& encounter : *encounters)
    {
        if (encounter->creditType == type && encounter->creditEntry == creditEntry)
        {
            ...
            if (encounter->lastEncounterDungeon)
            {
                ...
                SC_LOG_DEBUG("lfg", "UpdateEncounterState: Instance {} (instanceId {}) completed encounter {}. Credit Dungeon: {}", instance->GetMapName(), instance->GetInstanceId(), encounter->dbcEntry->Name[0], dungeonId);
                break;
            }
        }
    }
