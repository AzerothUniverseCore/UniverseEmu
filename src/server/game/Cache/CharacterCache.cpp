...
void CharacterCache::LoadCharacterCacheStorage()
{
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", "No character name data loaded, empty query");
        return;
    }
    ...
    SC_LOG_INFO("server.loading", "Loaded character infos for {} characters in {} ms", _characterCacheStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
