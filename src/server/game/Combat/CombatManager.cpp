...
bool CombatManager::SetInCombatWith(Unit* who, bool addSecondUnitSuppressed)
{
    ...
    if (PvPCombatReference* existingPvpRef = Syphrena::Containers::MapGetValuePtr(_pvpRefs, who->GetGUID()))
    {
        existingPvpRef->RefreshTimer();
        existingPvpRef->Refresh();
        return true;
    }
    if (CombatReference* existingPveRef = Syphrena::Containers::MapGetValuePtr(_pveRefs, who->GetGUID()))
    {
