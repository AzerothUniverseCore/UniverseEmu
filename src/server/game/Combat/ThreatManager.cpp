...
Syphrena::IteratorPair<ThreatManager::ThreatListIterator, std::nullptr_t> ThreatManager::GetUnsortedThreatList() const
{
...
Syphrena::IteratorPair<ThreatManager::ThreatListIterator, std::nullptr_t> ThreatManager::GetSortedThreatList() const
{
...
void ThreatManager::ProcessAIUpdates()
{
    ...
    for (ObjectGuid const& guid : v)
        if (ThreatReference const* ref = Syphrena::Containers::MapGetValuePtr(_myThreatListEntries, guid))
