...
void Syphrena::BroadcastTextBuilder::operator()(WorldPacket& data, LocaleConstant locale) const
{
...
size_t Syphrena::BroadcastTextBuilder::operator()(WorldPacket* data, LocaleConstant locale) const
{
    BroadcastText const* bct = sObjectMgr->GetBroadcastText(_textId);
...
void Syphrena::CustomChatTextBuilder::operator()(WorldPacket& data, LocaleConstant locale) const
{
    ChatHandler::BuildChatPacket(data, _msgType, _language, _source, _target, _text, 0, "", locale);
}

void Syphrena::SyphrenaStringChatBuilder::operator()(WorldPacket& data, LocaleConstant locale) const
{
    char const* text = sObjectMgr->GetSyphrenaString(_textId, locale);
