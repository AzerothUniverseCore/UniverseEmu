...
{
    SC_LOG_DEBUG("outdoorpvp", "Creating capture point {}", entry);

    // check info existence
    GameObjectTemplate const* goinfo = sObjectMgr->GetGameObjectTemplate(entry);
    if (!goinfo || goinfo->type != GAMEOBJECT_TYPE_CAPTURE_POINT)
    {
        SC_LOG_ERROR("outdoorpvp", "OutdoorPvP: GO {} is not capture point!", entry);
        return false;
    }
...
bool OPvPCapturePoint::DelCreature(uint32 type)
{
    ...
    if (!spawnId)
    {
        SC_LOG_DEBUG("outdoorpvp", "opvp creature type {} was already deleted", type);
        return false;
    }
    SC_LOG_DEBUG("outdoorpvp", "deleting opvp creature type {}", type);
...
void OutdoorPvP::HandlePlayerLeaveZone(Player* player, uint32 /*zone*/)
{
    ...
    SC_LOG_DEBUG("outdoorpvp", "Player {} left an outdoorpvp zone", player->GetName());
}
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    Syphrena::AnyPlayerInObjectRangeCheck checker(m_capturePoint, radius);
    Syphrena::PlayerListSearcher<Syphrena::AnyPlayerInObjectRangeCheck> searcher(m_capturePoint, players, checker);
    ...
    {
        //SC_LOG_ERROR("outdoorpvp", "{}->{}", m_OldState, m_State);
        if (oldTeam != m_team)
            ChangeTeam(oldTeam);
        ChangeState();
        return true;
...
void OutdoorPvP::SendDefenseMessage(uint32 zoneId, uint32 id)
{
    ...
    Syphrena::LocalizedPacketDo<DefenseMessageBuilder> localizer(builder);
