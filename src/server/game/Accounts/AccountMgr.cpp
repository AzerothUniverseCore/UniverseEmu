...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password, std::string email /*= ""*/)
{
    ...
    auto [salt, verifier] = Syphrena::Crypto::SRP6::MakeRegistrationData(username, password);
...
AccountOpResult AccountMgr::ChangeUsername(uint32 accountId, std::string newUsername, std::string newPassword)
{
    ...
    auto [salt, verifier] = Syphrena::Crypto::SRP6::MakeRegistrationData(newUsername, newPassword);
...
AccountOpResult AccountMgr::ChangePassword(uint32 accountId, std::string newPassword)
{
    ...
    Utf8ToUpperOnlyLatin(username);
    Utf8ToUpperOnlyLatin(newPassword);
    auto [salt, verifier] = Syphrena::Crypto::SRP6::MakeRegistrationData(username, newPassword);
...
bool AccountMgr::CheckPassword(uint32 accountId, std::string password)
{
    ...
    {
        Syphrena::Crypto::SRP6::Salt salt = (*result)[0].GetBinary<Syphrena::Crypto::SRP6::SALT_LENGTH>();
        Syphrena::Crypto::SRP6::Verifier verifier = (*result)[1].GetBinary<Syphrena::Crypto::SRP6::VERIFIER_LENGTH>();
        if (Syphrena::Crypto::SRP6::CheckLogin(username, password, salt, verifier))
            return true;
    }
...
void AccountMgr::LoadRBAC()
{
    ...
    SC_LOG_DEBUG("rbac", "AccountMgr::LoadRBAC");
    uint32 oldMSTime = getMSTime();
    uint32 count1 = 0;
    uint32 count2 = 0;
    uint32 count3 = 0;

    SC_LOG_DEBUG("rbac", "AccountMgr::LoadRBAC: Loading permissions");
    QueryResult result = LoginDatabase.Query("SELECT id, name FROM rbac_permissions");
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 account permission definitions. DB table `rbac_permissions` is empty.");
        return;
    }
    ...
    SC_LOG_DEBUG("rbac", "AccountMgr::LoadRBAC: Loading linked permissions");
    result = LoginDatabase.Query("SELECT id, linkedId FROM rbac_linked_permissions ORDER BY id ASC");
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 linked permissions. DB table `rbac_linked_permissions` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "RBAC Permission {} has itself as linked permission. Ignored", permissionId);
            continue;
        }
    ...
    SC_LOG_DEBUG("rbac", "AccountMgr::LoadRBAC: Loading default permissions");
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 default permission definitions. DB table `rbac_default_permissions` is empty.");
        return;
    }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} permission definitions, {} linked permissions and {} default permissions in {} ms", count1, count2, count3, GetMSTimeDiffToNow(oldMSTime));
}
...
{
    SC_LOG_TRACE("rbac", "AccountMgr::GetRBACPermission: {}", permissionId);
...
bool AccountMgr::HasPermission(uint32 accountId, uint32 permissionId, uint32 realmId)
{
    if (!accountId)
    {
        SC_LOG_ERROR("rbac", "AccountMgr::HasPermission: Wrong accountId 0");
        return false;
    }
    ...
    SC_LOG_DEBUG("rbac", "AccountMgr::HasPermission [AccountId: {}, PermissionId: {}, realmId: {}]: {}",
...
{
    SC_LOG_TRACE("rbac", "AccountMgr::GetRBACDefaultPermissions: secLevel {} - size: {}", secLevel, uint32(_defaultPermissions[secLevel].size()));
