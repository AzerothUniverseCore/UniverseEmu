...
SC_GAME_API extern ScriptMapMap sSpellScripts;
SC_GAME_API extern ScriptMapMap sEventScripts;
SC_GAME_API extern ScriptMapMap sWaypointScripts;

std::string GetScriptsTableNameByType(ScriptsType type);
ScriptMapMap* GetScriptsMapByType(ScriptsType type);
std::string GetScriptCommandName(ScriptCommands command);

struct SC_GAME_API InstanceSpawnGroupInfo
{
    enum
    {
...
struct SC_GAME_API SpellClickInfo
{
    uint32 spellId;
    uint8 castFlags;
...
typedef std::unordered_map<uint32, SyphrenaString> SyphrenaStringContainer;
...
SC_GAME_API bool normalizePlayerName(std::string& name);
...
SC_GAME_API extern LanguageDesc lang_description[LANGUAGES_COUNT];
...
class SC_GAME_API ObjectMgr
{
    ...
    public:
        ...
        typedef std::unordered_map<uint32, Syphrena::unique_trackable_ptr<Quest>> QuestContainer;
        ...
        Syphrena::IteratorPair<SpawnGroupLinkContainer::const_iterator> GetSpawnMetadataForGroup(uint32 groupId) const { return Syphrena::Containers::MapEqualRange(_spawnGroupMapStore, groupId); }
        ...
        SyphrenaString const* GetSyphrenaString(uint32 entry) const
        {
            SyphrenaStringContainer::const_iterator itr = _syphrenaStringStore.find(entry);
            if (itr == _syphrenaStringStore.end())
                return nullptr;
            return &itr->second;
        }
        char const* GetSyphrenaString(uint32 entry, LocaleConstant locale) const;
        char const* GetSyphrenaStringForDBCLocale(uint32 entry) const { return GetSyphrenaString(entry, DBCLocaleIndex); }
        ...
        Syphrena::IteratorPair<SpellClickInfoContainer::const_iterator> GetSpellClickInfoMapBounds(uint32 creature_id) const
        {
            return Syphrena::Containers::MapEqualRange(_spellClickInfoStore, creature_id);
        }
    ...
    private:
        ...
        SyphrenaStringContainer _syphrenaStringStore;
