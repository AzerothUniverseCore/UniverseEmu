...
void WardenCheckMgr::LoadWardenChecks()
{
    ...
    {
        SC_LOG_INFO("warden", ">> Warden disabled, loading checks skipped.");
        return;
    }
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 Warden checks. DB table `warden_checks` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "Warden check with id {} lists check type {} in `warden_checks`, which is not supported. Skipped.", id, type);
            continue;
        }

        if ((type == LUA_EVAL_CHECK) && (id > 9999))
        {
            SC_LOG_ERROR("sql.sql", "Warden Lua check with id {} found in `warden_checks`. Lua checks may have four-digit IDs at most. Skipped.", id);
            continue;
        }
        ...
        if (type == LUA_EVAL_CHECK)
        {
            ...
            {
                SC_LOG_ERROR("sql.sql", "Found over-long Lua check for Warden check with id {} in `warden_checks`. Max length is {}. Skipped.", id, WARDEN_MAX_LUA_CHECK_LENGTH);
                continue;
            }

            std::string str = Syphrena::StringFormat("{:04}", id);
            ASSERT(str.size() == 4);
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} warden checks in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void WardenCheckMgr::LoadWardenOverrides()
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_WARDEN_ENABLED))
    {
        SC_LOG_INFO("warden", ">> Warden disabled, loading check overrides skipped.");
        return;
    }
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 Warden action overrides. DB table `warden_action` is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (action > WARDEN_ACTION_BAN)
            SC_LOG_ERROR("warden", "Warden check override action out of range (ID: {}, action: {})", checkId, action);
        // Check if check actually exists before accessing the _checks vector
        else if (checkId >= _checks.size())
            SC_LOG_ERROR("warden", "Warden check action override for non-existing check (ID: {}, action: {}), skipped", checkId, action);
        else
        {
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} warden action overrides in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
