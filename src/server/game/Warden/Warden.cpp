...
{
    SC_LOG_DEBUG("warden", "Make module for client");
    InitializeModuleForClient(_module.emplace());

    _module->Id = Syphrena::Crypto::MD5::GetDigestOf(_module->CompressedData, _module->CompressedSize);
}

void Warden::SendModuleToClient()
{
    SC_LOG_DEBUG("warden", "Send module to client");
...
{
    SC_LOG_DEBUG("warden", "Request module");
...
void Warden::Update(uint32 diff)
{
    ...
    if (_dataSent)
    {
        ...
        if (maxClientResponseDelay > 0)
        {
            ...
            {
                SC_LOG_WARN("warden", "{} (latency: {}, IP: {}) exceeded Warden module response delay ({}) - disconnecting client",
...
bool Warden::IsValidCheckSum(uint32 checksum, uint8 const* data, const uint16 length)
{
    ...
    {
        SC_LOG_DEBUG("warden", "CHECKSUM IS NOT VALID");
        return false;
    }
    else
    {
        SC_LOG_DEBUG("warden", "CHECKSUM IS VALID");
        return true;
    }
}
...
{
    keyData hash;
    hash.bytes = Syphrena::Crypto::SHA1::GetDigestOf(data, size_t(length));
    uint32 checkSum = 0;
    for (uint8 i = 0; i < 5; ++i)
...
void Warden::HandleData(ByteBuffer& buff)
{
    ...
    uint8 opcode;
    buff >> opcode;
    SC_LOG_DEBUG("warden", "Got packet, opcode {:02X}, size {}", opcode, uint32(buff.size() - 1));
    buff.hexlike();
    ...
    case WARDEN_CMSG_MEM_CHECKS_RESULT:
        SC_LOG_DEBUG("warden", "NYI WARDEN_CMSG_MEM_CHECKS_RESULT received!");
        break;
    ...
    case WARDEN_CMSG_MODULE_FAILED:
        SC_LOG_DEBUG("warden", "NYI WARDEN_CMSG_MODULE_FAILED received!");
        break;
    default:
        SC_LOG_WARN("warden", "Got unknown warden opcode {:02X} of size {}.", opcode, uint32(buff.size() - 1));
        break;
    }
}
...
bool Warden::ProcessLuaCheckResponse(std::string const& msg)
{
    ...
    if (id < sWardenCheckMgr->GetMaxValidCheckId())
    {
        ...
        if (check.Type == LUA_EVAL_CHECK)
        {
            ...
            SC_LOG_WARN("warden", "{} failed Warden check {} ({}). Action: {}", _session->GetPlayerInfo(), id, EnumUtils::ToConstant(check.Type), penalty);
            return true;
        }
    }

    char const* penalty = ApplyPenalty(nullptr);
    SC_LOG_WARN("warden", "{} sent bogus Lua check response for Warden. Action: {}", _session->GetPlayerInfo(), penalty);
    return true;
}
