...
WardenWin::WardenWin() : Warden(), _serverTicks(0)
{
    for (WardenCheckCategory category : EnumUtils::Iterate<WardenCheckCategory>())
    {
        ...
        Syphrena::Containers::RandomShuffle(checks);
...
void WardenWin::Init(WorldSession* session, SessionKey const& K)
{
    _session = session;
    // Generate Warden Key
    SessionKeyGenerator<Syphrena::Crypto::SHA1> WK(K);
    ...
    SC_LOG_DEBUG("warden", "Server side warden for client {} initializing...", session->GetAccountId());
    SC_LOG_DEBUG("warden", "C->S Key: {}", ByteArrayToHexStr(_inputKey));
    SC_LOG_DEBUG("warden", "S->C Key: {}", ByteArrayToHexStr(_outputKey));
    SC_LOG_DEBUG("warden", "  Seed: {}", ByteArrayToHexStr(_seed));
    SC_LOG_DEBUG("warden", "Loading Module...");

    MakeModuleForClient();

    SC_LOG_DEBUG("warden", "Module Key: {}", ByteArrayToHexStr(_module->Key));
    SC_LOG_DEBUG("warden", "Module ID: {}", ByteArrayToHexStr(_module->Id));
    RequestModule();
}
...
{
    SC_LOG_DEBUG("warden", "Initialize module");
...
{
    SC_LOG_DEBUG("warden", "Request hash");
...
{
    // Verify key
    Syphrena::Crypto::SHA1::Digest response;
    buff.read(response);
    if (response != Module.ClientKeySeedHash)
    {
        char const* penalty = ApplyPenalty(nullptr);
        SC_LOG_WARN("warden", "{} failed hash reply. Action: {}", _session->GetPlayerInfo(), penalty);
        return;
    }

    SC_LOG_DEBUG("warden", "Request hash reply: succeed");
...
static constexpr uint8 GetCheckPacketBaseSize(WardenCheckType type)
{
    switch (type)
    {
        ...
        case MODULE_CHECK: return (4 + Syphrena::Crypto::HMAC_SHA1::DIGEST_LENGTH);
...
{
    SC_LOG_DEBUG("warden", "Request data from {} (account {}) - loaded: {}", _session->GetPlayerName(), _session->GetAccountId(), _session->GetPlayer() && !_session->PlayerLoading());
    ...
    for (WardenCheckCategory category : EnumUtils::Iterate<WardenCheckCategory>())
    {
        ...
        {
            SC_LOG_DEBUG("warden", "Finished all {} checks, re-shuffling", EnumUtils::ToConstant(category));
            Syphrena::Containers::RandomShuffle(checks);
            checksIt = checks.begin();
        }
    }
    ...
    Syphrena::Containers::RandomShuffle(_currentChecks);

    uint16 expectedSize = 4;
    Syphrena::Containers::EraseIf(_currentChecks,
    ...
    for (uint16 const id : _currentChecks)
    {
        ...
        switch (type)
        {
            ...
            case MODULE_CHECK:
            {
                std::array<uint8, 4> seed = Syphrena::Crypto::GetRandomBytes<4>();
                buff.append(seed);
                buff.append(Syphrena::Crypto::HMAC_SHA1::GetDigestOf(seed, check.Str));
                break;
            }
            /*case PROC_CHECK:
            {
    ...
    {
        SC_LOG_DEBUG("warden", "Finished building warden packet, size is %zu bytes", buff.size());
        SC_LOG_DEBUG("warden", "Sent checks: {}", idstring());
    }
    else
    {
        SC_LOG_WARN("warden", "Finished building warden packet, size is %zu bytes, but expected {} bytes!", buff.size(), expectedSize);
        SC_LOG_WARN("warden", "Sent checks: {}", idstring());
    }
...
{
    SC_LOG_DEBUG("warden", "Handle data");
    ...
    if (Length != (buff.size() - buff.rpos()))
    {
        buff.rfinish();
        char const* penalty = ApplyPenalty(nullptr);
        SC_LOG_WARN("warden", "{} sends manipulated warden packet. Action: {}", _session->GetPlayerInfo(), penalty);
        return;
    }
    ...
    if (!IsValidCheckSum(Checksum, buff.contents() + buff.rpos(), Length))
    {
        buff.rfinish();
        char const* penalty = ApplyPenalty(nullptr);
        SC_LOG_WARN("warden", "{} failed checksum. Action: {}", _session->GetPlayerInfo(), penalty);
        return;
    }
    ...
    // TIMING_CHECK
    {
        ...
        {
            char const* penalty = ApplyPenalty(nullptr);
            SC_LOG_WARN("warden", "{} failed timing check. Action: {}", _session->GetPlayerInfo(), penalty);
            return;
        }
        ...
        SC_LOG_DEBUG("warden", "Server tick count now:    {}", ticksNow);
        SC_LOG_DEBUG("warden", "Server tick count at req: {}", _serverTicks);
        SC_LOG_DEBUG("warden", "Client ticks in response: {}", newClientTicks);
        SC_LOG_DEBUG("warden", "Round trip response time: {} ms", ourTicks - newClientTicks);
    }
    ...
    for (uint16 const id : _currentChecks)
    {
        ...
        switch (check.Type)
        {
            case MEM_CHECK:
            {
                ...
                if (Mem_Result != 0)
                {
                    SC_LOG_DEBUG("warden", "RESULT MEM_CHECK not 0x00, CheckId {} account Id {}", id, _session->GetAccountId());
                    checkFailed = id;
                    continue;
                }
                ...
                {
                    SC_LOG_DEBUG("warden", "RESULT MEM_CHECK fail CheckId {} account Id {}", id, _session->GetAccountId());
                    SC_LOG_DEBUG("warden", "Expected: {}", ByteArrayToHexStr(expected));
                    SC_LOG_DEBUG("warden", "Got:      {}", ByteArrayToHexStr(response));
                    checkFailed = id;
                    continue;
                }

                SC_LOG_DEBUG("warden", "RESULT MEM_CHECK passed CheckId {} account Id {}", id, _session->GetAccountId());
                break;
            }
            case PAGE_CHECK_A:
            case PAGE_CHECK_B:
            ...
            case MODULE_CHECK:
            {
                ...
                {
                    SC_LOG_DEBUG("warden", "RESULT {} fail, CheckId {} account Id {}", EnumUtils::ToConstant(check.Type), id, _session->GetAccountId());
                    checkFailed = id;
                    continue;
                }

                SC_LOG_DEBUG("warden", "RESULT {} passed CheckId {} account Id {}", EnumUtils::ToConstant(check.Type), id, _session->GetAccountId());
                break;
            }
            case LUA_EVAL_CHECK:
            {
                ...
                SC_LOG_DEBUG("warden", "LUA_EVAL_CHECK CheckId {} account Id {} got in-warden dummy response ({})", id, _session->GetAccountId(), result);
                break;
            }
            case MPQ_CHECK:
            {
                ...
                if (Mpq_Result != 0)
                {
                    SC_LOG_DEBUG("warden", "RESULT MPQ_CHECK not 0x00 account id {}", _session->GetAccountId());
                    checkFailed = id;
                    continue;
                }

                std::vector<uint8> result;
                result.resize(Syphrena::Crypto::SHA1::DIGEST_LENGTH);
                buff.read(result.data(), result.size());
                if (result != sWardenCheckMgr->GetCheckResult(id)) // SHA1
                {
                    SC_LOG_DEBUG("warden", "RESULT MPQ_CHECK fail, CheckId {} account Id {}", id, _session->GetAccountId());
                    checkFailed = id;
                    continue;
                }

                SC_LOG_DEBUG("warden", "RESULT MPQ_CHECK passed, CheckId {} account Id {}", id, _session->GetAccountId());
                break;
            }
    ...
    if (checkFailed > 0)
    {
        ...
        SC_LOG_WARN("warden", "{} failed Warden check {} ({}). Action: {}", _session->GetPlayerInfo(), checkFailed, EnumUtils::ToConstant(check.Type), penalty);
    }
