...
void WardenMac::Init(WorldSession* pClient, SessionKey const& K)
{
    ...
    */
    ...
    SC_LOG_DEBUG("warden", "Server side warden for client {} initializing...", pClient->GetAccountId());
...
void WardenMac::HandleHashResult(ByteBuffer &buff)
{
    ...
    // Verify key
    Syphrena::Crypto::SHA1::Digest result;
    buff.read(result);
    if (result != Syphrena::Crypto::SHA1::GetDigestOf(reinterpret_cast<uint8*>(keyIn), 16))
    {
...
{
    SC_LOG_DEBUG("warden", "Request data");
...
void WardenMac::HandleCheckResult(ByteBuffer &buff)
{
    ...
    Syphrena::Crypto::SHA1 sha1;
    sha1.UpdateData(str);
    uint32 magic = 0xFEEDFACE;                              // unsure
    sha1.UpdateData((uint8*)&magic, 4);
    sha1.Finalize();

    std::array<uint8, Syphrena::Crypto::SHA1::DIGEST_LENGTH> sha1Hash;
    buff.read(sha1Hash.data(), sha1Hash.size());

    if (sha1Hash != sha1.GetDigest())
    {
        SC_LOG_DEBUG("warden", "Handle data failed: SHA1 hash is wrong!");
        //found = true;
    }

    std::array<uint8, 16> ourMD5Hash = Syphrena::Crypto::MD5::GetDigestOf(str);
    std::array<uint8, 16> theirsMD5Hash;
    buff.read(theirsMD5Hash);

    if (ourMD5Hash != theirsMD5Hash)
    {
        SC_LOG_DEBUG("warden", "Handle data failed: MD5 hash is wrong!");
        //found = true;
    }
