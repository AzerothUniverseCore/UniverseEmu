...
void QuestPoolMgr::LoadFromDB()
{
    ...
    // load template data from world DB
    {
        ...
        if (!result)
        {
            SC_LOG_INFO("server.loading", ">> Loaded 0 quest pools. DB table `quest_pool_members` is empty.");
            return;
        }
        ...
        do
        {
            ...
            if (fields[2].IsNull())
            {
                SC_LOG_ERROR("sql.sql", "Table `quest_pool_members` contains reference to non-existing pool {}. Skipped.", fields[1].GetUInt32());
                continue;
            }
            ...
            if (!quest)
            {
                SC_LOG_ERROR("sql.sql", "Table `quest_pool_members` contains reference to non-existing quest {}. Skipped.", questId);
                continue;
            }
            if (!quest->IsDailyOrWeekly() && !quest->IsMonthly())
            {
                SC_LOG_ERROR("sql.sql", "Table `quest_pool_members` contains reference to quest {}, which is neither daily, weekly nor monthly. Skipped.", questId);
                continue;
            }
    ...
    // load saved spawns from character DB
    {
        ...
        if (result)
        {
            ...
            do
            {
                ...
                {
                    SC_LOG_ERROR("sql.sql", "Table `pool_quest_save` contains reference to non-existant quest pool {}. Deleted.", poolId);
    ...
    for (auto pair : lookup)
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "Table `quest_pool_template` contains quest pool {} requesting {} spawns, but only has {} members. Requested spawns reduced.", pool.poolId, pool.numActive, pool.members.size());
        ...
        if (!doRegenerate)
        {
            ...
            for (size_t i = pool.members.size(); (i--);)
            {
                ...
                if (member.empty())
                {
                    SC_LOG_ERROR("sql.sql", "Table `quest_pool_members` contains no entries at index {} for quest pool {}. Index removed.", i, pool.poolId);
                ...
                for (auto it = member.begin(); (++it) != member.end();)
                {
                    ...
                    if (status != otherStatus)
                        SC_LOG_WARN("sql.sql", "Table `pool_quest_save` {} quest {} (in pool {}, index {}) saved, but its index is{} active (because quest {} is{} in the table). Set quest {} to {}active.", (status ? "does not have" : "has"), *it, pool.poolId, i, (status ? "" : " not"), member[0], (status ? "" : " not"), *it, (status ? "" : "in"));
                    if (otherStatus)
            ...
            for (uint32 quest : pool.activeQuests)
                SC_LOG_WARN("sql.sql", "Table `pool_quest_save` has saved quest {} for pool {}, but that quest is not part of the pool. Skipped.", quest, pool.poolId);
            ...
            {
                doRegenerate = true;
                SC_LOG_ERROR("sql.sql", "Table `pool_quest_save` has {} active members saved for pool {}, which requests {} active members. Pool spawns re-generated.", activeCount, pool.poolId, pool.numActive);
            }
        }
        ...
        for (QuestPool::Member const& member : pool.members)
        {
            for (uint32 quest : member)
            {
                ...
                if (ref)
                {
                    SC_LOG_ERROR("sql.sql", "Table `quest_pool_members` lists quest {} as member of pool {}, but it is already a member of pool {}. Skipped.", quest, pool.poolId, ref->poolId);
                    continue;
                }
                ref = &pool;
            }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} daily, {} weekly and {} monthly quest pools in {} ms", _dailyPools.size(), _weeklyPools.size(), _monthlyPools.size(), GetMSTimeDiffToNow(oldMSTime));
}
