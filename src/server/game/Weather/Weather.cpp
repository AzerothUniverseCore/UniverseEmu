    ...
    : m_map(map), m_zone(zoneId), m_weatherChances(weatherChances)
{
    ...
    SC_LOG_INFO("misc", "WORLD: Starting weather system for zone {} (change every {} minutes).", m_zone, (uint32)(m_timer.GetInterval() / (MINUTE*IN_MILLISECONDS)));
}
...
bool Weather::ReGenerate()
{
    ...
    SC_LOG_INFO("misc", "Generating a change in {} weather for zone {}.", seasonName[season], m_zone);
...
bool Weather::UpdateWeather()
{
    ...
    SC_LOG_INFO("misc", "Change the weather of zone {} to {}.", m_zone, wthstr);
