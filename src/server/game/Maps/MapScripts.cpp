...
inline Player* Map::_GetScriptPlayerSourceOrTarget(Object* source, Object* target, ScriptInfo const* scriptInfo) const
{
    ...
    if (!source && !target)
        SC_LOG_ERROR("scripts", "{} source and target objects are NULL.", scriptInfo->GetDebugInfo());
    else
    {
        ...
        if (!player)
            SC_LOG_ERROR("scripts", "{} neither source nor target object is player (source: TypeId: {}, Entry: {}, GUID: {}; target: TypeId: {}, Entry: {}, GUID: {}), skipping.",
...
inline Creature* Map::_GetScriptCreatureSourceOrTarget(Object* source, Object* target, ScriptInfo const* scriptInfo, bool bReverse) const
{
    ...
    else
    {
        ...
        if (!creature)
            SC_LOG_ERROR("scripts", "{} neither source nor target are creatures (source: TypeId: {}, Entry: {}, GUID: {}; target: TypeId: {}, Entry: {}, GUID: {}), skipping.",
                scriptInfo->GetDebugInfo(),
                source ? source->GetTypeId() : 0, source ? source->GetEntry() : 0, source ? source->GetGUID().GetCounter() : 0,
                target ? target->GetTypeId() : 0, target ? target->GetEntry() : 0, target ? target->GetGUID().GetCounter() : 0);
    }
...
inline GameObject* Map::_GetScriptGameObjectSourceOrTarget(Object* source, Object* target, ScriptInfo const* scriptInfo, bool bReverse) const
{
    ...
    else
    {
        ...
        if (!gameobject)
            SC_LOG_ERROR("scripts", "{} neither source nor target are gameobjects (source: TypeId: {}, Entry: {}, GUID: {}; target: TypeId: {}, Entry: {}, GUID: {}), skipping.",
                scriptInfo->GetDebugInfo(),
                source ? source->GetTypeId() : 0, source ? source->GetEntry() : 0, source ? source->GetGUID().GetCounter() : 0,
                target ? target->GetTypeId() : 0, target ? target->GetEntry() : 0, target ? target->GetGUID().GetCounter() : 0);
    }
...
inline Unit* Map::_GetScriptUnit(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    Unit* unit = nullptr;
    if (!obj)
        SC_LOG_ERROR("scripts", "{} {} object is NULL.", scriptInfo->GetDebugInfo(), isSource ? "source" : "target");
    else if (!obj->IsUnit())
        SC_LOG_ERROR("scripts", "{} {} object is not unit {}, skipping.",
            scriptInfo->GetDebugInfo(), isSource ? "source" : "target", obj->GetGUID().ToString());
    else
    {
        unit = obj->ToUnit();
        if (!unit)
            SC_LOG_ERROR("scripts", "{} {} object could not be cast to unit.",
...
inline Player* Map::_GetScriptPlayer(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    else
    {
        player = obj->ToPlayer();
        if (!player)
            SC_LOG_ERROR("scripts", "{} {} object is not a player {}.",
                scriptInfo->GetDebugInfo(), isSource ? "source" : "target", obj->GetGUID().ToString());
    }
    return player;
}
...
inline Creature* Map::_GetScriptCreature(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    else
    {
        ...
        if (!creature)
            SC_LOG_ERROR("scripts", "{} {} object is not a creature {}.", scriptInfo->GetDebugInfo(),
...
inline WorldObject* Map::_GetScriptWorldObject(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    if (!obj)
        SC_LOG_ERROR("scripts", "{} {} object is NULL.",
            scriptInfo->GetDebugInfo(), isSource ? "source" : "target");
    else
    {
        pWorldObject = dynamic_cast<WorldObject*>(obj);
        if (!pWorldObject)
            SC_LOG_ERROR("scripts", "{} {} object is not a world object {}.",
                scriptInfo->GetDebugInfo(), isSource ? "source" : "target", obj->GetGUID().ToString());
    }
    return pWorldObject;
}
...
inline void Map::_ScriptProcessDoor(Object* source, Object* target, ScriptInfo const* scriptInfo) const
{
    ...
    switch (scriptInfo->command)
    {
        ...
        default:
            SC_LOG_ERROR("scripts", "{} unknown command for _ScriptProcessDoor.", scriptInfo->GetDebugInfo());
            return;
    }
    if (!guid)
        SC_LOG_ERROR("scripts", "{} door guid is not specified.", scriptInfo->GetDebugInfo());
    else if (!source)
        SC_LOG_ERROR("scripts", "{} source object is NULL.", scriptInfo->GetDebugInfo());
    else if (!source->IsUnit())
        SC_LOG_ERROR("scripts", "{} source object is not unit {}, skipping.", scriptInfo->GetDebugInfo(),
            source->GetGUID().ToString());
    else
    {
        WorldObject* wSource = dynamic_cast <WorldObject*> (source);
        if (!wSource)
            SC_LOG_ERROR("scripts", "{} source object could not be cast to world object {}, skipping.",
                scriptInfo->GetDebugInfo(), source->GetGUID().ToString());
        else
        {
            GameObject* pDoor = _FindGameObject(wSource, guid);
            if (!pDoor)
                SC_LOG_ERROR("scripts", "{} gameobject was not found (guid: {}).", scriptInfo->GetDebugInfo(), guid);
            else if (pDoor->GetGoType() != GAMEOBJECT_TYPE_DOOR)
                SC_LOG_ERROR("scripts", "{} gameobject is not a door (GoType: {}, {}).",
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= GameTime::GetGameTime()))
    {
        ...
        if (step.sourceGUID)
        {
            switch (step.sourceGUID.GetHigh())
            {
                ...
                case HighGuid::Mo_Transport:
                    ...
                    break;
                default:
                    SC_LOG_ERROR("scripts", "{} source with unsupported high guid {}.",
        ...
        if (step.targetGUID)
        {
            switch (step.targetGUID.GetHigh())
            {
                ...
                case HighGuid::Mo_Transport:
                    ...
                    break;
                default:
                    SC_LOG_ERROR("scripts", "{} target with unsupported high guid {}.",
        ...
        switch (step.script->command)
        {
            case SCRIPT_COMMAND_TALK:
            {
                ...
                {
                    SC_LOG_ERROR("scripts", "{} invalid chat type ({}) specified, skipping.", step.script->GetDebugInfo(), step.script->Talk.ChatType);
                    break;
                }
                ...
                if (source)
                {
                    ...
                    if (!sourceUnit)
                    {
                        SC_LOG_ERROR("scripts", "{} source object ({}) is not an unit, skipping.", step.script->GetDebugInfo(), source->GetGUID().ToString());
                        break;
                    }
                    ...
                    switch (step.script->Talk.ChatType)
                    {
                        ...
                        case CHAT_TYPE_BOSS_WHISPER:
                        {
                            ...
                            if (!receiver)
                                SC_LOG_ERROR("scripts", "{} attempt to whisper to non-player unit, skipping.", step.script->GetDebugInfo());
                            else
            ...
            case SCRIPT_COMMAND_FIELD_SET:
                ...
                if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
                {
                    ...
                    if (step.script->FieldSet.FieldID <= OBJECT_FIELD_ENTRY || step.script->FieldSet.FieldID >= cSource->GetValuesCount())
                        SC_LOG_ERROR("scripts", "{} wrong field {} (max count: {}) in object (TypeId: {}, {}) specified, skipping.",
            ...
            case SCRIPT_COMMAND_FLAG_SET:
                ...
                if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
                {
                    ...
                    if (step.script->FlagToggle.FieldID <= OBJECT_FIELD_ENTRY || step.script->FlagToggle.FieldID >= cSource->GetValuesCount())
                        SC_LOG_ERROR("scripts", "{} wrong field {} (max count: {}) in object {} specified, skipping.",
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                if (!source)
                {
                    SC_LOG_ERROR("scripts", "{} source object is NULL.", step.script->GetDebugInfo());
                    break;
                }
                if (!target)
                {
                    SC_LOG_ERROR("scripts", "{} target object is NULL.", step.script->GetDebugInfo());
                    break;
                }
                ...
                if (player)
                {
                    ...
                    {
                        SC_LOG_ERROR("scripts", "{} source is not unit, gameobject or player {}, skipping.",
                ...
                else
                {
                    ...
                    if (player)
                    {
                        ...
                        {
                            SC_LOG_ERROR("scripts", "{} target is not unit, gameobject or player {}, skipping.",
                    ...
                    {
                        SC_LOG_ERROR("scripts", "{} neither source nor target is player (source: {}; target: {}), skipping.",
                            step.script->GetDebugInfo(), source->GetGUID().ToString(),
            ...
            case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
                ...
                {
                    SC_LOG_ERROR("scripts", "{} gameobject guid (datalong) is not specified.", step.script->GetDebugInfo());
                    break;
                }
                ...
                if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
                {
                    ...
                    if (!pGO)
                    {
                        SC_LOG_ERROR("scripts", "{} gameobject was not found (guid: {}).", step.script->GetDebugInfo(), step.script->RespawnGameobject.GOGuid);
                        break;
                    }
                    ...
                    {
                        SC_LOG_ERROR("scripts", "{} can not be used with gameobject of type {} (guid: {}).",
            ...
            case SCRIPT_COMMAND_TEMP_SUMMON_CREATURE:
            {
                ...
                if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
                {
                    if (!step.script->TempSummonCreature.CreatureEntry)
                        SC_LOG_ERROR("scripts", "{} creature entry (datalong) is not specified.", step.script->GetDebugInfo());
                    else
                    {
                        ...
                        if (!pSummoner->SummonCreature(step.script->TempSummonCreature.CreatureEntry, x, y, z, o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, Milliseconds(step.script->TempSummonCreature.DespawnDelay)))
                            SC_LOG_ERROR("scripts", "{} creature was not spawned (entry: {}).", step.script->GetDebugInfo(), step.script->TempSummonCreature.CreatureEntry);
                    }
                }
                break;
            }
            ...
            case SCRIPT_COMMAND_ACTIVATE_OBJECT:
                ...
                if (Unit* unit = _GetScriptUnit(source, true, step.script))
                {
                    ...
                    {
                        SC_LOG_ERROR("scripts", "{} target object is not gameobject {}, skipping.",
                            step.script->GetDebugInfo(), target->GetGUID().ToString());
                        break;
                    }
            ...
            case SCRIPT_COMMAND_CAST_SPELL:
            {
                if (!source && !target)
                {
                    SC_LOG_ERROR("scripts", "{} source and target objects are NULL.", step.script->GetDebugInfo());
                    break;
                }
                ...
                if (!uSource)
                {
                    SC_LOG_ERROR("scripts", "{} no source worldobject found for spell {}", step.script->GetDebugInfo(), step.script->CastSpell.SpellID);
                    break;
                }

                if (!uTarget)
                {
                    SC_LOG_ERROR("scripts", "{} no target worldobject found for spell {}", step.script->GetDebugInfo(), step.script->CastSpell.SpellID);
                    break;
                }
            ...
            case SCRIPT_COMMAND_LOAD_PATH:
                ...
                if (Unit* unit = _GetScriptUnit(source, true, step.script))
                {
                    if (!sWaypointMgr->GetPath(step.script->LoadPath.PathID))
                        SC_LOG_ERROR("scripts", "{} source object has an invalid path ({}), skipping.", step.script->GetDebugInfo(), step.script->LoadPath.PathID);
                    else
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                {
                    SC_LOG_ERROR("scripts", "{} creature entry is not specified, skipping.", step.script->GetDebugInfo());
                    break;
                }
                if (!step.script->CallScript.ScriptID)
                {
                    SC_LOG_ERROR("scripts", "{} script id is not specified, skipping.", step.script->GetDebugInfo());
                    break;
                }
                ...
                if (!cTarget)
                {
                    SC_LOG_ERROR("scripts", "{} target was not found (entry: {})", step.script->GetDebugInfo(), step.script->CallScript.CreatureEntry);
                    break;
                }
                ...
                if (!datamap)
                {
                    SC_LOG_ERROR("scripts", "{} unknown scriptmap ({}) specified, skipping.", step.script->GetDebugInfo(), step.script->CallScript.ScriptType);
                    break;
                }
            ...
            case SCRIPT_COMMAND_KILL:
                ...
                {
                    if (cSource->isDead())
                        SC_LOG_ERROR("scripts", "{} creature is already dead {}",
            ...
            default:
                SC_LOG_ERROR("scripts", "Unknown script command {}.", step.script->GetDebugInfo());
                break;
        }
