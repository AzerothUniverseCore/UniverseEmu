...
InstanceMap* MapInstanced::CreateInstance(uint32 InstanceId, InstanceSave* save, Difficulty difficulty, TeamId InstanceTeam)
{
    ...
    if (!entry)
    {
        SC_LOG_ERROR("maps", "CreateInstance: no entry for map {}", GetId());
        ABORT();
    }
    InstanceTemplate const* iTemplate = sObjectMgr->GetInstanceTemplate(GetId());
    if (!iTemplate)
    {
        SC_LOG_ERROR("maps", "CreateInstance: no instance template for map {}", GetId());
        ABORT();
    }
    ...
    SC_LOG_DEBUG("maps", "MapInstanced::CreateInstance: {} map instance {} for {} created with difficulty {}", save ? "" : "new ", InstanceId, GetId(), static_cast<uint32>(difficulty));
    ...
    Syphrena::unique_trackable_ptr<Map>& ptr = m_InstancedMaps[InstanceId];
    ptr.reset(map);
    map->SetWeakPtr(ptr);
...
BattlegroundMap* MapInstanced::CreateBattleground(uint32 InstanceId, Battleground* bg)
{
    ...
    SC_LOG_DEBUG("maps", "MapInstanced::CreateBattleground: map bg {} for {} created.", InstanceId, GetId());
