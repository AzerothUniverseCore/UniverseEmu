...
void MapManager::Initialize()
{
    ...
    if (num_threads > 1)
    {
        ...
        SC_LOG_ERROR("maps", "Map update threads set to {}, when Eluna only allows 1, changing to 1", num_threads);
        num_threads = 1;
    }
#endif
...
Map* MapManager::CreateBaseMap(uint32 id)
{
    ...
    if (map == nullptr)
    {
        ...
        Syphrena::unique_trackable_ptr<Map>& ptr = i_maps[id];
        ptr.reset(map);
        map->SetWeakPtr(ptr);
...
Map::EnterState MapManager::PlayerCannotEnter(uint32 mapid, Player* player, bool loginCheck)
{
    ...
    if (!player->IsAlive())
    {
        if (player->HasCorpse())
        {
            ...
            SC_LOG_DEBUG("maps", "MAP: Player '{}' has corpse in instance '{}' and can enter.", player->GetName(), mapName);
        }
        else
        {
            SC_LOG_DEBUG("maps", "Map::CanPlayerEnter - player '{}' is dead but does not have a corpse!", player->GetName());
        return Map::CANNOT_ENTER_CORPSE_IN_DIFFERENT_INSTANCE;
        }
    }
...
uint32 MapManager::GenerateInstanceId()
{
    ...
    {
        SC_LOG_ERROR("maps", "Instance ID overflow!! Can't continue, shutting down server. ");
