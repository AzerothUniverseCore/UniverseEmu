    ...
    struct shaman_botAI : public bot_ai
    {
        ...
        void CheckTotems(uint32 diff)
        {
            ...
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                    ...
                    me->GetDistance(_totems[i].second._pos) > _totems[i].second._effradius)
                {
                    ...
                    if (!to)
                    {
                        SC_LOG_ERROR("entities.player", "{} has unexpectingly lost totem in slot {}!", me->GetName(), i);
        ...
        void CheckEarthShield(uint32 diff)
        {
            ...
            else
            {
                ...
                if (!tanks.empty())
                {
                    Unit* target = tanks.size() == 1 ? *tanks.begin() : Syphrena::Containers::SelectRandomContainerElement(tanks);
        ...
        void JustSummoned(Creature* summon) override
        {
            if (summon->GetEntry() == BOT_PET_SPIRIT_WOLF)
            {
                ...
                if (!found)
                {
                    SC_LOG_ERROR("entities.unit", "Shaman_bot:JustSummoned() wolves array is full");
                    ASSERT(false);
                }
            }
        }
        ...
        void SummonedCreatureDespawn(Creature* summon) override
        {
            ...
            if (summon->GetEntry() == BOT_PET_SPIRIT_WOLF)
            {
                ...
                //if (!found)
                //{
                //    SC_LOG_ERROR("entities.unit", "Shaman_bot:SummonedCreatureDespawn() wolf is not found in array");
                //    ASSERT(false);
                //}
            }
        }
        ...
        void UnsummonAll() override
        {
            ...
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                if (_totems[i].first != ObjectGuid::Empty)
                {
                    ...
                    if (!to)
                    {
                        //SC_LOG_ERROR("entities.player", "{} has no totem in slot {} during remove!", me->GetName(), i);
                        continue;
                    }
                    to->ToTotem()->UnSummon();
                }
        ...
        void OnBotDespawn(Creature* summon) override
        {
            if (!summon)
            {
                SC_LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot {} received NULL", me->GetName());
                ASSERT(false);
                //UnsummonAll();
                return;
            }
            ...
            {
                //SC_LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot {} has despawned summon {} which is not a temp summon or not a totem...", me->GetName(), summon->GetName());
                return;
            }
            ...
            switch (totem->m_Properties->Slot)
            {
                ...
                default:
                    SC_LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot {} has despawned totem {} in unknown slot {}", me->GetName(), summon->GetName(), totem->m_Properties->ID);
                    return;
            }

            if (_totems[slot].first == ObjectGuid::Empty)
                SC_LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot {} has despawned totem {} while not having it registered!", me->GetName(), summon->GetName());
            else if (_totems[slot].second._type == BOT_TOTEM_NONE || _totems[slot].second._type >= BOT_TOTEM_END)
                SC_LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot {} has despawned totem {} with no type assigned!", me->GetName(), summon->GetName());

            //here we reset totem category cd (not totem spell cd) if totem is destroyed
            //SC_LOG_ERROR("entities.player", "OnBotDespawn(): {} despawned ({} : {})", summon->GetName(), summon->IsAlive() ? "alive" : summon->isDying() ? "justdied" : "unk", (uint32)summon->getDeathState());
        ...
        void OnBotSummon(Creature* summon) override
        {
            ...
            if (!totem || !totem->IsTotem())
            {
                //SC_LOG_ERROR("entities.player", "OnBotSummon(): Shaman bot {} has summoned creature {} which is not a temp summon or not a totem...", me->GetName(), summon->GetName());
                return;
            }
            ...
            switch (totem->m_Properties->Slot)
            {
                ...
                case SUMMON_SLOT_TOTEM_AIR:     slot = T_AIR;   break;
                default:
                    SC_LOG_ERROR("entities.player", "OnBotSummon(): Shaman bot {} has summoned totem {} with unknown type {}", me->GetName(), summon->GetName(), totem->m_Properties->ID);
                    return;
            }
            ...
            switch (createSpell)
            {
                ...
                default:
                {
                    SC_LOG_ERROR("scripts", "Unknown totem create spell {}!", createSpell);
            ...
            //SC_LOG_ERROR("entities.player", "shaman bot: summoned {} (type {}) at x={}, y={}, z={}",
    ...
    private:
        ...
        uint32 _getTotemsMask(std::map<uint32 /*type*/, uint32 /*curId*/>& idMap) const
        {
            ...
            for (Unit::AuraApplicationMap::const_iterator itr = aurapps.begin(); itr != aurapps.end(); ++itr)
            {
                ...
                //{
                //    SC_LOG_ERROR("spells","totemMask: unit {}, {} ({}), owner {} (crSp {}, base {}), istotem {}", target->GetName(),
