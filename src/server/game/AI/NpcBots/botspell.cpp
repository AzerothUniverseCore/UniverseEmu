...
void GenerateBotCustomSpellProcs()
{
    ...
    for (auto const& p : botSpellInfoOverrides)
    {
        ...
        if (!procSpellTypeMask)
        {
            for (SpellEffectInfo const& spellEffectInfo : spellInfo.GetEffects())
            {
                ...
                {
                    SC_LOG_ERROR("scripts", "Bot spell {} has ProcFlags {}, but it's of non-proc aura type, needs a correction", spellInfo.Id, spellInfo.ProcFlags);
                    break;
                }
            }
            continue;
    ...
    SC_LOG_INFO("server.loading", ">> Bot spell proc overrides generated for {} spells", uint32(botSpellProcOverrides.size()));
...
void GenerateBotCustomSpells()
{
    ...
    */
    ...
    SC_LOG_INFO("server.loading", ">> Bot spellInfo overrides generated for {} spells", uint32(botSpellInfoOverrides.size()));
