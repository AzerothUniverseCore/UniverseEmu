...
#include "World.h"
/*
NpcBot Pet System by Trickerer (https://github.com/trickerer/Syphrena-Bots; onlysuffering@gmail.com)
*/
...
void bot_pet_ai::CureGroup(uint32 cureSpell, uint32 diff)
{
    ...
    if (IAmFree())
    {
        ...
        if (!cureTargets.empty())
            me->CastSpell(Syphrena::Containers::SelectRandomContainerElement(cureTargets), cureSpell, false);
    ...
    if (!targets.empty())
        me->CastSpell(Syphrena::Containers::SelectRandomContainerElement(targets), cureSpell, false);
}
...
uint32 bot_pet_ai::GetData(uint32 data) const
{
    switch (data)
    {
        ...
        case BOTPETAI_MISC_MAX_ATTACKERS:
            return 0;
        default:
            SC_LOG_DEBUG("entities.unit", "bot_pet_ai::GetData(): unk data type {}!", data);
            return 0;
    }
}
...
void bot_pet_ai::SetPetStats(bool force)
{
    switch (myType)
    {
        ...
        case BOT_PET_LOCUST_SWARM:
            break;
        default:
            SC_LOG_ERROR("entities.player", "bot_pet_ai::SetPetStats(): unk pet type {}, aborting", myType);
            return;
    }
    ...
    if (force)
    {
        ...
        else
        {
            ...
            SC_LOG_ERROR("entities.unit", "SetPetStats(): pInfo is NULL, setting default stats for pet {}", myType);
        }
    }
    ...
    switch (petOwner->GetBotClass())
    {
        case BOT_CLASS_HUNTER:
            atpower += (level >= 90 ? 0.338f : 0.22f) * petOwner->GetTotalAttackPowerValue(RANGED_ATTACK);
            //Hunter vs. Wild
        ...
        case BOT_CLASS_WARLOCK:
            ...
            //SC_LOG_ERROR("entities.player", "SetPetStat(): atpower += 0.57 of {} = {}2f", spdtotal, atpower);
            break;
        case BOT_CLASS_DREADLORD:
            atpower += spdtotal * 6;
            break;
    ...
    if (AuraEffect* spelldam = me->GetAuraEffect(SPELLDAMAGE_PASSIVE, 0, me->GetGUID()))
    {
        ...
        {
            case BOT_CLASS_HUNTER:
                amount += int32(petOwner->GetTotalAttackPowerValue(RANGED_ATTACK) * (level >= 90 ? 0.18f : 0.1287f));
                break;
            case BOT_CLASS_WARLOCK:
    ...
    {
        case BOT_CLASS_HUNTER:
            stamValue += (level >= 90 ? 0.63f : 0.4f) * petOwner->GetBotAI()->GetTotalBotStat(BOT_STAT_MOD_STAMINA);
            break;
        case BOT_CLASS_WARLOCK:
    ...
    //SC_LOG_ERROR("entities.player", "SetPetStat(): hp stamval {}1f, stammult {}1f, base {}, total {}2f", stamValue, stamMult, botPet->GetCreateHealth(), m_totalhp);
    ...
    if (me->GetPowerType() == POWER_MANA)
    {
        ...
        //SC_LOG_ERROR("entities.player", "SetPetStat(): mana intValue {}1f, intMult {}1f, base {}, total {}2f", intValue, intMult, botPet->GetCreatePowerValue(POWER_MANA), m_totalmana);
...
void bot_pet_ai::RefreshAura(uint32 spellId, int8 count, Unit* target) const
{
    if (count < 0 || count > 10)
    {
        SC_LOG_ERROR("entities.player", "bot_pet_ai::RefreshAura(): count is out of bounds ({}) for bot {} (botclass: {}, entry: {})",
            int32(count), me->GetName(), uint32(petOwner->GetBotClass()), me->GetEntry());
        return;
    }
    if (!spellId)
    {
        SC_LOG_ERROR("entities.player", "bot_pet_ai::RefreshAura(): spellId is 0 for bot {} (botclass: {}, entry: {})",
    ...
    if (!spellInfo)
    {
        SC_LOG_ERROR("entities.player", "bot_pet_ai::RefreshAura(): Invalid spellInfo for spell {}! Bot - {} (botclass: {}, entry: {})",
...
void bot_pet_ai::InitSpellMap(uint32 basespell, bool forceadd, bool forwardRank)
{
    ...
    if (!info)
    {
        SC_LOG_ERROR("entities.player", "bot_pet_ai::InitSpellMap(): No SpellInfo found for base spell {}", basespell);
...
void bot_pet_ai::ReleaseSpellCooldown(uint32 basespell)
{
    ...
    {
        SC_LOG_ERROR("spells", "bot_pet_ai::ReleaseSpellCooldown is called for wrong spell {}!", basespell);
        return;
    }
...
void bot_pet_ai::AdjustTankingPosition() const
{
    ...
    //SC_LOG_ERROR("entities.player", "AdjustTankingPosition() by {}", me->GetName());
    ...
    //SC_LOG_ERROR("entities.player", "AdjustTankingPosition(): atts {}, behind {}", uint32(myattackers.size()), bCount);
...
{
    //SC_LOG_ERROR("entities.unit", "bot_pet_ai::IsSummonedBy for {} by {}", me->GetName(), summoner->GetName());
    //ASSERT(!petOwner);
...
void bot_pet_ai::OnBotPetSpellInterrupted(SpellSchoolMask schoolMask, uint32 unTimeMs)
{
    ...
    for (BotPetSpellMap::iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        //SC_LOG_ERROR("entities.player", "OnBotPetSpellInterrupted(): Adding cooldown ({}, new: {}) to spell {} (id: {}, schoolmask: {}), reqSchoolMask = {}",
...
bool bot_pet_ai::GlobalUpdate(uint32 diff)
{
    if (!petOwner)
    {
        SC_LOG_ERROR("entities.unit", "botpet:GlobalUpdate(): no owner!");
        return false;
    }
