...
public:
    ...
    struct dark_ranger_botAI : public bot_ai
    {
        ...
        void CheckBlackArrow(uint32 diff)
        {
            ...
            if (Unit* target = !targets.empty() ? Syphrena::Containers::SelectRandomContainerElement(targets) : nullptr)
            {
        ...
        void SummonBotPet(Unit* from)
        {
            ...
            {
                //SC_LOG_ERROR("entities.player", "bot_dranger_ai::SummonBotPet(): max minions");
                Unit* u = nullptr;
                ...
                //if (u)
                //    SC_LOG_ERROR("entities.player", "bot_dranger_ai::SummonBotPet(): found minion to erase(1)");
                //try 3: last resort
                if (!u)
                    u = *(_minions.begin());
                //if (u)
                //    SC_LOG_ERROR("entities.player", "bot_dranger_ai::SummonBotPet(): found minion to erase(2)");
