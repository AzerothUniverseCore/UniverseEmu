...
public:
    ...
    void AbortMe()
    {
        SC_LOG_ERROR("npcbots", "BotBattlegroundEnterEvent: Aborting bot {} bg {}!", _botGUID.GetEntry(), uint32(_bgQueueTypeId));
    ...
    void AbortAll()
    {
        SC_LOG_ERROR("npcbots", "BotBattlegroundEnterEvent: Aborting ALL bots by {} bg {}!", _playerGUID.GetCounter(), uint32(_bgQueueTypeId));
        AbortMe();
...
void SpawnWandererBot(uint32 bot_id, WanderNode const* spawnLoc, NpcBotRegistry* registry)
{
    ...
    SC_LOG_DEBUG("npcbots", "Spawning wandering bot: {} ({}) class {} race {} fac {}, location: mapId {} {} ({})",
    ...
    {
        delete bot;
        SC_LOG_FATAL("server.loading", "Cannot load npcbot from DB!");
        ASSERT(false);
    }
...
void BotDataMgr::DespawnWandererBot(uint32 entry)
{
    ...
    if (bot && bot->IsWandererBot())
    {
        ...
        SC_LOG_ERROR("npcbots", "DespawnWandererBot(): trying to despawn non-existing wanderer bot {} '{}'!", entry, bot ? bot->GetName() : "unknown");
}
...
private:
    ...
    WanderingBotsGenerator()
    {
        ...
        if (!result)
        {
            SC_LOG_WARN("server.loading", "Next bot id for autogeneration is not found! Resetting! (client cache may interfere with names)");
        ...
        bool immediate, PvPDifficultyEntry const* bracketEntry, NpcBotRegistry* registry)
    {
        ...
        auto const& spareBotPair = Syphrena::Containers::SelectRandomContainerElement(spareBotIdsPerClass);
        const uint8 bot_class = spareBotPair.first;
        auto const& cSet = spareBotPair.second;
        ASSERT(!cSet.empty());
        uint32 orig_entry = cSet.size() == 1 ? *cSet.cbegin() : Syphrena::Containers::SelectRandomContainerElement(cSet);
        ...
        WanderNode const* spawnLoc = Syphrena::Containers::SelectRandomContainerElement(level_nodes);
        ...
        CellCoord c = Syphrena::ComputeCellCoord(spawnLoc->m_positionX, spawnLoc->m_positionY);
        GridCoord g = Syphrena::ComputeGridCoord(spawnLoc->m_positionX, spawnLoc->m_positionY);
...
public:
    ...
    bool GenerateWanderingBotsToSpawn(uint32 count, int32 map_id, int32 team, bool immediate, PvPDifficultyEntry const* bracketEntry, NpcBotRegistry* registry, uint32& spawned)
    {
        ...
        Syphrena::Containers::RandomShuffle(brackets_shuffled);
...
void BotDataMgr::Update(uint32 diff)
{
    ...
    if (!_botsWanderCreaturesToDespawn.empty())
    {
        ...
        while (!_botsWanderCreaturesToDespawn.empty())
        {
            ...
            SC_LOG_DEBUG("npcbots", "Despawned wanderer bot {} '{}' (orig {})", bot_despawn_id, botName, origEntry);
        }
    }
...
void BotDataMgr::LoadNpcBots(bool spawn)
{
    ...
    SC_LOG_INFO("server.loading", "Starting NpcBot system...");
    ...
    if (result)
    {
        do
        {
            ...
            {
                SC_LOG_ERROR("server.loading", "Bot entry {} has appearance data but doesn't exist in `creature_template` table! Skipped.", entry);
                continue;
            }
        ...
        SC_LOG_INFO("server.loading", ">> Bot appearance data loaded");
    }
    else
        SC_LOG_INFO("server.loading", ">> Bots appearance data is not loaded. Table `creature_template_npcbot_appearance` is empty!");
    ...
    if (result)
    {
        do
        {
            ...
            if (!sObjectMgr->GetCreatureTemplate(entry))
            {
                SC_LOG_ERROR("server.loading", "Bot entry {} has extras data but doesn't exist in `creature_template` table! Skipped.", entry);
                continue;
            }
        ...
        SC_LOG_INFO("server.loading", ">> Bot race data loaded");
    }
    else
        SC_LOG_INFO("server.loading", ">> Bots race data is not loaded. Table `creature_template_npcbot_extras` is empty!");
    ...
    if (result)
    {
        do
        {
            ...
            if (!sObjectMgr->GetCreatureTemplate(entry))
            {
                SC_LOG_ERROR("server.loading", "Bot entry {} has transmog data but doesn't exist in `creature_template` table! Skipped.", entry);
                continue;
            }
        ...
        SC_LOG_INFO("server.loading", ">> Bot transmog data loaded");
    }
    else
        SC_LOG_INFO("server.loading", ">> Bots transmog data is not loaded. Table `characters_npcbot_transmog` is empty!");
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!sObjectMgr->GetCreatureTemplate(entry))
            {
                SC_LOG_ERROR("server.loading", "Bot entry {} doesn't exist in `creature_template` table! Skipped.", entry);
                continue;
            }
            ...
            {
                std::vector<std::string_view> tok = Syphrena::Tokenize(disabled_spells_str, ' ', false);
                for (std::vector<std::string_view>::size_type i = 0; i != tok.size(); ++i)
                    botData->disabled_spells.insert(*(Syphrena::StringTo<uint32>(tok[i])));
            }
        ...
        SC_LOG_INFO("server.loading", ">> Loaded {} bot data entries", datacounter);
        ...
        if (spawn)
        {
            for (std::list<uint32>::const_iterator itr = entryList.cbegin(); itr != entryList.cend(); ++itr)
            {
                ...
                if (!infores)
                {
                    SC_LOG_ERROR("server.loading", "Cannot spawn npcbot {} (id: {}), not found in `creature` table!", proto->Name, entry);
                    continue;
                }
                ...
                CellCoord c = Syphrena::ComputeCellCoord(pos_x, pos_y);
                GridCoord g = Syphrena::ComputeGridCoord(pos_x, pos_y);
                ASSERT(c.IsCoordValid(), "Invalid Cell coord!");
                ASSERT(g.IsCoordValid(), "Invalid Grid coord!");
                ...
                SC_LOG_DEBUG("server.loading", "bot {}: spawnId {}, full {}", entry, tableGuid, Guid.ToString());
                Creature* bot = map->GetCreature(Guid);
                if (!bot) //not in map, use storage
                {
                    //SC_LOG_DEBUG("server.loading", "bot {}: spawnId {}, is not in map on load", entry, tableGuid);
                    ...
                    {
                        SC_LOG_ERROR("server.loading", "bot {} is not in spawns list, consider re-spawning it!", entry);
                        continue;
                    }
                ...
                ASSERT(bot);
                if (!bot->FindMap())
                    SC_LOG_ERROR("server.loading", "bot {} is not in map!", entry);
                if (!bot->IsInWorld())
                    SC_LOG_ERROR("server.loading", "bot {} is not in world!", entry);
                if (!bot->IsAlive())
                {
                    SC_LOG_ERROR("server.loading", "bot {} is dead, respawning!", entry);
                    bot->Respawn();
                }

                SC_LOG_DEBUG("server.loading", ">> Spawned npcbot {} (id: {}, map: {}, grid: {}, cell: {})", proto->Name, entry, mapId, g.GetId(), c.GetId());
                botgrids.insert(g.GetId());
                ++botcounter;
            }

            SC_LOG_INFO("server.loading", ">> Spawned {} npcbot(s) within {} grid(s) in {} ms", botcounter, uint32(botgrids.size()), GetMSTimeDiffToNow(botoldMSTime));
        }
    }
    else
        SC_LOG_INFO("server.loading", ">> Loaded 0 npcbots. Table `characters_npcbot` is empty!");
...
{
    SC_LOG_INFO("server.loading", "Loading NPCBot group members...");
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 NPCBot group members. DB table `characters_npcbot_group_member` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            SC_LOG_WARN("server.loading", "Table `characters_npcbot_group_member` contains non-NPCBot creature {} which will not be loaded!", creature_id);
            continue;
        }
        ...
        if (Group* group = sGroupMgr->GetGroupByDbStoreId(fields[0].GetUInt32()))
        {
            ...
            SC_LOG_ERROR("misc", "BotDataMgr::LoadNpcBotGroupData: Consistency failed, can't find group (storage id: {})", fields[0].GetUInt32());

        ++count;

    } while (result->NextRow());

    SC_LOG_INFO("server.loading", ">> Loaded {} NPCBot group members in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}

void BotDataMgr::LoadNpcBotGearStorage()
{
    SC_LOG_INFO("server.loading", "Loading NPCBot items storage...");
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 NPCBot stored gear items. DB table `characters_npcbot_gear_storage` is empty!");
        return;
    }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} NPCBot stored items for {} bot owners in {} ms", count, uint32(player_guids.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void BotDataMgr::LoadWanderMap(bool reload)
{
    ...
    SC_LOG_INFO("server.loading", "Setting up wander map...");
    ...
    if (!wres)
    {
        SC_LOG_FATAL("server.loading", "Failed to load wander points: table `creature_template_npcbot_wander_nodes` is empty!");
        ASSERT(false);
    }
    ...
    do
    {
        ...
        if (!mapEntry)
        {
            SC_LOG_ERROR("server.loading", "WP {} has invalid map id {}!", id, mapId);
            continue;
        }

        if (minLevel == 1u && maxLevel == MAX_LEVEL)
            SC_LOG_WARN("server.loading", "WP {} has no levels set.", id);

        if (!minLevel || !maxLevel || minLevel > MAX_LEVEL || maxLevel > MAX_LEVEL || minLevel > maxLevel)
        {
            SC_LOG_WARN("server.loading", "WP {} has invalid levels min {} max {}! Setting to default...",
        ...
        {
            SC_LOG_WARN("server.loading", "WP {} has invalid flags {}! Removing all invalid flags...", id, flags);
        ...
        {
            SC_LOG_WARN("server.loading", "WP {} has BG-only flags {} for non-BG map {}! Removing...", id, (flags & nonbg_flags), mapEntry->ID);
            flags &= ~nonbg_flags;
        }
        ...
        {
            SC_LOG_WARN("server.loading", "WP {} has conflicting flags {}+{}! Removing both...",
        ...
        if (lstr.empty())
        {
            SC_LOG_ERROR("server.loading", "WP {} has no links!", id);
            continue;
        }
        std::vector<std::string_view> tok = Syphrena::Tokenize(lstr, ' ', false);
        for (std::vector<std::string_view>::size_type i = 0; i != tok.size(); ++i)
        {
            std::vector<std::string_view> link_str = Syphrena::Tokenize(tok[i], ':', false);
            ...
            ASSERT(Syphrena::StringTo<uint32>(link_str[0]) != std::nullopt, "Invalid links_str format: '%s'", std::string(tok[i].data(), tok[i].length()).c_str());
            ASSERT(Syphrena::StringTo<uint32>(link_str[1]) != std::nullopt, "Invalid links_str format: '%s'", std::string(tok[i].data(), tok[i].length()).c_str());
    ...
    for (WanderNode const* wp : all_spawn_nodes)
    {
        ...
        _wpMinSpawnLevelPerMapId[mapId] = std::min<uint8>((mincit != _wpMinSpawnLevelPerMapId.cend()) ? mincit->second : uint8(MAX_LEVEL), minLevel);
    ...
    for (auto& kv : spawn_node_exists_a)
    {
        if (!kv.second)
        {
            SC_LOG_FATAL("server.loading", "No valid Alliance spawn node for at least level {} on map {}! Spawning wandering bots is impossible! Aborting.",
    ...
    for (auto& kv : spawn_node_exists_h)
    {
        if (!kv.second)
        {
            SC_LOG_FATAL("server.loading", "No valid Horde spawn node for at least level {} on map {}! Spawning wandering bots is impossible! Aborting.",
                maxof_minclasslvl_nr, kv.first);
            spawn_node_minclasslvl_exists_all = false;
        }
    }
    for (auto& kv : spawn_node_exists_n)
    {
        if (!kv.second)
        {
            if (sMapStore.LookupEntry(kv.first)->IsBattlegroundOrArena())
                SC_LOG_INFO("server.loading", "No valid Neutral spawn node for at least level {} on non-continent map {}.", maxof_minclasslvl_ex, kv.first);
            else
            {
                SC_LOG_FATAL("server.loading", "No valid Neutral spawn node for at least level {} on map {}! Spawning wandering bots is impossible! Aborting.",
    ...
    std::array<bool, MAX_LEVEL> spawn_node_levels[TEAMS_COUNT]{ { false } };
    uint8 min_spawn_level = MAX_LEVEL;
    ...
    for (uint8 k = 0; k < TEAMS_COUNT; ++k)
    {
        ...
        {
            if (vec[i - 1] == false)
                SC_LOG_ERROR("server.loading", "No {} spawn node found for level {}! Wandering bots may cause a crash!", team_strs[k], i);
        }
    }
    ...
    for (auto const& vt : links_to_create)
    {
        ...
        {
            uint32 lid = *Syphrena::StringTo<uint32>(p.first);
            if (lid == vt.first)
            {
                SC_LOG_ERROR("server.loading", "WP {} has link {} which links to itself! Skipped.", vt.first, lid);
                continue;
            }
            ...
            if (!lwp)
            {
                SC_LOG_ERROR("server.loading", "WP {} has link {} which does not exist!", vt.first, lid);
                continue;
            }
            if (lwp->GetMapId() != vt.second.first->GetMapId())
            {
                SC_LOG_ERROR("server.loading", "WP {} map {} has link {} ON A DIFFERENT MAP {}!", vt.first, vt.second.first->GetMapId(), lid, lwp->GetMapId());
                continue;
            }
            ...
            if (lwpdist2d > MAX_WANDER_NODE_DISTANCE)
                SC_LOG_WARN("server.loading", "Warning! Link distance between WP {} and {} is too great ({})", vt.first, lid, lwpdist2d);
            if (lwpdist2d < MIN_WANDER_NODE_DISTANCE && is_continent)
                SC_LOG_WARN("server.loading", "Warning! Link distance between WP {} and {} is low ({})", vt.first, lid, lwpdist2d);
    ...
    WanderNode::DoForAllWPs([&](WanderNode const* wp) {
        ...
        {
            SC_LOG_DEBUG("server.loading", "Node {} ('{}') has single connection!", wp->GetWPId(), wp->GetName());
            ...
            {
                SC_LOG_DEBUG("server.loading", "Node {} ('{}') has single connection!", tn->GetWPId(), tn->GetName());
                ...
                ss << " is isolated!";
                SC_LOG_INFO("server.loading", "{}", ss.str());
            }
        }
    });

    SC_LOG_INFO("server.loading", ">> Loaded {} bot wander nodes ({} disabled) on {} maps (total {} tops) in {} ms",
...
void BotDataMgr::GenerateWanderingBots()
{
    ...
    SC_LOG_INFO("server.loading", "Spawning wandering bots...");
    ...
    {
        SC_LOG_FATAL("server.loading", "Only {} out of {} bots of enabled classes aren't spawned. Desired amount of wandering bots ({}) cannot be created. Aborting!",
    ...
    {
        SC_LOG_FATAL("server.loading", "Failed to spawn all {} bots ({} succeeded)!", wandering_bots_desired, spawned_count);
        ASSERT(false);
    }
...
bool BotDataMgr::GenerateBattlegroundBots(Player const* groupLeader, [[maybe_unused]] Group const* group, BattlegroundQueue* queue, PvPDifficultyEntry const* bracketEntry, GroupQueueInfo const* gqinfo)
{
    ...
    if (tarteamplayers == 0)
    {
        SC_LOG_INFO("npcbots", "[Disabled] BG {} wandering bots generation is disabled (not implemented?)", uint32(bgTypeId));
        return true;
    }
    ...
    for (auto const& kv : all_bgs)
    {
        if (kv.first == bgTypeId)
        {
            for (auto const& real_bg_pair : kv.second.m_Battlegrounds)
            {
                ...
                {
                    SC_LOG_INFO("npcbots", "[Already running] Found running BG {} inited by player {} ({}). Not generating bots",
    ...
    {
        SC_LOG_INFO("npcbots", "[No bots required] Failed to generate bots for BG {} inited by player {} ({})",
            uint32(bgTypeId), groupLeader->GetName(), groupLeader->GetGUID().GetCounter());
        return true;
    }
    ...
    {
        SC_LOG_INFO("npcbots", "[Not enough A bots] Failed to generate bots for BG {} inited by player {} ({})",
            uint32(bgTypeId), groupLeader->GetName(), groupLeader->GetGUID().GetCounter());
        return false;
    }
    if (queued_players_h + spare_bots_h < minteamplayers)
    {
        SC_LOG_INFO("npcbots", "[Not enough H bots] Failed to generate bots for BG {} inited by player {} ({})",
            uint32(bgTypeId), groupLeader->GetName(), groupLeader->GetGUID().GetCounter());
        return false;
    }
    ...
    if (needed_bots_count_a)
    {
        ...
        {
            SC_LOG_WARN("npcbots", "Failed to spawn {} ALLIANCE bots for BG {} '{}' queued A {} H {} req A {} H {} spare {}",
    ...
    if (needed_bots_count_h)
    {
        ...
        {
            SC_LOG_WARN("npcbots", "Failed to spawn {} HORDE bots for BG {} '{}' queued A {} H {} req A {} H {} spare {}",
...
{
    SC_LOG_INFO("server.loading", "Sorting wandering bot's gear...");
    ...
    if (dires)
    {
        ...
        SC_LOG_INFO("server.loading", ">> Loaded {} disabled wandering bots gear items", uint32(disabled_item_ids.size()));
    }
    else
        SC_LOG_INFO("server.loading", ">> Loaded 0 disabled wandering bots gear items. Table `creature_template_npcbot_disabled_items` is empty!");
    ...
    for (auto const& kv : all_item_templates)
    {
        ...
        switch (proto.Quality)
        {
            ...
            case ITEM_QUALITY_RARE:
                if (proto.RequiredLevel > 75 && proto.RequiredLevel < 90)
                    skip = true;
            [[fallthrough]];
            case ITEM_QUALITY_EPIC:
                if (!(proto.RequiredLevel >= 2 && proto.RequiredLevel <= MAX_LEVEL))
                    skip = true;
        ...
        {
            //SC_LOG_INFO("server.loading", "Item {} is disabled...", proto.ItemId);
            continue;
        }
        ...
        for (uint32 s = BOT_SLOT_MAINHAND; s < BOT_INVENTORY_SIZE; ++s)
        {
            ...
            for (uint32 lstep = 0; lstep < LEVEL_STEPS; ++lstep)
            {
                ...
                if (il_arr[lstep].empty())
                {
                    ...
                    //SC_LOG_WARN("server.loading", "No items for class {} slot {} at levels {}-{}!", c, s, minlvl, maxlvl);
                }
            }
        }
    }

    SC_LOG_INFO("server.loading", ">> Sorted wandering bots gear in {} ms", GetMSTimeDiffToNow(oldMSTime));
}
...
Item* BotDataMgr::GenerateWanderingBotItem(uint8 slot, uint8 botclass, uint8 level, std::function<bool(ItemTemplate const*)>&& check)
{
    ...
    if (!itemIdVec->empty())
    {
        ...
        if (!validVec.empty())
        {
            uint32 itemId = Syphrena::Containers::SelectRandomContainerElement(validVec);
...
bool BotDataMgr::GenerateWanderingBotItemEnchants(Item* item, uint8 slot, uint8 spec)
{
    ...
    uint32 enchant_id;
    enchant_id = valid_enchant_ids.empty() ? 0 : valid_enchant_ids.size() == 1u ? valid_enchant_ids.front() : Syphrena::Containers::SelectRandomContainerElement(valid_enchant_ids);
    if (enchant_id)
    {
...
void BotDataMgr::AddNpcBotData(uint32 entry, uint32 roles, uint8 spec, uint32 faction)
{
    ...
    SC_LOG_ERROR("sql.sql", "BotMgr::AddNpcBotData(): trying to add new data but entry already exists! entry = {}", entry);
}
...
void BotDataMgr::UpdateNpcBotData(uint32 entry, NpcBotDataUpdateType updateType, void* data)
{
    ...
    switch (updateType)
    {
        ...
        default:
            SC_LOG_ERROR("sql.sql", "BotDataMgr:UpdateNpcBotData: unhandled updateType {}", uint32(updateType));
            break;
    }
}
void BotDataMgr::UpdateNpcBotDataAll(uint32 playerGuid, NpcBotDataUpdateType updateType, void* data)
{
    ...
    switch (updateType)
    {
        ...
        //case NPCBOT_UPDATE_EQUIPS:
        default:
            SC_LOG_ERROR("sql.sql", "BotDataMgr:UpdateNpcBotDataAll: unhandled updateType {}", uint32(updateType));
            break;
    }
}
...
void BotDataMgr::RegisterBot(Creature const* bot)
{
    ...
    {
        SC_LOG_ERROR("entities.unit", "BotDataMgr::RegisterBot: bot {} ({}) already registered!",
    ...
    //SC_LOG_ERROR("entities.unit", "BotDataMgr::RegisterBot: registered bot {} ({})", bot->GetEntry(), bot->GetName());
}
void BotDataMgr::UnregisterBot(Creature const* bot)
{
    if (_existingBots.find(bot) == _existingBots.end())
    {
        SC_LOG_ERROR("entities.unit", "BotDataMgr::UnregisterBot: bot {} ({}) not found!",
            bot->GetEntry(), bot->GetName());
        return;
    }
    ...
    //SC_LOG_ERROR("entities.unit", "BotDataMgr::UnregisterBot: unregistered bot {} ({})", bot->GetEntry(), bot->GetName());
}
...
WanderNode const* BotDataMgr::GetNextWanderNode(WanderNode const* curNode, WanderNode const* lastNode, Position const* fromPos, Creature const* bot, uint8 lvl, bool random)
{
    ...
    if (curNode->GetLinks().empty() || random)
    {
        if (bot->IsInWorld() && !bot->GetMap()->IsBattlegroundOrArena())
        {
            ...
            if (!links.empty())
                return links.size() == 1u ? links.front() : Syphrena::Containers::SelectRandomContainerElement(links);
        }
    ...
    if (bot_ai::IsFlagCarrier(bot))
    {
        ...
        if (!flagDropNodes.empty())
            return flagDropNodes.size() == 1u ? flagDropNodes.front() : Syphrena::Containers::SelectRandomContainerElement(flagDropNodes);
    }
...
class SC_GAME_API WanderingBotXpGainFormulaScript : public FormulaScript
{
...
class SC_GAME_API BotDataMgrShutdownScript : public WorldScript
{
public:
