...
uint32 _targetBGPlayersPerTeamCount_BFG;
uint32 _targetBGPlayersPerTeamCount_TP;
uint32 _targetBGPlayersPerTeamCount_VOP;
bool _enableNpcBots;
...
bool _enableclass_bloodmage;
bool _enableclass_druid;
bool _enableclass_knight;
bool _enableclass_demon_hunter;
bool _enableclass_monk;
bool _enableclass_tamer;
bool _enableclass_hero;
bool _enableclass_evoker;
bool _enableclass_chronomancer;
...
float _mult_dmg_bloodmage;
float _mult_dmg_druid;
float _mult_dmg_knight;
float _mult_dmg_demon_hunter;
float _mult_dmg_monk;
float _mult_dmg_tamer;
float _mult_dmg_hero;
float _mult_dmg_evoker;
float _mult_dmg_chronomancer;
...
void BotMgr::LoadConfig(bool reload)
{
    ...
    _mult_dmg_bloodmage             = sConfigMgr->GetFloatDefault("NpcBot.Mult.Damage.BloodMage", 1.0f);
    _mult_dmg_druid                 = sConfigMgr->GetFloatDefault("NpcBot.Mult.Damage.Druid", 1.0f);
    _mult_dmg_knight                = sConfigMgr->GetFloatDefault("NpcBot.Mult.Damage.Knight", 1.0f);
    _mult_dmg_demon_hunter          = sConfigMgr->GetFloatDefault("NpcBot.Mult.Damage.DemonHunter", 1.0f);
    _mult_dmg_monk                  = sConfigMgr->GetFloatDefault("NpcBot.Mult.Damage.Monk", 1.0f);
    _mult_dmg_tamer                 = sConfigMgr->GetFloatDefault("NpcBot.Mult.Damage.Tamer", 1.0f);
    _mult_dmg_hero                  = sConfigMgr->GetFloatDefault("NpcBot.Mult.Damage.Hero", 1.0f);
    _mult_dmg_evoker                = sConfigMgr->GetFloatDefault("NpcBot.Mult.Damage.Evoker", 1.0f);
    _mult_dmg_chronomancer          = sConfigMgr->GetFloatDefault("NpcBot.Mult.Damage.Chronomancer", 1.0f);
    ...
    _enableclass_bloodmage          = sConfigMgr->GetBoolDefault("NpcBot.Classes.BloodMage.Enable", true);
    _enableclass_druid              = sConfigMgr->GetBoolDefault("NpcBot.Classes.Druid.Enable", true);
    _enableclass_knight             = sConfigMgr->GetBoolDefault("NpcBot.Classes.Knight.Enable", true);
    _enableclass_demon_hunter       = sConfigMgr->GetBoolDefault("NpcBot.Classes.DemonHunter.Enable", true);
    _enableclass_monk               = sConfigMgr->GetBoolDefault("NpcBot.Classes.Monk.Enable", true);
    _enableclass_tamer              = sConfigMgr->GetBoolDefault("NpcBot.Classes.Tamer.Enable", true);
    _enableclass_hero               = sConfigMgr->GetBoolDefault("NpcBot.Classes.Hero.Enable", true);
    _enableclass_evoker             = sConfigMgr->GetBoolDefault("NpcBot.Classes.Evoker.Enable", true);
    _enableclass_chronomancer       = sConfigMgr->GetBoolDefault("NpcBot.Classes.Chronomancer.Enable", true);
    ...
    _targetBGPlayersPerTeamCount_AB = sConfigMgr->GetIntDefault("NpcBot.WanderingBots.BG.TargetTeamPlayersCount.AB", 14);
    _targetBGPlayersPerTeamCount_EY = sConfigMgr->GetIntDefault("NpcBot.WanderingBots.BG.TargetTeamPlayersCount.EY", 14);
    ...
    std::vector<std::string_view> toks = Syphrena::Tokenize(mult_dps_by_levels, ',', false);
    ...
    {
        Optional<float> val = Syphrena::StringTo<float>(toks[i]);
        if (val == std::nullopt)
            SC_LOG_ERROR("server.loading", "NpcBot.Mult.Damage.Levels contains invalid float value '{}', set to default", toks[i]);
    ...
    std::vector<std::string_view> toks2 = Syphrena::Tokenize(wanderers_by_levels, ',', false);
    ...
    {
        Optional<uint32> val = Syphrena::StringTo<uint32>(toks2[i]);
        if (val == std::nullopt)
            SC_LOG_ERROR("server.loading", "NpcBot.Mult.Damage.Levels contains invalid uint32 value '{}', set to default", toks2[i]);
    ...
    std::string enabled_wander_node_maps = sConfigMgr->GetStringDefault("NpcBot.WanderingBots.Continents.Maps", "0,1,530,571,754,807,811");
    std::vector<std::string_view> toks3 = Syphrena::Tokenize(enabled_wander_node_maps, ',', false);
    for (decltype(toks3)::size_type i = 0; i != toks3.size(); ++i)
    {
        Optional<uint32> val = Syphrena::StringTo<uint32>(toks3[i]);
        if (val == std::nullopt)
        {
            SC_LOG_ERROR("server.loading", "NpcBot.WanderingBots.Continents.Maps contains invalid uint32 value '{}', skipped", toks3[i]);
            continue;
        }
        uint32 uval = val.value_or(uint32(0));
    ...
    {
        SC_LOG_ERROR("server.loading", "NpcBot.WanderingBots.Continents.Maps does not provide any valid maps! Wandering bots will not be spawned!");
...
void BotMgr::ResolveConfigConflicts()
{
    ...
    if (uint8 interFlags = (_noDpsTargetIconFlags & dpsFlags))
    {
        ...
        SC_LOG_ERROR("server.loading", "BotMgr::LoadConfig: NoDPSTargetIconMask intersects with dps targets flags {:#X}! Removed, new mask: {:#X}",
            uint32(interFlags), uint32(_noDpsTargetIconFlags));
    }

    if (!_enabled_wander_node_maps.empty())
    {
        uint8 minbotlevel = MAX_LEVEL;
        uint8 maxbotlevel = 0;
        ...
        for (int8 j = minbotlevel / 10 - 1; j >= 0; --j)
        {
            if (_botwanderer_pct_level_brackets[j] > 0)
            {
                ...
                SC_LOG_WARN("server.loading", "NpcBot.WanderingBots.Continents.Levels conflicts with NpcBot.WanderingBots.Continents.Maps: no map for levels {}-{}! Transferring extra {}% to levels {}-{}",
...
bool BotMgr::IsClassEnabled(uint8 m_class)
{
    switch (m_class)
    {
        ...
        case BOT_CLASS_BLOODMAGE:
            return _enableclass_bloodmage;
        case BOT_CLASS_DRUID:
            return _enableclass_druid;
        case BOT_CLASS_KNIGHT:
            return _enableclass_knight;
        case BOT_CLASS_DEMON_HUNTER:
            return _enableclass_demon_hunter;
        case BOT_CLASS_MONK:
            return _enableclass_monk;
        case BOT_CLASS_TAMER:
            return _enableclass_tamer;
        case BOT_CLASS_HERO:
            return _enableclass_hero;
        case BOT_CLASS_EVOKER:
            return _enableclass_evoker;
        case BOT_CLASS_CHRONOMANCER:
            return _enableclass_chronomancer;
        case BOT_CLASS_BM:
...
uint32 BotMgr::GetBGTargetTeamPlayersCount(BattlegroundTypeId bgTypeId)
{
    switch (bgTypeId)
    {
        ...
        case BATTLEGROUND_BFG:
            return _targetBGPlayersPerTeamCount_BFG;
        case BATTLEGROUND_TP:
            return _targetBGPlayersPerTeamCount_TP;
        case BATTLEGROUND_VOP:
            return _targetBGPlayersPerTeamCount_VOP;
        default:
            return 0;
    }
}
        ...
        if (bot->IsFreeBot())
        {
            ...
            if (newMap->IsBattleground())
            {
                ...
                if (!bot->IsAlive())
                {
                    ...
                    {
                        SC_LOG_ERROR("npcbots", "TeleportBot: Bot {} '{}' can't find SpiritHealer in bg {}!",
...
bool BotMgr::RemoveBotFromGroup(Creature* bot)
{
    ...
    //    SC_LOG_ERROR("entities.player", "RemoveBotFromGroup(): bot {} removed from group", bot->GetName());
    //else
    //    SC_LOG_ERROR("entities.player", "RemoveBotFromGroup(): RemoveMember() returned FALSE on bot {}", bot->GetName());
...
uint32 BotMgr::GetNpcBotCost(uint8 level, uint8 botclass)
{
    ...
    uint32 cost =
        ...
        (_npcBotsCost * level) / MAX_LEVEL; //50 - 100 gold
...
uint8 BotMgr::BotClassByClassName(std::string const& className)
{
    static const std::map<std::string, uint8> BotClassNamesMap = {
        ...
        { "bloodmage", BOT_CLASS_BLOODMAGE },
        { "druid", BOT_CLASS_DRUID },
        { "knight", BOT_CLASS_KNIGHT },
        { "demon_hunter", BOT_CLASS_DEMON_HUNTER },
        { "monk", BOT_CLASS_MONK },
        { "tamer", BOT_CLASS_TAMER },
        { "hero", BOT_CLASS_HERO },
        { "evoker", BOT_CLASS_EVOKER },
        { "chronomancer", BOT_CLASS_CHRONOMANCER },
    ...
    if (bot_class >= BOT_CLASS_EX_START)
    {
        switch (bot_class)
        {
            ...
            case BOT_CLASS_CRYPT_LORD:
                return BOT_CLASS_WARRIOR;
            default:
                SC_LOG_ERROR("npcbots", "GetPlayerClass: unknown Ex bot class {}!", bot_class);
                return BOT_CLASS_PALADIN;
        }
    }
    ...
    if (bot_class >= BOT_CLASS_EX_START)
    {
        switch (bot_class)
        {
            ...
            case BOT_CLASS_CRYPT_LORD:
                return RACE_UNDEAD_PLAYER;
            default:
                SC_LOG_ERROR("npcbots", "GetBotPlayerRace: unknown Ex bot class {}!", bot_class);
                return RACE_HUMAN;
        }
    }
...
int32 BotMgr::GetHPSTaken(Unit const* unit) const
{
    ...
    for (std::list<Unit*>::const_iterator itr = unitList.begin(); itr != unitList.end(); ++itr)
    {
        ...
        for (uint8 i = CURRENT_FIRST_NON_MELEE_SPELL; i != CURRENT_AUTOREPEAT_SPELL; ++i)
        {
            ...
            for (uint8 j = 0; j != MAX_SPELL_EFFECTS; ++j)
            {
                ...
                //SC_LOG_ERROR("entities.player", "BotMgr:pendingHeals: found {}'s {} on {} in {} ({}, total {})",
    ...
    //if (amount != 0)
    //    SC_LOG_ERROR("entities.player", "BotMgr:GetHPSTaken(): {} got {})", unit->GetName(), amount);
...
float BotMgr::GetBotDamageModByClass(uint8 botclass)
{
    switch (botclass)
    {
        ...
        case BOT_CLASS_BLOODMAGE:
            return _mult_dmg_bloodmage;
        case BOT_CLASS_DRUID:
            return _mult_dmg_druid;
        case BOT_CLASS_KNIGHT:
            return _mult_dmg_knight;
        case BOT_CLASS_DEMON_HUNTER:
            return _mult_dmg_demon_hunter;
        case BOT_CLASS_MONK:
            return _mult_dmg_monk;
        case BOT_CLASS_TAMER:
            return _mult_dmg_tamer;
        case BOT_CLASS_HERO:
            return _mult_dmg_hero;
        case BOT_CLASS_EVOKER:
            return _mult_dmg_evoker;
        case BOT_CLASS_CHRONOMANCER:
            return _mult_dmg_chronomancer;
        case BOT_CLASS_BM:
...
void BotMgr::InviteBotToBG(ObjectGuid botguid, GroupQueueInfo* ginfo, Battleground* bg)
{
    ...
    //SC_LOG_INFO("npcbots", "Battleground: invited NPCBot {} to BG instance {} bgtype {} '{}'",
