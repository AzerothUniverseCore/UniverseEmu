    ...
    static std::pair<uint8, uint8> GetZoneLevels(uint32 zoneId)
    {
        ...
        switch (zoneId)
        {
            ...
            case 493: // Moonglade
                return { 46, 60 };
            default:
                SC_LOG_ERROR("scripts", "GetZoneLevels: no choice for zoneId {}", zoneId);
                return { 1, 60 };
        }
    }
...
public:
    ...
    {
        CellCoord c = Syphrena::ComputeCellCoord(wp->m_positionX, wp->m_positionY);
        GridCoord g = Syphrena::ComputeGridCoord(wp->m_positionX, wp->m_positionY);
    ...
    static bool HandleNpcBotWPSetLevelsZoneCommand(ChatHandler* handler, Optional<uint8> minlevel, Optional<uint8> maxlevel)
    {
        ...
        {
            handler->PSendSysMessage("WP levels must be within bounds 1-%u, min <= max", uint32(MAX_LEVEL));
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        Optional<uint8> minlevel, Optional<uint8> maxlevel)
    {
        ...
        if (minlevel)
        {
            ...
            {
                handler->PSendSysMessage("Minlevel must be between 1 and %u!", uint32(MAX_LEVEL));
                handler->SetSentErrorMessage(true);
                return false;
            }
            if (maxlevel)
            {
                ...
                {
                    handler->PSendSysMessage("Maxlevel must be between 1 and %u!", uint32(MAX_LEVEL));
                    handler->SetSentErrorMessage(true);
                    return false;
                }
                if (*minlevel > *maxlevel)
        ...
        wp->SetLevels((!minlevel && !maxlevel) ? GetZoneLevels(GetZoneIdOverride(zoneId)) : std::pair{minlevel ? *minlevel : uint8(1), maxlevel ? *maxlevel : uint8(MAX_LEVEL)});
        wp->SetFlags(BotWPFlags(*flags));
    ...
    static bool HandleNpcBotOrderCastCommand(ChatHandler* handler, Optional<std::string> bot_name, Optional<std::string> spell_name, Optional<std::string_view> target_token)
    {
        ...
        else
        {
            ...
            bot = ccBots.empty() ? nullptr : ccBots.size() == 1 ? ccBots.front() : Syphrena::Containers::SelectRandomContainerElement(ccBots);
            if (!bot)
                bot = cBots.empty() ? nullptr : cBots.size() == 1 ? cBots.front() : Syphrena::Containers::SelectRandomContainerElement(cBots);
