    ...
    struct bot_druid_ai : public bot_ai
    {
        ...
        bool removeShapeshiftForm() override
        {
            ...
            //if (form != FORM_NONE)
            {
                ...
                {
                    //SC_LOG_ERROR("entities.player", "druid_bot::removeShapeshiftForm(): still has poweType {}!", uint32(me->GetPowerType()));
                    me->SetPowerType(POWER_MANA);
                }
                if (me->GetShapeshiftForm() != FORM_NONE)
                {
                    //SC_LOG_ERROR("entities.player", "druid_bot::removeShapeshiftForm(): still speshifted into {}!", uint32(me->GetShapeshiftForm()));
        ...
        void CheckBattleRez(uint32 diff)
        {
            ...
            if (Unit* targetOrCorpse = !targets.empty() ? Syphrena::Containers::SelectRandomContainerElement(targets) : nullptr)
            {
        ...
        void setStats(BotStances form)
        {
            ...
            switch (form)
            {
                case DRUID_BEAR_FORM:
                    ...
                    {
                        //SC_LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_RAGE");
                ...
                case DRUID_CAT_FORM:
                    ...
                    {
                        //SC_LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_ENERGY");
                ...
                case DRUID_MOONKIN_FORM:
                    if (me->GetPowerType() != POWER_MANA)
                    {
                        //SC_LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (moonkin)");
                        me->SetPowerType(POWER_MANA);
                    }
                ...
                case DRUID_TREE_FORM:
                    if (me->GetPowerType() != POWER_MANA)
                    {
                        //SC_LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (tree)");
                        me->SetPowerType(POWER_MANA);
                    }
                ...
                case DRUID_TRAVEL_FORM:
                    if (me->GetPowerType() != POWER_MANA)
                    {
                        //SC_LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (travel)");
                        me->SetPowerType(POWER_MANA);
                    }
                    break;
                case DRUID_AQUATIC_FORM:
                    if (me->GetPowerType() != POWER_MANA)
                    {
                        //SC_LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (aquatic)");
                        me->SetPowerType(POWER_MANA);
                    }
                    break;
                ...
                //    if (me->GetPowerType() != POWER_MANA)
                //    {
                //        //SC_LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (flight)");
                //        me->SetPowerType(POWER_MANA);
                //    }
                //    break;
                case BOT_STANCE_NONE:
                    if (me->GetPowerType() != POWER_MANA)
                    {
                        //SC_LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (deshape)");
                        me->SetPowerType(POWER_MANA);
                    }
                    break;
                default:
                    SC_LOG_ERROR("entities.player", "druid_bot::setStats(): NYI form {}", uint32(form));
                    setStats(BOT_STANCE_NONE);
                    return;
            }
        ...
        void SpellHitTarget(WorldObject* wtarget, SpellInfo const* spell) override
        {
            ...
            if (_form == DRUID_CAT_FORM)
            {
                    ...
                    baseId == RAKE_1 || baseId == RAVAGE_1 || baseId == SHRED_1)
                {
                    comboPoints++;
                    //debug
                    //SC_LOG_ERROR("entities.player", "druid_bot CP GEN: {} adds 1, now {}", spell->SpellName[0], uint32(comboPoints));
                    if (primalFuryProc)
                    {
                        comboPoints++;
                        //debug
                        //SC_LOG_ERROR("entities.player", "druid_bot CP EX: now {}", uint32(comboPoints));
                    }
                    if (comboPoints > 5)
                    {
                        comboPoints = 5;
                        //debug
                        //SC_LOG_ERROR("entities.player", "druid_bot CP NOR: now {}", uint32(comboPoints));
                    }
                }
                ...
                {
                    //debug
                    //SC_LOG_ERROR("entities.player", "druid_bot CP SPEND1: {} to 0", uint32(comboPoints));
                    if (lvl >= 25 && comboPoints > 0)
                    {
                        if (urand(1,100) <= uint32(comboPoints * 20))
                        {
                            ...
                            //debug
                            //SC_LOG_ERROR("entities.player", "druid_bot CP SPEND1: PS proc!");
                        }
                    }
                    comboPoints = 0;
                }
        ...
        void JustSummoned(Creature* summon) override
        {
            if (summon->GetEntry() == BOT_PET_FORCE_OF_NATURE)
            {
                ...
                if (!found)
                {
                    SC_LOG_ERROR("entities.unit", "Druid_bot:JustSummoned() treants array is full");
                    ASSERT(false);
                }
            }
        }
        ...
        {
            //SC_LOG_ERROR("entities.unit", "SummonedCreatureDespawn: {}'s {}", me->GetName(), summon->GetName());
            //if (summon == botPet)
            //    botPet = nullptr;
            if (summon->GetEntry() == BOT_PET_FORCE_OF_NATURE)
            {
                ...
                //if (!found)
                //{
                //    SC_LOG_ERROR("entities.unit", "Druid_bot:SummonedCreatureDespawn() treant is not found in array");
                //    ASSERT(false);
                //}
            }
        }
  ...
  /*custom*/lvl >= 22 ? InitSpellMap(PROWL_1) : RemoveSpell(PROWL_1); //base lvl 20
     ...
     /*tal*/lvl >= 60 && isRest ? InitSpellMap(WILD_GROWTH_1) : RemoveSpell(WILD_GROWTH_1);
        ...
        void ApplyClassPassives() const override
        {
            ...
            RefreshAura(GLYPH_NOURISH, level >= 90 ? 1 : 0);
            ...
            RefreshAura(T10_RESTO_P4_BONUS, level >= 90 ? 1 : 0);
            RefreshAura(T9_RESTO_P4_BONUS, level >= 90 ? 1 : 0);
            ...
            RefreshAura(T10_FERAL_P4_BONUS, level >= 90 ? 1 : 0);
        }
