    ...
    struct rogue_botAI : public bot_ai
    {
        ...
        void Attack(uint32 diff)
        {
            ...
            if (dist > 5)
            {
                ...
                //    SC_LOG_ERROR("entities.player", "rogue_bot: cannot attack target (dist)...");
                return;
            }
            ...
            {
                //SC_LOG_ERROR("entities.player", "bot_rogue: delaying attacks on gouged or blinded target...");
                return;
            }
        ...
        void ApplyClassDamageMultiplierMeleeSpell(int32& damage, SpellNonMeleeDamage& /*damageinfo*/, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/, bool iscrit) const override
        {
            ...
            if (lvl >= 90 && baseId == FAN_OF_KNIVES_1)
                pctbonus += 0.2f;
            ...
            if (spellInfo->NeedsComboPoints() && comboPoints)
            {
                if (lvl >= 10)
                {
                    if (irand(1,100) <= 20 * comboPoints)
                    {
                        ...
                        //SC_LOG_ERROR("entities.player", "rogue_bot CP SPEND1: RS proc!");
                    }
                }
            }
        ...
        void SpellHitTarget(WorldObject* wtarget, SpellInfo const* spell) override
        {
            ...
            {
                ++comboPoints;
                //SC_LOG_ERROR("entities.player", "rogue_bot CP GEN2: {} adds 1, now {}", spell->SpellName[0], uint32(comboPoints));
                if (comboPoints > 5)
                {
                    comboPoints = 5;
                    //SC_LOG_ERROR("entities.player", "rogue_bot CP NOR2: now {}", uint32(comboPoints));
                }
            }
                ...
                baseId == AMBUSH_1 || baseId == GARROTE_1 || baseId == CHEAP_SHOT_1/* || baseId == SHIV_1*/)
            {
                ...
                //SC_LOG_ERROR("entities.player", "rogue_bot CP GEN1: {} adds {}, now {}",
                ...
                {
                    ++comboPoints;
                    //SC_LOG_ERROR("entities.player", "rogue_bot CP GEN1: glyphSS proc, now {}", uint32(comboPoints));
                }

                if (comboPoints > 5)
                {
                    comboPoints = 5;
                    //SC_LOG_ERROR("entities.player", "rogue_bot CP NOR1: now {}", uint32(comboPoints));
                }
            }
            ...
            else if (spell->NeedsComboPoints() && comboPoints)
            {
                ...
                //SC_LOG_ERROR("entities.player", "rogue_bot CP SPEND1: {} to 0", tempCP);
                ...
                if (lvl >= 15)
                {
                    if (urand(1,100) <= 60)
                    {
                        ...
                        //SC_LOG_ERROR("entities.player", "rogue_bot CP SPEND1: RU proc!");
                    }
                }
            }
            ...
            {
                //SC_LOG_ERROR("entities.player", "rogue_bot Preparation hit!");
