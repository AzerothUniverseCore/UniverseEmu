...
void DPSTracker::_Release()
{
    for (DamageTakenMap::const_iterator itr = _damages.begin(); itr != _damages.end(); ++itr)
    {
        ...
        //SC_LOG_ERROR("entities.player", "DPSTracker::Release(): guidlow = {}, time = {}, tick damage {}, total {}, dps = {}",
...
{
    //SC_LOG_ERROR("entities.player", "DPSTracker::OnDamage(): on {}, damage {}", victim->GetName(), damage);
...
uint32 DPSTracker::GetDPSTaken(uint64 guid) const
{
    ...
    //SC_LOG_ERROR("entities.player", "DPSTracker::GetDPSTaken(): from {}, damage {}", guid, itr != _DPSes.end() ? itr->second : 0);
