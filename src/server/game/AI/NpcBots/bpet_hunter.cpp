    ...
    struct hunter_botpetAI : public bot_pet_ai
    {
        ...
        void DoPetActions(uint32 diff)
        {
                ...
                !me->HasAuraType(SPELL_AURA_PERIODIC_DAMAGE) && Rand() < 20)
            {
                ...
                Syphrena::AnyDeadUnitSpellTargetInRangeCheck check(me, 5.f, sSpellMgr->GetSpellInfo(CARRION_FEEDER_1), TARGET_CHECK_ENEMY);
                Syphrena::WorldObjectSearcher<Syphrena::AnyDeadUnitSpellTargetInRangeCheck> searcher(me, result, check);
