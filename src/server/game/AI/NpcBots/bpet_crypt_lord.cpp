    ...
    struct locust_swarm_botpetAI : public bot_pet_ai
    {
        ...
        void DoLocustActions(uint32 diff)
        {
            ...
            if (_targetRecheckTimer <= diff)
            {
                ...
                {
                    Syphrena::AnyUnfriendlyUnitInObjectRangeCheck check(petOwner, petOwner, LOCUST_SWARM_EFFECTIVE_RADIUS);
                    Syphrena::UnitListSearcher searcher(petOwner, targets, check);
                ...
                if (!targets.empty())
                {
                    opponent = targets.size() == 1 ? targets.front() : Syphrena::Containers::SelectRandomContainerElement(targets);
