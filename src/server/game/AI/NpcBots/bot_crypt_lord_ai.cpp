...
*/
    struct crypt_lord_botAI : public bot_ai
    {
        ...
        void CheckCarrionBeetles(uint32 diff)
        {
            ...
            Syphrena::CreatureSearcher searcher(me, creature, corpse_pred);
        ...
        void Attack(uint32 diff)
        {
                ...
                me->isAttackReady() && Rand() < 75)
            {
                ...
                if (chosen_targets)
                {
                    Unit* target = Syphrena::Containers::SelectRandomContainerElement(*chosen_targets);
    ...
    private:
        ...
        uint32 _getMaxLocusts() const
        {
            uint8 lvl = me->GetLevel();
            uint32 max_locusts;
            if (lvl >= MAX_LEVEL + BotDataMgr::GetLevelBonusForBotRank(me->GetCreatureTemplate()->rank))
