...
Original patch from: LordPsyan https://bitbucket.org/lordpsyan/syphrenacore-patches/src/3b8b9072280e/Individual/11185-BOTS-NPCBots.patch
*/
...
enum class BotWPFlags : uint32
{
    ...
    BOTWP_FLAG_SPAWN                    = 0x00000001, // wandering bots can spawn at this WP location // 1
    BOTWP_FLAG_ALLIANCE_ONLY            = 0x00000002, // only alliance bots can move here, SPAWN+A = only alliance bots can spawn at this WP location // 2
    BOTWP_FLAG_HORDE_ONLY               = 0x00000004, // only horde bots can move here, SPAWN+H = only horde bots can spawn at this WP location // 4
    BOTWP_FLAG_CAN_BACKTRACK_FROM       = 0x00000008, // can move back to WPs links even if other links exist // 8
    BOTWP_FLAG_MOVEMENT_IGNORES_FACTION = 0x00000010, // ignore faction flags when trying to select this WP as move point // 16
    BOTWP_FLAG_MOVEMENT_IGNORES_PATHING = 0x00000020, // do not generate path between 2 WPs having this flag // 32
    BOTWP_FLAG_BG_FLAG_DELIVER_TARGET   = 0x00000040, // <BG only> flag carrier destination marker // 64
    BOTWP_FLAG_BG_FLAG_PICKUP_TARGET    = 0x00000080, // <BG only> flag pick/activate up marker // 128
    BOTWP_FLAG_BG_BOSS_ROOM             = 0x00000100, // <BG only> boss room to attack as group / defend // 256
    BOTWP_FLAG_BG_MISC_OBJECTIVE_1      = 0x00000200, // <BG only> misc objective 1 (AV = mine) // 512
