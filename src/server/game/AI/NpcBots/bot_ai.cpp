...
Version 5.2.77a
Original idea: https://bitbucket.org/lordpsyan/syphrenacore-patches/src/3b8b9072280e/Individual/11185-BOTS-NPCBots.patch
TODO:
...
bot_ai::~bot_ai()
{
    SC_LOG_INFO("scripts", "bot_ai destructor call for {} ({})", me->GetName(), me->GetEntry());
...
const std::string& bot_ai::LocalizedNpcText(Player const* forPlayer, uint32 textId)
{
...

    {
        ...
        {
            SC_LOG_ERROR("entities.player", "NPCBots: bot text string #{} is not localized, at least for {}",
...
bool bot_ai::SetBotOwner(Player* newowner)
{
    ...
    {
        SC_LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): bot {} (id: {}) has master {} while trying to set to {}...",
            me->GetName(), me->GetEntry(), master->GetName(), newowner->GetName());
        return false;
    }
    if (!IAmFree())
    {
        SC_LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): minion bot {} (id: {}) IS NOT FREE (has master {}) while trying to set to {}",
            me->GetName(), me->GetEntry(), master->GetName(), newowner->GetName());
        return false;
    }
...
void bot_ai::CheckOwnerExpiry()
{
    ...
    if (timeNow >= lastLoginTime + expireTime)
    {
        ...
        SC_LOG_DEBUG("server.loading", ">> {}'s (guid: {}) ownership over bot {} ({}) has expired!",
...
SpellCastResult bot_ai::CheckBotCast(Unit const* victim, uint32 spellId) const
{
        ...
        (victim->GetLevel() + 10) < int32(spellInfo->GetFirstRankSpell()->SpellLevel))
    {
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            ...
            {
                //SC_LOG_ERROR("scripts", "CheckBotCast(): deny cast of {} by {} on low-level target {} (lvl {})",
    ...
    switch (_botclass)
    {
        ...
        case BOT_CLASS_CRYPT_LORD:
            break;
        default:
            SC_LOG_ERROR("entities.player", "CheckBotCast(): Unknown bot class {}", _botclass);
            break;
    }
...
bool bot_ai::doCast(Unit* victim, uint32 spellId, TriggerCastFlags flags)
{
    ...
    {
        SC_LOG_DEBUG("npcbots", "bot_ai::doCast(): {} (bot class {}) tried to cast spell {} on a dead target {}",
    ...
    //debug
    //SC_LOG_ERROR("entities.player", "CheckBotCast(): {} ({}) by {} on {}", m_botSpellInfo->SpellName[0], spellId, me->GetName(), victim->GetName());
...
/*
    ...
    if (result != SPELL_CAST_OK)
        SC_LOG_ERROR("entities.player", "doCast(): {} ({}) by {} on {} failed with {}",
...
*/
    ...
    if (!casted)
    {
        ...
        {
            if (DEBUG_BOT_ORDERS)
                SC_LOG_ERROR("entities.player", "doCast(): ordered spell {} is not casted!", m_botSpellInfo->Id);
...
void bot_ai::BotMovement(BotMovementType type, Position const* pos, Unit* target, bool generatePath, float speed) const
{
    ...
    switch (type)
    {
        ...
        case BOT_MOVE_POINT:
            ...
            break;
        default:
            SC_LOG_ERROR("scripts", "BotMovement: unhandled bot movement type {}", uint32(type));
            return;
    }
}
...
void bot_ai::BuffAndHealGroup(uint32 diff)
{
    ...
    if (IAmFree())
    {
        ...
        if (!targets2.empty() && BuffTarget(targets2.size() == 1 ? targets2.front() : Syphrena::Containers::SelectRandomContainerElement(targets2), diff))
            return;
    ...
    if (!pGroup)
    {
        ...
        if (HasRole(BOT_ROLE_HEAL))
        {
            ...
            if (!targets3.empty() && HealTarget(Syphrena::Containers::SelectRandomContainerElement(targets3), diff))
                return;
        }
        //buffs
        ...
        if (!targets4.empty() && BuffTarget(Syphrena::Containers::SelectRandomContainerElement(targets4), diff))
            return;
    ...
    if (HasRole(BOT_ROLE_HEAL))
    {
        ...
        if (!targets5.empty() && HealTarget(Syphrena::Containers::SelectRandomContainerElement(targets5), diff))
            return;
    }
    //buffs
    ...
    if (!targets6.empty() && BuffTarget(Syphrena::Containers::SelectRandomContainerElement(targets6), diff))
        return;
}
...
void bot_ai::ResurrectGroup(uint32 spell_id)
{
    ...
    //SC_LOG_ERROR("entities.player", "ResurrectGroup by {}", me->GetName());
    ...
    //SC_LOG_ERROR("entities.unit", "ResurrectGroup: {} found {} targets", me->GetName(), uint32(bottargets.size()));

    if (bottargets.empty())
        return;

    target = bottargets.size() < 2 ? bottargets.front() : Syphrena::Containers::SelectRandomContainerElement(bottargets);
...
void bot_ai::CureGroup(uint32 cureSpell, uint32 diff)
{
    ...
    if (IAmFree())
    {
        ...
        {
            if (doCast(Syphrena::Containers::SelectRandomContainerElement(cureTargets), cureSpell))
                return;
        }
    ...
    //SC_LOG_ERROR("entities.player", "{}: CureGroup() on {}", me->GetName(), pTarget->GetName());
    ...
    if (!targets.empty())
    {
        if (doCast(Syphrena::Containers::SelectRandomContainerElement(targets), cureSpell))
            return;
    }
}
...
void bot_ai::SetStats(bool force)
{
    ...
    uint8 mylevel = std::min<uint8>(master->GetLevel(), MAX_LEVEL);
    if (IsWanderer())
    {
        if (_baseLevel == 0) //this only happens once
        {
            ...
            if (me->GetMap()->IsBattlegroundOrArena())
                SC_LOG_DEBUG("npcbots", "BG bot {} id {} selected level {}...", me->GetName(), me->GetEntry(), uint32(_baseLevel));
            else
                SC_LOG_DEBUG("npcbots", "Wandering bot {} id {} selected level {}...", me->GetName(), me->GetEntry(), uint32(_baseLevel));
        }
    ...
    mylevel = std::min<uint8>(mylevel, MAX_LEVEL);
    ...
    if (force)
    {
        ...
        sObjectMgr->GetPlayerClassLevelInfo(GetPlayerClass(), std::min<uint8>(mylevel, MAX_LEVEL), _classinfo);

        PlayerLevelInfo info;
        sObjectMgr->GetPlayerLevelInfo(GetPlayerRace(), GetPlayerClass(), std::min<uint8>(mylevel, MAX_LEVEL), &info);
        for (uint8 i = STAT_STRENGTH; i != MAX_STATS; ++i)
    ...
    switch (myclass)
    {
        ...
        default:
            SC_LOG_ERROR("entities.player", "minion_ai: *etStats():Init - unknown bot class {}, real class: {}, _botclass: {}", myclass, GetPlayerClass(), _botclass);
            break;
    }
    ...
    switch (myclass)
    {
        ...
        default:
            SC_LOG_ERROR("entities.player", "_MeleeDamageUpdate(): NIY myclass {}!", uint32(myclass));
            strmult = 0.f; agimult = 0.f; break;
    }
    ...
    if (CanCrit())
    {
        ...
        if (_botclass == BOT_CLASS_MAGE && me->HasAuraTypeWithFamilyFlags(SPELL_AURA_MOD_RATING_FROM_STAT, SPELLFAMILY_MAGE, 0x40000))
            tempval += _getTotalBotStat(BOT_STAT_MOD_SPIRIT) * (mylevel >= 90 ? 0.95f : mylevel >= 90 ? 0.9f : mylevel >= 70 ? 0.75f : 0.55f);
        //Firestone: just emulate the rating bonus
        if (_botclass == BOT_CLASS_WARLOCK)
        {
            uint8 ratingBonus;
			if      (mylevel >= 90) ratingBonus = 56;
            else if (mylevel >= 90) ratingBonus = 49;
    ...
    if (CanDodge())
    {
        ...
        {
            //SC_LOG_ERROR("scripts", "BOT_CLASS_SEA_WITCH dodge: {} now in water", me->GetName());
            value += 50.f;
        }
...
void bot_ai::RefreshAura(uint32 spellId, int8 count, Unit* target) const
{
    if (count < 0 || count > 1)
    {
        SC_LOG_ERROR("entities.player", "bot_ai::RefreshAura(): count is out of bounds ({}) for bot {} (botclass: {}, entry: {})",
    ...
    if (!spellId)
    {
        SC_LOG_ERROR("entities.player", "bot_ai::RefreshAura(): spellId is 0 for bot {} (botclass: {}, entry: {})",
    ...
    if (!spellInfo)
    {
        SC_LOG_ERROR("entities.player", "bot_ai::RefreshAura(): Invalid spellInfo for spell {}! Bot - {} (botclass: {}, entry: {})",
...
Unit* bot_ai::_getVehicleTarget(BotVehicleStrats /*strat*/) const
{
            ...
            if (ObjectGuid guid = gr->GetTargetIcons()[i])
            {
                    ...
                    (BotMgr::GetRangedDPSTargetIconFlags() & GroupIconsFlags[i]))
                {
                    ...
                    if (Unit* unit = ObjectAccessor::GetUnit(*me, guid))
                    {
                        ...
                        {
                            //SC_LOG_ERROR("entities.unit", "_getTarget: found dps icon target {}", unit->GetName());
                            return unit;
                        }
                    }
                }
    ...
    if (mmover->IsAlive())
    {
            ...
            (mmover->GetDistance(mytar) > followdist || (mmover->GetDistance(mytar) > followdist * 0.75f && !mytar->IsWithinLOSInMap(veh)))))
        {
            //if (mytar)
            //{
            //    SC_LOG_ERROR("scripts", "_getVehicleTarget {}'s veh is too far from master - lost target ({} > {})",
    ...
    Syphrena::UnitSearcher <NearbyHostileVehicleTargetCheck> searcher(veh, t, check);
...
std::tuple<Unit*, Unit*> bot_ai::_getTargets(bool byspell, bool ranged, bool &reset) const
{
    ...
    //SC_LOG_ERROR("entities.player", "bot_ai::getTarget(): bot: {}", me->GetName());
    ...
    if (!IAmFree() && me->GetMap()->GetEntry() && !me->GetMap()->GetEntry()->IsWorldMap())
    {
        ...
        if (me->GetMapId() == 631 && isInWMOArea(WMOAreaGroupMarrowgar) && me->IsInCombat() && HasRole(BOT_ROLE_DPS) && !IsTank())
        {
            ...
            Syphrena::CreatureListSearcher searcher(me, cList, boneSpikeCheck);
            Cell::VisitAllObjects(me, searcher, 50.f);

            if (Creature* spike = cList.empty() ? nullptr : cList.size() == 1 ? cList.front() :
                Syphrena::Containers::SelectRandomContainerElement(cList))
            {
            ...
            mytar->GetEntry() != CREATURE_ICC_ICE_TOMB3 && mytar->GetEntry() != CREATURE_ICC_ICE_TOMB4))*/)
        {
            ...
            std::list<Creature*> cList;
            Syphrena::CreatureListSearcher searcher(master, cList, SiItCheck);
        ...
        if (me->GetMapId() == 631 && isInWMOArea(WMOAreaGroupLichKing) && me->IsInCombat() && HasRole(BOT_ROLE_DPS) && !IsTank())
        {
            ...
            Syphrena::CreatureSearcher searcher(me, valkyr, valkyrCheck);
            Cell::VisitAllObjects(me, searcher, 50.f);
            ...
            Syphrena::CreatureLastSearcher searcher2(usearcher, sphere, iceSphereCheck);
    ...
    if (gr && IsOffTank())
    {
        ...
        for (int8 i = TARGET_ICONS_COUNT - 1; i >= 0; --i)
        {
            ...
            if (BotMgr::GetOffTankTargetIconFlags() & GroupIconsFlags[i])
            {
                if (ObjectGuid guid = gr->GetTargetIcons()[i])
                {
                    ...
                    {
                        //SC_LOG_ERROR("entities.unit", "_getTarget: {} continues {}", me->GetName(), mytar->GetName());
                        return { mytar, mytar };
                    }
                    ...
                    if (Unit* unit = ObjectAccessor::GetUnit(*me, guid))
                    {
                        if (unit->IsVisible() && unit->isTargetableForAttack(false) && me->IsValidAttackTarget(unit) &&
                            unit->IsInCombat() && (CanSeeEveryone() || (me->CanSeeOrDetect(unit) && unit->InSamePhase(me))))
                        {
                            //SC_LOG_ERROR("entities.unit", "_getTarget: {} found new offtanking icon target {}", me->GetName(), unit->GetName());
                            Unit* tempTar = tankTar ? tankTar : unit;
                            tankTar = unit;
                            Unit* tVic = unit->GetVictim();
                            if (!tVic || (tVic != me && tVic->GetVictim() == unit && IsTank(tVic) && IsInBotParty(tVic)))
                            {
                                //SC_LOG_ERROR("entities.unit", "_getTarget: {} skipped {} ({})", me->GetName(), unit->GetName(), tVic->GetName());
                                tankTar = tempTar;
                                continue;
                            }
                        }
        ...
        if (tankTar)
        {
            //SC_LOG_ERROR("entities.unit", "_getTarget: {} returning {}", me->GetName(), tankTar->GetName());
                ...
                if (ObjectGuid guid = gr->GetTargetIcons()[i])
                {
                    ...
                    if (Unit* unit = ObjectAccessor::GetUnit(*me, guid))
                    {
                        if (unit->IsVisible() && unit->isTargetableForAttack(false) && me->IsValidAttackTarget(unit) &&
                            unit->IsInCombat() && (CanSeeEveryone() || (me->CanSeeOrDetect(unit) && unit->InSamePhase(me))))
                        {
                            //SC_LOG_ERROR("entities.unit", "_getTarget: {} found new mtanking icon target {}", me->GetName(), unit->GetName());
                            Unit* tempTar = tankTar ? tankTar : unit;
                            tankTar = unit;
                            Unit* tVic = unit->GetVictim();
                            if (!tVic || (tVic != me && tVic->GetVictim() == unit && IsTank(tVic) && IsInBotParty(tVic)))
                ...
                if (HasRole(BOT_ROLE_RANGED) && (BotMgr::GetRangedDPSTargetIconFlags() & GroupIconsFlags[i]))
                {
                    ...
                    if (Unit* unit = ObjectAccessor::GetUnit(*me, guid))
                    {
                        if (unit->IsVisible() && unit->isTargetableForAttack(false) && me->IsValidAttackTarget(unit) &&
                            unit->IsInCombat() && (CanSeeEveryone() || (me->CanSeeOrDetect(unit) && unit->InSamePhase(me))))
                        {
                            //SC_LOG_ERROR("entities.unit", "_getTarget: found rdps icon target {}", unit->GetName());
                            return { unit, unit };
                        }
                    }
                }
...
*/
    ...
    {
        //SC_LOG_ERROR("entities.player", "bot {} continues attack common target {}", me->GetName(), u->GetName());
        return { u, u };//forced
    }
    //Follow if...
    ...
    {
        _extendAttackRange(foldist);
        //SC_LOG_ERROR("entities.player", "bot {} ranged foldist {} spelldist {}", me->GetName(), foldist, spelldist);
    }
    ...
    if (dropTarget)
    {
        //SC_LOG_ERROR("entities.player", "bot {} cannot attack target {}, too far away or not in LoS", me->GetName(), mytar ? mytar->GetName() : "unk");
        mytar = nullptr;
    }
    ...
    {
        //SC_LOG_ERROR("entities.player", "bot {} starts attack master's target {}", me->GetName(), u->GetName());
        return { u, u };
    }
    ...
    {
        //SC_LOG_ERROR("entities.player", "bot {} continues attack its target {}", me->GetName(), mytar->GetName());
    ...
    if (!IAmFree())
    {
        if (!gr)
        {
            ...
            for (BotMap::const_iterator itr = map->begin(); itr != map->end(); ++itr)
            {
                ...
                {
                    //SC_LOG_ERROR("entities.player", "bot {} hooked {}'s victim {}", me->GetName(), bot->GetName(), u->GetName());
                    return { u, u };
                }
            }
        }
        else
        {
            for (GroupReference const* ref = gr->GetFirstMember(); ref != nullptr; ref = ref->next())
            {
                ...
                {
                    //SC_LOG_ERROR("entities.player", "bot {} hooked {}'s victim {}", me->GetName(), pl->GetName(), u->GetName());
                    return { u, u };
                }
    ...
    Syphrena::UnitListSearcher searcher(master->ToUnit(), unitList, check);
    ...
    if (IAmFree())
    {
        ...
        if (!closeList.empty())
        {
            ts[0].first = closeList.size() == 1 ? closeList.front() : Syphrena::Containers::SelectRandomContainerElement(closeList);
            ts[0].second = me->GetDistance(ts[0].first);
        }
        else if (!unitList.empty())
        {
            ts[0].first = unitList.size() == 1 ? unitList.front() : Syphrena::Containers::SelectRandomContainerElement(unitList);
            ts[0].second = me->GetDistance(ts[0].first);
        }
    }
    else
    ...
    //if (t)
    //    SC_LOG_ERROR("entities.player", "bot {} has found new target {}", me->GetName(), t->GetName());
...
bool bot_ai::CheckAttackTarget()
{
    ...
    switch (_botclass)
    {
        case BOT_CLASS_DRUID:
            switch (GetBotStance())
            {
                ...
                case BOT_STANCE_NONE:
                    ...
                    break;
                default:
                    SC_LOG_ERROR("entities.player", "bot_ai::CheckAttackTarget(): druid has NYI bot stance {}", uint32(GetBotStance()));
                    break;
            }
            break;
        case BOT_CLASS_PRIEST:
        ...
        case BOT_CLASS_CRYPT_LORD:
            break;
        default:
            SC_LOG_ERROR("entities.player", "bot_ai: CheckAttackTarget() - unknown bot class {}", _botclass);
            return false;
    }
    ...
    {
        //SC_LOG_ERROR("entities.player", "bot_ai: CheckAttackTarget() - bot {} lost target", me->GetName());
        if (me->GetVictim() || me->IsInCombat()/* || !me->GetThreatManager().isThreatListEmpty()*/)
        {
            //SC_LOG_ERROR("entities.player", "bot_ai: CheckAttackTarget() - bot {} Evades", me->GetName());
            if (me->GetVictim())
                me->AttackStop();
...
bool bot_ai::ProcessImmediateNonAttackTarget()
{
    ...
    if (me->GetMapId() == 580 && isInWMOArea(_lastWMOAreaId, WMOAreaGroupMuru)) // Sunwell - M'uru
    {
        ...
        {
            std::list<Creature*> cList;
            Syphrena::AllCreaturesOfEntryInRange check(me, 25744, 30.f); // Dark Fiend
            Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher(me, cList, check);
            Cell::VisitAllObjects(me, searcher, 30.f);

            //Dark Fiends do not die instantly, remove purged ones
            cList.remove_if(Syphrena::UnitAuraCheck(false, 45934)); // "Dark Fiend"
    ...
    if (me->GetMapId() == 564 && isInWMOArea(_lastWMOAreaId, WMOAreaGroupNajentus) && Rand() < 10) // Black Temple - High Warlord Naj'entus
    {
        if (Group const* gr = master->GetGroup())
        {
            if (Rand() < 4)
            {
                ...
                if (najentus && najentus->HasAuraTypeWithMiscvalue(SPELL_AURA_SCHOOL_IMMUNITY, 127)) // Tidal Shield
                {
                    ...
                    if (Player* pl = spiners.empty() ? nullptr : spiners.size() == 1u ? spiners.front() :
                        Syphrena::Containers::SelectRandomContainerElement(spiners))
                    {
            ...
            if (Unit* u = spines.empty() ? nullptr : spines.size() == 1u ? spines.front() :
                Syphrena::Containers::SelectRandomContainerElement(spines))
            {
    ...
    if (me->GetMapId() == 548 && isInWMOArea(_lastWMOAreaId, WMOAreaGroupVashj) && Rand() < 15) // Serpentshrine Cavern - Lady Vashj
    {
        ...
        if (!taintPlayers.empty() && alive_players <= 1)
        {
...
#endif
            std::list<Creature*> cList;
            Syphrena::AllCreaturesOfEntryInRange check(me, ShieldGeneratorTriggerNPC, 100.f); // Invis KV Shield Generator
            Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> csearcher(me, cList, check);
            ...
            Syphrena::GameObjectListSearcher gsearcher(me, gList, is_shield_go);
            ...
            if (!gList.empty())
            {
                Player* player = taintPlayers.size() == 1u ? taintPlayers.front() : Syphrena::Containers::SelectRandomContainerElement(taintPlayers);
                BotWhisper("Taking Tainted Core from you");
                GameObject* go = gList.size() == 1u ? gList.front() : Syphrena::Containers::SelectRandomContainerElement(gList);
#if defined(SYPHRENA_COMPILER)
        ...
        if (Group const* gr = master->GetGroup())
        {
            if (Rand() < 4)
            {
                ...
                if (!svalna)
                {
                    ...
                    Syphrena::AllCreaturesOfEntryInRange check(master, me->GetMap()->IsHeroic() ? CREATURE_SISTER_SVALNA_H : CREATURE_SISTER_SVALNA_N, 60.f);
                    Syphrena::CreatureSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher(master, svalna, check);
                ...
                if (svalna && svalna->HasAuraTypeWithMiscvalue(SPELL_AURA_SCHOOL_IMMUNITY, 127)) // Aether Shield
                {
                    ...
                    if (Player* pl = spearers.empty() ? nullptr : spearers.size() == 1u ? spearers.front() :
                        Syphrena::Containers::SelectRandomContainerElement(spearers))
                    {
            ...
            if (Unit* u = spears.empty() ? nullptr : spears.size() == 1u ? spears.front() :
                Syphrena::Containers::SelectRandomContainerElement(spears))
            {
                Creature* spear = nullptr;
                Syphrena::NearestCreatureEntryWithLiveStateInObjectRangeCheck check(*u, 38248, true, 5.f);
                Syphrena::CreatureSearcher<Syphrena::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(u, spear, check);
...
void bot_ai::CalculateAoeSpots(Unit const* unit, AoeSpotsVec& spots)
{
    ...
    Syphrena::WorldObjectListSearcher<NearbyHostileAoEDynobjectCheck> searcher(unit, doList, check, GRID_MAP_TYPE_MASK_DYNAMICOBJECT);
    //unit->VisitNearbyObject(60.f, searcher);
    Cell::VisitAllObjects(unit, searcher, 60.f);

    //if (!doList.empty())
    //    SC_LOG_ERROR("scripts", "CalculateAoeSpots {} aoes around {}", uint32(doList.size()), unit->GetName());
    ...
    for (std::list<WorldObject*>::const_iterator ci = doList.begin(); ci != doList.end(); ++ci)
    {
        ...
        {
            //SC_LOG_ERROR("scripts", "CalculateAoeSpots found {}'s aoe {} ({}) radius {} size {}",
    ...
    {
        std::list<Creature*> cList;
        Syphrena::AllCreaturesOfEntryInRange check2(unit->GetVehicleBase(), CREATURE_EOE_STATIC_FIELD, 60.f);
        Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher2(unit->GetVehicleBase(), cList, check2);
    ...
    {
        std::list<Creature*> cList;
        Syphrena::AllCreaturesOfEntryInRange check2(unit, CREATURE_ZA_FIRE_BOMB, 40.f);
        Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher2(unit, cList, check2);
    ...
    else if (unit->GetMapId() == 574)
    {
        ...
        Syphrena::CreatureSearcher searcher2(unit, creature, shadow_axe_check);
        Cell::VisitAllObjects(unit, searcher2, 40.f);
    ...
    {
        std::list<Creature*> cList;
        Syphrena::AllCreaturesOfEntryInRange check2(unit, CREATURE_ICC_OOZE_PUDDLE, 50.f);
        Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher2(unit, cList, check2);
...
void bot_ai::CalculateAttackPos(Unit* target, Position& pos, bool& force) const
{
    ...
    if (me->GetVehicle())
    {
        ...
        for (uint8 i = 0; i < 4; ++i)
        {
            ...
            if (isinaoe)
            {
                //SC_LOG_ERROR("scripts", "CalculateAttackPos {} veh skipped aoe pos", me->GetName());
                if (me->GetVehicleBase()->CanFly())
    ...
    if (!aoespots.empty())
    {
        ...
        {
            //if (!aoespots.empty())
            //    SC_LOG_ERROR("scripts", "CalculateAttackPos {} spot is still safe", me->GetName());
    ...
    if (!safespots.empty())
    {
        ...
        //SC_LOG_ERROR("scripts", "CalculateAttackPos {} safe spots, chosen at dist {}", uint32(safespots.size()), mindist);
...
void bot_ai::GetInPosition(bool force, Unit* newtarget, Position* mypos)
{
    ...
    if (IsRanged() || (!IAmFree() && !GetAoeSpots().empty()))
    {
        ...
        //SC_LOG_ERROR("scripts", "GetInPosition {} to {} dist {}, to pos {}", me->GetName(),
...
//        return;
    ...
    //SC_LOG_ERROR("entities.player", "AdjustTankPosition() by {}", me->GetName());
    ...
    //SC_LOG_ERROR("entities.player", "AdjustTankPosition(): atts {}, behind {}", uint32(myattackers.size()), bCount);
    ...
    for (uint8 slot = BOT_SLOT_MAINHAND; slot < BOT_INVENTORY_SIZE; ++slot)
    {
        if (_usableItemSlotsMask & (1ul << slot))
        {
            if (Item const* item = _equips[slot])
            {
                ...
                //SC_LOG_ERROR("scripts", "bot_ai::CheckUsableItems(): bot {}, slot {}, spell {}, target {}",
                ...
                SC_LOG_ERROR("scripts", "bot_ai::CheckUsableItems(): slot {} is in mask but no item exists in that slot!", uint32(slot));
        }
    }
}
...
WorldObject* bot_ai::GetNearbyRezTarget(float dist) const
{
    ...
    Syphrena::WorldObjectListSearcher <NearbyRezTargetCheck> searcher(me, list, check);
    ...
    return Syphrena::Containers::SelectRandomContainerElement(list);
}
...
Unit* bot_ai::FindImmunityShieldDispelTarget(float dist) const
{
    ...
    Syphrena::UnitSearcher <ImmunityShieldDispelTargetCheck> searcher(me, unit, check);
...
Unit* bot_ai::FindHostileDispelTarget(float dist, bool stealable) const
{
    ...
    Syphrena::UnitListSearcher <HostileDispelTargetCheck> searcher(me, unitList, check);
    Cell::VisitAllObjects(me, searcher, dist);
    //me->VisitNearbyObject(dist, searcher);
    ...
    return Syphrena::Containers::SelectRandomContainerElement(unitList);
}
...
Unit* bot_ai::FindAffectedTarget(uint32 spellId, ObjectGuid caster, float dist, uint8 hostile) const
{
    ...
    {
        SC_LOG_ERROR("entities.player", "bot_ai::FindAffectedTarget(): hostile = {} Setting to ALL...", hostile);
        hostile = 255;
    }
    ...
    Syphrena::UnitSearcher <AffectedTargetCheck> searcher(master, unit, check);
    Cell::VisitAllObjects(me, searcher, dist);
    //me->VisitNearbyObject(dist, searcher);
...
Unit* bot_ai::FindPolyTarget(float dist) const
{
    ...
    Syphrena::UnitListSearcher <PolyUnitCheck> searcher(me, unitList, check);
    Cell::VisitAllObjects(me, searcher, dist);
    //me->VisitNearbyObject(dist, searcher);
...
Unit* bot_ai::FindFearTarget(float dist) const
{
    ...
    Syphrena::UnitListSearcher <FearUnitCheck> searcher(me, unitList, check);
    Cell::VisitAllObjects(me, searcher, dist);
    //me->VisitNearbyObject(dist, searcher);
...
Unit* bot_ai::FindStunTarget(float dist) const
{
    ...
    Syphrena::UnitListSearcher <StunUnitCheck> searcher(me, unitList, check);
    Cell::VisitAllObjects(me, searcher, dist);
    //me->VisitNearbyObject(dist, searcher);
...
Unit* bot_ai::FindUndeadCCTarget(float dist, uint32 spellId, bool unattacked) const
{
    ...
    Syphrena::UnitListSearcher <UndeadCCUnitCheck> searcher(me, unitList, check);
    Cell::VisitAllObjects(me, searcher, dist);
    //me->VisitNearbyObject(dist, searcher);
...
Unit* bot_ai::FindRootTarget(float dist, uint32 spellId) const
{
    ...
    Syphrena::UnitListSearcher <RootUnitCheck> searcher(me, unitList, check);
    Cell::VisitAllObjects(me, searcher, dist);
    //me->VisitNearbyObject(dist, searcher);
...
Unit* bot_ai::FindCastingTarget(float maxdist, float mindist, uint32 spellId, uint8 minHpPct) const
{
    ...
    Syphrena::UnitListSearcher <CastingUnitCheck> searcher(me, unitList, check);
...
Unit* bot_ai::FindSplashTarget(float dist, Unit* To, float splashdist) const
{
    ...
    Syphrena::UnitSearcher <SecondEnemyCheck> searcher(me, unit, check);
    Cell::VisitAllObjects(me, searcher, dist);
    //me->VisitNearbyObject(dist, searcher);
...
Unit* bot_ai::FindSplashTarget(float dist, Unit* To, float splashdist, uint8 minTargets) const
{
    ...
    SecondEnemyCheck check(me, dist, splashdist, To, this);
    Syphrena::UnitListSearcher <SecondEnemyCheck> searcher(me, unitList, check);
    Cell::VisitAllObjects(me, searcher, dist);
    //me->VisitNearbyObject(dist, searcher);
...
Unit* bot_ai::FindTranquilTarget(float mindist, float maxdist) const
{
    ...
    Syphrena::UnitSearcher <TranquilTargetCheck> searcher(me, unit, check);
    Cell::VisitAllObjects(me, searcher, maxdist);
    //me->VisitNearbyObject(maxdist, searcher);
...
Unit* bot_ai::FindDistantTauntTarget(float maxdist, bool ally) const
{
    ...
    Syphrena::UnitListSearcher <FarTauntUnitCheck> searcher(me, unitList, check);
    Cell::VisitAllObjects(me, searcher, maxdist);
    //me->VisitNearbyObject(maxdist, searcher);
    ...
    Unit* unit = unitList.size() == 1 ? *unitList.begin() : Syphrena::Containers::SelectRandomContainerElement(unitList);
...
Unit* bot_ai::FindDrainTarget(float maxdist) const
{
    ...
    Syphrena::UnitLastSearcher <ManaDrainUnitCheck> searcher(me, unit, check);
    Cell::VisitAllObjects(me, searcher, maxdist);
    //me->VisitNearbyObject(maxdist, searcher);
...
void bot_ai::GetNearbyTargetsList(std::list<Unit*> &targets, float maxdist, uint8 CCoption, WorldObject const* source) const
{
    ...
    Syphrena::UnitListSearcher <NearbyHostileUnitCheck> searcher(me, targets, check);
    Cell::VisitAllObjects(me, searcher, maxdist);
    //me->VisitNearbyObject(maxdist, searcher);
}
...
void bot_ai::GetNearbyTargetsInConeList(std::list<Unit*> &targets, float maxdist) const
{
    ...
    Syphrena::UnitListSearcher <NearbyHostileUnitInConeCheck> searcher(me, targets, check);
    Cell::VisitAllObjects(me, searcher, maxdist);
    //me->VisitNearbyObject(maxdist, searcher);
}
...
void bot_ai::GetNearbyFriendlyTargetsList(std::list<Unit*> &targets, float maxdist) const
{
    ...
    Syphrena::UnitListSearcher <NearbyFriendlyUnitCheck> searcher(me, targets, check);
    Cell::VisitAllObjects(me, searcher, maxdist);
    //me->VisitNearbyObject(maxdist, searcher);
}
//////////
...
uint32 bot_ai::InitSpell(Unit const* caster, uint32 spell)
{
    ...
    if (!info)
    {
        SC_LOG_ERROR("entities.player", "InitSpell(): No SpellInfo found for spell {}", spell);
...
void bot_ai::InitSpellMap(uint32 basespell, bool forceadd, bool forwardRank)
{
    ...
    if (!info)
    {
        SC_LOG_ERROR("entities.player", "bot_ai::InitSpellMap(): No SpellInfo found for base spell {}", basespell);
    ...
    if (npcBotData && npcBotData->disabled_spells.find(basespell) != npcBotData->disabled_spells.end())
    {
        ...
        //SC_LOG_ERROR("entities.player", "bot_ai::InitSpellMap(): {} ({} -> {}) is disabled for {}!",
...
void bot_ai::SetSpellCategoryCooldown(SpellInfo const* spellInfo, uint32 msCooldown)
{
    ...
    for (BotSpellMap::const_iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        if (info && itr->first == spellInfo->Id && info->GetCategory() != category && info->StartRecoveryCategory != category)
        {
            ...
            {
                SC_LOG_ERROR("scripts", "Warning: SetSpellCategoryCooldown: {} has baseId {} but category {}, not {}!",
...
void bot_ai::ReleaseSpellCooldown(uint32 basespell)
{
    ...
    {
        SC_LOG_ERROR("spells", "bot_ai::ReleaseSpellCooldown is called for wrong spell {}!", basespell);
        return;
    }
    ...
    switch (myrace)
    {
        ...
        case RACE_DRAENEI:
            switch (myclass)
            {
                ...
                default:
                    SC_LOG_ERROR("entities.player", "RaceSpellForClass(): unknows race:class combo {}, {}", uint32(myrace), uint32(myclass));
                    return 0;
            }
            break;
        default:
...
void bot_ai::_OnHealthUpdate() const
{
    ...
    if (myclass == BOT_CLASS_DRUID && GetBotStance() != BOT_STANCE_NONE)
        myclass = GetBotStance();
    //SC_LOG_ERROR("entities.player", "_OnHealthUpdate(): updating bot {}", me->GetName());
    ...
    //SC_LOG_ERROR("entities.player", "class base health: {}", m_basehp);
    ...
    //SC_LOG_ERROR("entities.player", "bot's stats to health add: Stamina ({}), value: {}", stamValue, stamValue * 10.f);
    ...
    //SC_LOG_ERROR("entities.player", "health to add after stam mod: {}", hp_add);
    uint32 m_totalhp = m_basehp + int32(hp_add * (BotMgr::IsWanderingWorldBot(me) ? BotMgr::GetBotWandererHPMod() : BotMgr::GetBotHPMod()));
    //SC_LOG_ERROR("entities.player", "total base health: {}", m_totalhp);
    ...
    //SC_LOG_ERROR("entities.player", "overall hp: {}", me->GetMaxHealth());
...
void bot_ai::_OnManaUpdate() const
{
    ...
    //SC_LOG_ERROR("entities.player", "_OnManaUpdate(): updating bot {}", me->GetName());
    ...
    //SC_LOG_ERROR("entities.player", "classinfo base mana = {}", m_basemana);
...
void bot_ai::_UpdateWMOArea()
{
    ...
    if (WMOAreaTableEntry const* wmoEntry = GetWMOAreaTableEntryByTripple(rootId, adtId, groupId))
    {
        ...
        //SC_LOG_ERROR("scripts", "_UpdateWMOArea(): bot {}: area {}, wmoarea {}", me->GetName(), _lastAreaId, _lastWMOAreaId);
    }
}
...
void bot_ai::OnSpellHit(Unit* caster, SpellInfo const* spell)
{
    ...
    for (uint8 i = 0; i != MAX_SPELL_EFFECTS; ++i)
    {
        ...
        {
            //SC_LOG_ERROR("entities.unit", "OnSpellHit: mount on {}", me->GetName());
            if (!IAmFree())
                UnsummonAll();
            ...
            {
                //SC_LOG_ERROR("entities.unit", "OnSpellHit: modding flight speed");
                ...
                {
                    //SC_LOG_ERROR("entities.unit", "OnSpellHit: found aura");
...
void bot_ai::OnSpellHitTarget(Unit* /*target*/, SpellInfo const* spell)
{
    if (me->GetVehicle())
    {
        ...
        if (spellId == 56091 || spellId == 57090)
        {
            ...
            //SC_LOG_ERROR("scripts", "OnBotSpellGo(): veh cp spell {} now cp {}", curInfo->Id, uint32(vehcomboPoints));
        }
        //Engulf in Flames, Life Burst, Flame Shield   moved to globalupdate
        if (spellId == 56092 || spellId == 57143 || spellId == 57108)
        {
            vehcomboPoints = 0;
            //SC_LOG_ERROR("scripts", "OnSpellHitTarget(): veh cp waster {}", curInfo->Id);
        }
    }
}
//Update delay
...
bool bot_ai::OnGossipSelect(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action)
{
    ...
    switch (sender)
    {
            ...
            switch (_botclass)
            {
                ...
                case BOT_CLASS_ROGUE:
                {
                    ...
                    if (action == 1)
                    {
                        ...
                        Syphrena::GameObjectLastSearcher<NearestLockedGameObjectInRangeCheck> searcher(player, obj, check);
        ...
        case GOSSIP_SENDER_EQUIPMENT_SHOW: //equips change s2: send list of equippable items
        {
            ...
            //SC_LOG_ERROR("entities.player", "OnGossipSelect(bot): added {} item(s) to list of {} (requester: {})",
        ...
        case GOSSIP_SENDER_EQUIPMENT_BANK_DEPOSIT_ITEM:
        {
            ...
            if (!item)
            {
                SC_LOG_ERROR("npcbots", "GOSSIP_SENDER_EQUIPMENT_BANK_DEPOSIT_ITEM: item {} not found on player {}! Cheater?",
        ...
        case GOSSIP_SENDER_EQUIPMENT_BANK_WITHDRAW_ITEM:
        {
            ...
            if (!item)
            {
                SC_LOG_ERROR("npcbots", "GOSSIP_SENDER_EQUIPMENT_BANK_WITHDRAW_ITEM: item {} not found on player {}! Cheater?",
                    itemGuidLow, player->GetName());
                break;
            }
        ...
        case GOSSIP_SENDER_ABILITIES_USAGE_LIST_SUPPORT:
        {
            ...
            for (std::vector<uint32>::const_iterator itr = myspells->begin(); itr != myspells->end(); ++itr)
            {
                ...
                {
                    SC_LOG_ERROR("scripts", "bot_ai: gossip abilities list overflow with sender {} for bot class {}!",
        ...
        case GOSSIP_SENDER_SPEC:
        {
            ...
            switch (_botclass)
            {
                ...
                default:
                    SC_LOG_ERROR("entities.unit", "bot_ai:GOSSIP_SENDER_SPEC called for class {} with no specs!", uint32(_botclass));
                    return true;
            }
            ...
            if (!reason)
            {
                ...
                if (player->GetLevel() < minlvl)
                {
                    ...
                    switch (minlvl)
                    {
                        ...
                        default:
                            SC_LOG_ERROR("scripts", "No message exists for min class level {}!", uint32(minlvl));
        ...
        case GOSSIP_SENDER_DEBUG_ACTION:
        {
            ...
            switch (action - GOSSIP_ACTION_INFO_DEF)
            {
                ...
                case 6: //reload config
                {
                    ...
                    SC_LOG_INFO("misc", "Re-Loading config settings...");
...
void bot_ai::OnOwnerDamagedBy(Unit* attacker)
{
    ...
    switch (_botclass)
    {
        ...
        case BOT_CLASS_ARCHMAGE:
            ...
            break;
        default:
            //SC_LOG_ERROR("entities.player", "minion_ai: OnOwnerDamagedBy() - unknown bot class {}", uint8(_botclass));
            break;
    }
...
void bot_ai::FillKillReward(GameObject* go) const
{
    ...
    //gold
    float lvl = float(std::min<uint8>(me->GetLevel(), MAX_LEVEL));
    float gold = 125.0f;
...
bool bot_ai::_canLootItemForPlayer(Player* player, Creature* creature, uint8 slot) const
{
    ...
    {
        //SC_LOG_ERROR("scripts", "can't loot item {}, no item", slot);
        return false;
    }

    if (!qitem && item->is_blocked)
    {
        //SC_LOG_ERROR("scripts", "can't loot item {} ({}), blocked", slot, item->itemid);
        return false;
    }

    if (!item->rollWinnerGUID.IsEmpty() && item->rollWinnerGUID != player->GetGUID())
    {
        //SC_LOG_ERROR("scripts", "can't loot item {} ({}), roll won", slot, item->itemid);
        return false;
    }
    ...
    //SC_LOG_ERROR("scripts", "can't loot item {} ({}), can't store", slot, item->itemid);
    return false;
}
bool bot_ai::_canLootCreatureForPlayer(Player* player, Creature* creature, uint32 lootQualityMask, uint32 lootThreshold) const
{
    ...
    for (std::vector<LootItem>::const_iterator i = creature->loot.items.begin(); i != creature->loot.items.end(); ++i)
    {
        ...
        {
            //SC_LOG_ERROR("scripts", "item {} is blocked", i->itemid);
            continue;
        }

        if (!i->rollWinnerGUID.IsEmpty() && i->rollWinnerGUID != player->GetGUID())
        {
            //SC_LOG_ERROR("scripts", "can't loot item {} ({}), roll won", slot, i->itemid);
            continue;
        }
        ...
        if (!itemProto)
        {
            //SC_LOG_ERROR("scripts", "no item proto for itemId {}", i->itemid);
            return false;
        }

        if (itemProto->Quality >= lootThreshold)
        {
            //SC_LOG_ERROR("scripts", "item {} group quality threshold mismatch", i->itemid);
            continue;
        }

        if (!((1 << itemProto->Quality) & lootQualityMask))
        {
            //SC_LOG_ERROR("scripts", "item {} lootQualityMask mismatch", i->itemid);
            continue;
        }
    ...
    if (!canLoot)
    {
        ...
        if (q_itr != lootPlayerQuestItems.end())
        {
            ...
            for (NotNormalLootItemList::const_iterator qi = q_list->begin(); qi != q_list->end(); ++qi)
            {
                ...
                {
                    //SC_LOG_ERROR("scripts", "item {} is looted", i->itemid);
                    continue;
                }
...
bool bot_ai::_canLootCreature(Creature* creature) const
{
    ...
    //SC_LOG_ERROR("scripts", "lootQualityMask {}, lootThreshold {}", lootQualityMask, lootThreshold);
    ...
    for (std::vector<LootItem>::const_iterator ci = loot->quest_items.begin(); ci != loot->quest_items.end(); ++ci)
    {
        ...
        if (!itemProto)
        {
            //SC_LOG_ERROR("scripts", "no item proto for itemId {}", ci->itemid);
            return false;
        }
    ...
    if (!canLootQuality)
    {
        //SC_LOG_ERROR("scripts", "can't loot by quality");
        return false;
    }
    ...
    if (!canLootPlayers)
    {
        //SC_LOG_ERROR("scripts", "can't loot by canLootPlayers");
        return false;
    }
...
void bot_ai::_autoLootCreatureGold(Creature* creature) const
{
    ...
    if (loot->isLooted())
    {
        //SC_LOG_ERROR("scripts", "creature gold is looted, releasing");
...
void bot_ai::_autoLootCreatureItems(Player* receiver, Creature* creature, uint32 lootQualityMask, uint32 lootThreshold) const
{
    ...
    for (std::vector<LootItem>::iterator i = creature->loot.items.begin(); i != creature->loot.items.end(); ++i)
    {
        ...
        {
            //SC_LOG_ERROR("scripts", "looting {} ({}), quality {}, threshold {}",
    ...
    {
        //SC_LOG_ERROR("scripts", "creature items is looted, releasing");
        creature->AllLootRemovedFromCorpse();
        creature->RemoveDynamicFlag(UNIT_DYNFLAG_LOOTABLE);
        creature->loot.clear();
    }
...
void bot_ai::_autoLootCreature(Creature* creature)
{
    ...
    if (!receiver)
    {
        ...
        do
        {
            receiver = Syphrena::Containers::SelectRandomContainerElement(pLooters);
...
bool bot_ai::_resetEquipment(uint8 slot, ObjectGuid receiver)
{
    if (IsWanderer())
    {
        SC_LOG_ERROR("scripts", "bot_ai::_resetEquipment() is called for wanderer bot {} ({}), slot {}!",
    ...
    {
        SC_LOG_ERROR("entities.player", "minion_ai::_resetEquipment(): player {} ({}) failed to reset equipment for bot {} (id: {}) in slot {}",
...
void bot_ai::ApplyItemSetBonuses(Item* item, bool apply)
{
    if (item) //(un)equip, NOT from loops
    {
        ...
        for (uint8 i = 0; i != MAX_ITEM_SET_SPELLS; ++i)
        {
            ...
            //SC_LOG_ERROR("entities.player", "ApplyItemSetBonusesB: {}'s {}, {} ({}), {} ({}), icount {}",
    ...
    for (std::list<uint32>::const_iterator itr = itemSets.begin(); itr != itemSets.end(); ++itr)
    {
        ...
        for (uint8 j = 0; j != MAX_ITEM_SET_SPELLS; ++j)
        {
            ...
            //SC_LOG_ERROR("entities.player", "ApplyItemSetBonusesB (all): {}'s {}, {} ({}), {} ({}), c {}, req {}",
...
float bot_ai::_getItemGearStatScore(ItemTemplate const* iproto, uint8 forslot, Item const* item) const
{
    ...
    //SC_LOG_ERROR("scripts", "_getItemGearScore for {} - {}", proto->ItemId, proto->Name1);

    ItemStatBonus istats = {};
    //for (uint8 i = 0; i != MAX_BOT_ITEM_MOD; ++i)
    //    SC_LOG_ERROR("scripts", "_getItemGearScore at {} {}", uint32(i), istats[i]);
    ...
    //for (uint8 i = 0; i != MAX_BOT_ITEM_MOD; ++i)
    //    SC_LOG_ERROR("scripts", "_getItemGearScore total {} {}", uint32(i), istats[i]);
    ...
    //SC_LOG_ERROR("scripts", "_getItemGearScore total score {}", itemScore);
    return itemScore;
}
    ...
    if (_equips[slot])
    {
            ...
            (_equips[slot]->GetTemplate()->ItemId == transmogData->transmogs[slot].first || BotMgr::TransmogUseEquipmentSlots()))
        {
            ...
            if (item_id > 0)
            {
                ...
                {
                    SC_LOG_ERROR("scripts", "bot_ai::GetEquipDisplayId(): invalid item Id {} for bot {} {} slot {}",
...
void bot_ai::ApplyRacials()
{
    ...
    switch (myrace)
    {
        ...
        case RACE_DRAENEI:
            ...
            break;
        default:
            //SC_LOG_ERROR("entities.player", "bot_ai::ApplyRacePassives(): unknown race {} for bot {} ({})", uint32(me->GetRace()), me->GetName(), me->GetEntry());
            return;
    }
}
...
void bot_ai::InitSpec()
{
    ...
    //SC_LOG_ERROR("entities.unit", "bot_ai::InitSpec(): bot {} class {} spec: {}", me->GetEntry(), uint32(_botclass), uint32(spec));

    if (spec < BOT_SPEC_BEGIN || spec > BOT_SPEC_END)
    {
        SC_LOG_ERROR("entities.unit", "bot_ai::InitSpec(): spec ({}) is out of range for bot {}! Falling to default (1)...",
...
uint8 bot_ai::SelectSpecForClass(uint8 m_class)
{
    ...
    return specs.size() == 1 ? specs.front() : Syphrena::Containers::SelectRandomContainerElement(specs);
}
    ...
    if (IsWanderer())
    {
        ...
        for (uint8 i = BOT_SLOT_MAINHAND; i < BOT_INVENTORY_SIZE; ++i)
        {
            ...
            if (!item)
            {
                ...
                {
                    SC_LOG_WARN("npcbots", "Wanderer bot {} id {} level {} can't generate req gear in slot {}, generating standard item!",
    ...
    else
    {
        ...
        else
        {
            ...
            do
            {
                ...
                ASSERT(found);
                //ItemTemplate const* proto = item->GetTemplate();
                //SC_LOG_ERROR("entities.player", "minion_ai::InitEquips(): bot {} (id: {}): found item: for slot {}: {} (id: {}, guidLow: {})",
...
void bot_ai::_AddItemLink(Player const* forPlayer, Item const* item, std::ostringstream &str, bool addIcon) const
{
    ...
    //SC_LOG_ERROR("entities.player", "bot_ai::_AddItemLink(): {}", str.str());
}
//Unused
...
void bot_ai::_LocalizeItem(Player const* forPlayer, std::string &itemName, std::string &suffix, Item const* item) const
{
    ...
    //SC_LOG_ERROR("entities.player", "bot_ai::_LocalizeItem(): randomPropId = {}", randomPropId);
    if (!randomPropId)
        return;
    ...
    if (randomPropId > 0)
    {
        if (ItemRandomPropertiesEntry const* item_rand = sItemRandomPropertiesStore.LookupEntry(randomPropId))
        {
            ...
            //if (suffs)
            {
                ...
                //    SC_LOG_ERROR("entities.player", "bot_ai::_LocalizeItem(): rand prop suffix for loc {} = {}", i, suffs[i]);
                suffix = suffs[loc];
            }
        }
    }
    else
    {
        if (ItemRandomSuffixEntry const* item_rand = sItemRandomSuffixStore.LookupEntry(-randomPropId))
        {
            ...
            {
                //for (uint8 i = 0; i != MAX_LOCALES; ++i)
                //    SC_LOG_ERROR("entities.player", "bot_ai::_LocalizeItem(): rand suff suffix for loc {} = {}", i, suffs[i]);
                suffix = suffs[loc];
            }
        }
    }
...
void bot_ai::JustDied(Unit* u)
{
    ...
    if (IsTempBot())
    {
        //SC_LOG_ERROR("entities.player", "Unsummoning temp bot {} ({}), owner: {} ({})...",
    ...
    {
        SC_LOG_DEBUG("npcbots", "{} {} id {} class {} level {} WAS KILLED BY {} {} id {} class {} level {} on their way to {}!",
...
void bot_ai::KilledUnit(Unit* u)
{
    ...
    if (u->IsControlledByPlayer() || u->IsPvP() || u->IsNPCBotOrPet())
    {
        ...
        if (!me->GetMap()->IsBattlegroundOrArena())
        {
            if (IsWanderer())
            {
                SC_LOG_DEBUG("npcbots", "Wandering bot {} id {} class {} level {} KILLED {} {} id {} class {} level {} on their way to {}!",
            ...
            {
                SC_LOG_DEBUG("npcbots", "Bot {} id {} class {} level {} KILLED wandering bot {} id {} class {} level {} on their way to {}!",
                    me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()),
                    u->GetName(), u->GetEntry(), uint32(u->GetClass()), uint32(u->GetLevel()),
                    IsWanderer() ? _travel_node_cur->GetName() : "''");
            }
...
void bot_ai::OnBotSpellGo(Spell const* spell, bool ok)
{
    ...
    {
        if (DEBUG_BOT_ORDERS)
            SC_LOG_ERROR("entities.player", "doCast(): ordered spell {} by {} was {}!",
...
void bot_ai::OnBotOwnerSpellGo(Spell const* spell, bool ok)
{
    ...
    //SC_LOG_ERROR("entities.player", "OnBotOwnerSpellGo(): {} by {}", spellInfo->Id, master->GetName());
...
void bot_ai::OnBotSpellInterrupted(SpellSchoolMask schoolMask, uint32 unTimeMs)
{
    ...
    for (BotSpellMap::const_iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        {
            if (DEBUG_BOT_ORDERS)
                SC_LOG_ERROR("entities.player", "doCast(): ordered spell {} was interrupted!", info->Id);
            CompleteOrder(_orders.front());
        }

        itr->second->cooldown += unTimeMs;
        //SC_LOG_ERROR("entities.player", "OnBotSpellInterrupted(): Adding cooldown ({}, new: {}) to spell {} (id: {}, schoolmask: {}), reqSchoolMask = {}",
...
void bot_ai::CastBotItemCombatSpell(DamageInfo const& damageInfo, Item* item, ItemTemplate const* proto)
{
    ...
    if (canTrigger)
    {
        for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
        {
            ...
            if (!spellInfo)
            {
                //SC_LOG_ERROR("entities.player.items", "WORLD: unknown Item spellid {}", spellData.SpellId);
                continue;
            }
...
bool bot_ai::AddOrder(BotOrder&& order)
{
    ...
    {
        SC_LOG_ERROR("scripts", "bot_ai::AddOrder: orders limit reached for {} ({})!", me->GetName(), uint32(_orders.size()));
        return false;
    }
...
void bot_ai::CancelOrder(BotOrder const& order)
{
    if (_orders.empty())
    {
        SC_LOG_ERROR("scripts", "bot_ai::CancelOrder: {} orders are empty while trying to remove order type {}!",
            me->GetName(), uint32(order._type));
        return;
    }
    if (_orders.front()._type != order._type)
    {
        SC_LOG_ERROR("scripts", "bot_ai::CancelOrder: {} front order (type {}) is different from cur order (type {})!",
...
void bot_ai::CompleteOrder(BotOrder const& order)
{
    if (_orders.empty())
    {
        SC_LOG_ERROR("scripts", "bot_ai::CompleteOrder: {} orders are empty while trying to remove order type {}!",
            me->GetName(), uint32(order._type));
        return;
    }
    if (_orders.front()._type != order._type)
    {
        SC_LOG_ERROR("scripts", "bot_ai::CompleteOrder: {} front order (type {}) is different from cur order (type {})!",
            me->GetName(), uint32(_orders.front()._type), uint32(order._type));
        return;
    }
...
void bot_ai::_ProcessOrders()
{
    ...
    switch (order._type)
    {
        case BOT_ORDER_SPELLCAST:
        {
            ...
            {
                SC_LOG_ERROR("scripts", "bot_ai:_ProcessOrders: invalid spellCastParams.targetGuid " UI64FMTD "!", order.params.spellCastParams.targetGuid);
                CancelOrder(order);
                return;
            }
            ...
            {
                SC_LOG_ERROR("scripts", "bot_ai:_ProcessOrders: target " UI64FMTD " not found!", order.params.spellCastParams.targetGuid);
                CancelOrder(order);
                return;
            }
        ...
        default:
            SC_LOG_ERROR("scripts", "bot_ai:_ProcessOrders: invalid order type {}!", uint32(order._type));
            CancelOrder(order);
            return;
    }
}
        ...
        if (order_type == order._type)
        {
            switch (order_type)
            {
                case BOT_ORDER_SPELLCAST:
                    ...
                    break;
                default:
                    SC_LOG_ERROR("scripts", "bot_ai:IsLastOrder: invalid order type {}!", uint32(order_type));
                    break;
            }
        }
    }
...
void bot_ai::DoSkytalonVehicleStrats(uint32 diff)
{
    ...
    else if (drakePower >= 35)
    {
        ...
        else if (HasRole(BOT_ROLE_HEAL))
        {
            ...
            if (!targets1.empty())
                target = Syphrena::Containers::SelectRandomContainerElement(targets1);
    ...
    if (!drakespell)
    {
        SC_LOG_ERROR("scripts", "DoSkytalonVehicleStrats no spell for role mask {} cp {}, power {}, target {}",
            GetBotRoles(), uint32(vehcomboPoints), drakePower, target->GetName());
        return;
    }

    //SC_LOG_ERROR("scripts", "DoSkytalonVehicleStrats {} on {}", drakespell, target->GetName());
...
void bot_ai::DoRubyDrakeVehicleStrats(uint32 diff)
{
    ...
    if (!drakespell)
    {
        SC_LOG_ERROR("scripts", "DoRubyDrakeVehicleStrats no spell for target {}", target->GetName());
        return;
    }
...
void bot_ai::DoEmeraldDrakeVehicleStrats(uint32 diff)
{
    ...
    if (!target)
    {
        ...
        else
        {
            //random target
            std::list<Unit*> targets;
            Syphrena::AnyUnfriendlyUnitInObjectRangeCheck check(drake, drake, 60.f);
            Syphrena::UnitListSearcher <Syphrena::AnyUnfriendlyUnitInObjectRangeCheck> searcher(drake, targets, check);
            ...
            if (!targets.empty())
                target = Syphrena::Containers::SelectRandomContainerElement(targets);
        }
    }
    ...
    if (!drakespell)
    {
        SC_LOG_ERROR("scripts", "DoEmeraldDrakeVehicleStrats no spell for target {}", target->GetName());
        return;
    }
...
void bot_ai::DoAmberDrakeVehicleStrats(uint32 diff)
{
    ...
    if (!drakespell)
    {
        SC_LOG_ERROR("scripts", "DoAmberDrakeVehicleStrats no spell for target {}", target->GetName());
        return;
    }
...
void bot_ai::DoArgentMountVehicleStrats(uint32 diff)
{
    ...
    if (!mountspell)
    {
        SC_LOG_ERROR("scripts", "DoArgentMountVehicleStrats no spell for target {}", target->GetName());
        return;
    }
...
void bot_ai::DoVehicleStrats(BotVehicleStrats strat, uint32 diff)
{
    ...
    {
        //SC_LOG_ERROR("scripts", "DoVehicleStrats doing strat {}", uint32(strat));
        curVehStrat = strat;
    }

    //if (!master->GetVehicle() || me->GetVehicle()->GetCreatureEntry() != master->GetVehicle()->GetCreatureEntry())
    //{
    //    SC_LOG_ERROR("scripts", "DoVehicleStrats cannot do strats: master not on vehicle or on different one");
    //    return;
    //}
    ...
    switch (strat)
    {
        ...
        case BOT_VEH_STRAT_GENERIC:
            ...
            break;
        default:
            SC_LOG_ERROR("scripts", "Unhandled vehicle strat {}", uint32(strat));
            break;
    }
}
    ...
    switch (me->GetVehicleBase()->GetEntry())
    {
        ...
        */
        default:
            ...
            if (curVehStrat != strat)
                SC_LOG_DEBUG("scripts", "bot_ai DoVehicleActions: {} has to use generic strat for vehicle creature {} ({})",
...
bool bot_ai::GlobalUpdate(uint32 diff)
{
    ...
    {
        if (IAmFree())
            SC_LOG_ERROR("scripts", "bot_ai::GlobalUpdate is called for free bot not in world: {} ({}) class {} level {}",
                me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()));
        return false;
    }
    ...
    if (_updateTimerEx1 <= diff)
    {
        ...
        */
        ...
        {
            //SC_LOG_ERROR("spells", "bot_ai:UpdateEx by {}, found free master, my skills: {}:", me->GetName(), uint32(_roleMask & BOT_ROLE_MASK_GATHERING));
            ...
            Syphrena::WorldObjectListSearcher<NearbyObjectBySkillCheck> searcher(me, woList, check, GRID_MAP_TYPE_MASK_CREATURE|GRID_MAP_TYPE_MASK_GAMEOBJECT);
            Cell::VisitAllObjects(me, searcher, 20.f);
            //me->VisitNearbyObject(20.f, searcher);
            //SC_LOG_ERROR("spells", "bot_ai:UpdateEx: list is {}", woList.empty() ? "empty" : "not empty");
            if (!woList.empty())
            {
                ...
                wo = wo ? wo : Syphrena::Containers::SelectRandomContainerElement(woList);
                //SC_LOG_ERROR("spells", "bot_ai:UpdateEx: processing {}", wo->GetName());
        ...
        if (me->IsInWorld() && !IAmFree() && HasRole(BOT_ROLE_AUTOLOOT) && HasRole(BOT_ROLE_MASK_LOOTING) &&
            ...
            !HasBotCommandState(BOT_COMMAND_MASK_UNMOVING))
        {
            //SC_LOG_ERROR("scripts", "bot_ai Looting by {}, my mask: {}:", me->GetName(), uint32(_roleMask & BOT_ROLE_MASK_LOOTING));
            ...
            Syphrena::UnitListSearcher<NearbyLootableCreatureCheck> searcher(me, crList, check);
            ...
            for (std::list<Unit*>::iterator itr = crList.begin(); itr != crList.end();)
            {
                ...
                {
                    //SC_LOG_ERROR("scripts", "bot_ai Looting: cannot loot {} ({})", (*itr)->GetName(), (*itr)->GetGUIDLow());
                    crList.erase(itr++);
                    continue;
                }
                ++itr;
            ...
            if (!crList.empty())
            {
                Unit* un = Syphrena::Containers::SelectRandomContainerElement(crList);
                ...
                /*//SC_LOG_ERROR("scripts", "bot_ai Looting: processing {}", un->GetName());
                if (me->GetDistance(un) < INTERACTION_DISTANCE + 2.f)
                {
                    //SC_LOG_ERROR("scripts", "bot_ai Looting: looting {}", un->GetName());
                    _autoLootCreature(un->ToCreature());
                    _updateTimerEx1 = urand(500, 1000);
                }
                else
                {
                    //SC_LOG_ERROR("scripts", "bot_ai Looting: moving towards {}", un->GetName());
                    Position pos;
    ...
    if (checkAurasTimer <= lastdiff)
    {
        ...
        {
            //SC_LOG_ERROR("entities.player", "CheckAuras(): adding {} to group", me->GetName());
            ...
            (!mover->isMoving() || Rand() < 50) && !IsCasting(mover) && !IsShootingWand(mover))
        {
            if (Unit* victim = CanBotAttackOnVehicle() ? me->GetVictim() : mover->GetTarget() ? ObjectAccessor::GetUnit(*mover, mover->GetTarget()) : nullptr)
            {
                ...
                //SC_LOG_ERROR("scripts", "GetInPos prepare by {}", me->GetName());
                if (!IAmFree() && master->GetBotMgr()->GetBotAttackRangeMode() == BOT_ATTACK_RANGE_EXACT &&
                ...
                {
                    //SC_LOG_ERROR("scripts", "{} calculates attack pos to attack {}", me->GetName(), victim->GetName());
                    bool force = false;
                    CalculateAttackPos(victim, attackpos, force);
                    if (mover->GetExactDist2d(&attackpos) > (force ? 0.1f : 4.f) || (force && IsWanderer()))
                    {
                        //SC_LOG_ERROR("scripts", "{} moving to x {} y {} z {} to attack {}",
        ...
        (!me->GetVehicle() || (!CCed(me->GetVehicleBase(), true) && !me->GetVehicleBase()->GetTarget())))
    {
        ...
        else if (!IsCasting(mover) && (!IsShootingWand(mover) || Rand() < 10))
        {
            ...
            if (!mmover && me->GetMap()->IsBattleground() && !IsFlagCarrier(me))
            {
                ...
                Unit* nmover = nullptr;
                Syphrena::UnitSearcher searcher(me, nmover, flag_carrier_pred);
        ...
        if (IAmFree())
        {
            ...
            if (IsWanderer())
            {
                ...
                if (me->GetMap()->GetEntry()->IsContinent() || me->GetMap()->IsBattleground())
                {
                    ...
                    if (!nextNode)
                    {
                        SC_LOG_FATAL("scripts", "Bot {} ({}) is unable to get next travel node (1)! cur {}, last {}, position: {}. BOT WAS DISABLED",
                    ...
                    SC_LOG_TRACE("npcbots", "Bot {} id {} class {} level {} died on the way from node {} to {} ('{}'), NEW {} ('{}'), {}, dist {} yd!",
...
void bot_ai::Evade()
{
    ...
    if (IsWanderer())
    {
        ...
        {
            SC_LOG_DEBUG("npcbots", "Bot {} id {} class {} level {} map {} TELEPORTING to node {} ('{}') map {}, {}, dist {} yd!",
    ...
    if (dist > 1.5f || IsWanderer())
    {
        ...
        if (!me->isMoving())
        {
            ...
            if (dist > (me->GetMap()->GetEntry()->IsContinent() ? 15.0f : 3.0f))
            {
                ...
                {
                    SC_LOG_ERROR("npcbots", "Bot {} '{}' class {} level {} evade move point has invalid height {} (usepath: {})!\nWPs: cur {}, last {}\nPositions:\ncurrent: {}\ntarget: {}",
            ...
            else if (IsWanderer())
            {
                ...
                if (!nextNode)
                {
                    SC_LOG_FATAL("npcbots", "Bot {} ({}) is unable to get next travel node! cur {}, last {}, position: {}. BOT WAS DISABLED",
                        me->GetName(), me->GetEntry(), _travel_node_cur->GetWPId(), _travel_node_last ? _travel_node_last->GetWPId() : 0, me->GetPosition().ToString());
                    canUpdate = false;
                    return;
                }
                ...
                SC_LOG_TRACE("npcbots", "Bot {} id {} class {} level {} wandered from node {} to {}, next {} ('{}'), {}, dist {} yd!",
                    me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()), _travel_node_last ? _travel_node_last->GetWPId() : 0, _travel_node_cur->GetWPId(),
...
void bot_ai::GetNextEvadeMovePoint(Position& pos, bool& use_path) const
{
    ...
    while (path.GetPathType() == PATHFIND_BLANK || (path.GetPathType() & (PATHFIND_NOPATH | PATHFIND_SHORTCUT | PATHFIND_SHORT)))
    {
        if (std::fabs(fulldist - me->GetExactDist2d(pos)) > 15.0f)
        {
            ...
            Syphrena::NormalizeMapCoord(pos.m_positionX);
            Syphrena::NormalizeMapCoord(pos.m_positionY);
    ...
    if (me->IsInWater())
        SC_LOG_TRACE("npcbots", "Bot {} id {} class {} level {} is pathing from water!", me->GetName(), me->GetEntry(), uint32(_botclass), uint32(me->GetLevel()));
    ...
    switch (path.GetPathType())
    {
        ...
        case PATHFIND_FARFROMPOLY_END: //invalid end coords
            ...
            SC_LOG_DEBUG("npcbots", "Bot {} id {} class {} level {} can't find full path to node {} (res {}) from pos {}, falling back to default PF!",
    ...
    Syphrena::NormalizeMapCoord(mypos.m_positionX);
    Syphrena::NormalizeMapCoord(pos.m_positionY);
        ...
        case BATTLEGROUND_AV:
        {
            ...
            for (auto const& p : { std::pair{TEAM_ALLIANCE, CRETYPE_BOSS_A}, std::pair{TEAM_HORDE, CRETYPE_BOSS_H} })
            {
                ...
                if (boss->IsInCombat())
                {
                    ...
                    if (!vlinks.empty())
                        return vlinks.size() == 1u ? vlinks.front() : Syphrena::Containers::SelectRandomContainerElement(vlinks);
                }
            }
            ...
            for (TeamId teamId : { TEAM_ALLIANCE, TEAM_HORDE })
            {
                ...
                if (WanderNode const* dnode = defNode.second)
                {
                    ...
                    if (!defLinks.empty())
                        return defLinks.size() == 1u ? defLinks.front() : Syphrena::Containers::SelectRandomContainerElement(defLinks);
                }
                ...
                if (!assdlist.empty())
                    return assdlist.size() == 1u ? assdlist.front() : Syphrena::Containers::SelectRandomContainerElement(assdlist);
                if (!assalist.empty())
                    return assalist.size() == 1u ? assalist.front() : Syphrena::Containers::SelectRandomContainerElement(assalist);
            }
                ...
                for (BG_AV_CreaturePlace sptype : p.second)
                {
                    ...
                    if (mboss->IsAlive() && !mboss->IsInCombat() && me->IsWithinDist2d(mboss, SIZE_OF_GRIDS * 0.75f))
                    {
                        ...
                        if (!mlinks.empty())
                        {
                            ...
                            {
                                SC_LOG_DEBUG("npcbots", "Bot {} {} team {} goes for a mine! Cur node: {} {}",
                                    me->GetName(), me->GetEntry(), uint32(myTeamId), curNode->GetWPId(), curNode->GetName());
                                return mlinks.size() == 1u ? mlinks.front() : Syphrena::Containers::SelectRandomContainerElement(mlinks);
                            }
                        }
                    }
                }
            ...
            if (links.size() > 1)
            {
                SC_LOG_DEBUG("npcbots", "Bot {} {} team {} has no target point in BG_AV! Falling back to random ({} links)!. Cur node: {} {}",
    ...
    if (Battleground* bg = GetBG())
    {
        if (_travel_node_cur->HasFlag(BotWPFlags::BOTWP_FLAG_BG_FLAG_DELIVER_TARGET) && IsFlagCarrier(me, bg->GetTypeID()))
        {
            switch (bg->GetTypeID())
            {
                ...
                case BATTLEGROUND_TP:
                    if (bg->GetBotTeamId(me->GetGUID()) == TEAM_ALLIANCE && _travel_node_cur->HasFlag(BotWPFlags::BOTWP_FLAG_ALLIANCE_ONLY))
                        bg->HandleBotAreaTrigger(me, 5904);
                    if (bg->GetBotTeamId(me->GetGUID()) == TEAM_HORDE && _travel_node_cur->HasFlag(BotWPFlags::BOTWP_FLAG_HORDE_ONLY))
                        bg->HandleBotAreaTrigger(me, 5905);
                    break;
                default:
                    break;
            }
        ...
        if (_travel_node_cur->HasFlag(BotWPFlags::BOTWP_FLAG_BG_FLAG_PICKUP_TARGET) && !IsFlagCarrier(me, bg->GetTypeID()))
        {
            switch (bg->GetTypeID())
            {
                case BATTLEGROUND_AV:
                {
                    ...
                    //SC_LOG_ERROR("npcbots", "OnWanderNodeReached: [AV] Bot {} USES flag {} at node {}", me->GetName(), obj->GetName(), node);
                ...
                case BATTLEGROUND_WS:
                {
                    if (bg->GetBotTeamId(me->GetGUID()) == TEAM_ALLIANCE && _travel_node_cur->HasFlag(BotWPFlags::BOTWP_FLAG_HORDE_ONLY))
                    {
                        ...
                        {
                            //SC_LOG_ERROR("npcbots", "OnWanderNodeReached: [WSG] Horde flag dist: {}", me->GetExactDist(go));
                            if (me->IsMounted())
                                DismountBot();
                    ...
                    if (bg->GetBotTeamId(me->GetGUID()) == TEAM_HORDE && _travel_node_cur->HasFlag(BotWPFlags::BOTWP_FLAG_ALLIANCE_ONLY))
                    {
                        ...
                        {
                            //SC_LOG_ERROR("npcbots", "OnWanderNodeReached: [WSG] Alliance flag dist: {}", me->GetExactDist(go));
                            if (me->IsMounted())
                                DismountBot();
                            bg->EventBotClickedOnFlag(me, go);
                        }
                ...
                case BATTLEGROUND_AB:
                {
                    ...
                    if (node < BG_AB_DYNAMIC_NODES_COUNT)
                    {
                        ...
                        //SC_LOG_ERROR("npcbots", "OnWanderNodeReached: [AB] Bot {} USES flag {} at node {}",
                ...
                case BATTLEGROUND_BFG:
                {
                    ...
                    GameObject* obj = bg->GetBGObject(node * 8 + BG_BFG_OBJECT_BANNER_NEUTRAL);
                    while (node < BG_BFG_DYNAMIC_NODES_COUNT && (!obj || !me->IsWithinDistInMap(obj, 10.0f)))
                    {
                        ++node;
                        obj = bg->GetBGObject(node * 8 + BG_BFG_OBJECT_BANNER_NEUTRAL);
                    }
                    if (node < BG_BFG_DYNAMIC_NODES_COUNT)
                    {
                        TeamId teamId = bg->GetBotTeamId(me->GetGUID());
                        BattlegroundBFG const* bgbfg = dynamic_cast<BattlegroundBFG const*>(bg);

                        if (bgbfg->IsNodeOccupied(node, teamId) || bgbfg->IsNodeContested(node, teamId))
                            break;

                        //at this point node is either neutral or owned/contested by other team
                        uint8 new_bg_obj_type;
                        if (bgbfg->IsNodeOccupied(node, bg->GetOtherTeamId(teamId)))
                            new_bg_obj_type = BG_BFG_OBJECT_BANNER_HORDE;
                        else if (bgbfg->IsNodeContested(node, bg->GetOtherTeamId(teamId)))
                            new_bg_obj_type = BG_BFG_OBJECT_BANNER_CONT_H;
                        else
                            new_bg_obj_type = BG_BFG_OBJECT_BANNER_NEUTRAL;
                ...
                case BATTLEGROUND_TP:
                {
                    if (bg->GetBotTeamId(me->GetGUID()) == TEAM_ALLIANCE && _travel_node_cur->HasFlag(BotWPFlags::BOTWP_FLAG_HORDE_ONLY))
                    {
                        if (GameObject* go = bg->GetBGObject(BG_TP_OBJECT_H_FLAG, true))
                        {
                            //SC_LOG_ERROR("npcbots", "OnWanderNodeReached: [TP] Horde flag dist: {}", me->GetExactDist(go));
                            if (me->IsMounted())
                                DismountBot();
                            bg->EventBotClickedOnFlag(me, go);
                        }
                    ...
                    if (bg->GetBotTeamId(me->GetGUID()) == TEAM_HORDE && _travel_node_cur->HasFlag(BotWPFlags::BOTWP_FLAG_ALLIANCE_ONLY))
                    {
                        if (GameObject* go = bg->GetBGObject(BG_TP_OBJECT_A_FLAG, true))
                        {
                            //SC_LOG_ERROR("npcbots", "OnWanderNodeReached: [TP] Alliance flag dist: {}", me->GetExactDist(go));
                            if (me->IsMounted())
                                DismountBot();
                            bg->EventBotClickedOnFlag(me, go);
                        }
...
void bot_ai::AfterBotOwnerEnterVehicle()
{
        ...
        master->GetVehicleBase()->IsControlledByPlayer())
    {
        ...
        if (seat && seat->CanEnterOrExit())
        {
            ...
            if (!creEntry)
            {
                SC_LOG_DEBUG("scripts", "OnBotOwnerEnterVehicle: no vehicle selected for bot master veh {}!",
    ...
    //SC_LOG_ERROR("scripts", "OnBotOwnerEnterVehicle: master not in vehicle or no veh found for bot {}!", me->GetName());
}
...
void bot_ai::ChooseVehicleForEncounter(uint32 &creEntry, uint32 &vehEntry) const
{
    ...
    switch (mVeh->GetBase()->GetEntry())
    {
        ...
        default:
            ...
            SC_LOG_ERROR("scripts", "ChooseVehicleForEncounter: unhandled master vehicle creature {} ({})",
...
void bot_ai::SetContestedPvP()
{
    ...
    if (!me->HasUnitState(UNIT_STATE_ATTACK_PLAYER))
    {
        ...
        Syphrena::AIRelocationNotifier notifier(*me);
    ...
    if (botPet && !botPet->HasUnitState(UNIT_STATE_ATTACK_PLAYER))
    {
        ...
        Syphrena::AIRelocationNotifier notifier(*botPet);
        Cell::VisitWorldObjects(me, notifier, me->GetVisibilityRange());
    }
}
