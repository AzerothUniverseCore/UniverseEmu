...
bool SmartAI::LoadPath(uint32 entry)
{
    ...
    {
        Syphrena::NormalizeMapCoord(waypoint.x);
        Syphrena::NormalizeMapCoord(waypoint.y);
...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    {
        SC_LOG_ERROR("scripts.ai.sai", "SmartAI::PausePath: Creature wanted to pause waypoint (current waypoint: {}) movement while already paused, ignoring. ({})", _currentWaypointNode, me->GetGUID().ToString());
        return;
    }
...
class SmartTrigger : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) override
        {
            ...
            SC_LOG_DEBUG("scripts.ai", "AreaTrigger {} is using SmartTrigger script", trigger->ID);
            SmartScript script;
