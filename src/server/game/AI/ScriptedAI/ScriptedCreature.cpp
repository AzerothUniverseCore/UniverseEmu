...
{
    if (max)
        Syphrena::Containers::RandomResize(summons, max);
...
void ScriptedAI::DoPlaySoundToSet(WorldObject* source, uint32 soundId)
{
    ...
    {
        SC_LOG_ERROR("scripts.ai", "ScriptedAI::DoPlaySoundToSet: Invalid soundId {} used in DoPlaySoundToSet (Source: {})", soundId, source->GetGUID().ToString());
        return;
    }
...
{
    SC_LOG_DEBUG("scripts.ai", "ScriptedAI::ForceCombatStopForCreatureEntry: called on '{}'. Debug info: {}", me->GetGUID().ToString(), me->GetDebugInfo());

    std::list<Creature*> creatures;
    Syphrena::AllCreaturesOfEntryInRange check(me, entry, maxSearchRange);
    Syphrena::CreatureListSearcher<Syphrena::AllCreaturesOfEntryInRange> searcher(me, creatures, check);
...
void ScriptedAI::DoTeleportPlayer(Unit* unit, float x, float y, float z, float o)
{
    ...
    if (Player* player = unit->ToPlayer())
        ...
        SC_LOG_ERROR("scripts.ai", "ScriptedAI::DoTeleportPlayer: Creature {} Tried to teleport non-player unit ({}) to x: {} y:{} z: {} o: {}. Aborted.",
...
{
    Unit* unit = nullptr;
    Syphrena::FriendlyBelowHpPctEntryInRange u_check(me, entry, range, minHPDiff, excludeSelf);
    Syphrena::UnitLastSearcher<Syphrena::FriendlyBelowHpPctEntryInRange> searcher(me, unit, u_check);
    Cell::VisitAllObjects(me, searcher, range);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float range)
{
    ...
    Syphrena::CreatureListSearcher<Syphrena::FriendlyCCedInRange> searcher(me, list, u_check);
    Cell::VisitAllObjects(me, searcher, range);
...
{
    std::list<Creature*> list;
    Syphrena::FriendlyMissingBuffInRange u_check(me, range, uiSpellid);
    Syphrena::CreatureListSearcher<Syphrena::FriendlyMissingBuffInRange> searcher(me, list, u_check);
    Cell::VisitAllObjects(me, searcher, range);
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float minimumRange)
{
    ...
    Syphrena::PlayerAtMinimumRangeAway check(me, minimumRange);
    Syphrena::PlayerSearcher<Syphrena::PlayerAtMinimumRangeAway> searcher(me, player, check);
...
void BossAI::_DespawnAtEvade(Seconds delayToRespawn /*= 30s*/, Creature* who /*= nullptr*/)
{
    ...
    {
        SC_LOG_ERROR("scripts.ai", "BossAI::_DespawnAtEvade: called with delay of {} seconds, defaulting to 2 (me: {})", delayToRespawn.count(), me->GetGUID().ToString());
        delayToRespawn = 2s;
    }
    ...
    {
        SC_LOG_WARN("scripts.ai", "BossAI::_DespawnAtEvade: called on a temporary summon (who: {})", who->GetGUID().ToString());
        whoSummon->UnSummon();
        return;
    }
