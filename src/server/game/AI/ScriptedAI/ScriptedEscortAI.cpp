...
void EscortAI::EnterEvadeMode(EvadeReason /*why*/)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        ReturnToLastPoint();
        SC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::EnterEvadeMode: left combat and is now returning to last point ({})", me->GetGUID().ToString());
    }
    else
    {
...
void EscortAI::MovementInform(uint32 type, uint32 id)
{
    ...
    if (type == POINT_MOTION_TYPE)
    {
        ...
        {
            SC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::MovementInform: returned to before combat position ({})", me->GetGUID().ToString());
            me->SetWalk(!_running);
            RemoveEscortState(STATE_ESCORT_RETURNING);
        }
        else if (id == POINT_HOME)
        {
            SC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::MovementInform: returned to home location and restarting waypoint path ({})", me->GetGUID().ToString());
            _started = false;
        }
    }
    else if (type == WAYPOINT_MOTION_TYPE)
    {
        ...
        SC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::MovementInform: waypoint node {} reached ({})", waypoint.id, me->GetGUID().ToString());
...
void EscortAI::UpdateAI(uint32 diff)
{
        ...
        if (_pauseTimer.count() <= diff)
        {
            if (!HasEscortState(STATE_ESCORT_PAUSED))
            {
                ...
                if (_ended)
                {
                    ...
                    if (_despawnAtEnd)
                    {
                        SC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::UpdateAI: reached end of waypoints, despawning at end ({})", me->GetGUID().ToString());
                        if (_returnToStart)
                        {
                            ...
                            SC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::UpdateAI: returning to spawn location: {} ({})", respawnPosition.ToString(), me->GetGUID().ToString());
                        }
                    ...
                    SC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::UpdateAI: reached end of waypoints ({})", me->GetGUID().ToString());
    ...
    if (_despawnAtFar && HasEscortState(STATE_ESCORT_ESCORTING) && _playerGUID && !me->IsEngaged() && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (_playerCheckTimer <= diff)
        {
            ...
            {
                SC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::UpdateAI: failed because player/group was to far away or not found ({})", me->GetGUID().ToString());
...
void EscortAI::Start(bool isActiveAttacker /* = true*/, bool run /* = false */, ObjectGuid playerGUID /* = 0 */, Quest const* quest /* = nullptr */, bool instantRespawn /* = false */, bool canLoopPath /* = false */, bool resetWaypoints /* = true */)
{
    ...
    if (me->IsEngaged())
    {
        SC_LOG_ERROR("scripts.ai.escortai", "EscortAI::Start: (script: {}) attempts to Start while in combat ({})", me->GetScriptName(), me->GetGUID().ToString());
        return;
    }

    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        SC_LOG_ERROR("scripts.ai.escortai", "EscortAI::Start: (script: {}) attempts to Start while already escorting ({})", me->GetScriptName(), me->GetGUID().ToString());
        return;
    }
    ...
    {
        SC_LOG_ERROR("scripts.ai.escortai", "EscortAI::Start: (script: {}) is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn ({})", me->GetScriptName(), me->GetGUID().ToString());
        return;
    }
    ...
    SC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::Start: (script: {}) started with {} waypoints. ActiveAttacker = {}, Run = {}, Player = {} ({})",
...
void EscortAI::FillPointMovementListForCreature()
{
    ...
    for (WaypointNode const& value : path->nodes)
    {
        ...
        Syphrena::NormalizeMapCoord(node.x);
        Syphrena::NormalizeMapCoord(node.y);
