...
PlayerAI::PlayerAI(Player* player) : UnitAI(player), me(player),
    _selfSpec(Syphrena::Helpers::Entity::GetPlayerSpecialization(player)),
    _isSelfHealer(Syphrena::Helpers::Entity::IsPlayerHealer(player)),
    _isSelfRangedAttacker(Syphrena::Helpers::Entity::IsPlayerRangedAttacker(player))
{
}
...
{
    return (!who || who == me) ? _selfSpec : Syphrena::Helpers::Entity::GetPlayerSpecialization(who);
}

bool PlayerAI::IsHealer(Player const * who) const
{
    return (!who || who == me) ? _isSelfHealer : Syphrena::Helpers::Entity::IsPlayerHealer(who);
}

bool PlayerAI::IsRangedAttacker(Player const * who) const
{
    return (!who || who == me) ? _isSelfRangedAttacker : Syphrena::Helpers::Entity::IsPlayerRangedAttacker(who);
}
