...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (me->GetVictim() && me->EnsureVictim()->IsAlive())
    {
        ...
        if (NeedToStop())
        {
            SC_LOG_TRACE("scripts.ai.petai", "PetAI::UpdateAI: AI stopped attacking {}", me->GetGUID().ToString());
            StopAttack();
            return;
        }
...
void PetAI::HandleReturnMovement()
{
    ...
    if (!me->GetCharmInfo())
    {
        SC_LOG_WARN("scripts.ai.petai", "me->GetCharmInfo() is NULL in PetAI::HandleReturnMovement(). Debug info: {}", GetDebugInfo());
        return;
    }
...
bool PetAI::CanAttack(Unit* target)
{
    ...
    if (!me->GetCharmInfo())
    {
        SC_LOG_WARN("scripts.ai.petai", "me->GetCharmInfo() is NULL in PetAI::CanAttack(). Debug info: {}", GetDebugInfo());
        return false;
    }
