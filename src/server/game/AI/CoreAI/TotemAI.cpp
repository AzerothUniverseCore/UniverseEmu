...
void TotemAI::UpdateAI(uint32 /*diff*/)
{
    ...
    if (!victim || !victim->isTargetableForAttack() || !me->IsWithinDistInMap(victim, max_range) || me->IsFriendlyTo(victim) || !me->CanSeeOrDetect(victim))
    {
        ...
        Syphrena::NearestAttackableUnitInObjectRangeCheck u_check(me, me->GetCharmerOrOwnerOrSelf(), max_range);
        Syphrena::UnitLastSearcher<Syphrena::NearestAttackableUnitInObjectRangeCheck> checker(me, victim, u_check);
