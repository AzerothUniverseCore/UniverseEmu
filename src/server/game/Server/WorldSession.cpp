...
void WorldSession::SendPacket(WorldPacket const* packet)
{
    ...
    else
    {
        ...
        SC_LOG_DEBUG("misc", "Send all time packets count: {} bytes: {} avr.count/sec: {} avr.bytes/sec: {} time: {}", sendPacketCount, sendPacketBytes, float(sendPacketCount)/fullTime, float(sendPacketBytes)/fullTime, uint32(fullTime));
        SC_LOG_DEBUG("misc", "Send last min packets count: {} bytes: {} avr.count/sec: {} avr.bytes/sec: {}", sendLastPacketCount, sendLastPacketBytes, float(sendLastPacketCount)/minTime, float(sendLastPacketBytes)/minTime);
    ...
    SC_LOG_TRACE("network.opcode", "S->C: {} {}", GetPlayerInfo(), GetOpcodeNameForLogging(static_cast<OpcodeServer>(packet->GetOpcode())));
...
{
    SC_LOG_ERROR("network.opcode", "Received unexpected opcode {} Status: {} Reason: {} from {}",
...
void WorldSession::LogUnprocessedTail(WorldPacket* packet)
{
    ...
    SC_LOG_TRACE("network.opcode", "Unprocessed tail data (read stop at {} from {}) Opcode {} from {}",
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
    ...
    while (m_Socket && _recvQueue.next(packet, updater))
    {
        ...
        SC_METRIC_DETAILED_TIMER("worldsession_update_opcode_time", SC_METRIC_TAG("opcode", opHandle->Name));
        ...
        try
        {
            switch (opHandle->Status)
            {
                case STATUS_LOGGEDIN:
                    if (!_player)
                    {
                        ...
                        if (!m_playerRecentlyLogout)
                        {
                            ...
                            deletePacket = false;
                            SC_LOG_DEBUG("network", "Re-enqueueing packet with opcode {} with with status STATUS_LOGGEDIN. "
                ...
                case STATUS_AUTHED:
                    ...
                    break;
                case STATUS_NEVER:
                    SC_LOG_ERROR("network.opcode", "Received not allowed opcode {} from {}", GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode()))
                        , GetPlayerInfo());
                    break;
                case STATUS_UNHANDLED:
                    SC_LOG_DEBUG("network.opcode", "Received not handled opcode {} from {}", GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode()))
                        , GetPlayerInfo());
                    break;
            }
        }
        ...
        {
            SC_LOG_ERROR("network", "{} sent {} with an invalid link:\n{}", GetPlayerInfo(),
        ...
        {
            SC_LOG_ERROR("network", "{} sent {} which illegally contained a hyperlink:\n{}", GetPlayerInfo(),
                GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())), ihe.GetInvalidValue());
        ...
        {
            SC_LOG_ERROR("network", "PacketArrayMaxCapacityException: {} while parsing {} from {}.",
                pamce.what(), GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())), GetPlayerInfo());
        }
        catch (ByteBufferException const&)
        {
            SC_LOG_ERROR("network", "WorldSession::Update ByteBufferException occured while parsing a packet (opcode: {}) from client {}, accountid=%i. Skipped packet.",
    ...
    SC_METRIC_VALUE("processed_packets", processedPackets);
...
void WorldSession::LogoutPlayer(bool save)
{
    ...
    if (_player)
    {
        ...
        SC_METRIC_EVENT("player_events", "Logout", _player->GetName());
        ...
        SC_LOG_INFO("entities.player.character", "Account: {} (IP: {}) Logout Character:[{}] {} Level: {}, XP: {}/{} ({} left)",
        ...
        SC_LOG_DEBUG("network", "SESSION: Sent SMSG_LOGOUT_COMPLETE Message");
...
void WorldSession::KickPlayer(std::string const& reason)
{
    if (m_Socket)
    {
        SC_LOG_INFO("network.kick", "Account: {} Character: '{}' {} kicked with reason: {}", GetAccountId(), _player ? _player->GetName() : "<none>",
...
{
    if (Syphrena::Hyperlinks::CheckAllLinks(str))
        return true;

    SC_LOG_ERROR("network", "Player {}{} sent a message with an invalid link:\n{}", GetPlayer()->GetName(),
...
bool WorldSession::DisallowHyperlinksAndMaybeKick(std::string const& str)
{
    ...
    SC_LOG_ERROR("network", "Player {} {} sent a message which illegally contained a hyperlink:\n{}", GetPlayer()->GetName(),
                 GetPlayer()->GetGUID().ToString(), str);
...
{
    char const* format = GetSyphrenaString(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
char const* WorldSession::GetSyphrenaString(uint32 entry) const
{
    return sObjectMgr->GetSyphrenaString(entry, GetSessionDbLocaleIndex());
}
...
{
    SC_LOG_ERROR("network.opcode", "Received unhandled opcode {} from {}", GetOpcodeNameForLogging(static_cast<OpcodeClient>(null.GetOpcode())), GetPlayerInfo());
}

void WorldSession::Handle_EarlyProccess(WorldPacket& recvPacket)
{
    SC_LOG_ERROR("network.opcode", "Received opcode {} that must be processed in WorldSocket::ReadDataHandler from {}"
        , GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvPacket.GetOpcode())), GetPlayerInfo());
}

void WorldSession::Handle_ServerSide(WorldPacket& recvPacket)
{
    SC_LOG_ERROR("network.opcode", "Received server-side opcode {} from {}"
        , GetOpcodeNameForLogging(static_cast<OpcodeServer>(recvPacket.GetOpcode())), GetPlayerInfo());
}

void WorldSession::Handle_Deprecated(WorldPacket& recvPacket)
{
    SC_LOG_ERROR("network.opcode", "Received deprecated opcode {} from {}"
        , GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvPacket.GetOpcode())), GetPlayerInfo());
}
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask)
{
    ...
    do
    {
        ...
        {
            SC_LOG_ERROR("misc", "Table `{}` have invalid account data type ({}), ignore.",
        ...
        if ((mask & (1 << type)) == 0)
        {
            SC_LOG_ERROR("misc", "Table `{}` have non appropriate for table account data type ({}), ignore.",
                mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
            continue;
        }
...
void WorldSession::ReadMovementInfo(WorldPacket &data, MovementInfo* mi)
{
    ...
    { \
        if (check) \
        { \
            SC_LOG_DEBUG("entities.unit", "WorldSession::ReadMovementInfo: Violation of MovementFlags found ({}). " \
    ...
    if (mi->guid.IsEmpty())
    {
        SC_LOG_ERROR("entities.unit", "WorldSession::ReadMovementInfo: mi->guid is empty, opcode {}", static_cast<uint32>(data.GetOpcode()));
        return;
    }
    ...
    if (!mover)
    {
        SC_LOG_ERROR("entities.unit", "WorldSession::ReadMovementInfo: If the server allows the unit (GUID {}) to be moved by the client of player {}, the unit should still exist! Opcode {}",
            mi->guid.ToString(),
...
void WorldSession::ReadAddonsInfo(ByteBuffer &data)
{
    ...
    if (size > 0xFFFFF)
    {
        SC_LOG_DEBUG("addon", "WorldSession::ReadAddonsInfo: AddOnInfo too big, size {}", size);
        return;
    }
    ...
    if (uncompress(addonInfo.contents(), &uSize, data.contents() + pos, data.size() - pos) == Z_OK)
    {
        try
        {
            ...
            for (uint32 i = 0; i < addonsCount; ++i)
            {
                ...
                SC_LOG_DEBUG("addon", "AddOn: {} (CRC: 0x{:x}) - has key: 0x%x - URL CRC: 0x%x", addon.Name, publicKeyCrc, addon.HasKey, urlCrc);
                ...
                if (savedAddon)
                {
                    if (publicKeyCrc != savedAddon->CRC)
                    {
                        if (addon.HasKey)
                        {
                            ...
                            SC_LOG_WARN("addon", " Addon: {}: modified (CRC: 0x{:x}) - accountID {})", addon.Name, savedAddon->CRC, GetAccountId());
                        }
                        else
                    ...
                    {
                        addon.Status = Addons::SecureAddonInfo::SECURE_HIDDEN;
                        SC_LOG_DEBUG("addon", "Addon: {}: validated (CRC: 0x{:x}) - accountID {}", addon.Name, savedAddon->CRC, GetAccountId());
                    }
                }
                else
                {
                    addon.Status = Addons::SecureAddonInfo::BANNED;
                    SC_LOG_WARN("addon", "Addon: {}: not registered as known secure addon - accountId {}", addon.Name, GetAccountId());
                }
            }
            ...
            SC_LOG_DEBUG("addon", "AddOn: Newest banned addon timestamp: {}", _addons.LastBannedAddOnTimestamp);
        }
        catch (ByteBufferException const& e)
        {
            SC_LOG_DEBUG("addon", "AddOn: Addon packet read error! {}", e.what());
        }
    }
    else
        SC_LOG_DEBUG("addon", "AddOn: Addon packet uncompress error!");
}
...
void WorldSession::SendAddonsInfo()
{
    ...
    for (Addons::SecureAddonInfo const& addonInfo : _addons.SecureAddons)
    {
        ...
        if (infoProvided)
        {
            ...
            {
                SC_LOG_DEBUG("addon", "AddOn: {}: key missing: sending pubkey to accountID {}", addonInfo.Name, GetAccountId());
...
QueryCallback WorldSession::LoadPermissionsAsync()
{
    ...
    SC_LOG_DEBUG("rbac", "WorldSession::LoadPermissions [AccountId: {}, Name: {}, realmId: {}, secLevel: {}]",
...
bool WorldSession::HasPermission(uint32 permission)
{
    ...
    SC_LOG_DEBUG("rbac", "WorldSession::HasPermission [AccountId: {}, Name: {}, realmId: {}]",
...
{
    SC_LOG_DEBUG("rbac", "WorldSession::Invalidaterbac::RBACData [AccountId: {}, Name: {}, realmId: {}]",
                   _RBACData->GetId(), _RBACData->GetName(), realm.Id.Realm);
    delete _RBACData;
    _RBACData = nullptr;
}
...
bool WorldSession::DosProtection::EvaluateOpcode(WorldPacket& p, time_t time) const
{
    ...
    SC_LOG_WARN("network", "AntiDOS: Account {}, IP: {}, Ping: {}, Character: {}, flooding packet (opc: {} (0x%X), count: {})",
    ...
    switch (_policy)
    {
        ...
        case POLICY_KICK:
        {
            SC_LOG_WARN("network", "AntiDOS: Player kicked!");
        ...
        case POLICY_BAN:
        {
            ...
            SC_LOG_WARN("network", "AntiDOS: Player automatically banned for {} seconds.", duration);
            Session->KickPlayer("WorldSession::DosProtection::EvaluateOpcode AntiDOS");
            return false;
        }
...
uint32 WorldSession::AdjustClientMovementTime(uint32 time) const
{
    ...
    {
        SC_LOG_WARN("misc", "The computed movement time using clockDelta is erronous. Using fallback instead");
...
bool WorldSession::IsRightUnitBeingMoved(ObjectGuid guid)
{
    ...
    {
        SC_LOG_DEBUG("entities.unit", "Attempt at tampering movement data by Player {}", _player->GetName());
        return false;
    }
    ...
    {
        SC_LOG_DEBUG("entities.unit", "Bad or outdated movement data by Player {}", _player->GetName());
        return false;
    }
