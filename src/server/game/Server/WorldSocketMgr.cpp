...
bool WorldSocketMgr::StartWorldNetwork(Syphrena::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port, int threadCount)
{
    _tcpNoDelay = sConfigMgr->GetBoolDefault("Network.TcpNodelay", true);

    int const max_connections = SYPHRENA_MAX_LISTEN_CONNECTIONS;
    SC_LOG_DEBUG("misc", "Max allowed socket connections {}", max_connections);
    ...
    {
        SC_LOG_ERROR("misc", "Network.OutUBuff is wrong in your config file");
        return false;
    }
...
void WorldSocketMgr::OnSocketOpen(tcp::socket&& sock, uint32 threadIndex)
{
    ...
    if (_socketSystemSendBufferSize >= 0)
    {
        ...
        {
            SC_LOG_ERROR("misc", "WorldSocketMgr::OnSocketOpen sock.set_option(boost::asio::socket_base::send_buffer_size) err = {}", err.message());
            return;
        }
    }
    ...
    if (_tcpNoDelay)
    {
        ...
        if (err)
        {
            SC_LOG_ERROR("misc", "WorldSocketMgr::OnSocketOpen sock.set_option(boost::asio::ip::tcp::no_delay) err = {}", err.message());
            return;
        }
    }
