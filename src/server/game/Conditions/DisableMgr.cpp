...
void LoadDisables()
{
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 disables. DB table `disables` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "Invalid type {} specified in `disables` table, skipped.", type);
            continue;
        }
        ...
        switch (type)
        {
            case DISABLE_TYPE_SPELL:
                ...
                {
                    SC_LOG_ERROR("sql.sql", "Spell entry {} from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }

                if (!flags || flags > MAX_SPELL_DISABLE_TYPE)
                {
                    SC_LOG_ERROR("sql.sql", "Disable flags for spell {} are invalid, skipped.", entry);
                    continue;
                }

                if (flags & SPELL_DISABLE_MAP)
                {
                    for (std::string_view mapStr : Syphrena::Tokenize(params_0, ',', true))
                    {
                        if (Optional<uint32> mapId = Syphrena::StringTo<uint32>(mapStr))
                            data.params[0].insert(*mapId);
                        else
                            SC_LOG_ERROR("sql.sql", "Disable map '{}' for spell {} is invalid, skipped.", std::string(mapStr), entry);
                    }
                }

                if (flags & SPELL_DISABLE_AREA)
                {
                    for (std::string_view areaStr : Syphrena::Tokenize(params_1, ',', true))
                    {
                        if (Optional<uint32> areaId = Syphrena::StringTo<uint32>(areaStr))
                            data.params[1].insert(*areaId);
                        else
                            SC_LOG_ERROR("sql.sql", "Disable area '{}' for spell {} is invalid, skipped.", std::string(areaStr), entry);
                    }
                }
            ...
            case DISABLE_TYPE_LFG_MAP:
            {
                ...
                if (!mapEntry)
                {
                    SC_LOG_ERROR("sql.sql", "Map entry {} from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                ...
                switch (mapEntry->InstanceType)
                {
                    ...
                    case MAP_ARENA:
                        SC_LOG_ERROR("sql.sql", "Battleground map {} specified to be disabled in map case, skipped.", entry);
                        continue;
                }
                if (isFlagInvalid)
                {
                    SC_LOG_ERROR("sql.sql", "Disable flags for map {} are invalid, skipped.", entry);
                    continue;
                }
                break;
            }
            case DISABLE_TYPE_BATTLEGROUND:
                ...
                {
                    SC_LOG_ERROR("sql.sql", "Battleground entry {} from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    SC_LOG_ERROR("sql.sql", "Disable flags specified for battleground {}, useless data.", entry);
                break;
            case DISABLE_TYPE_OUTDOORPVP:
                if (entry > MAX_OUTDOORPVP_TYPES)
                {
                    SC_LOG_ERROR("sql.sql", "OutdoorPvPTypes value {} from `disables` is invalid, skipped.", entry);
                    continue;
                }
                if (flags)
                    SC_LOG_ERROR("sql.sql", "Disable flags specified for outdoor PvP {}, useless data.", entry);
                break;
            case DISABLE_TYPE_ACHIEVEMENT_CRITERIA:
                if (!sAchievementMgr->GetAchievementCriteria(entry))
                {
                    SC_LOG_ERROR("sql.sql", "Achievement Criteria entry {} from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    SC_LOG_ERROR("sql.sql", "Disable flags specified for Achievement Criteria {}, useless data.", entry);
                break;
                ...
                switch (mapEntry->InstanceType)
                {
                    case MAP_COMMON:
                        if (flags & VMAP::VMAP_DISABLE_AREAFLAG)
                            SC_LOG_INFO("misc", "Areaflag disabled for world map {}.", entry);
                        if (flags & VMAP::VMAP_DISABLE_LIQUIDSTATUS)
                            SC_LOG_INFO("misc", "Liquid status disabled for world map {}.", entry);
                        break;
                    case MAP_INSTANCE:
                    case MAP_RAID:
                        if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                            SC_LOG_INFO("misc", "Height disabled for instance map {}.", entry);
                        if (flags & VMAP::VMAP_DISABLE_LOS)
                            SC_LOG_INFO("misc", "LoS disabled for instance map {}.", entry);
                        break;
                    case MAP_BATTLEGROUND:
                        if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                            SC_LOG_INFO("misc", "Height disabled for battleground map {}.", entry);
                        if (flags & VMAP::VMAP_DISABLE_LOS)
                            SC_LOG_INFO("misc", "LoS disabled for battleground map {}.", entry);
                        break;
                    case MAP_ARENA:
                        if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                            SC_LOG_INFO("misc", "Height disabled for arena map {}.", entry);
                        if (flags & VMAP::VMAP_DISABLE_LOS)
                            SC_LOG_INFO("misc", "LoS disabled for arena map {}.", entry);
                        break;
                    default:
                        break;
                }
            ...
            case DISABLE_TYPE_MMAP:
            {
                ...
                {
                    case MAP_COMMON:
                        SC_LOG_INFO("misc", "Pathfinding disabled for world map {}.", entry);
                        break;
                    case MAP_INSTANCE:
                    case MAP_RAID:
                        SC_LOG_INFO("misc", "Pathfinding disabled for instance map {}.", entry);
                        break;
                    case MAP_BATTLEGROUND:
                        SC_LOG_INFO("misc", "Pathfinding disabled for battleground map {}.", entry);
                        break;
                    case MAP_ARENA:
                        SC_LOG_INFO("misc", "Pathfinding disabled for arena map {}.", entry);
                        break;
                    default:
                        break;
                }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} disables in {} ms", total_count, GetMSTimeDiffToNow(oldMSTime));
}
...
void CheckQuestDisables()
{
    ...
    if (!count)
    {
        SC_LOG_INFO("server.loading", ">> Checked 0 quest disables.");
        return;
    }
    ...
    for (DisableTypeMap::iterator itr = m_DisableMap[DISABLE_TYPE_QUEST].begin(); itr != m_DisableMap[DISABLE_TYPE_QUEST].end();)
    {
        ...
        {
            SC_LOG_ERROR("sql.sql", "Quest entry {} from `disables` doesn't exist, skipped.", entry);
            m_DisableMap[DISABLE_TYPE_QUEST].erase(itr++);
            continue;
        }
        if (itr->second.flags)
            SC_LOG_ERROR("sql.sql", "Disable flags specified for quest {}, useless data.", entry);
        ++itr;
    }

    SC_LOG_INFO("server.loading", ">> Checked {} quest disables in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
