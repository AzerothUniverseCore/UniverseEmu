    ...
    _navMeshQuery(nullptr)
{
    ...
    SC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::PathGenerator for {}", _source->GetGUID().ToString());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest)
{
    ...
    if (!Syphrena::IsValidMapCoord(destX, destY, destZ) || !Syphrena::IsValidMapCoord(x, y, z))
        return false;

    SC_METRIC_DETAILED_EVENT("mmap_events", "CalculatePath", "");
    ...
    SC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::CalculatePath() for {}", _source->GetGUID().ToString());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    {
        SC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPoly == 0 || endPoly == 0)");
        BuildShortcut();
    ...
    {
        SC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: farFromPoly distToStartPoly={:.3f} distToEndPoly={:.3f}", distToStartPoly, distToEndPoly);
        ...
        {
            SC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: underWater case");
        ...
        {
            SC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: flying case");
            if (Unit const* _sourceUnit = _source->ToUnit())
            {
    ...
    {
        SC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPoly == endPoly)");
    ...
    if (_polyLength)
    {
        for (; pathStartIndex < _polyLength; ++pathStartIndex)
        {
            ...
            {
                SC_LOG_ERROR("maps.mmaps", "Invalid poly ref in BuildPolyPath. _polyLength: {}, pathStartIndex: {},"
    ...
    {
        SC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPolyFound && endPolyFound)");
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (_useRaycast)
        {
            SC_LOG_ERROR("maps.mmaps", "PathGenerator::BuildPolyPath() called with _useRaycast with a previous path for unit {}", _source->GetGUID().ToString());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
            return;
        }
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            SC_LOG_ERROR("maps.mmaps", "Path Build failed\n{}", _source->GetDebugInfo());
        }

        SC_LOG_DEBUG("maps.mmaps", "++  m_polyLength={} prefixPolyLength={} suffixPolyLength={}", _polyLength, prefixPolyLength, suffixPolyLength);
    ...
    else
    {
        ...
        if (!_polyLength || dtStatusFailed(dtResult))
        {
            ...
            SC_LOG_ERROR("maps.mmaps", "{} Path Build failed: 0 length path", _source->GetGUID().ToString());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
            return;
        }
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    if (_useRaycast)
    {
        ...
        SC_LOG_ERROR("maps.mmaps", "PathGenerator::BuildPointPath() called with _useRaycast for unit {}", _source->GetGUID().ToString());
        BuildShortcut();
        _type = PATHFIND_NOPATH;
        return;
    }
    ...
    else if ( pointCount < 2 || dtStatusFailed(dtResult))
    {
        ...
        SC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath FAILED! path sized {} returned\n", pointCount);
        BuildShortcut();
    ...
    {
        SC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath FAILED! path sized {} returned, lower than limit set to {}", pointCount, _pointPathLimit);
        BuildShortcut();
    ...
    SC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath path type {} size {} poly-size {}", _type, pointCount, _polyLength);
}
...
{
    SC_LOG_DEBUG("maps.mmaps", "++ BuildShortcut :: making shortcut");
    ...
    *smoothPathSize = 0;
    ...
    while (npolys && nsmoothPath < maxSmoothPathSize)
    {
        ...
        if (dtStatusFailed(_navMeshQuery->getPolyHeight(polys[0], result, &result[1])))
            SC_LOG_DEBUG("maps.mmaps", "Cannot find height at position X: {} Y: {} Z: {} for {}", result[2], result[0], result[1], _source->GetDebugInfo());
        result[1] += 0.5f;
...
void PathGenerator::ShortenPathUntilDist(G3D::Vector3 const& target, float dist)
{
    ...
    {
        SC_LOG_ERROR("maps.mmaps", "PathGenerator::ReducePathLengthByDist called before path was successfully built");
        return;
    }
