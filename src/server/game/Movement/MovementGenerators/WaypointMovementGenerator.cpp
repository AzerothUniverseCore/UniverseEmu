...
void WaypointMovementGenerator<Creature>::DoInitialize(Creature* owner)
{
    ...
    if (!_path)
    {
        SC_LOG_ERROR("sql.sql", "WaypointMovementGenerator::DoInitialize: couldn't load path for creature ({}) (_pathId: {})", owner->GetGUID().ToString(), _pathId);
        return;
    }
...
void WaypointMovementGenerator<Creature>::OnArrived(Creature* owner)
{
    ...
    {
        SC_LOG_DEBUG("maps.script", "Creature movement start script {} at point {} for {}.", waypoint.eventId, _currentNode, owner->GetGUID().ToString());
        owner->ClearUnitState(UNIT_STATE_ROAMING_MOVE);
