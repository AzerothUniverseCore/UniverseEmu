...
void LootItemStorage::LoadStorageFromDB()
{
    ...
    if (result)
    {
        ...
        SC_LOG_INFO("server.loading", ">> Loaded {} stored item loots in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
    else
        SC_LOG_INFO("server.loading", ">> Loaded 0 stored item loots");
    ...
    if (result)
    {
        ...
        SC_LOG_INFO("server.loading", ">> Loaded {} stored item money in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
    else
        SC_LOG_INFO("server.loading", ">> Loaded 0 stored item money");
}
...
void LootItemStorage::AddNewStoredLoot(Loot* loot, Player* player)
{
    ...
    // read
    {
        ...
        if (itr != _lootItemStore.end())
        {
            SC_LOG_ERROR("misc", "Trying to store item loot by player: {} for container id: {} that is already in storage!", player->GetGUID().ToString(), loot->containerID);
            return;
        }
    }
