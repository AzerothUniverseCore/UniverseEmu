...
void PetitionMgr::LoadPetitions()
{
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 petitions.");
        return;
    }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} petitions in: {} ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void PetitionMgr::LoadSignatures()
{
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 Petition signs!");
        return;
    }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} Petition signs in {} ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
