...
/*static*/ void ChannelMgr::LoadFromDB()
{
    ...
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 custom chat channels. Custom channel saving is disabled.");
        return;
    }
    ...
    if (!result)
    {
        SC_LOG_INFO("server.loading", ">> Loaded 0 custom chat channels. DB table `channels` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            SC_LOG_ERROR("server.loading", "Failed to load custom chat channel '{}' from database - invalid utf8 sequence? Deleted.", dbName);
        ...
        if (!mgr)
        {
            SC_LOG_ERROR("server.loading", "Failed to load custom chat channel '{}' from database - invalid team {}. Deleted.", dbName, team);
            toDelete.push_back({ dbName, team });
            continue;
        }
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} custom chat channels in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
