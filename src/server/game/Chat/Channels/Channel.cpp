...
Channel::Channel(std::string const& name, uint32 team /*= 0*/, std::string const& banList) :
    ...
    _zoneEntry(nullptr)
{
    for (std::string_view guid : Syphrena::Tokenize(banList, ' ', false))
    {
        ObjectGuid banned(Syphrena::StringTo<uint64>(guid).value_or(0));
        if (!banned)
            continue;

        SC_LOG_DEBUG("chat.system", "Channel({}) loaded player {} into bannedStore", name, banned.ToString());
    ...
    if (channelId)
    {
        ...
        if (!(channelEntry->Flags & CHANNEL_DBC_FLAG_GLOBAL))
        {
            if (channelEntry->Flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                channelName = fmt::sprintf(channelEntry->Name[locale], sObjectMgr->GetSyphrenaString(LANG_CHANNEL_CITY, locale));
            else
...
void Channel::List(Player const* player) const
{
    ...
    SC_LOG_DEBUG("chat.system", "SMSG_CHANNEL_LIST {} Channel: {}",
...
{
    Syphrena::LocalizedPacketDo<Builder> localizer(builder);
