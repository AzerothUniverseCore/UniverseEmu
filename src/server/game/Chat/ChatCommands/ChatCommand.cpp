...
using ChatSubCommandMap = std::map<std::string_view, Syphrena::Impl::ChatCommands::ChatCommandNode, StringCompareLessI_T>;

void Syphrena::Impl::ChatCommands::ChatCommandNode::LoadFromBuilder(ChatCommandBuilder const& builder)
{
    if (std::holds_alternative<ChatCommandBuilder::InvokerEntry>(builder._data))
    {
        ASSERT(!_invoker, "Duplicate blank sub-command.");
        SyphrenaStrings help;
        std::tie(_invoker, help, _permission) = *(std::get<ChatCommandBuilder::InvokerEntry>(builder._data));
        if (help)
            _help.emplace<SyphrenaStrings>(help);
    }
    else
        LoadCommandsIntoMap(this, _subCommands, std::get<ChatCommandBuilder::SubCommandEntry>(builder._data));
}

/*static*/ void Syphrena::Impl::ChatCommands::ChatCommandNode::LoadCommandsIntoMap(ChatCommandNode* blank, ChatSubCommandMap& map, Syphrena::ChatCommands::ChatCommandTable const& commands)
{
    for (ChatCommandBuilder const& builder : commands)
    {
        ...
        {
            std::vector<std::string_view> const tokens = Syphrena::Tokenize(builder._name, COMMAND_DELIMITER, false);
...
/*static*/ ChatSubCommandMap const& Syphrena::Impl::ChatCommands::ChatCommandNode::GetTopLevelMap()
{
    if (COMMAND_MAP.empty())
        LoadCommandMap();
    return COMMAND_MAP;
}
/*static*/ void Syphrena::Impl::ChatCommands::ChatCommandNode::InvalidateCommandMap() { COMMAND_MAP.clear(); }
/*static*/ void Syphrena::Impl::ChatCommands::ChatCommandNode::LoadCommandMap()
{
    ...
    if (PreparedQueryResult result = WorldDatabase.Query(WorldDatabase.GetPreparedStatement(WORLD_SEL_COMMANDS)))
    {
        do
        {
            ...
            for (std::string_view key : Syphrena::Tokenize(name, COMMAND_DELIMITER, false))
            {
                ...
                {
                    SC_LOG_ERROR("sql.sql", "Table `command` contains data for non-existant command '{}'. Skipped.", name);
                    cmd = nullptr;
                    break;
                }
            }
            ...
            if (std::holds_alternative<std::string>(cmd->_help))
                SC_LOG_ERROR("sql.sql", "Table `command` contains duplicate data for command '{}'. Skipped.", name);

            if (std::holds_alternative<std::monostate>(cmd->_help))
                cmd->_help.emplace<std::string>(help);
            else
                SC_LOG_ERROR("sql.sql", "Table `command` contains legacy help text for command '{}', which uses `syphrena_string`. Skipped.", name);
...
void Syphrena::Impl::ChatCommands::ChatCommandNode::ResolveNames(std::string name)
{
    if (_invoker && std::holds_alternative<std::monostate>(_help))
        SC_LOG_WARN("sql.sql", "Table `command` is missing help text for command '{}'.", name);
...
void Syphrena::Impl::ChatCommands::ChatCommandNode::SendCommandHelp(ChatHandler& handler) const
{
    bool const hasInvoker = IsInvokerVisible(handler);
    if (hasInvoker)
    {
        if (std::holds_alternative<SyphrenaStrings>(_help))
            handler.SendSysMessage(std::get<SyphrenaStrings>(_help));
...
namespace Syphrena::Impl::ChatCommands
{
...
/*static*/ bool Syphrena::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(ChatHandler& handler, std::string_view cmdStr)
{
...
/*static*/ void Syphrena::Impl::ChatCommands::ChatCommandNode::SendCommandHelpFor(ChatHandler& handler, std::string_view cmdStr)
{
    ChatCommandNode const* cmd = nullptr;
    ChatSubCommandMap const* map = &GetTopLevelMap();
    for (std::string_view token : Syphrena::Tokenize(cmdStr, COMMAND_DELIMITER, false))
    {
        FilteredCommandListIterator it1(*map, handler, token);
        if (!it1)
...
/*static*/ std::vector<std::string> Syphrena::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmdStr)
{
    std::string path;
    ChatCommandNode const* cmd = nullptr;
    ChatSubCommandMap const* map = &GetTopLevelMap();
    ...
    while (!oldTail.empty())
    {
        ...
        { /* ok, so it1 points at a partially matching subcommand - let's see if there are others */
            ...
            { /* there are multiple matching subcommands - terminate here and show possibilities */
                ...
                auto possibility = ([prefix = std::string_view(path), suffix = std::string_view(newTail)](std::string_view match)
                {
                    if (prefix.empty())
                    {
                        return Syphrena::StringFormat("{}{}{}", match, COMMAND_DELIMITER, suffix);
                    }
                    else
                    {
                        return Syphrena::StringFormat("{}{}{}{}{}", prefix, COMMAND_DELIMITER, match, COMMAND_DELIMITER, suffix);
                    }
                });
        ...
        path = Syphrena::StringFormat("{}{}{}", path, COMMAND_DELIMITER, it1->first);
        cmd = &it1->second;
        map = &cmd->_subCommands;
    ...
    { /* there is some trailing text, leave it as is */
        ...
        { /* if we matched a command at some point, auto-complete it */
            return {
                Syphrena::StringFormat("{}{}{}", path, COMMAND_DELIMITER, oldTail)
            };
        }
        else
            return {};
    ...
    { /* offer all subcommands */
        auto possibility = ([prefix = std::string_view(path)](std::string_view match)
        {
            ...
            {
                return Syphrena::StringFormat("{}{}{}", prefix, COMMAND_DELIMITER, match);
            }
        });
...
bool Syphrena::Impl::ChatCommands::ChatCommandNode::IsInvokerVisible(ChatHandler const& who) const
{
    if (!_invoker)
        return false;
    if (who.IsConsole() && (_permission.AllowConsole == Syphrena::ChatCommands::Console::No))
        return false;
    return who.HasPermission(_permission.RequiredPermission);
}

bool Syphrena::Impl::ChatCommands::ChatCommandNode::HasVisibleSubCommands(ChatHandler const& who) const
{
    for (auto it = _subCommands.begin(); it != _subCommands.end(); ++it)
...
void Syphrena::ChatCommands::LoadCommandMap() { Syphrena::Impl::ChatCommands::ChatCommandNode::LoadCommandMap(); }
void Syphrena::ChatCommands::InvalidateCommandMap() { Syphrena::Impl::ChatCommands::ChatCommandNode::InvalidateCommandMap(); }
bool Syphrena::ChatCommands::TryExecuteCommand(ChatHandler& handler, std::string_view cmd) { return Syphrena::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(handler, cmd); }
void Syphrena::ChatCommands::SendCommandHelpFor(ChatHandler& handler, std::string_view cmd) { Syphrena::Impl::ChatCommands::ChatCommandNode::SendCommandHelpFor(handler, cmd); }
std::vector<std::string> Syphrena::ChatCommands::GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmd) { return Syphrena::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(handler, cmd); }
