...
ChatCommandResult Syphrena::Impl::ChatCommands::ArgInfo<AchievementEntry const*>::TryConsume(AchievementEntry const*& data, ChatHandler const* handler, std::string_view args)
{
    ...
    if (uint32* id = std::get_if<uint32>(&val))
        return FormatSyphrenaString(handler, LANG_CMDPARSER_ACHIEVEMENT_NO_EXIST, *id);
    return std::nullopt;
}
...
ChatCommandResult Syphrena::Impl::ChatCommands::ArgInfo<GameTele const*>::TryConsume(GameTele const*& data, ChatHandler const* handler, std::string_view args)
{
    ...
    if (val.holds_alternative<Hyperlink<tele>>())
        return FormatSyphrenaString(handler, LANG_CMDPARSER_GAME_TELE_ID_NO_EXIST, static_cast<uint32>(std::get<Hyperlink<tele>>(val)));
    else
        return FormatSyphrenaString(handler, LANG_CMDPARSER_GAME_TELE_NO_EXIST, STRING_VIEW_FMT_ARG(std::get<std::string_view>(val)));
}
...
ChatCommandResult Syphrena::Impl::ChatCommands::ArgInfo<ItemTemplate const*>::TryConsume(ItemTemplate const*& data, ChatHandler const* handler, std::string_view args)
{
    ...
    if (!result || (data = val.visit(ItemTemplateVisitor())))
        return result;
    if (uint32* id = std::get_if<uint32>(&val))
        return FormatSyphrenaString(handler, LANG_CMDPARSER_ITEM_NO_EXIST, *id);
    return std::nullopt;
}
...
ChatCommandResult Syphrena::Impl::ChatCommands::ArgInfo<Quest const*, void>::TryConsume(Quest const*& data, ChatHandler const* handler, std::string_view args)
{
    ...
    if (!result || (data = val.visit(QuestVisitor())))
        return result;
    if (uint32* id = std::get_if<uint32>(&val))
        return FormatSyphrenaString(handler, LANG_CMDPARSER_QUEST_NO_EXIST, *id);
    return std::nullopt;
}
...
ChatCommandResult Syphrena::Impl::ChatCommands::ArgInfo<SpellInfo const*>::TryConsume(SpellInfo const*& data, ChatHandler const* handler, std::string_view args)
{
    ...
    if (!result || (data = val.visit(SpellInfoVisitor())))
        return result;
    if (uint32* id = std::get_if<uint32>(&val))
        return FormatSyphrenaString(handler, LANG_CMDPARSER_SPELL_NO_EXIST, *id);
    return std::nullopt;
}
