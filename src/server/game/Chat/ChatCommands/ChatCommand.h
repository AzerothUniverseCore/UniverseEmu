...
#ifndef SYPHRENA_CHATCOMMAND_H
#define SYPHRENA_CHATCOMMAND_H
...
namespace Syphrena::ChatCommands
{
...
namespace Syphrena::Impl::ChatCommands
{
    ...
    struct MultiConsumer<Tuple, Optional<NestedNextType>, offset>
    {
        static ChatCommandResult TryConsumeTo(Tuple& tuple, ChatHandler const* handler, std::string_view args)
        {
            ...
            {
                return Syphrena::StringFormat("{} \"{}\"\n{} \"{}\"",
                    GetSyphrenaString(handler, LANG_CMDPARSER_EITHER), result2.GetErrorMessage(),
                    GetSyphrenaString(handler, LANG_CMDPARSER_OR), result1.GetErrorMessage());
            }
    ...
    template <typename T> struct HandlerToTuple { static_assert(Syphrena::dependant_false_v<T>, "Invalid command handler signature"); };
    ...
    struct CommandPermissions
    {
        ...
        CommandPermissions(rbac::RBACPermissions perm, Syphrena::ChatCommands::Console console) : RequiredPermission{ perm }, AllowConsole{ console } {}
        rbac::RBACPermissions RequiredPermission;
        Syphrena::ChatCommands::Console AllowConsole;
    };

    class ChatCommandNode
    {
        friend struct FilteredCommandListIterator;
        using ChatCommandBuilder = Syphrena::ChatCommands::ChatCommandBuilder;
        ...
        private:
            ...
            static void LoadCommandsIntoMap(ChatCommandNode* blank, std::map<std::string_view, ChatCommandNode, StringCompareLessI_T>& map, Syphrena::ChatCommands::ChatCommandTable const& commands);
            ...
            std::variant<std::monostate, SyphrenaStrings, std::string> _help;
...
namespace Syphrena::ChatCommands
{
    struct ChatCommandBuilder
    {
        friend class Syphrena::Impl::ChatCommands::ChatCommandNode;
        struct InvokerEntry
        {
            template <typename T>
            InvokerEntry(T& handler, SyphrenaStrings help, rbac::RBACPermissions permission, Syphrena::ChatCommands::Console allowConsole)
            ...
            Syphrena::Impl::ChatCommands::CommandInvoker _invoker;
            SyphrenaStrings _help;
            Syphrena::Impl::ChatCommands::CommandPermissions _permissions;
        ...
        template <typename TypedHandler>
        ChatCommandBuilder(char const* name, TypedHandler& handler, SyphrenaStrings help, rbac::RBACPermissions permission, Syphrena::ChatCommands::Console allowConsole)
            : _name{ ASSERT_NOTNULL(name) }, _data{ std::in_place_type<InvokerEntry>, handler, help, permission, allowConsole }
        {}

        template <typename TypedHandler>
        ChatCommandBuilder(char const* name, TypedHandler& handler, rbac::RBACPermissions permission, Syphrena::ChatCommands::Console allowConsole)
            : ChatCommandBuilder(name, handler, SyphrenaStrings(), permission, allowConsole)
        {}
        ...
        ChatCommandBuilder(char const* name, bool(&handler)(ChatHandler*, char const*), rbac::RBACPermissions permission, Syphrena::ChatCommands::Console allowConsole)
            : ChatCommandBuilder(name, handler, SyphrenaStrings(), permission, allowConsole)
        {}
        ...
        ChatCommandBuilder(char const* name, rbac::RBACPermissions permission, bool console, TypedHandler* handler, char const*)
            : ChatCommandBuilder(name, *handler, SyphrenaStrings(), permission, static_cast<Syphrena::ChatCommands::Console>(console))
        {}
    ...
    SC_GAME_API void LoadCommandMap();
    SC_GAME_API void InvalidateCommandMap();
    SC_GAME_API bool TryExecuteCommand(ChatHandler& handler, std::string_view cmd);
    SC_GAME_API void SendCommandHelpFor(ChatHandler& handler, std::string_view cmd);
    SC_GAME_API std::vector<std::string> GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmd);
}

// backwards compatibility with old patches
using ChatCommand [[deprecated("std::vector<ChatCommand> should be ChatCommandTable! (using namespace Syphrena::ChatCommands)")]] = Syphrena::ChatCommands::ChatCommandBuilder;
