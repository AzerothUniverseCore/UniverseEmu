...
#ifndef SYPHRENA_CHATCOMMANDARGS_H
#define SYPHRENA_CHATCOMMANDARGS_H
...
namespace Syphrena::Impl::ChatCommands
{
    ...
    struct ArgInfo { static_assert(Syphrena::dependant_false_v<T>, "Invalid command parameter type - see ChatCommandArgs.h for possible types"); };
    ...
    struct ArgInfo<T, std::enable_if_t<std::is_integral_v<T> || std::is_floating_point_v<T>>>
    {
        static ChatCommandResult TryConsume(T& val, ChatHandler const* handler, std::string_view args)
        {
            ...
            if (Optional<T> v = StringTo<T>(token, 0))
                ...
                return FormatSyphrenaString(handler, LANG_CMDPARSER_STRING_VALUE_INVALID, STRING_VIEW_FMT_ARG(token), Syphrena::GetTypeName<T>().c_str());
    ...
    struct ArgInfo<T, std::enable_if_t<std::is_enum_v<T>>>
    {
        ...
        static ChatCommandResult TryConsume(T& val, ChatHandler const* handler, std::string_view args)
        {
            ...
            if (next1)
                return FormatSyphrenaString(handler, LANG_CMDPARSER_STRING_VALUE_INVALID, STRING_VIEW_FMT_ARG(strVal), Syphrena::GetTypeName<T>().c_str());
            else
                return next1;
        }
    };
    ...
    struct ArgInfo<Syphrena::ChatCommands::Variant<Ts...>>
    {
        using V = std::variant<Ts...>;
        static constexpr size_t N = std::variant_size_v<V>;

        template <size_t I>
        static ChatCommandResult TryAtIndex([[maybe_unused]] Syphrena::ChatCommands::Variant<Ts...>& val, [[maybe_unused]] ChatHandler const* handler, [[maybe_unused]] std::string_view args)
        {
            if constexpr (I < N)
            {
                ...
                else
                {
                    ...
                    if (StringStartsWith(nestedResult.GetErrorMessage(), "\""))
                        return Syphrena::StringFormat("\"{}\"\n{} {}", thisResult.GetErrorMessage(), GetSyphrenaString(handler, LANG_CMDPARSER_OR), nestedResult.GetErrorMessage());
                    else
                        return Syphrena::StringFormat("\"{}\"\n{} \"{}\"", thisResult.GetErrorMessage(), GetSyphrenaString(handler, LANG_CMDPARSER_OR), nestedResult.GetErrorMessage());
                }
            }
        ...
        static ChatCommandResult TryConsume(Syphrena::ChatCommands::Variant<Ts...>& val, ChatHandler const* handler, std::string_view args)
        {
            ChatCommandResult result = TryAtIndex<0>(val, handler, args);
            if (result.HasErrorMessage() && (result.GetErrorMessage().find('\n') != std::string::npos))
                return Syphrena::StringFormat("{} {}", GetSyphrenaString(handler, LANG_CMDPARSER_EITHER), result.GetErrorMessage());
            return result;
        }
    };
    ...
    template <>
    struct SC_GAME_API ArgInfo<AchievementEntry const*>
    {
    ...
    template <>
    struct SC_GAME_API ArgInfo<GameTele const*>
    {
    ...
    template <>
    struct SC_GAME_API ArgInfo<ItemTemplate const*>
    {
    ...
    template <>
    struct SC_GAME_API ArgInfo<Quest const*>
    {
    ...
    template <>
    struct SC_GAME_API ArgInfo<SpellInfo const*>
    {
