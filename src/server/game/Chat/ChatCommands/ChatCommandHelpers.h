...
#ifndef SYPHRENA_CHATCOMMANDHELPERS_H
#define SYPHRENA_CHATCOMMANDHELPERS_H
...
namespace Syphrena::Impl::ChatCommands
{
    ...
    SC_GAME_API void SendErrorMessageToHandler(ChatHandler* handler, std::string_view str);
    SC_GAME_API char const* GetSyphrenaString(ChatHandler const* handler, SyphrenaStrings which);
    template <typename... Ts>
    std::string FormatSyphrenaString(ChatHandler const* handler, SyphrenaStrings which, Ts&&... args)
    {
        return fmt::sprintf(GetSyphrenaString(handler, which), std::forward<Ts>(args)...);
    }
}
