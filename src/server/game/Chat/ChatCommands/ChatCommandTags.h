...
#ifndef SYPHRENA_CHATCOMMANDTAGS_H
#define SYPHRENA_CHATCOMMANDTAGS_H
...
namespace Syphrena::Impl::ChatCommands
{
    struct ContainerTag
    {
        using ChatCommandResult = Syphrena::Impl::ChatCommands::ChatCommandResult;
    };
...
#define CHATCOMMANDS_IMPL_SPLIT_LITERAL_EXTRACT_CHAR(z, i, strliteral) \
        BOOST_PP_COMMA_IF(i) Syphrena::Impl::ChatCommands::GetChar(strliteral, i)
...
namespace Syphrena::ChatCommands
{
    ...
    |* Must inherit from Syphrena::Impl::ChatCommands::ContainerTag                          *|
    ...
    template <char... chars>
    struct ExactSequence : Syphrena::Impl::ChatCommands::ContainerTag
    {
        ...
        ChatCommandResult TryConsume(ChatHandler const* handler, std::string_view args) const
        {
            ...
            {
                auto [remainingToken, tail] = Syphrena::Impl::ChatCommands::tokenize(args.substr(_string.length()));
            ...
            return Syphrena::Impl::ChatCommands::FormatSyphrenaString(handler, LANG_CMDPARSER_EXACT_SEQ_MISMATCH, STRING_VIEW_FMT_ARG(_string), STRING_VIEW_FMT_ARG(start));
        }
...
#define EXACT_SEQUENCE(str) Syphrena::ChatCommands::ExactSequence<CHATCOMMANDS_IMPL_SPLIT_LITERAL(str)>

    struct Tail : std::string_view, Syphrena::Impl::ChatCommands::ContainerTag
    {
    ...
    struct WTail : std::wstring, Syphrena::Impl::ChatCommands::ContainerTag
    {
        ...
        ChatCommandResult TryConsume(ChatHandler const* handler, std::string_view args)
        {
            if (Utf8toWStr(args, *this))
                ...
                return Syphrena::Impl::ChatCommands::GetSyphrenaString(handler, LANG_CMDPARSER_INVALID_UTF8);
        }
    };

    struct QuotedString : std::string, Syphrena::Impl::ChatCommands::ContainerTag
    {
        using value_type = std::string;

        SC_GAME_API ChatCommandResult TryConsume(ChatHandler const* handler, std::string_view args);
    };

    struct SC_GAME_API AccountIdentifier : Syphrena::Impl::ChatCommands::ContainerTag
    {
    ...
    struct SC_GAME_API PlayerIdentifier : Syphrena::Impl::ChatCommands::ContainerTag
    {
    ...
    struct Hyperlink : Syphrena::Impl::ChatCommands::ContainerTag
    {
        ...
        ChatCommandResult TryConsume(ChatHandler const* handler, std::string_view args)
        {
            Syphrena::Hyperlinks::HyperlinkInfo info = Syphrena::Hyperlinks::ParseSingleHyperlink(args);
            ...
            if (!linktag::StoreTo(val, info.data))
                return Syphrena::Impl::ChatCommands::GetSyphrenaString(handler, LANG_CMDPARSER_LINKDATA_INVALID);

            // finally, skip any potential delimiters
            auto [token, next] = Syphrena::Impl::ChatCommands::tokenize(info.tail);
    ...
    using namespace ::Syphrena::Hyperlinks::LinkTags;
}
...
namespace Syphrena::ChatCommands
{
    ...
    struct Variant : public std::variant<T1, Ts...>
    {
        ...
        using first_type = Syphrena::Impl::ChatCommands::tag_base_t<T1>;
        static constexpr bool have_operators = Syphrena::Impl::ChatCommands::are_all_assignable<first_type, Syphrena::Impl::ChatCommands::tag_base_t<Ts>...>::value;

        template <bool C = have_operators>
        std::enable_if_t<C, first_type> operator*() const
        {
            return visit(Syphrena::Impl::CastToVisitor<first_type>());
        }
        ...
        template <bool C = have_operators>
        friend std::enable_if_t<C, std::ostream&> operator<<(std::ostream& os, Syphrena::ChatCommands::Variant<T1, Ts...> const& v)
        {
            return (os << *v);
        }
    };
...
template <typename T1, typename... Ts>
struct fmt::formatter<Syphrena::ChatCommands::Variant<T1, Ts...>> : ostream_formatter { };

template <typename T1, typename... Ts>
struct fmt::printf_formatter<Syphrena::ChatCommands::Variant<T1, Ts...>> : formatter<T1>
{
