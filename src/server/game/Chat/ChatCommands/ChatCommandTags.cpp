...
using namespace Syphrena::Impl::ChatCommands;

ChatCommandResult Syphrena::ChatCommands::QuotedString::TryConsume(ChatHandler const* handler, std::string_view args)
{
    if (args.empty())
        return std::nullopt;
    if ((args[0] != '"') && (args[0] != '\''))
...
Syphrena::ChatCommands::AccountIdentifier::AccountIdentifier(WorldSession& session)
    : _id(session.GetAccountId()), _name(session.GetAccountName()), _session(&session) {}

ChatCommandResult Syphrena::ChatCommands::AccountIdentifier::TryConsume(ChatHandler const* handler, std::string_view args)
{
    ...
    if (!Utf8ToUpperOnlyLatin(_name))
        return GetSyphrenaString(handler, LANG_CMDPARSER_INVALID_UTF8);
    ...
    Optional<uint32> id = Syphrena::StringTo<uint32>(text, 10);
    if (!id)
        return FormatSyphrenaString(handler, LANG_CMDPARSER_ACCOUNT_NAME_NO_EXIST, STRING_VIEW_FMT_ARG(_name));
    _id = *id;
    _session = sWorld->FindSession(_id);
    ...
    if (AccountMgr::GetName(_id, _name))
        ...
        return FormatSyphrenaString(handler, LANG_CMDPARSER_ACCOUNT_ID_NO_EXIST, _id);
}

Optional<Syphrena::ChatCommands::AccountIdentifier> Syphrena::ChatCommands::AccountIdentifier::FromTarget(ChatHandler* handler)
{
...
ChatCommandResult Syphrena::ChatCommands::PlayerIdentifier::TryConsume(ChatHandler const* handler, std::string_view args)
{
    ...
    if (val.holds_alternative<ObjectGuid::LowType>())
    {
        ...
        if ((_player = ObjectAccessor::FindPlayerByLowGUID(_guid.GetCounter())))
            ...
            return FormatSyphrenaString(handler, LANG_CMDPARSER_CHAR_GUID_NO_EXIST, _guid.ToString().c_str());
        return next;
    }
    else
    {
        ...
        if (!normalizePlayerName(_name))
            return FormatSyphrenaString(handler, LANG_CMDPARSER_CHAR_NAME_INVALID, STRING_VIEW_FMT_ARG(_name));
        ...
        if ((_player = ObjectAccessor::FindPlayerByName(_name)))
            ...
            return FormatSyphrenaString(handler, LANG_CMDPARSER_CHAR_NAME_NO_EXIST, STRING_VIEW_FMT_ARG(_name));
        return next;
    }
}

Syphrena::ChatCommands::PlayerIdentifier::PlayerIdentifier(Player& player)
    : _name(player.GetName()), _guid(player.GetGUID()), _player(&player) {}

/*static*/ Optional<Syphrena::ChatCommands::PlayerIdentifier> Syphrena::ChatCommands::PlayerIdentifier::FromTarget(ChatHandler* handler)
{
    if (Player* player = handler->GetPlayer())
        if (Player* target = player->GetSelectedPlayer())
            return { *target };
...
/*static*/ Optional<Syphrena::ChatCommands::PlayerIdentifier> Syphrena::ChatCommands::PlayerIdentifier::FromSelf(ChatHandler* handler)
{
    if (Player* player = handler->GetPlayer())
        return { *player };
    return std::nullopt;
