...
char const* ChatHandler::GetSyphrenaString(uint32 entry) const
{
    return m_session->GetSyphrenaString(entry);
}
...
void ChatHandler::SendSysMessage(std::string_view str, bool escapeCharacters)
{
    ...
    {
        std::vector<std::string_view> tokens = Syphrena::Tokenize(msg, '|', true);
    ...
    WorldPacket data;
    for (std::string_view line : Syphrena::Tokenize(str, '\n', true))
    {
...
{
    SendSysMessage(GetSyphrenaString(entry));
}

bool ChatHandler::_ParseCommands(std::string_view text)
{
    if (Syphrena::ChatCommands::TryExecuteCommand(*this, text))
        return true;
...
bool ChatHandler::ParseCommands(std::string_view text)
{
    ...
    if (text[1] == Syphrena::Impl::ChatCommands::COMMAND_DELIMITER)
        return false;
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    Syphrena::NearestGameObjectCheck check(*pl);
    Syphrena::GameObjectLastSearcher<Syphrena::NearestGameObjectCheck> searcher(pl, obj, check);
...
ObjectGuid::LowType ChatHandler::extractLowGuidFromLink(char* text, HighGuid& guidHigh)
{
    ...
    switch (type)
    {
        ...
        case GUID_LINK_CREATURE:
        {
            ...
            ObjectGuid::LowType lowguid = Syphrena::StringTo<ObjectGuid::LowType>(idS).value_or(0);
            return lowguid;
        }
...
char const* CliHandler::GetSyphrenaString(uint32 entry) const
{
    return sObjectMgr->GetSyphrenaStringForDBCLocale(entry);
}
...
{
    return GetSyphrenaString(LANG_CONSOLE_COMMAND);
}
...
bool AddonChannelCommandHandler::ParseCommands(std::string_view str)
{
    if (str.length() < 17)
        return false;
    if (!StringStartsWith(str, "SyphrenaCore\t"))
        return false;
    char opcode = str[12];
    echo = &str[13];
