...
bool ReputationMgr::IsAtWar(uint32 faction_id) const
{
    ...
    if (!factionEntry)
    {
        SC_LOG_ERROR("misc", "ReputationMgr::IsAtWar: Can't get AtWar flag of {} for unknown faction (faction id) #{}.", _player->GetName(), faction_id);
        return 0;
    }
...
int32 ReputationMgr::GetReputation(uint32 faction_id) const
{
    ...
    if (!factionEntry)
    {
        SC_LOG_ERROR("misc", "ReputationMgr::GetReputation: Can't get reputation of {} for unknown faction (faction id) #{}.", _player->GetName(), faction_id);
        return 0;
    }
