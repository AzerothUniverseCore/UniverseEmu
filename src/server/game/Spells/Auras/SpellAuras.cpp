...
_flags(AFLAG_NONE), _effectsToApply(effMask), _needClientUpdate(false)
{
    ...
    if (GetBase()->CanBeSentToClient())
    {
        ...
        if (slot < MAX_AURAS)
        {
            ...
            SetNeedClientUpdate();
            SC_LOG_DEBUG("spells", "Aura: {} Effect: {} put to unit visible auras slot: {}", GetBase()->GetId(), GetEffectMask(), slot);
        }
        else
            SC_LOG_ERROR("spells", "Aura: {} Effect: {} could not find empty unit visible slot", GetBase()->GetId(), GetEffectMask());
    }
...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    SC_LOG_DEBUG("spells", "AuraApplication::_HandleEffect: {}, apply: {}: amount: {}", aurEff->GetAuraType(), apply, aurEff->GetAmount());
...
void Aura::_UnapplyForTarget(Unit* target, Unit* caster, AuraApplication* auraApp)
{
    ...
    {
        SC_LOG_ERROR("spells", "Aura::_UnapplyForTarget, target: {}, caster: {}, spell:{} was not found in owners application map!",
    ...
    for (auto itr = targets.begin(); itr != targets.end();)
    {
        ...
        else
        {
            ...
            if (!GetOwner()->IsSelfOrInSameMap(itr->first))
            {
                ...
                SC_LOG_FATAL("spells", "Aura {}: Owner {} (map {}) is not in the same map as target {} (map {}).", GetSpellInfo()->Id,
                ...
                if (GetSpellInfo()->SpellFamilyFlags[2] & 0x2)
                {
                    ...
                    if (aurEff)
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            default:
                                SC_LOG_ERROR("spells", "Aura::HandleAuraSpecificMods: Unknown rank of Crypt Fever/Ebon Plague ({}) found", aurEff->GetId());
                        }
                ...
                if (GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400)
                {
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 98, 0))
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            default:
                                SC_LOG_ERROR("spells", "Aura::HandleAuraSpecificMods: Unknown rank of Improved Fear ({}) found", aurEff->GetId());
                        }
                        if (spellId)
...
void Aura::LoadScripts()
{
    ...
    for (auto itr = m_loadedScripts.begin(); itr != m_loadedScripts.end(); ++itr)
    {
        SC_LOG_DEBUG("spells", "Aura::LoadScripts: Script `{}` for aura `{}` is loaded now", (*itr)->_GetScriptName()->c_str(), m_spellInfo->Id);
        (*itr)->Register();
    }
}
...
void UnitAura::FillTargetMap(std::unordered_map<Unit*, uint8>& targets, Unit* caster)
{
    ...
    for (SpellEffectInfo const& spellEffectInfo : GetSpellInfo()->GetEffects())
    {
        ...
        {
            Syphrena::WorldObjectSpellAreaTargetCheck check(radius, GetUnitOwner(), ref, GetUnitOwner(), m_spellInfo, selectionType, condList);
            Syphrena::UnitListSearcher<Syphrena::WorldObjectSpellAreaTargetCheck> searcher(GetUnitOwner(), units, check);
...
void DynObjAura::FillTargetMap(std::unordered_map<Unit*, uint8>& targets, Unit* /*caster*/)
{
    ...
    for (SpellEffectInfo const& spellEffectInfo : GetSpellInfo()->GetEffects())
    {
        ...
        Syphrena::WorldObjectSpellAreaTargetCheck check(radius, GetDynobjOwner(), dynObjOwnerCaster, dynObjOwnerCaster, m_spellInfo, selectionType, condList);
        Syphrena::UnitListSearcher<Syphrena::WorldObjectSpellAreaTargetCheck> searcher(GetDynobjOwner(), units, check);
