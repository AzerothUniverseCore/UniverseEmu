...
{
    SC_LOG_DEBUG("spells", "WORLD: Spell Effect DUMMY");
}

void Spell::EffectUnused()
{
    // NOT USED BY ANY SPELL OR USELESS OR IMPLEMENTED IN DIFFERENT WAY IN SYPHRENA
}
...
void Spell::EffectDummy()
{
    ...
    SC_LOG_DEBUG("spells", "Spell ScriptStart spellid {} in EffectDummy({})", m_spellInfo->Id, uint32(effectInfo->EffectIndex));
...
void Spell::EffectTriggerSpell()
{
            ...
            case 25155:
            {
                switch (m_spellInfo->Id)
                {
                    ...
                    case 34756:
                    {
                        ...
                        //SC_LOG_ERROR("entities.unit", "TriggerSpell(%u from %u): %s on %s base val %i,",
    ...
    {
        SC_LOG_WARN("spells.effect.nospell", "Spell::EffectTriggerSpell: Spell {} [EffectIndex: {}] does not have triggered spell.", m_spellInfo->Id, uint32(effectInfo->EffectIndex));
        return;
    }
    ...
    if (!spellInfo)
    {
        SC_LOG_ERROR("spells.effect.nospell", "Spell::EffectTriggerSpell spell {} tried to trigger unknown spell {}", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerMissileSpell()
{
    ...
    if (triggered_spell_id == 0)
    {
        SC_LOG_WARN("spells.effect.nospell", "Spell::EffectTriggerMissileSpell: Spell {} [EffectIndex: {}] does not have triggered spell.", m_spellInfo->Id, uint32(effectInfo->EffectIndex));
        return;
    }
    ...
    if (!spellInfo)
    {
        SC_LOG_ERROR("spells.effect.nospell", "Spell::EffectTriggerMissileSpell spell {} tried to trigger unknown spell {}.", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectForceCast()
{
    ...
    if (triggered_spell_id == 0)
    {
        SC_LOG_WARN("spells.effect.nospell", "Spell::EffectForceCast: Spell {} [EffectIndex: {}] does not have triggered spell.", m_spellInfo->Id, uint32(effectInfo->EffectIndex));
        return;
    }
    ...
    if (!spellInfo)
    {
        SC_LOG_ERROR("spells.effect.nospell", "Spell::EffectForceCast of spell {}: triggering unknown spell id {}.", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerRitualOfSummoning()
{
    ...
    if (triggered_spell_id == 0)
    {
        SC_LOG_WARN("spells.effect.nospell", "Spell::EffectTriggerRitualOfSummoning: Spell {} [EffectIndex: {}] does not have triggered spell.", m_spellInfo->Id, uint32(effectInfo->EffectIndex));
        return;
    }
    ...
    if (!spellInfo)
    {
        SC_LOG_ERROR("spells.effect.nospell", "EffectTriggerRitualOfSummoning of spell {}: triggering unknown spell id {}.", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTeleportUnits()
{
    ...
    if (!m_targets.HasDst())
    {
        SC_LOG_ERROR("spells", "Spell::EffectTeleportUnits - does not have a destination for spellId {}.", m_spellInfo->Id);
        return;
    }
...
void Spell::EffectUnlearnSpecialization()
{
    ...
    SC_LOG_DEBUG("spells", "Spell: Player {} has unlearned spell {} from Npc {}", player->GetGUID().ToString(), spellToUnlearn, m_caster->GetGUID().ToString());
}
...
void Spell::EffectSendEvent()
{
    ...
    SC_LOG_DEBUG("spells", "Spell ScriptStart {} for spellid {} in EffectSendEvent ", effectInfo->MiscValue, m_spellInfo->Id);
...
void Spell::EffectHeal()
{
    ...
    else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, unitCaster))
    {
        ...
        if (!targetAura)
        {
            SC_LOG_ERROR("spells", "Target ({}) has the aurastate AURA_STATE_SWIFTMEND, but no matching aura.", unitTarget->GetGUID().ToString());
            return;
        }
...
void Spell::EffectHealthLeech()
{
    ...
    SC_LOG_DEBUG("spells", "HealthLeech :{}", damage);
...
void Spell::SendLoot(ObjectGuid guid, LootType loottype)
{
    ...
    if (gameObjTarget)
    {
        ...
        {
            SC_LOG_ERROR("entities.player.cheat", "Possible hacking attempt: Player {} {} tried to loot a gameobject {} which is on respawn timer without being in GM mode!",
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_CHEST:
                ...
                {
                    SC_LOG_DEBUG("spells", "Chest ScriptStart id {} for GO {}", gameObjTarget->GetGOInfo()->chest.eventId, gameObjTarget->GetSpawnId());
...
void Spell::EffectOpenLock()
{
    ...
    if (m_caster->GetTypeId() != TYPEID_PLAYER)
    {
        SC_LOG_DEBUG("spells", "WORLD: Open Lock - No Player Caster!");
        return;
    }
    ...
    {
        SC_LOG_DEBUG("spells", "WORLD: Open Lock - No GameObject/Item Target!");
        return;
    }
...
void Spell::EffectSummonType()
{
    ...
    if (!properties)
    {
        SC_LOG_ERROR("spells", "EffectSummonType: Unhandled summon type {}.", effectInfo->MiscValueB);
        return;
    }
...
void Spell::EffectLearnSpell()
{
    ...
    SC_LOG_DEBUG("spells", "Spell: Player {} has learned spell {} from Npc {}", player->GetGUID().ToString(), spellToLearn, m_caster->GetGUID().ToString());
}
...
void Spell::EffectAddHonor()
{
    ...
    if (m_CastItem)
    {
        ...
        SC_LOG_DEBUG("spells", "SpellEffect::AddHonor (spell_id {}) rewards {} honor points (item {}) for player {}", m_spellInfo->Id, damage/10, m_CastItem->GetEntry(), unitTarget->ToPlayer()->GetGUID().ToString());
        return;
    }
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = Syphrena::Honor::hk_honor_at_level(unitTarget->GetLevel(), float(damage));
        unitTarget->ToPlayer()->RewardHonor(nullptr, 1, honor_reward);
        SC_LOG_DEBUG("spells", "SpellEffect::AddHonor (spell_id {}) rewards {} honor points (scale) to player {}", m_spellInfo->Id, honor_reward, unitTarget->ToPlayer()->GetGUID().ToString());
    }
    else
    {
        //maybe we have correct honor_gain in damage already
        unitTarget->ToPlayer()->RewardHonor(nullptr, 1, damage);
        SC_LOG_DEBUG("spells", "SpellEffect::AddHonor (spell_id {}) rewards {} honor points (non scale) for player {}", m_spellInfo->Id, damage, unitTarget->ToPlayer()->GetGUID().ToString());
    }
}
...
void Spell::EffectEnchantItemPrismatic()
{
    ...
    // support only enchantings with add socket in this slot
    {
        ...
        if (!add_socket)
        {
            SC_LOG_ERROR("spells", "Spell::EffectEnchantItemPrismatic: attempt to apply the enchant spell {} with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC ({}), but without ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET ({}), not supported yet.",
...
void Spell::EffectEnchantItemTmp()
{
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN && m_spellInfo->SpellFamilyFlags[0] & 0x400000)
    {
        ...
        switch (damage)
        {
            ...
            default:
                SC_LOG_ERROR("spells", "Spell::EffectEnchantItemTmp: Damage {} not handled in S'RW.", damage);
                return;
        }
        ...
        if (!spellInfo)
        {
            SC_LOG_ERROR("spells", "Spell::EffectEnchantItemTmp: unknown spell id {}", spell_id);
            return;
    ...
    if (!enchant_id)
    {
        SC_LOG_ERROR("spells", "Spell {} Effect {} (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) has enchanting id 0.", m_spellInfo->Id, uint32(effectInfo->EffectIndex));
        return;
    }
    ...
    if (!pEnchant)
    {
        SC_LOG_ERROR("spells", "Spell {} Effect {} (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) has a non-existing enchanting id {} ", m_spellInfo->Id, uint32(effectInfo->EffectIndex), enchant_id);
        return;
    }
...
void Spell::EffectTaunt()
{
    ...
    // Affliction can hit some entities that can't have a threat list (including players' pets)
    if (m_spellInfo->Id == 720034)
        if (unitTarget->GetTypeId() != TYPEID_PLAYER && unitTarget->GetTarget() != unitCaster->GetGUID())
            unitCaster->CastSpell(unitTarget, 720039, true);
...
void Spell::EffectScriptEffect()
{
    ...
    // normal DB scripted effect
    SC_LOG_DEBUG("spells", "Spell ScriptStart spellid {} in EffectScriptEffect({})", m_spellInfo->Id, uint32(effectInfo->EffectIndex));
    m_caster->GetMap()->ScriptsStart(sSpellScripts, uint32(m_spellInfo->Id | (effectInfo->EffectIndex << 24)), m_caster, unitTarget);
}
...
void Spell::EffectStuck()
{
    ...
    SC_LOG_DEBUG("spells", "Spell Effect: Stuck");
    SC_LOG_DEBUG("spells", "Player {} {} used the auto-unstuck feature at map {} ({}, {}, {}).", player->GetName(), player->GetGUID().ToString(), player->GetMapId(), player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
...
void Spell::EffectForceDeselect()
{
    ...
    Syphrena::MessageDistDelivererToHostile notifierBreak(unitCaster, &data, dist);
    Cell::VisitWorldObjects(unitCaster, notifierBreak, dist);

    // and selection
    data.Initialize(SMSG_CLEAR_TARGET, 8);
    data << uint64(unitCaster->GetGUID());
    Syphrena::MessageDistDelivererToHostile notifierClear(unitCaster, &data, dist);
...
void Spell::EffectPullTowards()
{
    ...
    {
        SC_LOG_ERROR("spells", "Spell {} with SPELL_EFFECT_PULL_TOWARDS called with invalid speedZ. {}", m_spellInfo->Id, GetDebugInfo());
        return;
    }
...
void Spell::EffectPullTowardsDest()
{
    ...
    if (!m_targets.HasDst())
    {
        SC_LOG_ERROR("spells", "Spell {} with SPELL_EFFECT_PULL_TOWARDS_DEST has no dest target", m_spellInfo->Id);
        return;
    }
    ...
    if (!std::isfinite(speedZ))
    {
        SC_LOG_ERROR("spells", "Spell {} with SPELL_EFFECT_PULL_TOWARDS_DEST called with invalid speedZ. {}", m_spellInfo->Id, GetDebugInfo());
        return;
    }
...
void Spell::EffectTransmitted()
{
    ...
    if (!goinfo)
    {
        SC_LOG_ERROR("sql.sql", "Gameobject (Entry: {}) does not exist and is not created by spell (ID: {}) cast.", name_id, m_spellInfo->Id);
        return;
    }
    ...
    SC_LOG_DEBUG("spells", "AddObject at SpellEfects.cpp EffectTransmitted");
    //unitCaster->AddGameObject(pGameObj);
...
void Spell::EffectSkill()
{
    ...
    SC_LOG_DEBUG("spells", "WORLD: SkillEFFECT");
}
...
void Spell::EffectSkinPlayerCorpse()
{
    ...
    SC_LOG_DEBUG("spells", "Effect: SkinPlayerCorpse");
...
void Spell::EffectStealBeneficialBuff()
{
    ...
    SC_LOG_DEBUG("spells", "Effect: StealBeneficialBuff");
...
void Spell::EffectPlayMusic()
{
    ...
    {
        SC_LOG_ERROR("spells", "EffectPlayMusic: Sound (Id: {}) does not exist in spell {}.", soundid, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectPlaySound()
{
    ...
    if (!sSoundEntriesStore.LookupEntry(soundId))
    {
        SC_LOG_ERROR("spells", "EffectPlaySound: Sound (Id: {}) does not exist in spell {}.", soundId, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectBind()
{
    ...
    SC_LOG_DEBUG("spells", "EffectBind: New homebind X: {}, Y: {}, Z: {}, MapId: {}, AreaId: {}",
