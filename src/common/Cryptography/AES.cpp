...
Syphrena::Crypto::AES::AES(bool encrypting) : _ctx(EVP_CIPHER_CTX_new()), _encrypting(encrypting)
{
...
Syphrena::Crypto::AES::~AES()
{
    EVP_CIPHER_CTX_free(_ctx);
}

void Syphrena::Crypto::AES::Init(Key const& key)
{
    int status = EVP_CipherInit_ex(_ctx, nullptr, nullptr, key.data(), nullptr, -1);
    ASSERT(status);
}

bool Syphrena::Crypto::AES::Process(IV const& iv, uint8* data, size_t length, Tag& tag)
{
