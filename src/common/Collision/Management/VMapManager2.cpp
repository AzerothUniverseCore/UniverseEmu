    ...
    */
    ...
    WorldModel* VMapManager2::acquireModelInstance(const std::string& basepath, const std::string& filename, uint32 flags/* Only used when creating the model */)
    {
        ...
        if (model == iLoadedModelFiles.end())
        {
            ...
            {
                SC_LOG_ERROR("misc", "VMapManager2: could not load '{}{}.vmo'", basepath, filename);
                delete worldmodel;
                return nullptr;
            }
            SC_LOG_DEBUG("maps", "VMapManager2: loading file '{}{}'", basepath, filename);
    ...
    void VMapManager2::releaseModelInstance(const std::string &filename)
    {
        ...
        if (model == iLoadedModelFiles.end())
        {
            SC_LOG_ERROR("misc", "VMapManager2: trying to unload non-loaded file '{}'", filename);
            return;
        }
        if (model->second.decRefCount() == 0)
        {
            SC_LOG_DEBUG("maps", "VMapManager2: unloading file '{}'", filename);
