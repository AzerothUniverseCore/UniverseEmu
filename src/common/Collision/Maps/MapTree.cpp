...
namespace VMAP
{
    ...
    class AreaInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                SC_LOG_DEBUG("maps", "AreaInfoCallback: trying to intersect '{}'", prims[entry].name);
#endif
    ...
    class LocationInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                SC_LOG_DEBUG("maps", "LocationInfoCallback: trying to intersect '{}'", prims[entry].name);
#endif
    ...
    */
    ...
    bool StaticMapTree::InitMap(const std::string &fname, VMapManager2* vm)
    {
        ...
        ModelSpawn spawn;
#ifdef VMAP_DEBUG
        SC_LOG_DEBUG("maps", "StaticMapTree::InitMap() : map isTiled: {}", static_cast<uint32>(iIsTiled));
#endif
        if (!iIsTiled && ModelSpawn::readFromFile(rf, spawn))
        {
            WorldModel* model = vm->acquireModelInstance(iBasePath, spawn.name, spawn.flags);
            SC_LOG_DEBUG("maps", "StaticMapTree::InitMap() : loading {}", spawn.name);
            if (model)
            {
            ...
            {
                success = false;
                SC_LOG_ERROR("misc", "StaticMapTree::InitMap() : could not acquire WorldModel pointer for '{}'", spawn.name);
            }
        }
    ...
    bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        ...
        if (!iTreeValues)
        {
            SC_LOG_ERROR("misc", "StaticMapTree::LoadMapTile() : tree has not been initialized [{}, {}]", tileX, tileY);
            return false;
        }
        bool result = true;
        ...
        if (tf)
        {
            ...
            for (uint32 i=0; i<numSpawns && result; ++i)
            {
                ...
                if (result)
                {
                    ...
                    WorldModel* model = vm->acquireModelInstance(iBasePath, spawn.name, spawn.flags);
                    if (!model)
                        SC_LOG_ERROR("misc", "StaticMapTree::LoadMapTile() : could not acquire WorldModel pointer [{}, {}]", tileX, tileY);
                    ...
                    if (fread(&referencedVal, sizeof(uint32), 1, tf) == 1)
                    {
                        if (!iLoadedSpawns.count(referencedVal))
                        {
                            ...
                            {
                                SC_LOG_ERROR("maps", "StaticMapTree::LoadMapTile() : invalid tree element ({}/{}) referenced in tile {}", referencedVal, iNTreeValues, tilefile);
                                continue;
                            }
                        ...
                        else
                        {
                            ...
                            if (iTreeValues[referencedVal].ID != spawn.ID)
                                SC_LOG_DEBUG("maps", "StaticMapTree::LoadMapTile() : trying to load wrong spawn in node");
                            else if (iTreeValues[referencedVal].name != spawn.name)
                                SC_LOG_DEBUG("maps", "StaticMapTree::LoadMapTile() : name collision on GUID={}", spawn.ID);
#endif
                        }
                    }
                    else
            ...
            iLoadedTiles[packTileID(tileX, tileY)] = false;
        SC_METRIC_EVENT("map_events", "LoadMapTile",
    ...
    void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        ...
        {
            SC_LOG_ERROR("misc", "StaticMapTree::UnloadMapTile() : trying to unload non-loaded tile - Map:{} X:{} Y:{}", iMapID, tileX, tileY);
            return;
        }
                ...
                for (uint32 i=0; i<numSpawns && result; ++i)
                {
                    ...
                    if (result)
                    {
                        ...
                        else
                        {
                            ...
                            SC_LOG_ERROR("misc", "StaticMapTree::UnloadMapTile() : trying to unload non-referenced model '{}' (ID:{})", spawn.name, spawn.ID);
        ...
        SC_METRIC_EVENT("map_events", "UnloadMapTile",
            "Map: " + std::to_string(iMapID) + " TileX: " + std::to_string(tileX) + " TileY: " + std::to_string(tileY));
    }
