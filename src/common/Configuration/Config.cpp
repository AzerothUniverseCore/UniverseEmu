...
namespace
{
    ...
    {
        std::string envKey = "SC_" + IniKeyToEnvVarKey(key);
...
T ConfigMgr::GetValueDefault(std::string const& name, T def, bool quiet) const
{
    ...
    catch (bpt::ptree_bad_path const&)
    {
        ...
        if (envVar)
        {
            Optional<T> castedVar = Syphrena::StringTo<T>(*envVar);
            if (!castedVar)
            {
                SC_LOG_ERROR("server.loading", "Bad value defined for name {} in environment variables, going to use default instead", name);
                return def;
            }

            if (!quiet)
                SC_LOG_WARN("server.loading", "Missing name {} in config file {}, recovered with environment '{}' value.", name, _filename, envVar->c_str());

            return *castedVar;
        }
        else if (!quiet)
        {
            SC_LOG_WARN("server.loading", "Missing name {} in config file {}, add \"{} = {}\" to this file",
                name, _filename, name, def);
        }
    }
    catch (bpt::ptree_bad_data const&)
    {
        SC_LOG_ERROR("server.loading", "Bad value defined for name {} in config file {}, going to use {} instead",
            name, _filename, def);
    }
...
bool ConfigMgr::GetBoolDefault(std::string const& name, bool def, bool quiet) const
{
    ...
    val.erase(std::remove(val.begin(), val.end(), '"'), val.end());
    Optional<bool> boolVal = Syphrena::StringTo<bool>(val);
    if (boolVal)
        return *boolVal;
    else
    {
