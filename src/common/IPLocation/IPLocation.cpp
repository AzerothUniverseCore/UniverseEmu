...
void IpLocationStore::Load()
{
    ...
    SC_LOG_INFO("server.loading", "Loading IP Location Database...");
    ...
    if (!databaseFile)
    {
        SC_LOG_ERROR("server.loading", "IPLocation: No ip database file exists ({}).", databaseFilePath);
        return;
    }

    if (!databaseFile.is_open())
    {
        SC_LOG_ERROR("server.loading", "IPLocation: Ip database file ({}) can not be opened.", databaseFilePath);
        return;
    }
    ...
    while (databaseFile.good())
    {
        ...
        Optional<uint32> from = Syphrena::StringTo<uint32>(ipFrom);
        if (!from)
            continue;

        Optional<uint32> to = Syphrena::StringTo<uint32>(ipTo);
        if (!to)
            continue;
    ...
    SC_LOG_INFO("server.loading", ">> Loaded {} ip location entries.", _ipLocationStore.size());
}

IpLocationRecord const* IpLocationStore::GetLocationRecord(std::string const& ipAddress) const
{
    boost::system::error_code error;
    boost::asio::ip::address_v4 address = Syphrena::Net::make_address_v4(ipAddress, error);
    if (error)
        return nullptr;

    uint32 ip = Syphrena::Net::address_to_uint(address);
