...
void Log::CreateAppenderFromConfig(std::string const& appenderName)
{
    ...
    std::vector<std::string_view> tokens = Syphrena::Tokenize(options, ',', true);
    ...
    AppenderType type = AppenderType(Syphrena::StringTo<uint8>(tokens[0]).value_or(APPENDER_INVALID));
    LogLevel level = LogLevel(Syphrena::StringTo<uint8>(tokens[1]).value_or(LOG_LEVEL_INVALID));
    ...
    if (size > 2)
    {
        if (Optional<uint8> flagsVal = Syphrena::StringTo<uint8>(tokens[2]))
...
void Log::CreateLoggerFromConfig(std::string const& appenderName)
{
    ...
    level = LogLevel(Syphrena::StringTo<uint8>(tokens[0]).value_or(LOG_LEVEL_INVALID));
    if (level > NUM_ENABLED_LOG_LEVELS)
    {
    ...
    for (std::string_view appenderName : Syphrena::Tokenize(tokens[1], ' ', false))
    {
...
void Log::OutMessageImpl(std::string_view filter, LogLevel level, Syphrena::FormatStringView messageFormat, Syphrena::FormatArgs messageFormatArgs)
{
    write(std::make_unique<LogMessage>(level, filter, Syphrena::StringVFormat(messageFormat, messageFormatArgs)));
}

void Log::OutCommandImpl(uint32 account, Syphrena::FormatStringView messageFormat, Syphrena::FormatArgs messageFormatArgs)
{
    write(std::make_unique<LogMessage>(LOG_LEVEL_INFO, "commands.gm", Syphrena::StringVFormat(messageFormat, messageFormatArgs), Syphrena::ToString(account)));
}
...
void Log::write(std::unique_ptr<LogMessage> msg) const
{
    ...
    if (_ioContext)
    {
        ...
        Syphrena::Asio::post(*_ioContext, Syphrena::Asio::bind_executor(*_strand, [logOperation]() { logOperation->call(); }));
    }
    else
...
std::string Log::GetTimestampStr()
{
    ...
    return Syphrena::StringFormat("{:04}-{:02}-{:02}_{:02}-{:02}-{:02}",
...
void Log::Initialize(Syphrena::Asio::IoContext* ioContext)
{
    if (ioContext)
    {
        _ioContext = ioContext;
        _strand = new Syphrena::Asio::Strand(*ioContext);
    }
