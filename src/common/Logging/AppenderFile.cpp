...
{
    if (args.size() < 4)
        throw InvalidAppenderArgsException(Syphrena::StringFormat("Log::CreateAppenderFromConfig: Missing file name for appender {}", name));
    ...
    if (5 < args.size())
    {
        if (Optional<uint32> size = Syphrena::StringTo<uint32>(args[5]))
            _maxFileSize = *size;
        else
            throw InvalidAppenderArgsException(Syphrena::StringFormat("Log::CreateAppenderFromConfig: Invalid size '{}' for appender {}", args[5], name));
    }
...
void AppenderFile::_write(LogMessage const* message)
{
    ...
    if (_dynamicName)
    {
        char namebuf[SYPHRENA_PATH_MAX];
        snprintf(namebuf, SYPHRENA_PATH_MAX, _fileName.c_str(), message->param1.c_str());
