...
template<typename T>
auto MakeSCLogSink(T&& callback)
    -> SCLogSink<typename std::decay<T>::type>
{
...
#pragma warning(pop)
    ...
    if (!secure)
    {
        SC_LOG_TRACE(logger, "Starting process \"{}\" with arguments: \"{}\".",
    ...
    auto outInfo = MakeSCLogSink([&](std::string_view msg)
    {
        SC_LOG_INFO(logger, "{}", msg);
    });

    auto outError = MakeSCLogSink([&](std::string_view msg)
    {
        SC_LOG_ERROR(logger, "{}", msg);
    });
    ...
    if (!secure)
    {
        SC_LOG_TRACE(logger, ">> Process \"{}\" finished with return value {}.",
            executable, result);
    }
