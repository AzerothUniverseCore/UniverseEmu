...
SC_COMMON_API int StartProcess(std::string const& executable, std::vector<std::string> const& args,
...
/// When the process is marked as secure no arguments are leaked to logs.
/// Note that most executables expect it's name as the first argument.
SC_COMMON_API std::shared_ptr<AsyncProcessResult>
...
SC_COMMON_API std::string SearchExecutableInPath(std::string const& filename);
