...
namespace Syphrena
{
    SC_COMMON_API std::vector<std::string_view> Tokenize(std::string_view str, char sep, bool keepEmpty);
...
SC_COMMON_API Optional<int32> MoneyStringToMoney(std::string const& moneyString);

SC_COMMON_API struct tm* localtime_r(time_t const* time, struct tm *result);
SC_COMMON_API time_t LocalTimeToUTCTime(time_t time);
SC_COMMON_API time_t GetLocalHourTimestamp(time_t time, uint8 hour, bool onlyAfterTime = true);
SC_COMMON_API tm TimeBreakdown(time_t t);

SC_COMMON_API std::string secsToTimeString(uint64 timeInSecs, TimeFormat timeFormat = TimeFormat::FullText, bool hoursOnly = false);
SC_COMMON_API uint32 TimeStringToSecs(std::string const& timestring);
SC_COMMON_API std::string TimeToTimestampStr(time_t t);
SC_COMMON_API std::string TimeToHumanReadable(time_t t);
...
// UTF8 handling
SC_COMMON_API bool Utf8toWStr(std::string_view utf8str, std::wstring& wstr);

// in wsize==max size of buffer, out wsize==real string size
SC_COMMON_API bool Utf8toWStr(char const* utf8str, size_t csize, wchar_t* wstr, size_t& wsize);
...
SC_COMMON_API bool WStrToUtf8(std::wstring_view wstr, std::string& utf8str);
// size==real string size
SC_COMMON_API bool WStrToUtf8(wchar_t const* wstr, size_t size, std::string& utf8str);

// set string to "" if invalid utf8 sequence
SC_COMMON_API size_t utf8length(std::string& utf8str);
SC_COMMON_API void utf8truncate(std::string& utf8str, size_t len);
...
SC_COMMON_API void wstrToUpper(std::wstring& str);
SC_COMMON_API void wstrToLower(std::wstring& str);
SC_COMMON_API void strToUpper(std::string& str);
SC_COMMON_API void strToLower(std::string& str);

SC_COMMON_API std::wstring GetMainPartOfName(std::wstring const& wname, uint32 declension);

SC_COMMON_API bool utf8ToConsole(std::string_view utf8str, std::string& conStr);
SC_COMMON_API bool consoleToUtf8(std::string_view conStr, std::string& utf8str);
SC_COMMON_API bool Utf8FitTo(std::string_view str, std::wstring_view search);
SC_COMMON_API void utf8printf(FILE* out, const char *str, ...);
SC_COMMON_API void vutf8printf(FILE* out, const char *str, va_list* ap);
SC_COMMON_API bool Utf8ToUpperOnlyLatin(std::string& utf8String);

#if SYPHRENA_PLATFORM == SYPHRENA_PLATFORM_WINDOWS
SC_COMMON_API bool ReadWinConsole(std::string& str, size_t size = 256);
SC_COMMON_API bool WriteWinConsole(std::string_view str, bool error = false);
#endif

SC_COMMON_API Optional<std::size_t> RemoveCRLF(std::string& str);

SC_COMMON_API bool IsIPAddress(char const* ipaddress);

SC_COMMON_API uint32 CreatePIDFile(std::string const& filename);
SC_COMMON_API uint32 GetPID();

namespace Syphrena::Impl
{
    SC_COMMON_API std::string ByteArrayToHexStr(uint8 const* bytes, size_t length, bool reverse = false);
    SC_COMMON_API void HexStrToByteArray(std::string_view str, uint8* out, size_t outlen, bool reverse = false);
}

template <typename Container>
std::string ByteArrayToHexStr(Container const& c, bool reverse = false)
{
    return Syphrena::Impl::ByteArrayToHexStr(std::data(c), std::size(c), reverse);
}

template <size_t Size>
void HexStrToByteArray(std::string_view str, std::array<uint8, Size>& buf, bool reverse = false)
{
    Syphrena::Impl::HexStrToByteArray(str, buf.data(), Size, reverse);
}
template <size_t Size>
...
inline std::vector<uint8> HexStrToByteVector(std::string_view str, bool reverse = false)
{
    ...
    buf.resize(sz);
    Syphrena::Impl::HexStrToByteArray(str, buf.data(), sz, reverse);
    return buf;
}

SC_COMMON_API bool StringEqualI(std::string_view str1, std::string_view str2);
...
SC_COMMON_API bool StringContainsStringI(std::string_view haystack, std::string_view needle);
template <typename T>
...
SC_COMMON_API bool StringCompareLessI(std::string_view a, std::string_view b);
...
namespace Syphrena
{
    namespace Impl
    {
        SC_COMMON_API std::string GetTypeName(std::type_info const&);
    }
template <typename T>
