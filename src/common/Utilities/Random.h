...
SC_COMMON_API int32 irand(int32 min, int32 max);

/* Return a random number in the range min..max (inclusive). */
SC_COMMON_API uint32 urand(uint32 min, uint32 max);
...
SC_COMMON_API uint32 urandms(uint32 min, uint32 max);

/* Return a random number in the range 0 .. UINT32_MAX. */
SC_COMMON_API uint32 rand32();
...
SC_COMMON_API Milliseconds randtime(Milliseconds min, Milliseconds max);

/* Return a random number in the range min..max */
SC_COMMON_API float frand(float min, float max);

/* Return a random double from 0.0 to 1.0 (exclusive). */
SC_COMMON_API double rand_norm();

/* Return a random double from 0.0 to 100.0 (exclusive). */
SC_COMMON_API double rand_chance();

/* Return a random number in the range 0..count (exclusive) with each value having a different chance of happening */
SC_COMMON_API uint32 urandweighted(size_t count, double const* chances);
...
*/
class SC_COMMON_API RandomEngine
{
public:
