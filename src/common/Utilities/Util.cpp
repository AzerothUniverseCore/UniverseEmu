...
std::vector<std::string_view> Syphrena::Tokenize(std::string_view str, char sep, bool keepEmpty)
{
...
std::string secsToTimeString(uint64 timeInSecs, TimeFormat timeFormat, bool hoursOnly)
{
    ...
    {
        if (days)
            return Syphrena::StringFormat("{}:{:02}:{:02}:{:02}", days, hours, minutes, secs);
        else if (hours)
            return Syphrena::StringFormat("{}:{:02}:{:02}", hours, minutes, secs);
        else if (minutes)
            return Syphrena::StringFormat("{}:{:02}", minutes, secs);
        else
            return Syphrena::StringFormat("0:{:02}", secs);
    }
...
Optional<int32> MoneyStringToMoney(std::string const& moneyString)
{
    ...
    for (std::string_view token : Syphrena::Tokenize(moneyString, ' ', false))
    {
        uint32 unit;
        ...
        Optional<uint32> amount = Syphrena::StringTo<uint32>(token.substr(0, token.length() - 1));
        if (amount)
            money += (unit * *amount);
        else
            return std::nullopt;
...
std::string TimeToTimestampStr(time_t t)
{
    ...
    return Syphrena::StringFormat("{:04}-{:02}-{:02}_{:02}-{:02}-{:02}", aTm.tm_year + 1900, aTm.tm_mon + 1, aTm.tm_mday, aTm.tm_hour, aTm.tm_min, aTm.tm_sec);
}
...
bool IsIPAddress(char const* ipaddress)
{
    ...
    Syphrena::Net::make_address(ipaddress, error);
    return !error;
}
...
bool Utf8toWStr(char const* utf8str, size_t csize, wchar_t* wstr, size_t& wsize)
{
    try
    {
        Syphrena::CheckedBufferOutputIterator<wchar_t> out(wstr, wsize);
...
std::string Syphrena::Impl::ByteArrayToHexStr(uint8 const* bytes, size_t arrayLen, bool reverse /* = false */)
{
    int32 init = 0;
    int32 end = arrayLen;
    int8 op = 1;
    ...
    for (int32 i = init; i != end; i += op)
        Syphrena::StringFormatTo(inserter, "{:02X}", bytes[i]);

    return result;
}

void Syphrena::Impl::HexStrToByteArray(std::string_view str, uint8* out, size_t outlen, bool reverse /*= false*/)
{
    ...
    for (int32 i = init; i != end; i += 2 * op)
        out[j++] = Syphrena::StringTo<uint8>(str.substr(i, 2), 16).value_or(0);
}
...
std::string Syphrena::Impl::GetTypeName(std::type_info const& info)
{
