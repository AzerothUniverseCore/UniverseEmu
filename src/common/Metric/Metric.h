...
private:
    ...
    std::unique_ptr<Syphrena::Asio::DeadlineTimer> _batchTimer;
    std::unique_ptr<Syphrena::Asio::DeadlineTimer> _overallStatusTimer;
...
public:
    ...
    void Initialize(std::string const& realmName, Syphrena::Asio::IoContext& ioContext, std::function<void()> overallStatusLogger);
...
#define SC_METRIC_TAG(name, value) MetricTag(name, value)

#define SC_METRIC_DO_CONCAT(a, b) a ## b
#define SC_METRIC_CONCAT(a, b) SC_METRIC_DO_CONCAT(a, b)
#define SC_METRIC_UNIQUE_NAME(name) SC_METRIC_CONCAT(name, __LINE__)

#if defined PERFORMANCE_PROFILING || defined WITHOUT_METRICS
#define SC_METRIC_EVENT(category, title, description) ((void)0)
#define SC_METRIC_VALUE(category, value, ...) ((void)0)
#define SC_METRIC_TIMER(category, ...) ((void)0)
#define SC_METRIC_DETAILED_EVENT(category, title, description) ((void)0)
#define SC_METRIC_DETAILED_TIMER(category, ...) ((void)0)
#define SC_METRIC_DETAILED_NO_THRESHOLD_TIMER(category, ...) ((void)0)
#else
#  if SYPHRENA_PLATFORM != SYPHRENA_PLATFORM_WINDOWS
#define SC_METRIC_EVENT(category, title, description)                  \
        do {                                                           \
            if (sMetric->IsEnabled())                                  \
                sMetric->LogEvent(category, title, description);       \
        } while (0)
#define SC_METRIC_VALUE(category, value, ...)                          \
        do {                                                           \
            if (sMetric->IsEnabled())                                  \
...
#define SC_METRIC_VALUE(category, value, ...)                          \
        ...
        __pragma(warning(pop))
#  endif
#define SC_METRIC_TIMER(category, ...)                                                                           \
        auto SC_METRIC_UNIQUE_NAME(__sc_metric_stop_watch) = MakeMetricStopWatch([&](TimePoint start)            \
        {                                                                                                        \
            sMetric->LogValue(category, std::chrono::steady_clock::now() - start, ##__VA_ARGS__);                \
        });
#  if defined WITH_DETAILED_METRICS
#define SC_METRIC_DETAILED_TIMER(category, ...)                                                                  \
        auto SC_METRIC_UNIQUE_NAME(__sc_metric_stop_watch) = MakeMetricStopWatch([&](TimePoint start)            \
        {                                                                                                        \
...
#define SC_METRIC_DETAILED_NO_THRESHOLD_TIMER(category, ...) SC_METRIC_TIMER(category, ##__VA_ARGS__)
#define SC_METRIC_DETAILED_EVENT(category, title, description) SC_METRIC_EVENT(category, title, description)
#  else
#define SC_METRIC_DETAILED_EVENT(category, title, description) ((void)0)
#define SC_METRIC_DETAILED_TIMER(category, ...) ((void)0)
#define SC_METRIC_DETAILED_NO_THRESHOLD_TIMER(category, ...) ((void)0)
