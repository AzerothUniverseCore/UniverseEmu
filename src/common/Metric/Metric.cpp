...
void Metric::Initialize(std::string const& realmName, Syphrena::Asio::IoContext& ioContext, std::function<void()> overallStatusLogger)
{
    _dataStream = std::make_unique<boost::asio::ip::tcp::iostream>();
    _realmName = FormatInfluxDBTagValue(realmName);
    _batchTimer = std::make_unique<Syphrena::Asio::DeadlineTimer>(ioContext);
    _overallStatusTimer = std::make_unique<Syphrena::Asio::DeadlineTimer>(ioContext);
...
bool Metric::Connect()
{
    ...
    if (error)
    {
        SC_LOG_ERROR("metric", "Error connecting to '{}:{}', disabling Metric. Error message : {}",
...
void Metric::LoadFromConfigs()
{
    ...
    {
        SC_LOG_ERROR("metric", "'Metric.Interval' config set to {}, overriding to 1.", _updateInterval);
        _updateInterval = 1;
    }
    ...
    {
        SC_LOG_ERROR("metric", "'Metric.OverallStatusInterval' config set to {}, overriding to 1.", _overallStatusTimerInterval);
    ...
    if (_enabled && !previousValue)
    {
        ...
        {
            SC_LOG_ERROR("metric", "'Metric.ConnectionInfo' not specified in configuration file.");
            return;
        }

        std::vector<std::string_view> tokens = Syphrena::Tokenize(connectionInfo, ';', true);
        if (tokens.size() != 3)
        {
            SC_LOG_ERROR("metric", "'Metric.ConnectionInfo' specified with wrong format in configuration file.");
            return;
        }
...
void Metric::SendBatch()
{
    ...
    if (status_code != 204)
    {
        SC_LOG_ERROR("metric", "Error sending data, returned HTTP code: {}", status_code);
    }
...
void Metric::Unload()
{
    ...
    if (_enabled && Syphrena::Asio::get_io_context(*_batchTimer).stopped())
    {
        _enabled = false;
        SendBatch();
    }
...
template SC_COMMON_API std::string Metric::FormatInfluxDBValue(int8);
template SC_COMMON_API std::string Metric::FormatInfluxDBValue(uint8);
template SC_COMMON_API std::string Metric::FormatInfluxDBValue(int16);
template SC_COMMON_API std::string Metric::FormatInfluxDBValue(uint16);
template SC_COMMON_API std::string Metric::FormatInfluxDBValue(int32);
template SC_COMMON_API std::string Metric::FormatInfluxDBValue(uint32);
template SC_COMMON_API std::string Metric::FormatInfluxDBValue(int64);
template SC_COMMON_API std::string Metric::FormatInfluxDBValue(uint64);
