...
uint32 ReadBuild(int locale)
{
    ...
    std::string filename = Syphrena::StringFormat("component.wow-{}.txt", langs[locale]);
...
void ExtractMapsFromMpq(uint32 build)
{
    ...
    for(uint32 z = 0; z < map_count; ++z)
    {
        ...
        mpqMapName = Syphrena::StringFormat("World\\Maps\\{}\\{}.wdt", map_ids[z].name, map_ids[z].name);
        WDT_file wdt;
...
//            printf("Error loading %s map wdt data\n", map_ids[z].name);
        ...
        for(uint32 y = 0; y < WDT_MAP_SIZE; ++y)
        {
            for(uint32 x = 0; x < WDT_MAP_SIZE; ++x)
            {
                ...
                mpqFileName = Syphrena::StringFormat("World\\Maps\\{}\\{}_{}_{}.adt", map_ids[z].name, map_ids[z].name, x, y);
                outputFileName = Syphrena::StringFormat("{}/maps/{:03}{:02}{:02}.map", output_path, map_ids[z].id, y, x);
...
{
    std::string fileName = Syphrena::StringFormat("{}/Data/{}/locale-{}.MPQ", input_path, langs[locale], langs[locale]);
...
void LoadCommonMPQFiles()
{
    ...
    {
        fileName = Syphrena::StringFormat("{}/Data/{}", input_path, CONF_mpq_list[i]);
        if (boost::filesystem::exists(fileName))
            new MPQArchive(fileName.c_str());
    }
}
...
{
    Syphrena::Banner::Show("Map & DBC Extractor", [](char const* text) { printf("%s\n", text); }, nullptr);
    ...
    {
        std::string filename = Syphrena::StringFormat("{}/Data/{}/locale-{}.MPQ", input_path, langs[i], langs[i]);
        if (boost::filesystem::exists(filename))
        {
