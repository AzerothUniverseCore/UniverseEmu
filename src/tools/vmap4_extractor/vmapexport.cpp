...
bool ExtractSingleWmo(std::string& fname)
{
    ...
    if (froot.nGroups !=0)
    {
        for (uint32 i = 0; i < froot.nGroups; ++i)
        {
            ...
            WMOGroup fgroup(Syphrena::StringFormat("{}_{:03}.wmo", temp, i));
...
bool fillArchiveNameVector(std::vector<std::string>& pArchiveNames)
{
    ...
    if (!scan_patches(Syphrena::StringFormat("{}patch", input_path).c_str(), pArchiveNames))
        return(false);
    ...
    {
        printf("Locale: %s\n", locale.c_str());
        if(scan_patches(Syphrena::StringFormat("{}{}/patch-{}", input_path, locale, locale).c_str(), pArchiveNames))
            foundOne = true;
    }
...
{
    Syphrena::Banner::Show("VMAP data extractor", [](char const* text) { printf("%s\n", text); }, nullptr);
